...
Nebula is exploring a Monero-settled layer 2 focused on quantum resistance,
speed, DeFi features, low fees, and privacy by default. The initial protocol
plan is documented in [docs/NEBULA_LAYER2.md](docs/NEBULA_LAYER2.md), with a
stdlib-only executable spec in [utils/nebula_l2](utils/nebula_l2) and a Rust
core migration target in [utils/nebula_l2_rs](utils/nebula_l2_rs) covering
private notes, native assets, AMMs, stable pools, private multi-hop AMM route
swaps, dark-pool atomic swaps, lending, contract calls with committed private
arguments, scoped private event disclosures, committed private storage, scoped
storage disclosures, batched execution, metered contract execution receipts,
native contract escrow, ML-KEM-shaped wallet sessions, append-only event chain
roots, timelocked contract upgrade governance, DAO governor contracts, vault
allowance commitment roots, paymasters, bridge flows, DA records,
relay-path commitments for mempool admissions and wallet sessions,
policy-limited sponsored fee calls, signed paymaster pause/resume and
replenishment governance, emergency close refunds, refill planning,
relayer-bound refill authorization receipts, refill failure receipts, relayer
challenge windows, slashing hooks, bonded relayer stake settlement, relayer
delayed unbonding, stake-weighted relayer selection, performance-weighted refill
routing, proof-bound budget-capped claimable relayer reward accounting and batch
claim bundles with quote roots, expiry windows, settlement monitors, and stale
quote invalidation reports, inclusion deadlines, re-quote backoff scoring,
relayer reputation counters, devnet performance profiles for latency,
throughput, DA/proof bandwidth, fee curves, signed benchmark runs, calibration
records, Rust fee-profile parity logic, Rust native-asset plus AMM
state-transition logic, Rust private batch swaps, multi-hop route swaps,
dark-pool atomic swaps, Rust private lending markets with oracle roots,
position commitments, borrow/repay/liquidation records, and lending roots, and
Rust counter-contract execution receipts, native contract escrow
deposits/withdrawals, and timelocked contract upgrade roots,
Rust block/state commitments with DA shards, PQ validator votes, privacy-proof
aggregates, signed validity certificates, Rust Monero settlement commitments
with epoch checkpoints, fixed-format anchors, bridge signer sets, hashed
deposit observations, withdrawal queues and release delays, signed reserve
reports, withdrawal challenge evidence, privacy-preserving Monero monitor roots
for RPC/ZMQ/block/tx/anchor/withdrawal/reorg observations, Rust encrypted
mempool commitments with sequencer preconfirmations, omission evidence, and
forced inclusion receipts, Rust wallet-facing status queries for pending,
included, anchored, and Monero-final states, transaction-status receipts, and
Rust paymaster fee sponsorship with private deposits, policy caps, governance,
relayer rewards and bonded slashing hooks, Rust Wasm runtime-boundary manifests,
private execution receipts, host-call/storage-delta roots, and block
`wasm_runtime_root` commitments, Rust Wasm bytecode validation for host imports,
memory caps, required exports, and validation hashes, Rust wallet sync views
with scan tags, watched nullifiers, private-note, mempool, contract/runtime,
bridge, and paymaster history roots, Rust account registry state with
recovery-signed key rotation, retired signer labels, wallet-session revocation,
stale node-network checks, and `account_root`/`wallet_session_root`
commitments, Rust local sequencer orchestration for encrypted admission,
preconfirmation, typed DeFi submission, fee-density block packing,
staged DeFi admission mirrors, state-safe canonical DeFi inclusion,
KEM-envelope mempool and wallet-session records, capacity-aware
preconfirmation target heights, encrypted batch receipts, relay fairness
tickets, anti-censorship mempool lane commitments, low-fee smoothing lanes,
stake-weighted proposer slots, fast-finality votes and certificates,
consensus slashing evidence, Rust prover-network proof jobs, signed prover
receipts, proof-market fee resources, DA sampling receipts, watchtower audit
and challenge reports, signed network node advertisements, root inventory
gossip, admission inventory gossip, encrypted gossip envelopes, peer scoring,
root-conflict evidence, block production,
status, wallet scans with Monero evidence ingestion, and epoch anchors, and
Monero anchor simulation with devnet
validity certificates, privacy-proof
aggregates, and watchtower audit/challenge reports,
including forced inclusion for omitted encrypted mempool admissions, bridge
deposit observations bound to threshold PQ bridge signer sets, withdrawal holds
and release-rate limits, withdrawal queue signatures bound to PQ bridge signer
sets, PQ bridge signer-set rotation, threshold-signed bridge emergency controls,
sealed-swap order commitments, solver bids, Rust sealed AMM batch execution,
and solver settlement receipts with clearing-price, aggregate-surplus, and
block-committed settlement receipt roots plus explicit auction expiry.
