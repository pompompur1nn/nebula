...
pub type PrivateL2LowFeePqConfidentialSequencerBlobFeeRebateNettingRouterRuntimeResult<T> =
    Result<T>;
pub type Runtime = State;

pub const PRIVATE_L2_LOW_FEE_PQ_CONFIDENTIAL_SEQUENCER_BLOB_FEE_REBATE_NETTING_ROUTER_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-private-l2-low-fee-pq-confidential-sequencer-blob-fee-rebate-netting-router-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_LOW_FEE_PQ_CONFIDENTIAL_SEQUENCER_BLOB_FEE_REBATE_NETTING_ROUTER_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const PQ_ENVELOPE_SUITE: &str = "ml-kem-1024+ml-dsa-87-netting-router-fee-rebate-envelope-v1";
pub const NETTING_ROUTER_SUITE: &str =
    "private-l2-low-fee-confidential-sequencer-rebate-netting-router-v1";
pub const BALANCE_COMMITMENT_SUITE: &str = "pedersen+bulletproofs-plus-router-balance-root-v1";
pub const SETTLEMENT_PROOF_SUITE: &str =
    "slh-dsa-shake-256f-netting-router-settlement-proof-root-v1";
pub const FAST_PATH_PROOF_SUITE: &str = "ml-dsa-87-fast-netting-router-attestation-root-v1";
pub const NULLIFIER_SUITE: &str = "confidential-netting-router-nullifier-set-v1";
pub const PUBLIC_RECORD_SUITE: &str =
    "roots-only-confidential-sequencer-blob-fee-rebate-netting-router-record-v1";
pub const PRIVACY_BOUNDARY: &str =
    "roots_only_no_plaintext_accounts_amounts_addresses_view_keys_blob_payloads_or_secret_keys";
pub const DEVNET_L2_NETWORK: &str = "nebula-private-l2-netting-router-devnet";
pub const DEVNET_ROUTER_ID: &str = "private-l2-sequencer-blob-fee-rebate-netting-router-devnet";
pub const DEVNET_OPERATOR_SET_ID: &str = "private-l2-netting-router-operator-set-devnet";
...
pub const DEFAULT_ROUTER_EPOCH_BLOCKS: u64 = 720;
...
pub const DEFAULT_MAX_ROUTING_QUEUE_ENTRIES: usize = 4_194_304;
...
const D_STATE: &str = "PL2-LF-PQ-CONF-SEQ-BLOB-REBATE-NETTING-ROUTER:STATE";
const D_CONFIG: &str = "PL2-LF-PQ-CONF-SEQ-BLOB-REBATE-NETTING-ROUTER:CONFIG";
const D_COUNTERS: &str = "PL2-LF-PQ-CONF-SEQ-BLOB-REBATE-NETTING-ROUTER:COUNTERS";
const D_ROOTS: &str = "PL2-LF-PQ-CONF-SEQ-BLOB-REBATE-NETTING-ROUTER:ROOTS";
const D_DEBITS: &str = "PL2-LF-PQ-CONF-SEQ-BLOB-REBATE-NETTING-ROUTER:DEBITS";
const D_CREDITS: &str = "PL2-LF-PQ-CONF-SEQ-BLOB-REBATE-NETTING-ROUTER:CREDITS";
const D_WINDOWS: &str = "PL2-LF-PQ-CONF-SEQ-BLOB-REBATE-NETTING-ROUTER:WINDOWS";
const D_SETTLEMENTS: &str = "PL2-LF-PQ-CONF-SEQ-BLOB-REBATE-NETTING-ROUTER:SETTLEMENTS";
const D_BUCKETS: &str = "PL2-LF-PQ-CONF-SEQ-BLOB-REBATE-NETTING-ROUTER:BUCKETS";
const D_ROUTES: &str = "PL2-LF-PQ-CONF-SEQ-BLOB-REBATE-NETTING-ROUTER:ROUTES";
const D_QUEUE: &str = "PL2-LF-PQ-CONF-SEQ-BLOB-REBATE-NETTING-ROUTER:QUEUE";
const D_ATTESTATIONS: &str = "PL2-LF-PQ-CONF-SEQ-BLOB-REBATE-NETTING-ROUTER:ATTESTATIONS";
const D_NULLIFIERS: &str = "PL2-LF-PQ-CONF-SEQ-BLOB-REBATE-NETTING-ROUTER:NULLIFIERS";
const D_PUBLIC_RECORDS: &str = "PL2-LF-PQ-CONF-SEQ-BLOB-REBATE-NETTING-ROUTER:PUBLIC-RECORDS";
...
pub struct Config {
    ...
    pub netting_router_suite: String,
    ...
    pub max_route_commitments: usize,
    pub max_routing_queue_entries: usize,
...
impl Config {
    pub fn devnet() -> Self {
        Self {
            ...
            netting_router_suite: NETTING_ROUTER_SUITE.to_string(),
            ...
            router_epoch_blocks: DEFAULT_ROUTER_EPOCH_BLOCKS,
            ...
            max_route_commitments: DEFAULT_MAX_ROUTE_COMMITMENTS,
            max_routing_queue_entries: DEFAULT_MAX_ROUTING_QUEUE_ENTRIES,
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "netting_router_suite": self.netting_router_suite,
            ...
            "router_epoch_blocks": self.router_epoch_blocks,
...
pub struct Counters {
    ...
    pub routing_entries_enqueued: u64,
...
impl Counters {
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "routing_entries_enqueued": self.routing_entries_enqueued,
...
pub struct Roots {
    ...
    pub route_commitments_root: String,
    pub routing_queue_root: String,
...
impl Roots {
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "route_commitments_root": self.route_commitments_root,
            "routing_queue_root": self.routing_queue_root,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct RouteCommitmentInput {
    pub route_id: String,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct RoutingQueueEntryInput {
    pub entry_id: String,
    pub route_id: String,
...
impl RouteCommitment {
    ...
    pub fn state_root(&self) -> String {
        record_root(D_ROUTES, &self.public_record())
    }
}
...
impl RoutingQueueEntry {
    pub fn selectable(&self, height: u64) -> bool {
        self.status == QueueStatus::Pending && height <= self.deadline_height
    }
    pub fn public_record(&self) -> Value {
        json!({
    ...
    pub fn state_root(&self) -> String {
        record_root(D_QUEUE, &self.public_record())
    }
}
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct PqRouterAttestation {
    pub attestation_id: String,
    pub kind: AttestationKind,
    pub subject_id: String,
    pub subject_root: String,
...
pub struct State {
    ...
    pub route_commitments: BTreeMap<String, RouteCommitment>,
    pub routing_queue: BTreeMap<String, RoutingQueueEntry>,
    pub attestations: BTreeMap<String, PqRouterAttestation>,
    pub nullifiers: BTreeSet<String>,
    pub public_records: BTreeMap<String, RouterPublicRecord>,
}
impl State {
    ...
    pub fn advertise_route(&mut self, input: RouteCommitmentInput) -> Result<String> {
        self.config.validate()?;
        self.ensure_capacity(
            self.route_commitments.len(),
            self.config.max_route_commitments,
            "route commitments",
        )?;
        self.ensure_unique_id(&input.route_id, "route commitment")?;
        self.validate_low_fee(input.max_fee_bps)?;
        self.validate_privacy(input.min_privacy_set_size, input.min_pq_security_bits)?;
        self.validate_commitment(&input.route_commitment_root, "route commitment root")?;
        ...
        let id = input.route_id.clone();
        let route = RouteCommitment {
            route_id: id.clone(),
            route_class: input.route_class,
            lane: input.lane,
            status: RouteStatus::Advertised,
            route_commitment_root: input.route_commitment_root,
        ...
        self.route_commitments.insert(id.clone(), route);
        self.counters.routes_advertised = self.counters.routes_advertised.saturating_add(1);
        self.emit_public_record("route_commitment", &id)?;
        self.recompute_roots();
        Ok(id)
    }
    pub fn enqueue_route_entry(&mut self, input: RoutingQueueEntryInput) -> Result<String> {
        self.config.validate()?;
        self.ensure_capacity(
            self.routing_queue.len(),
            self.config.max_routing_queue_entries,
            "routing queue entries",
        )?;
        self.ensure_unique_id(&input.entry_id, "routing queue entry")?;
        self.validate_low_fee(input.priority_fee_bps)?;
        self.validate_privacy(input.privacy_set_size, input.pq_security_bits)?;
        self.validate_commitment(&input.queue_commitment_root, "queue commitment root")?;
        ...
        let route = self
            .route_commitments
            .get(&input.route_id)
            .ok_or_else(|| "missing route commitment".to_string())?;
        if !route.status.selectable() {
            return Err("route is not selectable".to_string());
        }
        if input.priority_fee_bps > route.max_fee_bps {
            return Err("entry fee exceeds selected route cap".to_string());
        }
        if input.privacy_set_size < route.min_privacy_set_size
            || input.pq_security_bits < route.min_pq_security_bits
        {
            return Err("entry does not satisfy route privacy or pq floor".to_string());
        }
        ...
        let entry = RoutingQueueEntry {
            ...
            queue_commitment_root: input.queue_commitment_root,
        ...
        self.routing_queue.insert(id.clone(), entry);
        self.counters.routing_entries_enqueued =
            self.counters.routing_entries_enqueued.saturating_add(1);
        self.emit_public_record("routing_queue_entry", &id)?;
        self.recompute_roots();
        Ok(id)
    }
    pub fn select_best_route_entry(&mut self) -> Result<Option<String>> {
        let selected = self
            .routing_queue
            .iter()
            .filter_map(|(entry_id, entry)| {
                let route = self.route_commitments.get(&entry.route_id)?;
                if entry.selectable(self.height) && route.status.selectable() {
                    Some((
                        route
                            .priority_score()
        ...
        if let Some(entry) = self.routing_queue.get_mut(&entry_id) {
            entry.status = QueueStatus::Routed;
            entry.selected_height = Some(self.height);
        }
        if let Some(route) = self.route_commitments.get_mut(&route_id) {
            route.status = RouteStatus::Selected;
            route.selected_entries = route.selected_entries.saturating_add(1);
        }
    ...
    pub fn post_attestation(&mut self, input: AttestationInput) -> Result<String> {
        ...
        let attestation = PqRouterAttestation {
        ...
        self.emit_public_record("pq_router_attestation", &id)?;
        self.recompute_roots();
        Ok(id)
    }
    ...
    pub fn advance(&mut self, input: AdvanceInput) -> Result<()> {
        ...
        } else if self.config.router_epoch_blocks > 0 {
            self.epoch = self.height / self.config.router_epoch_blocks;
        }
        self.expire_stale_notes();
        self.expire_stale_windows();
        self.expire_stale_queue();
        self.recompute_roots();
        Ok(())
    }
    ...
    pub fn recompute_roots(&mut self) {
        ...
        self.roots.route_commitments_root = map_root(D_ROUTES, &self.route_commitments);
        self.roots.routing_queue_root = map_root(D_QUEUE, &self.routing_queue);
        ...
        self.roots.state_root = record_root(
            ...
            &json!({
                ...
                "route_commitments_root": self.roots.route_commitments_root,
                "routing_queue_root": self.roots.routing_queue_root,
    ...
    fn ensure_unique_id(&self, id: &str, label: &str) -> Result<()> {
        ...
        if self.debit_notes.contains_key(id)
            ...
            || self.route_commitments.contains_key(id)
            || self.routing_queue.contains_key(id)
    ...
    fn subject_root(&self, record_kind: &str, subject_id: &str) -> Result<String> {
        match record_kind {
            ...
            "route_commitment" => self
                .route_commitments
                .get(subject_id)
                .map(RouteCommitment::state_root)
                .ok_or_else(|| "missing route commitment for public record".to_string()),
            "routing_queue_entry" => self
                .routing_queue
                .get(subject_id)
                .map(RoutingQueueEntry::state_root)
                .ok_or_else(|| "missing routing queue entry for public record".to_string()),
            "pq_router_attestation" => self
                .attestations
                .get(subject_id)
                .map(PqRouterAttestation::state_root)
    ...
    fn expire_stale_queue(&mut self) {
        for entry in self.routing_queue.values_mut() {
            if entry.status == QueueStatus::Pending && self.height > entry.deadline_height {
...
pub fn devnet() -> State {
    ...
    let bucket_id = state
        .open_liquidity_bucket(LiquidityBucketInput {
            bucket_id: "devnet-netting-router-liquidity-bucket-0001".to_string(),
            lane: RouterLane::SequencerBlobFee,
    ...
    let route_id = state
        .advertise_route(RouteCommitmentInput {
            route_id: "devnet-low-fee-fast-rebate-route-0001".to_string(),
            route_class: RouteClass::FastFeeRebate,
            lane: RouterLane::UserRebate,
            route_commitment_root: "devnet-route-commitment-root-0001".to_string(),
            fee_policy_root: "devnet-route-fee-policy-root-0001".to_string(),
            privacy_policy_root: "devnet-route-privacy-policy-root-0001".to_string(),
            liquidity_policy_root: "devnet-route-liquidity-policy-root-0001".to_string(),
            settlement_policy_root: "devnet-route-settlement-policy-root-0001".to_string(),
            pq_envelope_root: "devnet-route-pq-envelope-root-0001".to_string(),
            max_fee_bps: DEFAULT_TARGET_FEE_BPS,
            min_privacy_set_size: DEFAULT_MIN_PRIVACY_SET_SIZE,
            min_pq_security_bits: DEFAULT_MIN_PQ_SECURITY_BITS,
            priority_hint: 240,
        })
        .expect("devnet route commitment");
    let debit_id = state
    ...
    let _queue_id = state
        .enqueue_route_entry(RoutingQueueEntryInput {
            entry_id: "devnet-routing-queue-entry-0001".to_string(),
            route_id: route_id.clone(),
            debit_note_id: Some(debit_id.clone()),
            credit_note_id: Some(credit_id.clone()),
            window_id: None,
            queue_commitment_root: "devnet-routing-queue-root-0001".to_string(),
        ...
        .expect("devnet routing queue entry");
    state
        .select_best_route_entry()
        .expect("devnet route selection");
    state
