...
pub type MoneroL2PqPrivateJamtisSeraphisSpendlinkDecoyRefreshRebateRouterRuntimeResult<T> =
    Result<T>;
pub type Runtime = State;

pub const MONERO_L2_PQ_PRIVATE_JAMTIS_SERAPHIS_SPENDLINK_DECOY_REFRESH_REBATE_ROUTER_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-monero-l2-pq-private-jamtis-seraphis-spendlink-decoy-refresh-rebate-router-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    MONERO_L2_PQ_PRIVATE_JAMTIS_SERAPHIS_SPENDLINK_DECOY_REFRESH_REBATE_ROUTER_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const STATE_ROOT_DOMAIN: &str =
    "MONERO-L2-PQ-PRIVATE-JAMTIS-SERAPHIS-SPENDLINK-DECOY-REFRESH-REBATE-ROUTER";
pub const DEVNET_L2_NETWORK: &str = "nebula-private-l2-devnet";
pub const DEVNET_MONERO_NETWORK: &str = "monero-devnet";
pub const DEVNET_FEE_ASSET_ID: &str = "piconero-devnet";
pub const DEVNET_REBATE_ASSET_ID: &str = "refresh-router-rebate-credit-devnet";
pub const DEVNET_ROUTE_ASSET_ID: &str = "refresh-route-share-devnet";
...
pub const DEFAULT_TARGET_RING_SIZE: u16 = 160;
...
pub const DEFAULT_MIN_ROUTING_QUALITY_BPS: u64 = 9_180;
...
pub const DEFAULT_MAX_ROUTE_HOPS: u8 = 6;
pub const DEFAULT_MIN_ROUTE_SPLIT_BPS: u64 = 500;
pub const DEFAULT_MAX_REFRESH_UNITS_PER_ROUTE: u64 = 32_768;
pub const DEFAULT_MAX_REFRESH_UNITS_PER_EPOCH: u64 = 524_288;
pub const DEFAULT_MIN_REBATE_SOLVENCY_BPS: u64 = 9_420;
...
pub const ROUTER_INTENT_SCHEME: &str =
    "jamtis-seraphis-spendlink-decoy-refresh-rebate-router-intent-root-v1";
pub const ROUTER_POOL_SCHEME: &str = "pq-private-jamtis-seraphis-decoy-refresh-router-pool-root-v1";
pub const ROUTER_QUOTE_SCHEME: &str = "defi-style-private-refresh-rebate-router-quote-root-v1";
pub const ROUTE_PLAN_SCHEME: &str = "shielded-jamtis-seraphis-refresh-rebate-route-plan-root-v1";
pub const ROUTE_SPLIT_SCHEME: &str = "private-refresh-rebate-router-split-commitment-root-v1";
pub const ROUTE_SETTLEMENT_SCHEME: &str =
    "defi-style-private-refresh-rebate-router-settlement-root-v1";
pub const ROUTER_PQ_ATTESTATION_SCHEME: &str =
    "ML-DSA-87+SLH-DSA-SHAKE-256f-jamtis-seraphis-refresh-rebate-router-attestation-v1";
pub const ROUTER_AUDIT_SCHEME: &str = "low-fee-private-refresh-rebate-router-audit-root-v1";
pub const PUBLIC_RECORD_SCHEME: &str =
    "roots-only-jamtis-seraphis-spendlink-decoy-refresh-rebate-router-public-record-v1";
pub const PRIVACY_BOUNDARY: &str =
    "roots_only_no_addresses_view_keys_key_images_amounts_output_indices_viewtags_spendlinks_ring_members_decoy_graphs_solver_identities_route_prices_liquidity_owner_ids_split_witnesses_or_settlement_witnesses";
...
pub struct Config {
    ...
    pub min_routing_quality_bps: u64,
    ...
    pub min_route_split_bps: u64,
    pub max_refresh_units_per_route: u64,
...
impl Config {
    pub fn devnet() -> Self {
        Self {
            ...
            route_asset_id: DEVNET_ROUTE_ASSET_ID.to_string(),
            ...
            min_routing_quality_bps: DEFAULT_MIN_ROUTING_QUALITY_BPS,
            ...
            min_route_split_bps: DEFAULT_MIN_ROUTE_SPLIT_BPS,
            max_refresh_units_per_route: DEFAULT_MAX_REFRESH_UNITS_PER_ROUTE,
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        ensure_bps(self.min_routing_quality_bps, "min routing quality")?;
        ...
        ensure_bps(self.min_route_split_bps, "min route split")?;
        ...
        ensure(self.max_route_hops > 0, "max route hops must be positive")?;
        ensure(
            self.max_refresh_units_per_epoch >= self.max_refresh_units_per_route,
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "route_asset_id": self.route_asset_id,
            ...
            "min_routing_quality_bps": self.min_routing_quality_bps,
            ...
            "min_route_split_bps": self.min_route_split_bps,
            "max_refresh_units_per_route": self.max_refresh_units_per_route,
...
impl Roots {
    pub fn empty(config: &Config, counters: &Counters) -> Self {
        Self {
            ...
            router_pool_root: empty_root("router-pools"),
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "router_pool_root": self.router_pool_root,
...
impl RouteIntentInput {
    pub fn into_entry(self, config: &Config) -> Result<RouteIntentEntry> {
        ...
        ensure_bps(self.max_user_fee_bps, "intent max user fee")?;
            ...
            "intent user fee exceeds router max",
        )?;
        ensure(
...
pub struct RouterPoolInput {
    ...
    pub status: RouterPoolStatus,
}

impl RouterPoolInput {
    pub fn into_entry(self, config: &Config) -> Result<RouterPoolEntry> {
        ensure(!self.pool_id.is_empty(), "router pool id empty")?;
        ensure(
            self.fee_asset_id == config.fee_asset_id,
            "router pool fee asset mismatch",
        )?;
        ensure(
            self.rebate_asset_id == config.rebate_asset_id,
            "router pool rebate asset mismatch",
        )?;
        ensure(
            self.route_asset_id == config.route_asset_id,
            "router pool route asset mismatch",
        )?;
        ensure_bps(self.max_user_fee_bps, "router pool max user fee")?;
        ensure(
            self.max_user_fee_bps <= config.max_user_refresh_fee_bps,
            "router pool user fee above max",
        )?;
        ensure(
            self.rebate_cover_bps >= config.min_rebate_cover_bps,
            "router pool rebate cover below min",
        )?;
        ensure(
            self.solvency_bps >= config.min_rebate_solvency_bps,
            "router pool solvency below min",
        )?;
        ensure(
            self.liquidity_depth_bps >= config.min_liquidity_depth_bps,
            "router pool liquidity depth below min",
        )?;
        ensure(
            self.solver_diversity_bps >= config.min_solver_diversity_bps,
            "router pool solver diversity below min",
        )?;
        Ok(RouterPoolEntry {
...
impl RouterPoolEntry {
    pub fn public_record(&self) -> Value {
        json!({
            "scheme": ROUTER_POOL_SCHEME,
            "pool_id": self.pool_id,
            "provider_bucket": self.provider_bucket,
            "lane": self.lane,
            "fee_asset_id": self.fee_asset_id,
    ...
    pub fn state_root(&self) -> String {
        root_from_record("router-pool", &self.public_record())
    }
}

#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct RouteQuoteInput {
    pub quote_id: String,
    pub pool_id: String,
...
impl RouteQuoteInput {
    pub fn into_entry(self, config: &Config) -> Result<RouteQuoteEntry> {
        ...
        ensure_bps(self.route_rebate_bps, "route quote rebate")?;
            ...
            self.routing_quality_bps >= config.min_routing_quality_bps,
        ...
        Ok(RouteQuoteEntry {
            ...
            rebate_cover_bps: self.rebate_cover_bps,
            routing_quality_bps: self.routing_quality_bps,
            liquidity_depth_bps: self.liquidity_depth_bps,
...
impl RoutePlanInput {
    pub fn into_entry(self, config: &Config) -> Result<RoutePlanEntry> {
            ...
            self.hop_count <= config.max_route_hops,
            "route plan exceeds max hops",
        )?;
        ensure_bps(self.route_weight_bps, "route plan weight")?;
        ensure(
            self.route_weight_bps >= config.min_route_split_bps,
            ...
            self.refresh_unit_bucket <= config.max_refresh_units_per_route,
...
impl RouteSplitEntry {
    pub fn validate(&self, config: &Config) -> Result<()> {
        ...
        ensure_bps(self.split_weight_bps, "route split weight")?;
            ...
            self.split_weight_bps >= config.min_route_split_bps,
...
impl RouteSettlementEntry {
    ...
    pub fn public_record(&self) -> Value {
        json!({
            "scheme": ROUTE_SETTLEMENT_SCHEME,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct PqRouterAttestationEntry {
...
impl PqRouterAttestationEntry {
    ...
    pub fn public_record(&self) -> Value {
        json!({
            "scheme": ROUTER_PQ_ATTESTATION_SCHEME,
    ...
    pub fn state_root(&self) -> String {
        root_from_record("pq-router-attestation", &self.public_record())
    }
}

#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct LowFeeRouterAuditEntry {
    pub audit_id: String,
    pub plan_id: String,
    pub settlement_id: String,
    ...
    pub status: RouterAuditStatus,
}

impl LowFeeRouterAuditEntry {
    pub fn validate(&self, config: &Config) -> Result<()> {
    ...
    pub fn state_root(&self) -> String {
        root_from_record("low-fee-router-audit", &self.public_record())
    }
}
...
pub struct State {
    ...
    pub router_pools: BTreeMap<String, RouterPoolEntry>,
    pub route_quotes: BTreeMap<String, RouteQuoteEntry>,
    pub route_plans: BTreeMap<String, RoutePlanEntry>,
    pub route_splits: BTreeMap<String, RouteSplitEntry>,
    pub route_settlements: BTreeMap<String, RouteSettlementEntry>,
    pub pq_attestations: BTreeMap<String, PqRouterAttestationEntry>,
    pub low_fee_audits: BTreeMap<String, LowFeeRouterAuditEntry>,
...
impl State {
    pub fn new(config: Config, l2_height: u64, monero_height: u64, epoch: u64) -> Result<Self> {
        ...
        Ok(Self {
            ...
            router_pools: BTreeMap::new(),
            route_quotes: BTreeMap::new(),
            route_plans: BTreeMap::new(),
            route_splits: BTreeMap::new(),
            route_settlements: BTreeMap::new(),
    ...
    pub fn devnet() -> Self {
        ...
        .expect("devnet router config is valid");
        state.seed_devnet();
        state
    }

    pub fn insert_route_intent(&mut self, input: RouteIntentInput) -> Result<String> {
    ...
    pub fn insert_router_pool(&mut self, input: RouterPoolInput) -> Result<String> {
        let entry = input.into_entry(&self.config)?;
        ensure(
            !self.router_pools.contains_key(&entry.pool_id),
            "router pool already exists",
        )?;
        let root = entry.state_root();
        ...
        self.router_pools.insert(entry.pool_id.clone(), entry);
        self.refresh_roots();
        Ok(root)
    }

    pub fn insert_route_quote(&mut self, input: RouteQuoteInput) -> Result<String> {
        let entry = input.into_entry(&self.config)?;
        ensure(
            !self.route_quotes.contains_key(&entry.quote_id),
        ...
        self.route_quotes.insert(entry.quote_id.clone(), entry);
        self.refresh_roots();
        Ok(root)
    }

    pub fn insert_route_plan(&mut self, input: RoutePlanInput) -> Result<String> {
        let entry = input.into_entry(&self.config)?;
        ensure(
            !self.route_plans.contains_key(&entry.plan_id),
        ...
        self.route_plans.insert(entry.plan_id.clone(), entry);
        self.refresh_roots();
        Ok(root)
    }
    ...
    pub fn insert_route_split(&mut self, entry: RouteSplitEntry) -> Result<String> {
        entry.validate(&self.config)?;
            ...
            !self.route_splits.contains_key(&entry.split_id),
        ...
        self.route_splits.insert(entry.split_id.clone(), entry);
        self.refresh_roots();
        Ok(root)
    }
    ...
    pub fn insert_route_settlement(&mut self, entry: RouteSettlementEntry) -> Result<String> {
        entry.validate(&self.config)?;
            ...
            !self.route_settlements.contains_key(&entry.settlement_id),
        ...
        self.route_settlements
            .insert(entry.settlement_id.clone(), entry);
        self.refresh_roots();
        Ok(root)
    }

    pub fn insert_pq_attestation(&mut self, entry: PqRouterAttestationEntry) -> Result<String> {
        entry.validate(&self.config)?;
        ensure(
            !self.pq_attestations.contains_key(&entry.attestation_id),
            "router attestation already exists",
        )?;
        let root = entry.state_root();
    ...
    pub fn insert_low_fee_audit(&mut self, entry: LowFeeRouterAuditEntry) -> Result<String> {
        entry.validate(&self.config)?;
        ensure(
            !self.low_fee_audits.contains_key(&entry.audit_id),
            "router audit already exists",
        )?;
        let root = entry.state_root();
        self.counters.audits += 1;
        if entry.status == RouterAuditStatus::Quarantined
            || entry.status == RouterAuditStatus::Regression
        {
    ...
    pub fn expire_height_sensitive_entries(&mut self, monero_height: u64) -> u64 {
        ...
        for quote in self.route_quotes.values_mut() {
            if quote.expires_at_height <= monero_height && quote.status.active() {
                quote.status = RouteQuoteStatus::Expired;
                expired += 1;
            }
        }
        for plan in self.route_plans.values_mut() {
            if plan.expires_at_height <= monero_height
                && matches!(
                    plan.status,
                    RoutePlanStatus::Draft
                        | RoutePlanStatus::Scored
                        | RoutePlanStatus::Reserved
                        | RoutePlanStatus::Split
                        | RoutePlanStatus::Executing
                )
            {
                plan.status = RoutePlanStatus::Expired;
                expired += 1;
            }
        }
        for split in self.route_splits.values_mut() {
    ...
    pub fn route_health_root(&self) -> String {
        let record = json!({
            ...
            "routing_quality_floor_bps": self.config.min_routing_quality_bps,
    ...
    fn seed_devnet(&mut self) {
        let intent_id = "rebate-router-intent-devnet-0".to_string();
        let pool_id = "rebate-router-pool-devnet-0".to_string();
        let quote_id = "rebate-router-quote-devnet-0".to_string();
        let plan_id = "rebate-router-plan-devnet-0".to_string();
        let split_id = "rebate-router-split-devnet-0".to_string();
        let settlement_id = "rebate-router-settlement-devnet-0".to_string();

        self.insert_route_intent(RouteIntentInput {
            intent_id: intent_id.clone(),
            lane: RouterLane::DexSwap,
            epoch: self.epoch,
            monero_height_bucket: bucket(self.monero_height, self.config.public_bucket_size),
        ...
        self.insert_router_pool(RouterPoolInput {
            ...
            pool_commitment_root: root_from_parts("devnet-router-pool", &[HashPart::Str(&pool_id)]),
            liquidity_policy_root: root_from_parts(
                "devnet-router-liquidity-policy",
                &[HashPart::Str(&pool_id)],
            ),
            rebalance_proof_root: root_from_parts(
                "devnet-router-rebalance",
                &[HashPart::Str(&pool_id)],
            ),
            mev_resistance_root: root_from_parts(
                "devnet-router-pool-mev",
                &[HashPart::Str(&pool_id)],
            ),
            status: RouterPoolStatus::Rebalanced,
        })
        .expect("devnet router pool inserts");

        self.insert_route_quote(RouteQuoteInput {
            quote_id: quote_id.clone(),
            pool_id: pool_id.clone(),
            solver_bucket: "devnet-router-solver-bucket-0".to_string(),
        ...
        self.insert_route_plan(RoutePlanInput {
            ...
            quote_root,
            pool_root,
            split_root: root_from_parts("devnet-router-planned-splits", &[HashPart::Str(&plan_id)]),
            hop_count: 3,
            ...
            route_transcript_root: root_from_parts(
                "devnet-router-transcript",
                &[HashPart::Str(&plan_id)],
            ),
            privacy_budget_root: root_from_parts(
                "devnet-router-privacy-budget",
                &[HashPart::Str(&plan_id)],
            ),
            mev_resistance_root: root_from_parts("devnet-router-mev", &[HashPart::Str(&plan_id)]),
            expires_at_height: self.monero_height + self.config.route_plan_ttl_blocks,
            status: RoutePlanStatus::Settled,
        })
        ...
        self.insert_route_settlement(RouteSettlementEntry {
            ...
            intent_id: intent_id.clone(),
            split_root: map_root(
                "devnet-router-settlement-splits",
                [(&split_id as &str, self.route_splits[&split_id].state_root())],
            ),
            pool_root: map_root(
                "devnet-router-settlement-pools",
                [(&pool_id as &str, self.router_pools[&pool_id].state_root())],
            ),
            quote_root: map_root(
                "devnet-router-settlement-quotes",
                [(&quote_id as &str, self.route_quotes[&quote_id].state_root())],
            ),
        ...
        self.insert_pq_attestation(PqRouterAttestationEntry {
            attestation_id: "pq-refresh-rebate-router-attestation-devnet-0".to_string(),
            plan_id: plan_id.clone(),
            settlement_id: settlement_id.clone(),
            signer_set_root: root_from_parts("devnet-router-pq-signers", &[HashPart::Str("0")]),
            pq_transcript_root: root_from_parts(
                "devnet-router-pq-transcript",
                &[HashPart::Str(&settlement_id)],
            ),
            route_integrity_root: root_from_parts(
                "devnet-router-integrity",
                &[HashPart::Str(&settlement_id)],
            ),
            spendlink_absence_root: root_from_parts(
                "devnet-router-spendlink-absence",
                &[HashPart::Str(&settlement_id)],
            ),
        ...
        .expect("devnet pq router attestation inserts");

        self.insert_low_fee_audit(LowFeeRouterAuditEntry {
            audit_id: "refresh-rebate-router-low-fee-audit-devnet-0".to_string(),
            plan_id,
            settlement_id,
            ...
            fee_sample_root: root_from_parts("devnet-router-fee-samples", &[HashPart::Str("0")]),
            rebate_sample_root: root_from_parts(
                "devnet-router-rebate-samples",
                &[HashPart::Str("0")],
            ),
            route_fairness_root: root_from_parts("devnet-router-fairness", &[HashPart::Str("0")]),
            privacy_regression_root: root_from_parts(
                "devnet-router-privacy-regression",
                &[HashPart::Str("0")],
            ),
            accounting_evidence_root: root_from_parts(
                "devnet-router-accounting-evidence",
                &[HashPart::Str("0")],
            ),
            status: RouterAuditStatus::Accepted,
        })
        .expect("devnet low fee router audit inserts");

        self.publish_roots_only_record(
            "roots-only-refresh-rebate-router-public-record-devnet-0",
