...
pub type PrivateL2LowFeePqConfidentialProverBlobFeeRebateAuctionRuntimeResult<T> = Result<T>;
pub type Runtime = State;
pub const PRIVATE_L2_LOW_FEE_PQ_CONFIDENTIAL_PROVER_BLOB_FEE_REBATE_AUCTION_RUNTIME_PROTOCOL_VERSION: &str = "nebula-private-l2-low-fee-pq-confidential-prover-blob-fee-rebate-auction-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_LOW_FEE_PQ_CONFIDENTIAL_PROVER_BLOB_FEE_REBATE_AUCTION_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const SEALED_BUILDER_CREDIT_SUITE: &str = "ML-KEM-1024-sealed-prover-fee-credit-v1";
pub const PQ_BUILDER_ATTESTATION_SUITE: &str =
    "ML-DSA-87+SLH-DSA-SHAKE-256f-prover-blob-fee-netting-v1";
...
pub const LOW_FEE_BATCH_NETTING_SUITE: &str = "private-low-fee-prover-batch-netting-v1";
pub const PUBLIC_RECORD_SUITE: &str =
    "roots-only-no-prover-addresses-no-credit-amounts-no-blob-payloads-v1";
pub const DEVNET_L2_NETWORK: &str = "nebula-monero-private-l2-devnet";
pub const DEVNET_VAULT_ID: &str = "prover-blob-fee-rebate-auction-devnet";
...
impl PublicRecordKind {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::ProverSetRoot => "prover_set_root",
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            max_prover_fee_bps: DEFAULT_MAX_BUILDER_FEE_BPS,
...
impl Config {
    ...
    pub fn public_record(&self) -> Value {
        json!({"chain_id":self.chain_id,"protocol_version":self.protocol_version,"schema_version":SCHEMA_VERSION,"hash_suite":HASH_SUITE,"sealed_prover_credit_suite":SEALED_BUILDER_CREDIT_SUITE,"pq_prover_attestation_suite":PQ_BUILDER_ATTESTATION_SUITE,"blob_proof_bucket_suite":BLOB_PROOF_BUCKET_SUITE,"rebate_coupon_suite":REBATE_COUPON_SUITE,"congestion_smoothing_suite":CONGESTION_SMOOTHING_SUITE,"low_fee_batch_netting_suite":LOW_FEE_BATCH_NETTING_SUITE,"public_record_suite":PUBLIC_RECORD_SUITE,"l2_network":self.l2_network,"vault_id":self.vault_id,"fee_asset_id":self.fee_asset_id,"blob_fee_asset_id":self.blob_fee_asset_id,"proof_fee_asset_id":self.proof_fee_asset_id,"min_privacy_set_size":self.min_privacy_set_size,"target_privacy_set_size":self.target_privacy_set_size,"min_pq_security_bits":self.min_pq_security_bits,"epoch_seconds":self.epoch_seconds,"netting_window_slots":self.netting_window_slots,"coupon_ttl_slots":self.coupon_ttl_slots,"attestation_ttl_slots":self.attestation_ttl_slots,"min_batch_items":self.min_batch_items,"max_batch_items":self.max_batch_items,"max_prover_fee_bps":self.max_prover_fee_bps,"protocol_fee_bps":self.protocol_fee_bps,"target_rebate_bps":self.target_rebate_bps,"smoothing_alpha_bps":self.smoothing_alpha_bps,"congestion_target_bps":self.congestion_target_bps,"congestion_max_multiplier_bps":self.congestion_max_multiplier_bps,"low_fee_floor_micro_units":self.low_fee_floor_micro_units,"blob_byte_price_micro_units":self.blob_byte_price_micro_units,"proof_step_price_micro_units":self.proof_step_price_micro_units,"devnet_l2_height":self.devnet_l2_height,"devnet_epoch":self.devnet_epoch})
    }
...
pub struct LowFeeBatchNetting {
    ...
    pub open_slot: u64,
    pub close_slot: u64,
    pub prover_ids: BTreeSet<String>,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct RegisterProverInput {
    pub prover_id: String,
    pub pq_key_commitment: String,
    pub sealed_credit_root: String,
    pub privacy_set_size: u64,
...
pub struct State {
    pub config: Config,
    pub provers: BTreeMap<String, ProverProfile>,
...
impl State {
    ...
    pub fn counters(&self) -> Counters {
        Counters {
            provers: self.provers.len() as u64,
    ...
    pub fn register_prover(&mut self, input: RegisterProverInput) -> Result<String> {
        if self.provers.contains_key(&input.prover_id) {
            return Err(format!("prover already registered: {}", input.prover_id));
        }
        if input.privacy_set_size < self.config.min_privacy_set_size {
            return Err("prover privacy set below minimum".to_string());
        }
        if input.pq_security_bits < self.config.min_pq_security_bits {
            return Err("prover pq security below minimum".to_string());
        }
        if input.max_fee_bps > self.config.max_prover_fee_bps {
            return Err("prover fee cap exceeds vault maximum".to_string());
        }
        let p = ProverProfile {
            prover_id: input.prover_id.clone(),
        ...
        self.provers.insert(input.prover_id, p);
        Ok(root)
    }
    pub fn seal_prover_fee_credit(&mut self, input: SealCreditInput) -> Result<String> {
        let b = self
            .provers
            .get(&input.prover_id)
            .ok_or_else(|| format!("unknown prover: {}", input.prover_id))?;
        if !b.status.accepts_flow() {
            return Err("prover cannot seal credits while inactive".to_string());
        }
        ...
        let c = SealedProverFeeCredit {
            credit_id: input.credit_id.clone(),
            prover_id: input.prover_id,
            ...
            low_fee_hint_bps: input.low_fee_hint_bps.min(self.config.max_prover_fee_bps),
        };
    ...
    pub fn open_cost_bucket(&mut self, input: OpenCostBucketInput) -> Result<String> {
        ...
        if !self.provers.contains_key(&input.prover_id) {
            return Err(format!("unknown prover: {}", input.prover_id));
        }
    ...
    pub fn submit_pq_prover_attestation(
        &mut self,
        input: SubmitAttestationInput,
    ) -> Result<String> {
        let b = self
            .provers
            .get_mut(&input.prover_id)
            .ok_or_else(|| format!("unknown prover: {}", input.prover_id))?;
    ...
    pub fn propose_low_fee_batch(&mut self, input: ProposeBatchInput) -> Result<String> {
        ...
        for id in &input.prover_ids {
            if !self.provers.contains_key(id) {
                return Err(format!("unknown prover in batch: {}", id));
            }
        }
        let mut gross = 0u128;
        ...
        let transcript = record_root(
            D_BATCHES,
            &json!({"batch_id":input.batch_id,"provers":input.prover_ids,"credits":input.credit_ids,"buckets":input.bucket_ids,"gross":gross,"netted":netted,"rebate":rebate}),
        );
    ...
    pub fn finalize_batch_netting(&mut self, batch_id: &str) -> Result<String> {
        ...
        if batch.attestation_ids.is_empty() {
            return Err("batch requires pq prover attestation".to_string());
        }
    ...
    pub fn roots(&self) -> Roots {
        Roots {
            ...
            prover_root: map_root(D_BUILDERS, &self.provers),
...
pub fn devnet() -> State {
    ...
    tags.insert("devnet-prover".to_string());
    tags.insert("monero-private-l2".to_string());
    let _ = state.register_prover(RegisterProverInput {
        prover_id: "prover-devnet-alpha".to_string(),
    ...
    let _ = state.seal_prover_fee_credit(SealCreditInput {
    ...
    let prover_ids = BTreeSet::from(["prover-devnet-alpha".to_string()]);
    ...
    let _ = state.submit_pq_prover_attestation(SubmitAttestationInput {
