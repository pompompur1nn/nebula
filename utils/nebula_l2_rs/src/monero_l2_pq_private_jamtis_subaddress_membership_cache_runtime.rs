...
pub type Result<T> = MoneroL2PqPrivateJamtisSubaddressMembershipCacheRuntimeResult<T>;
pub type MoneroL2PqPrivateJamtisSubaddressMembershipCacheRuntimeResult<T> =
    std::result::Result<T, String>;
pub type Runtime = State;

pub const MONERO_L2_PQ_PRIVATE_JAMTIS_SUBADDRESS_MEMBERSHIP_CACHE_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-monero-l2-pq-private-jamtis-subaddress-membership-cache-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    MONERO_L2_PQ_PRIVATE_JAMTIS_SUBADDRESS_MEMBERSHIP_CACHE_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const JAMTIS_SUBADDRESS_COHORT_SCHEME: &str =
    "monero-jamtis-subaddress-cohort-commitment-root-v1";
pub const JAMTIS_SUBADDRESS_MEMBERSHIP_CACHE_SCHEME: &str =
    "monero-jamtis-subaddress-membership-cache-root-v1";
pub const OUTPUT_MEMBERSHIP_PROOF_HINT_SCHEME: &str =
    "view-key-safe-jamtis-output-membership-proof-hint-root-v1";
pub const DECOY_AGE_BUCKET_SCHEME: &str = "monero-jamtis-decoy-age-bucket-root-v1";
pub const PQ_CACHE_AUDITOR_ATTESTATION_SCHEME: &str =
    "ML-DSA-87+SLH-DSA-SHAKE-256f-jamtis-cache-auditor-attestation-v1";
...
pub const PRIVACY_REDACTION_BUDGET_SCHEME: &str =
    "jamtis-subaddress-membership-cache-redaction-budget-root-v1";
pub const LOW_FEE_CACHE_REBATE_SCHEME: &str =
    "low-fee-private-jamtis-subaddress-membership-cache-rebate-root-v1";
pub const PUBLIC_RECORD_SCHEME: &str =
    "deterministic-public-jamtis-subaddress-membership-cache-record-v1";
pub const PRIVACY_BOUNDARY: &str =
    "roots_only_no_plaintext_subaddresses_view_keys_key_images_amounts_output_indices_or_wallet_graphs";
pub const DEVNET_L2_NETWORK: &str = "nebula-private-l2-devnet";
pub const DEVNET_MONERO_NETWORK: &str = "monero-devnet";
pub const DEVNET_FEE_ASSET_ID: &str = "piconero-devnet";
pub const DEVNET_L2_HEIGHT: u64 = 3_084_000;
pub const DEVNET_MONERO_HEIGHT: u64 = 3_760_000;
pub const DEVNET_EPOCH: u64 = 15_360;
pub const MAX_BPS: u64 = 10_000;
pub const DEFAULT_MIN_RING_SIZE: u16 = 16;
pub const DEFAULT_TARGET_RING_SIZE: u16 = 96;
pub const DEFAULT_MIN_COHORT_SUBADDRESSES: u64 = 65_536;
pub const DEFAULT_TARGET_COHORT_SUBADDRESSES: u64 = 262_144;
...
pub struct Config {
    ...
    pub min_cohort_subaddresses: u64,
    pub target_cohort_subaddresses: u64,
...
impl Config {
    pub fn devnet() -> Self {
        Self {
            ...
            min_cohort_subaddresses: DEFAULT_MIN_COHORT_SUBADDRESSES,
            target_cohort_subaddresses: DEFAULT_TARGET_COHORT_SUBADDRESSES,
    ...
    pub fn validate(&self) -> Result<()> {
            ...
            self.target_cohort_subaddresses >= self.min_cohort_subaddresses,
            "target subaddress cohort must cover minimum",
        )?;
        ensure(
            self.target_cohort_outputs >= self.min_cohort_outputs,
            "target output cohort must cover minimum",
        )?;
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "min_cohort_subaddresses": self.min_cohort_subaddresses,
            "target_cohort_subaddresses": self.target_cohort_subaddresses,
    ...
    pub fn state_root(&self) -> String {
        root_from_record(
            "jamtis-subaddress-membership-cache-config",
            &self.public_record(),
        )
    }
}
...
pub struct Counters {
    ...
    pub output_membership_hints: u64,
...
impl Counters {
    ...
    pub fn state_root(&self) -> String {
        root_from_record(
            "jamtis-subaddress-membership-cache-counters",
            &self.public_record(),
        )
    }
}
...
pub struct Roots {
    pub subaddress_cohorts_root: String,
    pub membership_caches_root: String,
    pub output_membership_hints_root: String,
...
impl Roots {
    pub fn empty() -> Self {
        Self {
            subaddress_cohorts_root: empty_root(JAMTIS_SUBADDRESS_COHORT_SCHEME),
            membership_caches_root: empty_root(JAMTIS_SUBADDRESS_MEMBERSHIP_CACHE_SCHEME),
            output_membership_hints_root: empty_root(OUTPUT_MEMBERSHIP_PROOF_HINT_SCHEME),
            decoy_age_buckets_root: empty_root(DECOY_AGE_BUCKET_SCHEME),
            pq_attestations_root: empty_root(PQ_CACHE_AUDITOR_ATTESTATION_SCHEME),
            ...
            deterministic_state_root: empty_root("jamtis-subaddress-membership-cache-state"),
        }
    }
...
pub struct SubaddressCohort {
    ...
    pub subaddress_count_bucket: u64,
...
impl SubaddressCohort {
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "subaddress_count_bucket": self.subaddress_count_bucket,
            "output_count_bucket": self.output_count_bucket,
            "subaddress_set_root": self.subaddress_set_root,
    ...
    pub fn state_root(&self) -> String {
        root_from_record("jamtis-subaddress-cohort", &self.public_record())
    }
}
...
pub struct MembershipCache {
    ...
    pub cache_transcript_root: String,
...
impl MembershipCache {
    ...
    pub fn state_root(&self) -> String {
        root_from_record("jamtis-subaddress-membership-cache", &self.public_record())
    }
}

#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct OutputMembershipProofHint {
    pub hint_id: String,
    pub cohort_id: String,
    pub cache_id: String,
    pub ring_size: u16,
...
impl OutputMembershipProofHint {
    pub fn public_record(&self) -> Value {
        json!(self)
    }

    pub fn state_root(&self) -> String {
        root_from_record("jamtis-output-membership-proof-hint", &self.public_record())
    }
}
...
impl DecoyAgeBucket {
    ...
    pub fn state_root(&self) -> String {
        root_from_record("jamtis-decoy-age-bucket", &self.public_record())
    }
}

#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct PqCacheAuditorAttestation {
...
impl PqCacheAuditorAttestation {
    pub fn public_record(&self) -> Value {
        json!(self)
    }

    pub fn state_root(&self) -> String {
        root_from_record("jamtis-pq-cache-auditor-attestation", &self.public_record())
    }
}
...
impl WalletScanHint {
    ...
    pub fn state_root(&self) -> String {
        root_from_record("jamtis-wallet-scan-hint", &self.public_record())
    }
}
...
impl PrivacyRedactionBudget {
    ...
    pub fn state_root(&self) -> String {
        root_from_record("jamtis-privacy-redaction-budget", &self.public_record())
    }
}
...
impl LowFeeCacheRebate {
    ...
    pub fn state_root(&self) -> String {
        root_from_record("jamtis-low-fee-cache-rebate", &self.public_record())
    }
}
...
pub struct State {
    ...
    pub membership_caches: BTreeMap<String, MembershipCache>,
    pub output_membership_hints: BTreeMap<String, OutputMembershipProofHint>,
    pub decoy_age_buckets: BTreeMap<String, DecoyAgeBucket>,
    pub pq_attestations: BTreeMap<String, PqCacheAuditorAttestation>,
...
impl State {
    pub fn new(config: Config, l2_height: u64, monero_height: u64, epoch: u64) -> Result<Self> {
        ...
        let mut state = Self {
            ...
            output_membership_hints: BTreeMap::new(),
    ...
    pub fn public_record(&self) -> Value {
        json!({
            "runtime": "monero_l2_pq_private_jamtis_subaddress_membership_cache_runtime",
    ...
    pub fn insert_subaddress_cohort(&mut self, cohort: SubaddressCohort) -> Result<()> {
        ensure(
            cohort.subaddress_count_bucket >= self.config.min_cohort_subaddresses,
            "subaddress cohort is below configured privacy floor",
        )?;
        ensure(
            cohort.output_count_bucket >= self.config.min_cohort_outputs,
            "output cohort is below configured privacy floor",
        )?;
        self.subaddress_cohorts
            .insert(cohort.cohort_id.clone(), cohort);
        self.counters.subaddress_cohorts = self.subaddress_cohorts.len() as u64;
        self.refresh_roots();
        Ok(())
    }

    pub fn insert_membership_cache(&mut self, cache: MembershipCache) -> Result<()> {
        ensure(
            self.subaddress_cohorts.contains_key(&cache.cohort_id),
    ...
    pub fn insert_output_membership_hint(&mut self, hint: OutputMembershipProofHint) -> Result<()> {
        ensure(
            hint.ring_size >= self.config.min_ring_size,
            "output membership hint ring size below minimum",
        )?;
        ensure(
            self.membership_caches.contains_key(&hint.cache_id),
            "output membership hint references unknown cache",
        )?;
        ensure(
            self.decoy_age_buckets
                .contains_key(&hint.decoy_age_bucket_id),
            "output membership hint references unknown decoy bucket",
        )?;
        self.output_membership_hints
            .insert(hint.hint_id.clone(), hint);
        self.counters.output_membership_hints = self.output_membership_hints.len() as u64;
        self.refresh_roots();
        Ok(())
    }
    ...
    pub fn insert_decoy_age_bucket(&mut self, bucket: DecoyAgeBucket) -> Result<()> {
            ...
            "decoy age bucket upper bound must cover lower bound",
        )?;
        self.decoy_age_buckets
    ...
    pub fn insert_pq_attestation(&mut self, attestation: PqCacheAuditorAttestation) -> Result<()> {
        ensure(
            attestation.pq_security_bits >= self.config.min_pq_security_bits,
            "PQ auditor attestation security below minimum",
        )?;
        ensure(
            self.membership_caches.contains_key(&attestation.cache_id),
            "PQ auditor attestation references unknown cache",
        )?;
    ...
    pub fn insert_wallet_scan_hint(&mut self, hint: WalletScanHint) -> Result<()> {
            ...
            "wallet scan hint redaction units exceed budget",
        )?;
        ensure(
            self.subaddress_cohorts.contains_key(&hint.cohort_id),
            "wallet scan hint references unknown cohort",
        )?;
    ...
    pub fn insert_redaction_budget(&mut self, budget: PrivacyRedactionBudget) -> Result<()> {
            ...
            "redaction budget reservation exceeds total",
        )?;
    ...
    pub fn refresh_roots(&mut self) {
        self.roots.subaddress_cohorts_root = map_root(
            JAMTIS_SUBADDRESS_COHORT_SCHEME,
            self.subaddress_cohorts
                .iter()
                .map(|(id, cohort)| (id.as_str(), cohort.state_root())),
        );
        self.roots.membership_caches_root = map_root(
            JAMTIS_SUBADDRESS_MEMBERSHIP_CACHE_SCHEME,
            self.membership_caches
                .iter()
                .map(|(id, cache)| (id.as_str(), cache.state_root())),
        );
        self.roots.output_membership_hints_root = map_root(
            OUTPUT_MEMBERSHIP_PROOF_HINT_SCHEME,
            self.output_membership_hints
                .iter()
        ...
        self.roots.pq_attestations_root = map_root(
            PQ_CACHE_AUDITOR_ATTESTATION_SCHEME,
            self.pq_attestations
                .iter()
    ...
    fn state_root_without_cached_roots(&self) -> String {
        root_from_parts(
            "jamtis-subaddress-membership-cache-state",
            &[
                ...
                HashPart::Str(&self.roots.subaddress_cohorts_root),
                HashPart::Str(&self.roots.membership_caches_root),
                HashPart::Str(&self.roots.output_membership_hints_root),
...
pub fn devnet() -> State {
    ...
    .expect("devnet JAMTIS subaddress membership cache config is valid");

    let cohort_id = "jamtis-subaddress-cohort-devnet-0".to_string();
    let cache_id = "jamtis-subaddress-membership-cache-devnet-0".to_string();
    let bucket_id = "jamtis-decoy-age-bucket-devnet-0".to_string();
    state
        .insert_subaddress_cohort(SubaddressCohort {
            cohort_id: cohort_id.clone(),
            cohort_kind: CohortKind::BridgeWithdrawal,
            epoch: DEVNET_EPOCH,
            monero_height: DEVNET_MONERO_HEIGHT,
            subaddress_count_bucket: DEFAULT_TARGET_COHORT_SUBADDRESSES,
            output_count_bucket: DEFAULT_TARGET_COHORT_OUTPUTS,
            subaddress_set_root: root_from_parts(
                "devnet-jamtis-subaddress-set",
                &[HashPart::Str(&cohort_id)],
            ),
            output_membership_root: root_from_parts(
                "devnet-jamtis-output-membership",
                &[HashPart::Str(&cohort_id)],
            ),
            status: CacheStatus::Warm,
        })
        .expect("devnet JAMTIS cohort inserts");
    state
        .insert_decoy_age_bucket(DecoyAgeBucket {
            bucket_id: bucket_id.clone(),
            lower_age_blocks: 720,
            upper_age_blocks: 10_080,
            output_count_bucket: DEFAULT_TARGET_COHORT_OUTPUTS,
            decoy_distribution_root: root_from_parts(
                "devnet-jamtis-decoy-distribution",
                &[HashPart::Str(&bucket_id)],
            ),
        })
        .expect("devnet JAMTIS decoy bucket inserts");
    state
        .insert_membership_cache(MembershipCache {
            cache_id: cache_id.clone(),
            cohort_id: cohort_id.clone(),
            membership_tree_root: root_from_parts(
                "devnet-jamtis-membership-tree",
                &[
                    HashPart::Str(&cohort_id),
                    HashPart::U64(DEVNET_MONERO_HEIGHT),
                ],
            ),
            cache_transcript_root: root_from_parts(
                "devnet-jamtis-cache-transcript",
                &[HashPart::Str(&cache_id)],
            ),
            proof_hint_root: root_from_parts(
                "devnet-jamtis-proof-hints",
                &[HashPart::Str(&cache_id)],
            ),
            expires_at_monero_height: DEVNET_MONERO_HEIGHT + DEFAULT_CACHE_TTL_BLOCKS,
            status: CacheStatus::Attested,
        })
        .expect("devnet JAMTIS membership cache inserts");
    state
        .insert_output_membership_hint(OutputMembershipProofHint {
            hint_id: "jamtis-output-membership-proof-hint-devnet-0".to_string(),
            cohort_id: cohort_id.clone(),
            cache_id: cache_id.clone(),
            ring_size: DEFAULT_TARGET_RING_SIZE,
            decoy_age_bucket_id: bucket_id,
            redacted_hint_root: root_from_parts(
                "devnet-jamtis-redacted-output-hint",
                &[HashPart::Str("0")],
            ),
            wallet_scan_hint_root: root_from_parts(
                "devnet-jamtis-wallet-scan-hint-link",
                &[HashPart::Str("0")],
            ),
            status: HintStatus::Published,
        })
        .expect("devnet JAMTIS output membership hint inserts");
    state
        .insert_pq_attestation(PqCacheAuditorAttestation {
            attestation_id: "jamtis-pq-cache-auditor-attestation-devnet-0".to_string(),
            ...
            attestation_root: root_from_parts(
                "devnet-jamtis-pq-cache-auditor-attestation",
                &[HashPart::Str(&cache_id)],
            ),
            status: AttestationStatus::Quorum,
        })
        .expect("devnet JAMTIS PQ cache auditor attestation inserts");
    state
        .insert_wallet_scan_hint(WalletScanHint {
            hint_id: "jamtis-wallet-scan-hint-devnet-0".to_string(),
            cohort_id: cohort_id.clone(),
            scan_window_root: root_from_parts(
                "devnet-jamtis-scan-window",
                &[HashPart::Str(&cohort_id)],
            ),
            view_tag_bucket_root: root_from_parts(
                "devnet-jamtis-view-tag-bucket",
                &[HashPart::Str(&cohort_id)],
            ),
            expires_at_monero_height: DEVNET_MONERO_HEIGHT + DEFAULT_HINT_TTL_BLOCKS,
            redaction_units: 10,
        })
        .expect("devnet JAMTIS wallet scan hint inserts");
    state
        .insert_redaction_budget(PrivacyRedactionBudget {
            budget_id: "jamtis-redaction-budget-devnet-0".to_string(),
        ...
        .expect("devnet JAMTIS redaction budget inserts");
    state
        .insert_low_fee_rebate(LowFeeCacheRebate {
            rebate_id: "jamtis-low-fee-cache-rebate-devnet-0".to_string(),
            cache_id,
            cohort_id,
            fee_asset_id: DEVNET_FEE_ASSET_ID.to_string(),
            fee_rebate_amount_bucket: 16_000,
            sponsor_receipt_root: root_from_parts(
                "devnet-jamtis-low-fee-sponsor-receipt",
                &[HashPart::Str("0")],
            ),
        })
        .expect("devnet JAMTIS low-fee rebate inserts");
