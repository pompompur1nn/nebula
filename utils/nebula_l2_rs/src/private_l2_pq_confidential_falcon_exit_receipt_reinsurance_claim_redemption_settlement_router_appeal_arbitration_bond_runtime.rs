...
pub type PrivateL2PqConfidentialFalconExitReceiptReinsuranceClaimRedemptionSettlementRouterAppealArbitrationBondRuntimeResult<
    T,
> = Result<T>;
pub type Runtime = State;

pub const PRIVATE_L2_PQ_CONFIDENTIAL_FALCON_EXIT_RECEIPT_REINSURANCE_CLAIM_REDEMPTION_SETTLEMENT_ROUTER_APPEAL_ARBITRATION_BOND_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-private-l2-pq-confidential-falcon-exit-receipt-reinsurance-claim-redemption-settlement-router-appeal-arbitration-bond-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_PQ_CONFIDENTIAL_FALCON_EXIT_RECEIPT_REINSURANCE_CLAIM_REDEMPTION_SETTLEMENT_ROUTER_APPEAL_ARBITRATION_BOND_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const FALCON_ROUTER_APPEAL_ARBITRATION_BOND_SETTLEMENT_SUITE: &str =
    "falcon-1024-ntru-exit-receipt-reinsurance-claim-redemption-settlement-router-appeal-arbitration-bond-v1";
pub const CONFIDENTIAL_EXIT_RECEIPT_SUITE: &str =
    "falcon-confidential-exit-receipt-router-appeal-arbitration-commitment-v1";
pub const CLAIM_REDEMPTION_AUTH_SUITE: &str =
    "falcon-1024-private-claim-redemption-router-appeal-arbitration-authorization-v1";
pub const REINSURANCE_SETTLEMENT_ACCOUNTING_SUITE: &str =
    "roots-only-falcon-reinsurance-claim-redemption-router-appeal-arbitration-accounting-v1";
pub const LOW_FEE_SETTLEMENT_BATCH_SUITE: &str =
    "low-fee-falcon-claim-redemption-router-appeal-arbitration-batch-v1";
...
pub const ROOTS_ONLY_PUBLIC_RECORD_SUITE: &str =
    "roots-only-falcon-claim-redemption-settlement-router-appeal-arbitration-public-record-v1";
...
pub const DEFAULT_MIN_ARBITRATORS: u16 = 7;
pub const DEFAULT_ARBITRATOR_THRESHOLD_BPS: u16 = 6_700;
pub const DEFAULT_EVIDENCE_SCORE_THRESHOLD: u16 = 700;
pub const DEFAULT_REINSURANCE_CHALLENGE_PACKET_LIMIT: u16 = 64;
pub const DEFAULT_MIN_ARBITRATOR_BOND_ATOMIC: u64 = 18_000_000;
pub const DEFAULT_MAX_ARBITRATOR_BOND_ATOMIC: u64 = 360_000_000;
pub const DEFAULT_BOND_REBATE_BPS: u16 = 2_500;
pub const DEFAULT_BOND_SLASH_BPS: u16 = 7_500;
...
pub enum SettlementAccountingSide {
    ...
    ProtocolFee,
    BatchFee,
    ArbitratorBondEscrow,
    ArbitratorBondSlash,
    ArbitratorBondRefund,
    LowFeeBondRebate,
}
...
impl SettlementAccountingSide {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ArbitratorBondEscrow => "arbitrator_bond_escrow",
            Self::ArbitratorBondSlash => "arbitrator_bond_slash",
            Self::ArbitratorBondRefund => "arbitrator_bond_refund",
            Self::LowFeeBondRebate => "low_fee_bond_rebate",
        }
    }
}
...
impl AppealStatus {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::Routed => "routed",
            Self::ArbitratorQuorumMet => "arbitrator_quorum_met",
...
pub enum AppealDecision {
    UpholdClaimant,
    UpholdRouter,
    RemandForReinsuranceArbitration,
...
impl AppealDecision {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::RemandForReinsuranceArbitration => "remand_for_reinsurance_arbitration",
...
impl AppealRouteKind {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::FastPqArbitration => "fast_pq_arbitration",
...
pub struct Config {
    ...
    pub falcon_router_appeal_arbitration_bond_settlement_suite: String,
    ...
    pub min_arbitrators: u16,
    pub arbitrator_threshold_bps: u16,
    pub evidence_score_threshold: u16,
    pub reinsurance_challenge_packet_limit: u16,
    pub min_arbitrator_bond_atomic: u64,
    pub max_arbitrator_bond_atomic: u64,
    pub bond_rebate_bps: u16,
...
impl Config {
    pub fn devnet() -> Self {
        Self {
            ...
            falcon_router_appeal_arbitration_bond_settlement_suite:
                FALCON_ROUTER_APPEAL_ARBITRATION_BOND_SETTLEMENT_SUITE.to_string(),
            ...
            min_arbitrators: DEFAULT_MIN_ARBITRATORS,
            arbitrator_threshold_bps: DEFAULT_ARBITRATOR_THRESHOLD_BPS,
            evidence_score_threshold: DEFAULT_EVIDENCE_SCORE_THRESHOLD,
            reinsurance_challenge_packet_limit: DEFAULT_REINSURANCE_CHALLENGE_PACKET_LIMIT,
            min_arbitrator_bond_atomic: DEFAULT_MIN_ARBITRATOR_BOND_ATOMIC,
            max_arbitrator_bond_atomic: DEFAULT_MAX_ARBITRATOR_BOND_ATOMIC,
            bond_rebate_bps: DEFAULT_BOND_REBATE_BPS,
            bond_slash_bps: DEFAULT_BOND_SLASH_BPS,
    ...
    pub fn validate(&self) -> Result<()> {
        if self.min_pq_security_bits < DEFAULT_MIN_PQ_SECURITY_BITS {
            return Err(
                "pq security bits below falcon-router appeal arbitration bond settlement minimum"
                    .to_string(),
            );
        }
        ...
        {
            return Err(
                "invalid falcon-router appeal arbitration bond settlement parameter schedule"
                    .to_string(),
            );
        }
        ...
        if self.min_arbitrators < 3
            || self.arbitrator_threshold_bps < 5_000
            || self.arbitrator_threshold_bps > 10_000
            || self.evidence_score_threshold == 0
            || self.evidence_score_threshold > 1_000
            || self.reinsurance_challenge_packet_limit == 0
        {
            return Err("invalid pq appeal arbitration quorum policy".to_string());
        }
        if self.min_arbitrator_bond_atomic == 0
            || self.max_arbitrator_bond_atomic < self.min_arbitrator_bond_atomic
            || self.max_arbitrator_bond_atomic > self.settlement_escrow_atomic
            || self.bond_rebate_bps > 10_000
            || self.bond_slash_bps > 10_000
            || self.bond_rebate_bps + self.bond_slash_bps > 10_000
        {
            return Err("invalid pq arbitrator bond escrow policy".to_string());
        }
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "suites": {
                "falcon_router_appeal_settlement": self.falcon_router_appeal_arbitration_bond_settlement_suite,
            ...
            "arbitration": {
                "min_arbitrators": self.min_arbitrators,
                "arbitrator_threshold_bps": self.arbitrator_threshold_bps,
                "evidence_score_threshold": self.evidence_score_threshold,
                "reinsurance_challenge_packet_limit": self.reinsurance_challenge_packet_limit,
                "min_arbitrator_bond_atomic": self.min_arbitrator_bond_atomic,
                "max_arbitrator_bond_atomic": self.max_arbitrator_bond_atomic,
                "bond_rebate_bps": self.bond_rebate_bps,
                "bond_slash_bps": self.bond_slash_bps,
            },
            "privacy": {
...
pub struct Roots {
    ...
    pub arbitrator_quorum_root: String,
...
impl Default for Roots {
    fn default() -> Self {
        ...
        Self {
            ...
            arbitrator_quorum_root: empty.clone(),
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct AppealArbitratorQuorumInput {
    pub appeal_id: String,
    pub arbitrator_set_root: String,
    pub arbitrator_pq_key_set_root: String,
    pub arbitrator_bond_escrow_root: String,
    pub arbitrator_bond_atomic: u64,
    pub vote_commitment_root: String,
    pub quorum_certificate_root: String,
    pub falcon_quorum_signature_root: String,
    pub mldsa_quorum_signature_root: String,
    pub slhdsa_quorum_signature_root: String,
...
pub struct AppealEvidenceInput {
    pub appeal_id: String,
    pub confidential_evidence_bundle_root: String,
    pub reinsurance_claim_evidence_root: String,
    pub reinsurance_challenge_packet_root: String,
    pub challenge_packet_count: u16,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct AppealArbitratorQuorum {
...
impl AppealArbitratorQuorum {
    pub fn from_input(config: &Config, input: AppealArbitratorQuorumInput) -> Result<Self> {
        require_non_empty(&[
            (&input.appeal_id, "appeal id"),
            (&input.arbitrator_set_root, "arbitrator set root"),
            (
                &input.arbitrator_pq_key_set_root,
                "arbitrator pq key set root",
            ),
            (
                &input.arbitrator_bond_escrow_root,
                "arbitrator bond escrow root",
            ),
            (&input.vote_commitment_root, "vote commitment root"),
            (&input.quorum_certificate_root, "quorum certificate root"),
            (
                &input.falcon_quorum_signature_root,
                "falcon quorum signature root",
            ),
            (
                &input.mldsa_quorum_signature_root,
                "ml-dsa quorum signature root",
            ),
            (
                &input.slhdsa_quorum_signature_root,
                "slh-dsa quorum signature root",
            ),
        ])?;
        if input.threshold == 0
            || input.arbitrators < input.threshold
            || input.arbitrators < config.min_arbitrators
            || u32::from(input.threshold) * 10_000
                < u32::from(input.arbitrators) * u32::from(config.arbitrator_threshold_bps)
        {
            return Err("appeal arbitrator quorum threshold not met".to_string());
        }
        if input.pq_security_bits < config.min_pq_security_bits {
            return Err("appeal arbitrator quorum below configured pq security".to_string());
        }
        if input.arbitrator_bond_atomic < config.min_arbitrator_bond_atomic
            || input.arbitrator_bond_atomic > config.max_arbitrator_bond_atomic
        {
            return Err("arbitrator bond escrow outside configured policy".to_string());
        }
        let quorum_id = deterministic_id(
            "appeal-arbitrator-quorum",
            &[
                HashPart::Str(&input.appeal_id),
                HashPart::Str(&input.arbitrator_set_root),
                HashPart::Str(&input.arbitrator_pq_key_set_root),
                HashPart::Str(&input.arbitrator_bond_escrow_root),
        ...
        Ok(Self {
            ...
            arbitrator_set_root: input.arbitrator_set_root,
            arbitrator_pq_key_set_root: input.arbitrator_pq_key_set_root,
            arbitrator_bond_escrow_root: input.arbitrator_bond_escrow_root,
            arbitrator_bond_atomic: input.arbitrator_bond_atomic,
            vote_commitment_root: input.vote_commitment_root,
            quorum_certificate_root: input.quorum_certificate_root,
            falcon_quorum_signature_root: input.falcon_quorum_signature_root,
            mldsa_quorum_signature_root: input.mldsa_quorum_signature_root,
            slhdsa_quorum_signature_root: input.slhdsa_quorum_signature_root,
            decision: input.decision,
            arbitrators: input.arbitrators,
    ...
    pub fn roots_only_record(&self) -> Value {
        json!({
            ...
            "appeal_id": self.appeal_id,
            "arbitrator_set_root": self.arbitrator_set_root,
            "arbitrator_pq_key_set_root": self.arbitrator_pq_key_set_root,
            "arbitrator_bond_escrow_root": self.arbitrator_bond_escrow_root,
            "arbitrator_bond_commitment_root": value_root("arbitrator-bond-escrow", &json!({
                "quorum_id": self.quorum_id,
                "arbitrator_bond_atomic": self.arbitrator_bond_atomic,
            })),
            "vote_commitment_root": self.vote_commitment_root,
            "quorum_certificate_root": self.quorum_certificate_root,
            "falcon_quorum_signature_root": self.falcon_quorum_signature_root,
            "mldsa_quorum_signature_root": self.mldsa_quorum_signature_root,
            "slhdsa_quorum_signature_root": self.slhdsa_quorum_signature_root,
...
impl AppealEvidence {
    pub fn from_input(config: &Config, input: AppealEvidenceInput) -> Result<Self> {
        require_non_empty(&[
            (&input.appeal_id, "appeal id"),
            (
                &input.confidential_evidence_bundle_root,
                "confidential evidence bundle root",
            ),
            (
            ...
            (
                &input.reinsurance_challenge_packet_root,
                "reinsurance challenge packet root",
            ),
        ...
        if input.challenge_packet_count == 0
            || input.challenge_packet_count > config.reinsurance_challenge_packet_limit
        {
            return Err(
                "reinsurance challenge packet count outside arbitration policy".to_string(),
            );
        }
        if input.evidence_score < config.evidence_score_threshold || input.evidence_score > 1_000 {
            return Err(
                "confidential appeal evidence score below arbitration threshold".to_string(),
            );
        }
        let evidence_id = deterministic_id(
            ...
            &[
                HashPart::Str(&input.appeal_id),
                HashPart::Str(&input.confidential_evidence_bundle_root),
                HashPart::Str(&input.reinsurance_claim_evidence_root),
                HashPart::Str(&input.reinsurance_challenge_packet_root),
        ...
        Ok(Self {
            evidence_id,
            appeal_id: input.appeal_id,
            confidential_evidence_bundle_root: input.confidential_evidence_bundle_root,
            reinsurance_claim_evidence_root: input.reinsurance_claim_evidence_root,
            reinsurance_challenge_packet_root: input.reinsurance_challenge_packet_root,
            challenge_packet_count: input.challenge_packet_count,
    ...
    pub fn roots_only_record(&self) -> Value {
        json!({
            ...
            "appeal_id": self.appeal_id,
            "confidential_evidence_bundle_root": self.confidential_evidence_bundle_root,
            "reinsurance_claim_evidence_root": self.reinsurance_claim_evidence_root,
            "reinsurance_challenge_packet_root": self.reinsurance_challenge_packet_root,
            "challenge_packet_count": self.challenge_packet_count,
            ...
            "privacy_redaction_root": self.privacy_redaction_root,
            "evidence_score_commitment_root": value_root("appeal-evidence-score", &json!({
                "evidence_id": self.evidence_id,
...
pub struct State {
    ...
    pub appeal_arbitrator_quorums: BTreeMap<String, AppealArbitratorQuorum>,
...
impl State {
    pub fn new(config: Config, height: u64, epoch: u64, slot: u64) -> Result<Self> {
        ...
        let mut state = Self {
            ...
            appeal_arbitrator_quorums: BTreeMap::new(),
    ...
    pub fn devnet() -> Self {
        ...
        state
            .anchor_appeal_evidence(AppealEvidenceInput {
                ...
                confidential_evidence_bundle_root: sample_root("appeal-confidential-evidence", 1),
                reinsurance_claim_evidence_root: sample_root("appeal-reinsurance-evidence", 1),
                reinsurance_challenge_packet_root: sample_root(
                    "appeal-reinsurance-challenge-packet",
                    1,
                ),
                challenge_packet_count: 4,
                ...
                evidence_score: DEFAULT_EVIDENCE_SCORE_THRESHOLD,
        ...
        state
            .record_arbitrator_quorum(AppealArbitratorQuorumInput {
                appeal_id: appeal_id.clone(),
                arbitrator_set_root: sample_root("appeal-arbitrators", 1),
                arbitrator_pq_key_set_root: sample_root("appeal-arbitrator-pq-keys", 1),
                arbitrator_bond_escrow_root: sample_root("appeal-arbitrator-bond-escrow", 1),
                arbitrator_bond_atomic: DEFAULT_MIN_ARBITRATOR_BOND_ATOMIC * 7,
                vote_commitment_root: sample_root("appeal-votes", 1),
                quorum_certificate_root: sample_root("appeal-quorum-cert", 1),
                falcon_quorum_signature_root: sample_root("appeal-falcon-quorum-signature", 1),
                mldsa_quorum_signature_root: sample_root("appeal-mldsa-quorum-signature", 1),
                slhdsa_quorum_signature_root: sample_root("appeal-slhdsa-quorum-signature", 1),
                decision: AppealDecision::RemandForReinsuranceArbitration,
                arbitrators: 7,
                threshold: 5,
                pq_security_bits: DEFAULT_MIN_PQ_SECURITY_BITS,
    ...
    pub fn anchor_appeal_evidence(&mut self, input: AppealEvidenceInput) -> Result<String> {
        let evidence = AppealEvidence::from_input(&self.config, input)?;
        let appeal = self
            .appeal_cases
    ...
    pub fn grant_low_fee_bond_rebate(
        &mut self,
        appeal_id: &str,
        sponsor_commitment_root: String,
        fee_market_snapshot_root: String,
        credited_slot: u64,
    ) -> Result<String> {
        let appeal = self
            .appeal_cases
            .get(appeal_id)
            .ok_or_else(|| "unknown appeal for low-fee bond rebate".to_string())?;
        let rebate_micro_units =
            bps_amount(appeal.appeal_bond_atomic, self.config.bond_rebate_bps).max(1);
        let credit_commitment_root = value_root(
            "low-fee-bond-rebate",
            &json!({
                "appeal_id": appeal_id,
                "rebate_micro_units": rebate_micro_units,
                "credited_slot": credited_slot,
            }),
        );
        self.grant_low_fee_appeal_credit(LowFeeAppealCreditInput {
            appeal_id: appeal_id.to_string(),
            sponsor_commitment_root,
            credit_commitment_root,
            fee_market_snapshot_root,
            credit_micro_units: rebate_micro_units.min(self.config.max_batch_fee_micro_units),
            credited_slot,
        })
    }

    pub fn record_arbitrator_quorum(
        &mut self,
        input: AppealArbitratorQuorumInput,
    ) -> Result<String> {
        let quorum = AppealArbitratorQuorum::from_input(&self.config, input)?;
        let appeal = self
            .appeal_cases
            .get_mut(&quorum.appeal_id)
            .ok_or_else(|| "unknown appeal for arbitrator quorum".to_string())?;
        appeal.status = match quorum.decision {
            AppealDecision::QuarantineForFraudProof => {
                self.counters.quarantined_appeals += 1;
                AppealStatus::Quarantined
            }
            _ => AppealStatus::ArbitratorQuorumMet,
        };
        let quorum_id = quorum.quorum_id.clone();
        self.counters.arbitrator_quorums += 1;
        self.appeal_arbitrator_quorums
            .insert(quorum_id.clone(), quorum);
        self.refresh_roots();
        Ok(quorum_id)
    }

    pub fn lock_arbitrator_bond(&mut self, input: AppealArbitratorQuorumInput) -> Result<String> {
        let appeal_id = input.appeal_id.clone();
        let escrow_root = input.arbitrator_bond_escrow_root.clone();
        let bond_atomic = input.arbitrator_bond_atomic;
        let decided_slot = input.decided_slot;
        let quorum_id = self.record_arbitrator_quorum(input)?;
        let settlement_id = self
            .appeal_cases
            .get(&appeal_id)
            .map(|appeal| appeal.settlement_id.clone())
            .ok_or_else(|| "unknown appeal for arbitrator bond escrow".to_string())?;
        self.append_accounting_entry(SettlementAccountingEntryInput {
            settlement_id,
            side: SettlementAccountingSide::ArbitratorBondEscrow,
            amount_atomic: bond_atomic,
            account_commitment_root: escrow_root,
            memo_commitment_root: value_root(
                "arbitrator-bond-lock-memo",
                &json!({
                    "appeal_id": appeal_id,
                    "quorum_id": quorum_id,
    ...
    pub fn settle_appeal(&mut self, input: AppealSettlementOutcomeInput) -> Result<String> {
        ...
        appeal.status = match outcome.decision {
            ...
            AppealDecision::RemandForReinsuranceArbitration => {
    ...
    pub fn slash_arbitrator_bond(
        ...
        let appeal = self
            .appeal_cases
            .get(appeal_id)
            .ok_or_else(|| "unknown appeal for arbitrator bond slash".to_string())?;
        let amount_atomic = bps_amount(appeal.appeal_bond_atomic, self.config.bond_slash_bps);
        let settlement_id = appeal.settlement_id.clone();
        self.append_accounting_entry(SettlementAccountingEntryInput {
            settlement_id,
            side: SettlementAccountingSide::ArbitratorBondSlash,
            amount_atomic,
            account_commitment_root: slash_commitment_root,
            memo_commitment_root: value_root(
                "arbitrator-bond-slash-memo",
                &json!({
                    "appeal_id": appeal_id,
    ...
    pub fn refund_arbitrator_bond(
        &mut self,
        appeal_id: &str,
        refund_commitment_root: String,
        effective_slot: u64,
    ) -> Result<String> {
        let appeal = self
            .appeal_cases
            .get(appeal_id)
            .ok_or_else(|| "unknown appeal for arbitrator bond refund".to_string())?;
        let refund_atomic = appeal.appeal_bond_atomic.saturating_sub(bps_amount(
            appeal.appeal_bond_atomic,
            self.config.bond_slash_bps,
        ));
        let settlement_id = appeal.settlement_id.clone();
        self.append_accounting_entry(SettlementAccountingEntryInput {
            settlement_id,
            side: SettlementAccountingSide::ArbitratorBondRefund,
            amount_atomic: refund_atomic,
            account_commitment_root: refund_commitment_root,
            memo_commitment_root: value_root(
                "arbitrator-bond-refund-memo",
                &json!({
                    "appeal_id": appeal_id,
                    "refund_atomic": refund_atomic,
                    "effective_slot": effective_slot,
                }),
            ),
            effective_slot,
    ...
    pub fn publish_settlement_outcome(
        &mut self,
        input: AppealSettlementOutcomeInput,
    ) -> Result<String> {
        self.settle_appeal(input)
    }
    ...
    fn apply_accounting_counter(&mut self, entry: &SettlementAccountingEntry) {
        match entry.side {
            ...
            SettlementAccountingSide::ArbitratorBondEscrow => {
                self.counters.total_escrowed_atomic = self
                    .counters
                    .total_escrowed_atomic
                    .saturating_add(entry.amount_atomic);
            }
            SettlementAccountingSide::ArbitratorBondSlash => {
                self.counters.total_protocol_fee_atomic = self
                    .counters
                    .total_protocol_fee_atomic
                    .saturating_add(entry.amount_atomic);
            }
            SettlementAccountingSide::ArbitratorBondRefund
            | SettlementAccountingSide::LowFeeBondRebate => {
                self.counters.total_refunded_atomic = self
                    .counters
                    .total_refunded_atomic
                    .saturating_add(entry.amount_atomic);
    ...
    fn compute_roots(&self) -> Roots {
        ...
        let arbitrator_quorum_root = record_root(
            "appeal-arbitrator-quorums",
            self.appeal_arbitrator_quorums
                .values()
                .map(AppealArbitratorQuorum::roots_only_record)
                .collect(),
        );
        ...
        let public_record_root = value_root(
            ...
            &json!({
                ...
                "arbitrator_quorum_root": arbitrator_quorum_root,
        ...
        let state_root = domain_hash(
            ...
            &[
                ...
                HashPart::Str(&arbitrator_quorum_root),
...
pub fn public_record(state: &State) -> Value {
    json!({
        ...
        "FALCON_ROUTER_APPEAL_ARBITRATION_BOND_SETTLEMENT_SUITE": FALCON_ROUTER_APPEAL_ARBITRATION_BOND_SETTLEMENT_SUITE,
        ...
        "redacted_private_records": {
            ...
            "arbitrator_quorums": state.roots.arbitrator_quorum_root,
