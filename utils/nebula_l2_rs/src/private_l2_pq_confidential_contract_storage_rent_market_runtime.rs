...
pub type PrivateL2PqConfidentialContractStorageRentMarketRuntimeResult<T> = Result<T>;
pub type Runtime = State;

pub const PROTOCOL_VERSION: &str =
    "nebula-private-l2-pq-confidential-contract-storage-rent-market-runtime-v1";
pub const PRIVATE_L2_PQ_CONFIDENTIAL_CONTRACT_STORAGE_RENT_MARKET_RUNTIME_PROTOCOL_VERSION: &str =
    PROTOCOL_VERSION;
pub const PRIVATE_L2_PQ_CONFIDENTIAL_CONTRACT_STORAGE_RENT_MARKET_RUNTIME_SCHEMA_VERSION: u64 = 1;
pub const PRIVATE_L2_PQ_CONFIDENTIAL_CONTRACT_STORAGE_RENT_MARKET_RUNTIME_HASH_SUITE: &str =
    "SHAKE256-domain-separated-canonical-json";
pub const PRIVATE_L2_PQ_CONFIDENTIAL_CONTRACT_STORAGE_RENT_MARKET_RUNTIME_PQ_ATTESTATION_SUITE:
    &str = "ML-DSA-87+SLH-DSA-SHAKE-256f-storage-rent-publisher-v1";
pub const PRIVATE_L2_PQ_CONFIDENTIAL_CONTRACT_STORAGE_RENT_MARKET_RUNTIME_STORAGE_ENCRYPTION_SUITE: &str = "ML-KEM-1024+XChaCha20Poly1305+view-tagged-storage-envelope-v1";
pub const PRIVATE_L2_PQ_CONFIDENTIAL_CONTRACT_STORAGE_RENT_MARKET_RUNTIME_NULLIFIER_SUITE: &str =
    "monero-l2-nullifier-fence-storage-rent-v1";
pub const PRIVATE_L2_PQ_CONFIDENTIAL_CONTRACT_STORAGE_RENT_MARKET_RUNTIME_DEVNET_HEIGHT: u64 =
    1_386_000;
pub const PRIVATE_L2_PQ_CONFIDENTIAL_CONTRACT_STORAGE_RENT_MARKET_RUNTIME_MAX_BPS: u64 = 10_000;
...
pub struct Config {
    ...
    pub storage_encryption_suite: String,
...
pub struct StorageCommitmentRecord {
    ...
    pub encrypted_storage_root: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct MaliciousStorageChallengeRecord {
...
impl MaliciousStorageChallengeRecord {
    pub fn public_record(&self) -> Value {
        json!(self)
    }
}
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct SubmitStorageCommitmentRequest {
    pub lane_id: String,
    pub publisher_commitment: String,
    pub storage_class: StorageClass,
    pub encrypted_storage_root: String,
...
impl SubmitStorageCommitmentRequest {
    pub fn public_record(&self) -> Value {
        json!(self)
    }
}
...
impl RegisterCacheLaneRequest {
    pub fn validate(
        &self,
        config: &Config,
    ) -> PrivateL2PqConfidentialContractStorageRentMarketRuntimeResult<()> {
...
impl SubmitStorageCommitmentRequest {
    pub fn validate(
        ...
        for (n, v) in [
            ...
            ("encrypted_storage_root", &self.encrypted_storage_root),
        ...
        if self.size_bytes == 0 {
            return Err("storage commitment cannot be empty".to_string());
        }
        if self.min_privacy_set < config.min_privacy_set
            || self.pq_security_bits < config.min_pq_security_bits
        {
            return Err("storage privacy or pq security below floor".to_string());
        }
        Ok(())
    }
}
...
pub struct State {
    ...
    pub challenges: BTreeMap<String, MaliciousStorageChallengeRecord>,
...
impl State {
    pub fn new(
        config: Config,
    ) -> PrivateL2PqConfidentialContractStorageRentMarketRuntimeResult<Self> {
        config.validate()?;
        Ok(Self {
            config,
            counters: Counters::default(),
            current_height:
                PRIVATE_L2_PQ_CONFIDENTIAL_CONTRACT_STORAGE_RENT_MARKET_RUNTIME_DEVNET_HEIGHT,
            lanes: BTreeMap::new(),
    ...
    pub fn devnet() -> PrivateL2PqConfidentialContractStorageRentMarketRuntimeResult<Self> {
    ...
    ) -> PrivateL2PqConfidentialContractStorageRentMarketRuntimeResult<String> {
    ...
    pub fn submit_storage_commitment(
        &mut self,
        request: SubmitStorageCommitmentRequest,
    ) -> PrivateL2PqConfidentialContractStorageRentMarketRuntimeResult<String> {
        request.validate(&self.config)?;
        ...
        if !lane.status.accepts_commitments() {
            return Err("cache lane is not accepting storage commitments".to_string());
        }
        ...
        let commitment_id = storage_commitment_id(&request, self.counters.commitment_sequence);
        let record = StorageCommitmentRecord {
            commitment_id: commitment_id.clone(),
            lane_id: request.lane_id.clone(),
            publisher_commitment: request.publisher_commitment,
            storage_class: request.storage_class,
            encrypted_storage_root: request.encrypted_storage_root,
            storage_commitment: request.storage_commitment,
    ...
    pub fn attest_publisher(
        ...
        let commitment = self
            ...
            .ok_or_else(|| "unknown storage commitment".to_string())?;
        ...
        if self.current_height > commitment.expires_at_height {
            ...
            return Err("storage commitment expired".to_string());
        }
    ...
    pub fn challenge_malicious_code(
        ...
        let challenge_id =
            malicious_storage_challenge_id(&request, self.counters.challenge_sequence);
        self.challenges.insert(
            challenge_id.clone(),
            MaliciousStorageChallengeRecord {
    ...
    pub fn slash_publisher_verifier(
        ...
        if let Some(commitment_id) = &request.commitment_id {
            ...
            if matches!(
                request.reason,
                SlashingReason::MaliciousStorage
    ...
    pub fn roots(&self) -> Roots {
        ...
        let commitment_root = public_record_root(
            "storage-rent-market:commitments",
            &self
                .commitments
                .values()
                .map(StorageCommitmentRecord::public_record)
                .collect::<Vec<_>>(),
        );
        let attestation_root = public_record_root(
            "storage-rent-market:attestations",
            &self
                .attestations
                .values()
        ...
        let invalidation_root = public_record_root(
            "storage-rent-market:invalidations",
            &self
                .invalidations
                .values()
        ...
        let challenge_root = public_record_root(
            "storage-rent-market:challenges",
            &self
                .challenges
                .values()
                .map(MaliciousStorageChallengeRecord::public_record)
                .collect::<Vec<_>>(),
        );
        let slashing_root = public_record_root(
            "storage-rent-market:slashing",
            &self
                .slashing_events
                .values()
        ...
        let privacy_fence_root = public_record_root(
            "storage-rent-market:privacy-fences",
            &self
                .privacy_fences
                .values()
        ...
        let nullifiers = self
            ...
            .map(|v| json!(v))
            .collect::<Vec<_>>();
        let nullifier_root = public_record_root("storage-rent-market:nullifiers", &nullifiers);
    ...
    pub fn commitment(&self, commitment_id: &str) -> Option<&StorageCommitmentRecord> {
    ...
    pub fn lane_fee_quote(
        ...
        lane_id: &str,
        call_units: u64,
    ) -> PrivateL2PqConfidentialContractStorageRentMarketRuntimeResult<Value> {
        let lane = self
            .lanes
            .get(lane_id)
            .ok_or_else(|| "unknown cache lane".to_string())?;
...
pub fn storage_commitment_id(request: &SubmitStorageCommitmentRequest, sequence: u64) -> String {
...
pub fn privacy_fence_id(request: &SubmitStorageCommitmentRequest, sequence: u64) -> String {
    deterministic_id(
        "PRIVACY-FENCE-ID",
        sequence,
...
pub fn malicious_storage_challenge_id(
    request: &ChallengeMaliciousCodeRequest,
    sequence: u64,
) -> String {
...
pub fn deterministic_id(kind: &str, sequence: u64, record: &Value) -> String {
    domain_hash(
        &format!("PRIVATE-L2-PQ-STORAGE-CACHE-MARKET:{kind}"),
        &[
...
pub fn state_root_from_record(record: &Value) -> String {
    domain_hash(
        "PRIVATE-L2-PQ-STORAGE-CACHE-MARKET:STATE-ROOT",
...
pub fn lane_record_root(record: &CacheLaneRecord) -> String {
    deterministic_record_root(
        "PRIVATE-L2-PQ-STORAGE-CACHE-MARKET:LANE-RECORD-ROOT",
        &record.public_record(),
    )
}
pub fn commitment_record_root(record: &StorageCommitmentRecord) -> String {
    deterministic_record_root(
        "PRIVATE-L2-PQ-STORAGE-CACHE-MARKET:COMMITMENT-RECORD-ROOT",
        &record.public_record(),
    )
}
pub fn attestation_record_root(record: &PublisherAttestationRecord) -> String {
    deterministic_record_root(
        "PRIVATE-L2-PQ-STORAGE-CACHE-MARKET:ATTESTATION-RECORD-ROOT",
        &record.public_record(),
    )
}
pub fn slot_record_root(record: &VerifierCacheSlotRecord) -> String {
    deterministic_record_root(
        "PRIVATE-L2-PQ-STORAGE-CACHE-MARKET:SLOT-RECORD-ROOT",
        &record.public_record(),
    )
}
pub fn ticket_record_root(record: &WarmCallTicketRecord) -> String {
    deterministic_record_root(
        "PRIVATE-L2-PQ-STORAGE-CACHE-MARKET:TICKET-RECORD-ROOT",
        &record.public_record(),
    )
}
pub fn rebate_record_root(record: &FeeRebateRecord) -> String {
    deterministic_record_root(
        "PRIVATE-L2-PQ-STORAGE-CACHE-MARKET:REBATE-RECORD-ROOT",
        &record.public_record(),
    )
}
pub fn invalidation_record_root(record: &InvalidationReceiptRecord) -> String {
    deterministic_record_root(
        "PRIVATE-L2-PQ-STORAGE-CACHE-MARKET:INVALIDATION-RECORD-ROOT",
        &record.public_record(),
    )
}
pub fn privacy_fence_record_root(record: &PrivacyFenceRecord) -> String {
    deterministic_record_root(
        "PRIVATE-L2-PQ-STORAGE-CACHE-MARKET:PRIVACY-FENCE-RECORD-ROOT",
        &record.public_record(),
    )
}
pub fn challenge_record_root(record: &MaliciousStorageChallengeRecord) -> String {
    deterministic_record_root(
        "PRIVATE-L2-PQ-STORAGE-CACHE-MARKET:CHALLENGE-RECORD-ROOT",
        &record.public_record(),
    )
}
pub fn slashing_record_root(record: &SlashingEvidenceRecord) -> String {
    deterministic_record_root(
        "PRIVATE-L2-PQ-STORAGE-CACHE-MARKET:SLASHING-RECORD-ROOT",
        &record.public_record(),
    )
}
fn required(
...
fn bps_amount(amount: u128, bps: u64) -> u128 {
    amount.saturating_mul(bps as u128)
        / PRIVATE_L2_PQ_CONFIDENTIAL_CONTRACT_STORAGE_RENT_MARKET_RUNTIME_MAX_BPS as u128
}
