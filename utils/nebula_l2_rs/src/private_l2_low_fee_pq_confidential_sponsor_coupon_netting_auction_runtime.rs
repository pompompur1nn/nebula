...
pub type PrivateL2LowFeePqConfidentialSponsorCouponNettingAuctionRuntimeResult<T> = Result<T>;
pub type Runtime = State;

pub const PRIVATE_L2_LOW_FEE_PQ_CONFIDENTIAL_SPONSOR_COUPON_NETTING_AUCTION_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-private-l2-low-fee-pq-confidential-sponsor-coupon-netting-auction-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_LOW_FEE_PQ_CONFIDENTIAL_SPONSOR_COUPON_NETTING_AUCTION_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const DEVNET_L2_NETWORK: &str = "nebula-private-l2-devnet";
pub const DEVNET_FEE_ASSET_ID: &str = "piconero-private-l2-devnet";
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const ROUTER_SCHEME: &str = "low-fee-private-sponsor-coupon-netting-auction-v1";
pub const VOUCHER_LOT_SCHEME: &str = "ml-kem-1024-sealed-sponsor-coupon-lot-v1";
pub const PQ_SPONSOR_ATTESTATION_SCHEME: &str =
    "ML-DSA-87+SLH-DSA-SHAKE-256f-sponsor-attestation-v1";
pub const REDEMPTION_RECEIPT_SCHEME: &str = "confidential-sponsor-coupon-redemption-receipt-v1";
pub const ROUTE_CAP_SCHEME: &str = "low-fee-netting-cap-root-v1";
pub const REBATE_ACCOUNTING_SCHEME: &str = "multi-sponsor-rebate-accounting-root-v1";
pub const REDACTION_BUDGET_SCHEME: &str = "coupon-nettingr-redaction-budget-root-v1";
pub const OPERATOR_SUMMARY_SCHEME: &str = "operator-safe-coupon-nettingr-summary-root-v1";
pub const PRIVACY_BOUNDARY: &str =
    "roots_only_no_plaintext_sponsor_balances_coupon_amounts_accounts_or_decryption_keys";
...
pub struct Config {
    ...
    pub max_netting_sponsors: usize,
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            auction_scheme: ROUTER_SCHEME.to_string(),
            coupon_lot_scheme: VOUCHER_LOT_SCHEME.to_string(),
            ...
            netting_cap_scheme: ROUTE_CAP_SCHEME.to_string(),
            ...
            coupon_ttl_blocks: DEFAULT_VOUCHER_TTL_BLOCKS,
            redaction_epoch_blocks: DEFAULT_REDACTION_EPOCH_BLOCKS,
            max_netting_sponsors: DEFAULT_MAX_ROUTE_SPONSORS,
...
impl Config {
    pub fn validate(&self) -> Result<()> {
        ...
        if self.max_netting_sponsors == 0 {
            return Err("max_netting_sponsors must be positive".to_string());
        }
        Ok(())
    }
    pub fn public_record(&self) -> Value {
        json!({ "protocol_version": PROTOCOL_VERSION, "schema_version": SCHEMA_VERSION, "chain_id": self.chain_id, "l2_network": self.l2_network, "fee_asset_id": self.fee_asset_id, "hash_suite": self.hash_suite, "auction_scheme": self.auction_scheme, "coupon_lot_scheme": self.coupon_lot_scheme, "pq_sponsor_attestation_scheme": self.pq_sponsor_attestation_scheme, "redemption_receipt_scheme": self.redemption_receipt_scheme, "netting_cap_scheme": self.netting_cap_scheme, "rebate_accounting_scheme": self.rebate_accounting_scheme, "redaction_budget_scheme": self.redaction_budget_scheme, "operator_summary_scheme": self.operator_summary_scheme, "privacy_boundary": self.privacy_boundary, "low_fee_bps": self.low_fee_bps, "max_user_fee_bps": self.max_user_fee_bps, "target_rebate_bps": self.target_rebate_bps, "sponsor_cover_bps": self.sponsor_cover_bps, "fast_path_latency_ms": self.fast_path_latency_ms, "min_privacy_set_size": self.min_privacy_set_size, "target_privacy_set_size": self.target_privacy_set_size, "min_pq_security_bits": self.min_pq_security_bits, "target_pq_security_bits": self.target_pq_security_bits, "attestation_ttl_blocks": self.attestation_ttl_blocks, "coupon_ttl_blocks": self.coupon_ttl_blocks, "redaction_epoch_blocks": self.redaction_epoch_blocks, "max_netting_sponsors": self.max_netting_sponsors, "operator_bucket_size": self.operator_bucket_size })
    }
...
impl Counters {
    pub fn public_record(&self) -> Value {
        json!({ "sponsor_cohorts": self.sponsor_cohorts, "coupon_nettings": self.coupon_nettings, "sealed_coupon_lots": self.sealed_coupon_lots, "pq_sponsor_attestations": self.pq_sponsor_attestations, "redemption_receipts": self.redemption_receipts, "netting_caps": self.netting_caps, "rebate_accounts": self.rebate_accounts, "redaction_budgets": self.redaction_budgets, "operator_summaries": self.operator_summaries, "runtime_events": self.runtime_events, "fast_path_nettings": self.fast_path_nettings, "private_nettings": self.private_nettings, "rejected_nettings": self.rejected_nettings, "total_coupon_units": self.total_coupon_units, "redeemed_coupon_units": self.redeemed_coupon_units, "rebated_fee_units": self.rebated_fee_units })
    }
}
...
impl Roots {
    pub fn public_record(&self) -> Value {
        json!({ "config_root": self.config_root, "sponsor_cohort_root": self.sponsor_cohort_root, "coupon_netting_root": self.coupon_netting_root, "sealed_coupon_lot_root": self.sealed_coupon_lot_root, "pq_sponsor_attestation_root": self.pq_sponsor_attestation_root, "redemption_receipt_root": self.redemption_receipt_root, "netting_cap_root": self.netting_cap_root, "rebate_accounting_root": self.rebate_accounting_root, "redaction_budget_root": self.redaction_budget_root, "operator_summary_root": self.operator_summary_root, "runtime_event_root": self.runtime_event_root, "public_record_root": self.public_record_root })
    }
...
impl SponsorCohort {
    pub fn new(
        ...
        Self {
            ...
            sponsor_count,
            available_coupon_units: units,
            reserved_coupon_units: 0,
            redeemed_coupon_units: 0,
            ...
            opened_height,
            expires_height: opened_height.saturating_add(config.coupon_ttl_blocks),
            labels: BTreeSet::new(),
        }
    }
    pub fn reserve(&mut self, units: u64) -> Result<()> {
        ...
        if units > self.available_coupon_units {
            return Err("insufficient cohort coupon units".to_string());
        }
        self.available_coupon_units = self.available_coupon_units.saturating_sub(units);
        self.reserved_coupon_units = self.reserved_coupon_units.saturating_add(units);
    ...
    pub fn redeem_reserved(&mut self, units: u64) -> Result<()> {
        if units > self.reserved_coupon_units {
            return Err("redeem exceeds reserved units".to_string());
        }
        self.reserved_coupon_units = self.reserved_coupon_units.saturating_sub(units);
        self.redeemed_coupon_units = self.redeemed_coupon_units.saturating_add(units);
        if self.available_coupon_units == 0 && self.reserved_coupon_units == 0 {
            self.status = SponsorCohortStatus::Settling;
        }
        Ok(())
    }
    pub fn public_record(&self) -> Value {
        json!({ "kind": "sponsor_cohort", "protocol_version": PROTOCOL_VERSION, "cohort_id": self.cohort_id, "sponsor_commitment_root": self.sponsor_commitment_root, "funding_nullifier_root": self.funding_nullifier_root, "status": self.status, "sponsor_count": self.sponsor_count, "available_coupon_units": self.available_coupon_units, "reserved_coupon_units": self.reserved_coupon_units, "redeemed_coupon_units": self.redeemed_coupon_units, "low_fee_bps": self.low_fee_bps, "sponsor_cover_bps": self.sponsor_cover_bps, "pq_security_bits": self.pq_security_bits, "privacy_set_size": self.privacy_set_size, "opened_height": self.opened_height, "expires_height": self.expires_height, "labels": self.labels.iter().cloned().collect::<Vec<_>>() })
    }
    ...
    pub fn utilization_bps(&self) -> u64 {
        bps(
            self.reserved_coupon_units
                .saturating_add(self.redeemed_coupon_units),
            self.available_coupon_units
                .saturating_add(self.reserved_coupon_units)
                .saturating_add(self.redeemed_coupon_units),
        )
    }
}
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct CouponNetting {
    pub netting_id: String,
    pub cohort_id: String,
    pub cap_id: String,
    pub lot_id: String,
    pub netting_commitment: String,
    ...
    pub privacy_set_size: u64,
    pub pq_security_bits: u16,
    pub status: CouponNettingStatus,
...
impl CouponNetting {
    pub fn new(
        ...
        let netting_id = netting_id.into();
        let cohort_id = cohort_id.into();
        let cap_id = cap_id.into();
        let lot_id = lot_id.into();
        Self {
            netting_commitment: runtime_id(
                "ROUTE-COMMITMENT",
                &[
                    HashPart::Str(&netting_id),
            ...
            payer_note_commitment: runtime_id(
                "PAYER-NOTE",
                &[HashPart::Str(&netting_id), HashPart::U64(height)],
            ),
            netting_id,
            cohort_id,
            cap_id,
            ...
            privacy_set_size: config.target_privacy_set_size,
            pq_security_bits: config.target_pq_security_bits,
            status: CouponNettingStatus::Draft,
            created_height: height,
            expires_height: height.saturating_add(config.coupon_ttl_blocks),
    ...
    pub fn attach_sponsor_commitment(
        ...
        if self.sponsor_commitments.len() >= config.max_netting_sponsors {
            return Err("netting sponsor fanout exceeds config".to_string());
        }
        self.sponsor_commitments.push(commitment.into());
        self.status = CouponNettingStatus::SponsorMatched;
        Ok(())
    }
    pub fn mark_attested(&mut self) {
        self.status = CouponNettingStatus::Attested;
    }
    pub fn mark_redeemed(&mut self) -> Result<()> {
        if !self.status.redeemable() {
            return Err("netting is not redeemable".to_string());
        }
        self.status = CouponNettingStatus::Redeemed;
        Ok(())
    }
    pub fn effective_user_fee_units(&self) -> u64 {
        self.coupon_units.saturating_mul(self.max_user_fee_bps) / MAX_BPS
    }
    pub fn sponsor_cover_units(&self) -> u64 {
        self.coupon_units.saturating_mul(self.sponsor_cover_bps) / MAX_BPS
    }
    pub fn public_record(&self) -> Value {
        json!({ "kind": "coupon_netting", "protocol_version": PROTOCOL_VERSION, "netting_id": self.netting_id, "cohort_id": self.cohort_id, "cap_id": self.cap_id, "lot_id": self.lot_id, "netting_commitment": self.netting_commitment, "payer_note_commitment": self.payer_note_commitment, "fee_asset_id": self.fee_asset_id, "coupon_units": self.coupon_units, "max_user_fee_bps": self.max_user_fee_bps, "sponsor_cover_bps": self.sponsor_cover_bps, "expected_latency_ms": self.expected_latency_ms, "privacy_set_size": self.privacy_set_size, "pq_security_bits": self.pq_security_bits, "status": self.status, "created_height": self.created_height, "expires_height": self.expires_height, "sponsor_commitments": self.sponsor_commitments })
    }
    pub fn netting_root(&self) -> String {
...
impl SealedCouponLot {
    pub fn new(
        ...
        Self {
            ...
            coupon_commitment_root: runtime_id(
                "VOUCHER-COMMITMENTS",
    ...
    pub fn reserve(&mut self, units: u64) -> Result<()> {
        if !self.status.available() {
            return Err("coupon lot is unavailable".to_string());
        }
        if self.remaining_units() < units {
            return Err("coupon lot capacity exceeded".to_string());
        }
        self.units_reserved = self.units_reserved.saturating_add(units);
        self.status = CouponLotStatus::Reserved;
        Ok(())
    }
    pub fn redeem(&mut self, units: u64) -> Result<()> {
        ...
        self.status = if self.remaining_units() == 0 && self.units_reserved == 0 {
            CouponLotStatus::Exhausted
        } else {
            CouponLotStatus::PartiallyRedeemed
        };
        Ok(())
    }
    ...
    pub fn public_record(&self) -> Value {
        json!({ "kind": "sealed_coupon_lot", "protocol_version": PROTOCOL_VERSION, "lot_id": self.lot_id, "cohort_id": self.cohort_id, "sealed_ciphertext_root": self.sealed_ciphertext_root, "coupon_commitment_root": self.coupon_commitment_root, "nullifier_root": self.nullifier_root, "units_total": self.units_total, "units_reserved": self.units_reserved, "units_redeemed": self.units_redeemed, "min_split_units": self.min_split_units, "max_split_units": self.max_split_units, "status": self.status, "pq_kem": self.pq_kem, "opened_height": self.opened_height, "expires_height": self.expires_height })
    }
...
impl RedemptionReceipt {
    pub fn from_netting(
        ...
        netting: &CouponNetting,
        height: u64,
        config: &Config,
    ) -> Self {
        let receipt_id = receipt_id.into();
        let charged_fee_units = netting.effective_user_fee_units();
        let rebated_fee_units = netting
            .coupon_units
        ...
        Self {
            redemption_nullifier: runtime_id(
                ...
                &[
                    ...
                    HashPart::Str(&netting.netting_id),
                ],
            ),
            inclusion_root: runtime_id(
                "REDEMPTION-INCLUSION",
                &[HashPart::Str(&netting.netting_id), HashPart::U64(height)],
            ),
            receipt_id,
            netting_id: netting.netting_id.clone(),
            lot_id: netting.lot_id.clone(),
            cohort_id: netting.cohort_id.clone(),
            redeemed_units: netting.coupon_units,
            charged_fee_units,
            rebated_fee_units,
            latency_ms: netting.expected_latency_ms,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct NettingCap {
    pub cap_id: String,
    pub cohort_id: String,
    pub max_units_per_epoch: u64,
    pub used_units: u64,
    pub max_nettings_per_epoch: u64,
    pub used_nettings: u64,
    pub min_privacy_set_size: u64,
    pub max_user_fee_bps: u64,
...
impl NettingCap {
    ...
    pub fn consume(&mut self, units: u64) -> Result<()> {
        if matches!(
            ...
            return Err("netting cap is closed".to_string());
        }
        if self.used_units.saturating_add(units) > self.max_units_per_epoch {
            self.status = CapStatus::Exhausted;
            return Err("netting cap units exhausted".to_string());
        }
        if self.used_nettings.saturating_add(1) > self.max_nettings_per_epoch {
            self.status = CapStatus::HardLimited;
            return Err("netting cap count exhausted".to_string());
        }
        self.used_units = self.used_units.saturating_add(units);
        self.used_nettings = self.used_nettings.saturating_add(1);
    ...
    pub fn public_record(&self) -> Value {
        json!({ "kind": "netting_cap", "protocol_version": PROTOCOL_VERSION, "cap_id": self.cap_id, "cohort_id": self.cohort_id, "max_units_per_epoch": self.max_units_per_epoch, "used_units": self.used_units, "max_nettings_per_epoch": self.max_nettings_per_epoch, "used_nettings": self.used_nettings, "min_privacy_set_size": self.min_privacy_set_size, "max_user_fee_bps": self.max_user_fee_bps, "status": self.status, "epoch": self.epoch })
    }
...
impl OperatorSummary {
    pub fn public_record(&self) -> Value {
        json!({ "kind": "operator_summary", "protocol_version": PROTOCOL_VERSION, "summary_id": self.summary_id, "audience": self.audience, "height": self.height, "cohort_root": self.cohort_root, "netting_root": self.netting_root, "receipt_root": self.receipt_root, "rebate_root": self.rebate_root, "redaction_root": self.redaction_root, "active_cohorts": self.active_cohorts, "fast_path_nettings": self.fast_path_nettings, "median_latency_ms": self.median_latency_ms, "low_fee_bps": self.low_fee_bps, "public_record_root": self.public_record_root })
    }
...
pub struct State {
    ...
    pub coupon_nettings: BTreeMap<String, CouponNetting>,
    ...
    pub netting_caps: BTreeMap<String, NettingCap>,
...
impl State {
    ...
    pub fn insert_coupon_lot(&mut self, lot: SealedCouponLot) -> Result<()> {
        ...
        self.sealed_coupon_lots.insert(lot.lot_id.clone(), lot);
        self.refresh();
        Ok(())
    }
    pub fn insert_netting_cap(&mut self, cap: NettingCap) -> Result<()> {
        if !self.sponsor_cohorts.contains_key(&cap.cohort_id) {
            return Err("cap references unknown cohort".to_string());
        }
        self.netting_caps.insert(cap.cap_id.clone(), cap);
        self.refresh();
        Ok(())
    }
    ...
    pub fn reserve_netting(&mut self, mut netting: CouponNetting) -> Result<String> {
        self.config.validate()?;
        let cohort = self
            .sponsor_cohorts
            .get_mut(&netting.cohort_id)
            .ok_or_else(|| "netting references unknown cohort".to_string())?;
        let lot = self
            .sealed_coupon_lots
            .get_mut(&netting.lot_id)
            .ok_or_else(|| "netting references unknown coupon lot".to_string())?;
        let cap = self
            .netting_caps
            .get_mut(&netting.cap_id)
            .ok_or_else(|| "netting references unknown cap".to_string())?;
        if netting.privacy_set_size < self.config.min_privacy_set_size {
            netting.status = CouponNettingStatus::Rejected;
            return Err("netting privacy set below minimum".to_string());
        }
        cap.consume(netting.coupon_units)?;
        cohort.reserve(netting.coupon_units)?;
        lot.reserve(netting.coupon_units)?;
        netting.status = CouponNettingStatus::FastPathReady;
        let netting_id = netting.netting_id.clone();
        self.emit(
            RuntimeEventKind::NettingMatched,
            &netting_id,
            &netting.netting_root(),
            netting.created_height,
            json!({"coupon_units": netting.coupon_units, "latency_ms": netting.expected_latency_ms}),
        );
        self.coupon_nettings.insert(netting_id.clone(), netting);
        self.refresh();
        Ok(netting_id)
    }
    ...
    pub fn redeem_netting(
        ...
        let netting = self
            .coupon_nettings
            .get_mut(netting_id)
            .ok_or_else(|| "unknown netting".to_string())?;
        netting.mark_redeemed()?;
        let receipt = RedemptionReceipt::from_netting(receipt_id, netting, height, &self.config);
        if let Some(cohort) = self.sponsor_cohorts.get_mut(&netting.cohort_id) {
            cohort.redeem_reserved(netting.coupon_units)?;
        }
        if let Some(lot) = self.sealed_coupon_lots.get_mut(&netting.lot_id) {
            lot.redeem(netting.coupon_units)?;
        }
        for account in self
            .rebate_accounting
            .values_mut()
            .filter(|account| account.cohort_id == netting.cohort_id)
        {
            account.accrue(receipt.rebated_fee_units);
        }
        let receipt_id = receipt.receipt_id.clone();
        self.emit(RuntimeEventKind::CouponRedeemed, &receipt_id, &receipt.receipt_root(), height, json!({"redeemed_units": receipt.redeemed_units, "rebated_fee_units": receipt.rebated_fee_units}));
    ...
    pub fn publish_operator_summary(
        ...
        let summary = OperatorSummary {
            ...
            netting_root: self.roots.coupon_netting_root.clone(),
            ...
            active_cohorts: self
                ...
                .count() as u64,
            fast_path_nettings: self
                .coupon_nettings
                .values()
                .filter(|netting| {
                    netting.status == CouponNettingStatus::FastPathReady || netting.status.charged()
                })
                .count() as u64,
    ...
    pub fn privacy_score_bps(&self) -> u64 {
        let private_nettings = self
            .coupon_nettings
            .values()
            .filter(|netting| {
                netting.privacy_set_size >= self.config.target_privacy_set_size
                    && netting.pq_security_bits >= self.config.target_pq_security_bits
            })
            .count() as u64;
        bps(private_nettings, self.coupon_nettings.len() as u64)
    }
    pub fn speed_score_bps(&self) -> u64 {
        let fast_nettings = self
            .coupon_nettings
            .values()
            .filter(|netting| netting.expected_latency_ms <= self.config.fast_path_latency_ms)
            .count() as u64;
        bps(fast_nettings, self.coupon_nettings.len() as u64)
    }
    pub fn low_fee_score_bps(&self) -> u64 {
        let low_fee_nettings = self
            .coupon_nettings
            .values()
            .filter(|netting| netting.max_user_fee_bps <= self.config.max_user_fee_bps)
            .count() as u64;
        bps(low_fee_nettings, self.coupon_nettings.len() as u64)
    }
    pub fn pq_score_bps(&self) -> u64 {
        let pq_nettings = self
            .coupon_nettings
            .values()
            .filter(|netting| netting.pq_security_bits >= self.config.min_pq_security_bits)
            .count() as u64;
        bps(pq_nettings, self.coupon_nettings.len() as u64)
    }
    ...
    fn refresh(&mut self) {
        ...
        self.state_root = private_l2_low_fee_pq_confidential_sponsor_coupon_netting_auction_state_root_from_record(&self.public_record_without_state_root());
    }
    fn compute_counters(&self) -> Counters {
        Counters {
            sponsor_cohorts: self.sponsor_cohorts.len() as u64,
            coupon_nettings: self.coupon_nettings.len() as u64,
            sealed_coupon_lots: self.sealed_coupon_lots.len() as u64,
            pq_sponsor_attestations: self.pq_sponsor_attestations.len() as u64,
            redemption_receipts: self.redemption_receipts.len() as u64,
            netting_caps: self.netting_caps.len() as u64,
            ...
            fast_path_nettings: self
                ...
                .count() as u64,
            private_nettings: self
                .coupon_nettings
                .values()
                .filter(|netting| netting.privacy_set_size >= self.config.min_privacy_set_size)
                .count() as u64,
            rejected_nettings: self
                .coupon_nettings
                .values()
                .filter(|netting| netting.status == CouponNettingStatus::Rejected)
                .count() as u64,
            total_coupon_units: self
                .sealed_coupon_lots
                .values()
                .map(|lot| lot.units_total)
                .sum(),
            redeemed_coupon_units: self
                .redemption_receipts
                .values()
    ...
    fn compute_roots(&self) -> Roots {
        Roots {
            ...
            coupon_netting_root: collection_root(
                "VOUCHER-ROUTES",
                self.coupon_nettings
                    .values()
                    .map(CouponNetting::public_record)
                    .collect(),
            ),
            sealed_coupon_lot_root: collection_root(
                "SEALED-VOUCHER-LOTS",
            ...
            netting_cap_root: collection_root(
                "ROUTE-CAPS",
                self.netting_caps
                    .values()
                    .map(NettingCap::public_record)
                    .collect(),
            ),
    ...
    fn public_record_without_state_root(&self) -> Value {
        json!({ "kind": "private_l2_low_fee_pq_confidential_sponsor_coupon_netting_auction_runtime", "protocol_version": PROTOCOL_VERSION, "schema_version": SCHEMA_VERSION, "config": self.config.public_record(), "roots": self.roots.public_record(), "counters": self.counters.public_record(), "sponsor_cohorts": self.sponsor_cohorts.values().map(SponsorCohort::public_record).collect::<Vec<_>>(), "coupon_nettings": self.coupon_nettings.values().map(CouponNetting::public_record).collect::<Vec<_>>(), "sealed_coupon_lots": self.sealed_coupon_lots.values().map(SealedCouponLot::public_record).collect::<Vec<_>>(), "pq_sponsor_attestations": self.pq_sponsor_attestations.values().map(PqSponsorAttestation::public_record).collect::<Vec<_>>(), "redemption_receipts": self.redemption_receipts.values().map(RedemptionReceipt::public_record).collect::<Vec<_>>(), "netting_caps": self.netting_caps.values().map(NettingCap::public_record).collect::<Vec<_>>(), "rebate_accounting": self.rebate_accounting.values().map(RebateAccounting::public_record).collect::<Vec<_>>(), "redaction_budgets": self.redaction_budgets.values().map(RedactionBudget::public_record).collect::<Vec<_>>(), "operator_summaries": self.operator_summaries.values().map(OperatorSummary::public_record).collect::<Vec<_>>(), "runtime_events": self.runtime_events.iter().map(RuntimeEvent::public_record).collect::<Vec<_>>(), "public_records": self.public_records })
    }
}
impl State {
    pub fn generated_operator_bucket_001_record(&self) -> Value {
        let bucket_size = self.config.operator_bucket_size.max(1);
        let netting_count = self.coupon_nettings.len() as u64;
        let receipt_count = self.redemption_receipts.len() as u64;
        let bucket_floor = (1u64 - 1).saturating_mul(bucket_size);
        json!({"kind":"operator_bucket","bucket_index":1,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_002_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":2,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_003_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":3,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_004_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":4,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_005_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":5,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_006_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":6,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_007_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":7,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_008_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":8,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_009_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":9,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_010_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":10,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_011_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":11,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_012_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":12,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_013_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":13,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_014_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":14,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_015_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":15,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_016_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":16,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_017_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":17,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_018_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":18,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_019_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":19,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_020_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":20,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_021_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":21,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_022_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":22,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_024_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":24,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_025_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":25,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_026_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":26,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_027_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":27,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_028_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":28,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_029_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":29,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_031_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":31,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_032_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":32,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_033_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":33,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_034_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":34,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_035_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":35,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_036_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":36,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_037_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":37,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_038_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":38,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_039_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":39,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_040_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":40,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_041_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":41,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_042_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":42,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_043_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":43,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_045_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":45,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_046_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":46,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_047_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":47,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_048_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":48,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_049_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":49,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_050_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":50,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_051_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":51,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_052_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":52,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_053_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":53,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_054_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":54,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_055_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":55,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_056_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":56,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_057_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":57,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
impl State {
    pub fn generated_operator_bucket_058_record(&self) -> Value {
        ...
        json!({"kind":"operator_bucket","bucket_index":58,"protocol_version":PROTOCOL_VERSION,"bucket_floor":bucket_floor,"bucket_ceiling":bucket_floor.saturating_add(bucket_size),"netting_count":netting_count,"receipt_count":receipt_count,"low_fee_score_bps":self.low_fee_score_bps(),"privacy_score_bps":self.privacy_score_bps(),"speed_score_bps":self.speed_score_bps(),"pq_score_bps":self.pq_score_bps(),"state_root":self.state_root})
    }
...
pub fn demo() -> State {
    ...
    state
        .insert_coupon_lot(SealedCouponLot::new(
            "lot-alpha-0001",
    ...
    state
        .insert_netting_cap(NettingCap::new(
            "cap-alpha-epoch-1",
            "cohort-low-fee-alpha",
            1,
            &config,
    ...
    let mut netting_a = CouponNetting::new(
        "netting-alpha-0001",
        "cohort-low-fee-alpha",
        "cap-alpha-epoch-1",
        "lot-alpha-0001",
    ...
    netting_a
        ...
        .expect("sponsor fanout");
    state
        .reserve_netting(netting_a)
        .expect("valid netting alpha");
    let mut netting_b = CouponNetting::new(
        "netting-beta-0001",
        "cohort-fast-private-beta",
        "cap-beta-epoch-1",
        "lot-beta-0001",
    ...
    netting_b.expected_latency_ms = 320;
    netting_b
        .attach_sponsor_commitment("sponsor-beta-commitment-a", &config)
        .expect("sponsor fanout");
    state
        .reserve_netting(netting_b)
        .expect("valid netting beta");
    let netting_alpha_root = state
        .coupon_nettings
        .get("netting-alpha-0001")
        .expect("netting exists")
        .netting_root();
    state
    ...
    let netting_beta_root = state
        .coupon_nettings
        .get("netting-beta-0001")
        .expect("netting exists")
        .netting_root();
    state
        .accept_attestation(PqSponsorAttestation::new(
    ...
    state
        .redeem_netting("netting-alpha-0001", "receipt-alpha-0001", 1_012)
        .expect("valid alpha redemption");
    state
        .redeem_netting("netting-beta-0001", "receipt-beta-0001", 1_013)
...
pub fn state_root(state: &State) -> String {
    private_l2_low_fee_pq_confidential_sponsor_coupon_netting_auction_state_root_from_record(
        &state.public_record_without_state_root(),
    )
}

pub fn private_l2_low_fee_pq_confidential_sponsor_coupon_netting_auction_state_root_from_record(
    record: &Value,
) -> String {
