...
pub type PrivateL2FastPqConfidentialDaBlobWitnessPrefetchRuntimeResult<T> = Result<T>;
pub type Runtime = State;

pub const PRIVATE_L2_FAST_PQ_CONFIDENTIAL_DA_BLOB_WITNESS_PREFETCH_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-private-l2-fast-pq-confidential-da-blob-witness-prefetch-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_FAST_PQ_CONFIDENTIAL_DA_BLOB_WITNESS_PREFETCH_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const PQ_SIGNATURE_SUITE: &str =
    "ML-DSA-87+SLH-DSA-SHAKE-256f-confidential-da-blob-witness-prefetch-v1";
pub const PQ_KEM_SUITE: &str = "ML-KEM-1024+hybrid-x25519-confidential-blob-hints-v1";
pub const RECEIPT_HINT_SCHEME: &str = "private-l2-confidential-blob-hint-root-v1";
pub const WITNESS_PREFETCH_SCHEME: &str = "private-l2-da_availability-witness-prefetch-root-v1";
pub const RECEIPT_WINDOW_SCHEME: &str = "private-l2-da-blob-witness-window-root-v1";
pub const ATTESTATION_SCHEME: &str = "pq-da_availability-prefetch-attestation-root-v1";
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct WitnessPrefetch {
    pub lane_id: String,
    pub class: LaneClass,
    pub mode: LaneMode,
    pub operator_id: String,
    pub da_availability_committee_id: String,
...
impl WitnessPrefetch {
    ...
    pub fn new(
        ...
        da_availability_committee_id: impl Into<String>,
        capacity_units: u64,
        prefetch_depth: u64,
        ...
        Self {
            ...
            da_availability_committee_id: da_availability_committee_id.into(),
            capacity_units,
            reserved_units: 0,
            prefetch_depth,
            target_latency_ms,
    ...
    pub fn root(&self) -> String {
        ...
        domain_hash(WITNESS_PREFETCH_SCHEME, &[HashPart::Json(&record)], 32)
    }
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "da_availability_committee_id": self.da_availability_committee_id,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct DaBlobWitnessWindow {
    pub window_id: String,
    pub lane_id: String,
    pub status: BlobWindowStatus,
...
impl DaBlobWitnessWindow {
    pub fn new(
        ...
        Self {
            ...
            lane_id: lane_id.into(),
            status: BlobWindowStatus::Planned,
            start_height,
            end_height,
            receipt_commitment_root: "blob-window-empty".to_string(),
    ...
    pub fn open(&mut self) {
        if matches!(self.status, BlobWindowStatus::Planned) {
            self.status = BlobWindowStatus::Open;
        }
    }

    pub fn mark_prefetching(&mut self) {
        if matches!(self.status, BlobWindowStatus::Open) {
            self.status = BlobWindowStatus::Prefetching;
        }
    }
    ...
    pub fn seal(
        ...
        self.status = BlobWindowStatus::Sealed;
    }

    pub fn settle(&mut self) {
        if matches!(self.status, BlobWindowStatus::Sealed) {
            self.status = BlobWindowStatus::Settled;
        }
    }

    pub fn invalidate(&mut self) {
        self.status = BlobWindowStatus::Invalidated;
    }
...
impl EncryptedBlobHint {
    ...
    pub fn new(
        ...
        Self {
            ...
            lane_id: lane_id.into(),
            window_id: window_id.into(),
            status: BlobHintStatus::Encrypted,
    ...
    pub fn mark_prefetched(&mut self, fee_charged: u128) -> Result<()> {
        ...
        self.status = BlobHintStatus::Prefetched;
        Ok(())
    }

    pub fn mark_attested(&mut self) {
        if matches!(self.status, BlobHintStatus::Prefetched) {
            self.status = BlobHintStatus::Attested;
        }
    }

    pub fn consume(&mut self, height: u64) {
        self.status = BlobHintStatus::Consumed;
    ...
    pub fn attach_rebate(&mut self, rebate_id: impl Into<String>) {
        ...
        if matches!(self.status, BlobHintStatus::Consumed) {
            self.status = BlobHintStatus::Rebated;
        }
    }

    pub fn redact(&mut self) {
        self.status = BlobHintStatus::Redacted;
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct PqDaAvailabilityAttestation {
    pub attestation_id: String,
    pub lane_id: String,
    pub window_id: String,
    pub hint_ids: Vec<String>,
    pub da_availability_id: String,
...
impl PqDaAvailabilityAttestation {
    #[allow(clippy::too_many_arguments)]
    pub fn new(
        ...
        window_id: impl Into<String>,
        hint_ids: Vec<String>,
        da_availability_id: impl Into<String>,
        ...
        Self {
            ...
            window_id: window_id.into(),
            hint_ids,
            da_availability_id: da_availability_id.into(),
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "da_availability_id": self.da_availability_id,
...
impl OperatorSummary {
    ...
    pub fn record_hint(&mut self, hint: &EncryptedBlobHint) {
        if matches!(
            hint.status,
            BlobHintStatus::Prefetched | BlobHintStatus::Attested
        ) {
            self.hints_prefetched = self.hints_prefetched.saturating_add(1);
        }
        if matches!(
            hint.status,
            BlobHintStatus::Consumed | BlobHintStatus::Rebated
        ) {
...
pub struct State {
    ...
    pub height: u64,
    pub config: Config,
    pub lanes: BTreeMap<String, WitnessPrefetch>,
    pub windows: BTreeMap<String, DaBlobWitnessWindow>,
    pub encrypted_hints: BTreeMap<String, EncryptedBlobHint>,
    pub pq_attestations: BTreeMap<String, PqDaAvailabilityAttestation>,
...
impl State {
    ...
    pub fn register_lane(&mut self, lane: WitnessPrefetch) -> Result<()> {
    ...
    pub fn open_window(&mut self, mut window: DaBlobWitnessWindow) -> Result<()> {
        if self.windows.len() >= self.config.max_windows
    ...
    pub fn submit_encrypted_hint(&mut self, hint: EncryptedBlobHint) -> Result<()> {
        if self.encrypted_hints.len() >= self.config.max_hints
    ...
    pub fn add_attestation(&mut self, mut attestation: PqDaAvailabilityAttestation) -> Result<()> {
        if self.pq_attestations.len() >= self.config.max_attestations
            && !self
                .pq_attestations
    ...
    pub fn post_invalidation_fence(&mut self, fence: InvalidationFence) -> Result<()> {
        ...
        for hint_id in &fence.invalidated_hint_ids {
            if let Some(hint) = self.encrypted_hints.get_mut(hint_id) {
                hint.status = BlobHintStatus::Invalidated;
            }
        }
    ...
    pub fn expire_stale(&mut self, height: u64) {
        ...
        for window in self.windows.values_mut() {
            if window.status.live() && height > window.end_height.saturating_add(1) {
                window.status = BlobWindowStatus::Expired;
            }
        }
    }
    ...
    pub fn counters(&self) -> Counters {
        Counters {
            ...
            redacted_hints: self
                .encrypted_hints
                .values()
                .filter(|hint| matches!(hint.status, BlobHintStatus::Redacted))
                .count() as u64,
    ...
    pub fn roots(&self) -> Roots {
        ...
        let lane_root = merkle_root(
            ...
            &self
                .lanes
                .values()
                .map(WitnessPrefetch::public_record)
                .collect::<Vec<_>>(),
        );
        let window_root = merkle_root(
            ...
            &self
                .windows
                .values()
                .map(DaBlobWitnessWindow::public_record)
                .collect::<Vec<_>>(),
        );
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "lanes": self.lanes.values().map(WitnessPrefetch::public_record).collect::<Vec<_>>(),
            "windows": self.windows.values().map(DaBlobWitnessWindow::public_record).collect::<Vec<_>>(),
            "encrypted_hints": self.encrypted_hints.values().map(EncryptedBlobHint::public_record).collect::<Vec<_>>(),
            "pq_attestations": self.pq_attestations.values().map(PqDaAvailabilityAttestation::public_record).collect::<Vec<_>>(),
...
pub fn devnet() -> State {
    ...
    let fast_lane = WitnessPrefetch::new(
        "lane-fast-receipts",
    ...
    let bridge_lane = WitnessPrefetch::new(
    ...
    let window_a = DaBlobWitnessWindow::new(
        "window-fast-2160000",
        "lane-fast-receipts",
        DEVNET_HEIGHT,
    ...
    let window_b = DaBlobWitnessWindow::new(
...
pub fn demo() -> State {
    ...
    let hint_a = EncryptedBlobHint::new(
        "hint-fast-a",
        "lane-fast-receipts",
        "window-fast-2160000",
        "nullifier-fast-a",
    ...
    let hint_b = EncryptedBlobHint::new(
        "hint-bridge-a",
        "lane-bridge-receipts",
        "window-bridge-2160000",
        "nullifier-bridge-a",
    ...
    state
        ...
        .expect("demo consume");
    let attestation = PqDaAvailabilityAttestation::new(
        "attestation-fast-a",
        "lane-fast-receipts",
        "window-fast-2160000",
        vec!["hint-fast-a".to_string()],
        "da_availability-alpha-0",
        "committee-pq-alpha",
