...
pub type PrivateL2PqConfidentialContractFheStateRentRebateRuntimeResult<T> = Result<T>;
...
pub const PRIVATE_L2_PQ_CONFIDENTIAL_CONTRACT_FHE_STATE_RENT_REBATE_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-private-l2-pq-confidential-contract-fhe-state-rent-rebate-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_PQ_CONFIDENTIAL_CONTRACT_FHE_STATE_RENT_REBATE_RUNTIME_PROTOCOL_VERSION;
...
pub const DETERMINISTIC_PUBLIC_RECORD_SCHEME: &str =
    "deterministic-FHE-STATE-RENT-REBATE-public-record-and-roots-v1";
...
impl State {
    ...
    ) -> PrivateL2PqConfidentialContractFheStateRentRebateRuntimeResult<String> {
        ensure!(
    ...
    fn issue_low_fee_storage_rebate(
        ...
        let rebate_commitment = deterministic_record_root(
            "PRIVATE-L2-PQ-FHE-STATE-RENT-REBATE:REBATE-COMMITMENT",
            &record_seed,
        );
        let eligibility_root = deterministic_record_root(
            "PRIVATE-L2-PQ-FHE-STATE-RENT-REBATE:REBATE-ELIGIBILITY",
            &record_seed,
        );
    ...
    fn publish_public_record(
        ...
        domain: &str,
        subject_id: &str,
    ) -> PrivateL2PqConfidentialContractFheStateRentRebateRuntimeResult<()> {
        ensure!(
        ...
        let record_root = deterministic_record_root(
            "PRIVATE-L2-PQ-FHE-STATE-RENT-REBATE:PUBLIC-RECORD",
            &record_seed,
        );
...
pub fn devnet() -> State {
    ...
    tags.insert("FHE-STATE-RENT-REBATE".to_string());
    ...
    let namespace_id = state
        .register_fhe_namespace(RegisterFheNamespaceRequest {
            ...
            policy_root: "policy:root:FHE-STATE-RENT-REBATE-devnet".to_string(),
...
pub fn deterministic_id(kind: &str, sequence: u64, record: &Value) -> String {
    domain_hash(
        &format!("PRIVATE-L2-PQ-FHE-STATE-RENT-REBATE:{kind}"),
        &[
...
pub fn public_record_root(domain: &str, records: &[Value]) -> String {
    merkle_root(
        &format!("PRIVATE-L2-PQ-FHE-STATE-RENT-REBATE:{domain}-ROOT"),
        records,
    )
}
...
pub fn state_root_from_record(record: &Value) -> String {
    domain_hash(
        "PRIVATE-L2-PQ-FHE-STATE-RENT-REBATE:STATE-ROOT",
...
fn empty_root(domain: &str) -> String {
    merkle_root(
        &format!("PRIVATE-L2-PQ-FHE-STATE-RENT-REBATE:{domain}-EMPTY"),
        &[],
    )
}
