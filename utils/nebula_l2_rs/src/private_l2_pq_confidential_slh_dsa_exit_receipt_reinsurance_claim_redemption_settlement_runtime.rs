...
pub type PrivateL2PqConfidentialSlhDsaExitReceiptReinsuranceClaimRedemptionSettlementRuntimeResult<
    T,
> = Result<T>;
pub type Runtime = State;

pub const PRIVATE_L2_PQ_CONFIDENTIAL_SLH_DSA_EXIT_RECEIPT_REINSURANCE_CLAIM_REDEMPTION_SETTLEMENT_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-private-l2-pq-confidential-slh-dsa-exit-receipt-reinsurance-claim-redemption-settlement-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_PQ_CONFIDENTIAL_SLH_DSA_EXIT_RECEIPT_REINSURANCE_CLAIM_REDEMPTION_SETTLEMENT_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const SLH_DSA_SETTLEMENT_SUITE: &str =
    "slh-dsa-hash-based-exit-receipt-reinsurance-claim-redemption-settlement-v1";
pub const CONFIDENTIAL_EXIT_RECEIPT_SUITE: &str =
    "slh-dsa-confidential-exit-receipt-settlement-commitment-v1";
pub const CLAIM_REDEMPTION_AUTH_SUITE: &str =
    "hash-based-slh-dsa-private-claim-redemption-authorization-v1";
pub const REINSURANCE_SETTLEMENT_ACCOUNTING_SUITE: &str =
    "roots-only-slh-dsa-reinsurance-claim-redemption-settlement-accounting-v1";
pub const LOW_FEE_SETTLEMENT_BATCH_SUITE: &str =
    "low-fee-slh-dsa-claim-redemption-settlement-batch-v1";
...
pub const ROOTS_ONLY_PUBLIC_RECORD_SUITE: &str =
    "roots-only-slh-dsa-claim-redemption-settlement-public-record-v1";
pub const DEVNET_HEIGHT: u64 = 9_944_000;
pub const DEVNET_EPOCH: u64 = 41_433;
pub const DEVNET_SLOT: u64 = 608;
...
pub const DEFAULT_SLH_DSA_FORS_HEIGHT: u8 = 9;
pub const DEFAULT_SLH_DSA_SIGNATURE_BYTES_CAP: u32 = 49_856;
...
impl ProofMode {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SlhDsaHashAttestation => "slh_dsa_hash_attestation",
...
pub struct Config {
    ...
    pub slh_dsa_settlement_suite: String,
    ...
    pub slh_dsa_fors_height: u8,
    pub slh_dsa_signature_bytes_cap: u32,
    ...
    pub slh_dsa_authorization_required: bool,
...
impl Config {
    pub fn devnet() -> Self {
        Self {
            ...
            slh_dsa_settlement_suite: SLH_DSA_SETTLEMENT_SUITE.to_string(),
            ...
            slh_dsa_fors_height: DEFAULT_SLH_DSA_FORS_HEIGHT,
            ...
            slh_dsa_authorization_required: true,
    ...
    pub fn validate(&self) -> Result<()> {
        if self.min_pq_security_bits < DEFAULT_MIN_PQ_SECURITY_BITS {
            return Err("pq security bits below slh-dsa settlement minimum".to_string());
        }
        if self.slh_dsa_tree_height < 12
            || self.slh_dsa_layer_count == 0
            || self.slh_dsa_fors_trees < 30
            || self.slh_dsa_fors_height < 8
        {
            return Err("invalid slh-dsa settlement parameter schedule".to_string());
        }
        if self.slh_dsa_signature_bytes_cap < 16_000 {
            return Err("slh-dsa signature cap below expected hash-based floor".to_string());
        }
        ...
        if !self.slh_dsa_authorization_required
        ...
        {
            return Err(
                "slh-dsa, confidentiality, nullifier, reserve, and roots-only gates are mandatory"
                    .to_string(),
            );
        }
        Ok(())
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "suites": {
                "slh_dsa_settlement": self.slh_dsa_settlement_suite,
            ...
            "security": {
                ...
                "slh_dsa_tree_height": self.slh_dsa_tree_height,
                "slh_dsa_layer_count": self.slh_dsa_layer_count,
                "slh_dsa_fors_trees": self.slh_dsa_fors_trees,
                "slh_dsa_fors_height": self.slh_dsa_fors_height,
                "slh_dsa_signature_bytes_cap": self.slh_dsa_signature_bytes_cap,
            },
            "timing": {
...
pub struct Counters {
    ...
    pub slh_dsa_authorizations: u64,
...
pub struct Roots {
    ...
    pub slh_dsa_authorization_root: String,
...
impl Default for Roots {
    fn default() -> Self {
        ...
        Self {
            ...
            slh_dsa_authorization_root: empty.clone(),
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct SlhDsaClaimAuthorizationInput {
    pub settlement_id: String,
    pub slh_dsa_public_key_root: String,
    pub slh_dsa_signature_root: String,
    pub message_digest_root: String,
    pub hypertree_auth_path_root: String,
    pub fors_transcript_root: String,
    pub randomized_hash_root: String,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct SlhDsaClaimAuthorization {
...
impl SlhDsaClaimAuthorization {
    pub fn from_input(config: &Config, input: SlhDsaClaimAuthorizationInput) -> Result<Self> {
        require_non_empty(&[
            (&input.settlement_id, "settlement id"),
            (&input.slh_dsa_public_key_root, "slh-dsa public key root"),
            (&input.slh_dsa_signature_root, "slh-dsa signature root"),
            (&input.message_digest_root, "message digest root"),
            (&input.hypertree_auth_path_root, "hypertree auth path root"),
            (&input.fors_transcript_root, "fors transcript root"),
            (&input.randomized_hash_root, "randomized hash root"),
        ])?;
        if input.pq_security_bits < config.min_pq_security_bits {
            return Err("slh-dsa authorization below configured pq security".to_string());
        }
        if input.signature_bytes == 0 || input.signature_bytes > config.slh_dsa_signature_bytes_cap
        {
            return Err("slh-dsa authorization signature size outside policy".to_string());
        }
        let authorization_id = deterministic_id(
            "slh-dsa-claim-authorization",
            &[
                HashPart::Str(&input.settlement_id),
                HashPart::Str(&input.slh_dsa_public_key_root),
                HashPart::Str(&input.slh_dsa_signature_root),
        ...
        Ok(Self {
            authorization_id,
            settlement_id: input.settlement_id,
            slh_dsa_public_key_root: input.slh_dsa_public_key_root,
            slh_dsa_signature_root: input.slh_dsa_signature_root,
            message_digest_root: input.message_digest_root,
            hypertree_auth_path_root: input.hypertree_auth_path_root,
            fors_transcript_root: input.fors_transcript_root,
            randomized_hash_root: input.randomized_hash_root,
            pq_security_bits: input.pq_security_bits,
            signature_bytes: input.signature_bytes,
            tree_height: config.slh_dsa_tree_height,
            layer_count: config.slh_dsa_layer_count,
            fors_trees: config.slh_dsa_fors_trees,
            fors_height: config.slh_dsa_fors_height,
    ...
    pub fn roots_only_record(&self) -> Value {
        json!({
            ...
            "hypertree_auth_path_root": self.hypertree_auth_path_root,
            "fors_transcript_root": self.fors_transcript_root,
            "randomized_hash_root": self.randomized_hash_root,
...
pub struct State {
    ...
    pub slh_dsa_authorizations: BTreeMap<String, SlhDsaClaimAuthorization>,
...
impl State {
    pub fn new(config: Config, height: u64, epoch: u64, slot: u64) -> Result<Self> {
        ...
        let mut state = Self {
            ...
            slh_dsa_authorizations: BTreeMap::new(),
    ...
    pub fn devnet() -> Self {
        ...
        state
            .authorize_claim(SlhDsaClaimAuthorizationInput {
                settlement_id: settlement_id.clone(),
                slh_dsa_public_key_root: sample_root("slh-dsa-pk", 1),
                slh_dsa_signature_root: sample_root("slh-dsa-sig", 1),
                message_digest_root: sample_root("message-digest", 1),
                hypertree_auth_path_root: sample_root("hypertree-path", 1),
                fors_transcript_root: sample_root("fors-transcript", 1),
                randomized_hash_root: sample_root("randomized-hash", 1),
    ...
    pub fn authorize_claim(&mut self, input: SlhDsaClaimAuthorizationInput) -> Result<String> {
        let authorization = SlhDsaClaimAuthorization::from_input(&self.config, input)?;
        let claim = self
            .settlement_claims
            .get_mut(&authorization.settlement_id)
            .ok_or_else(|| "unknown settlement for slh-dsa authorization".to_string())?;
        if authorization.authorized_slot > claim.expires_slot {
            return Err("slh-dsa authorization outside settlement window".to_string());
        }
        claim.status = SettlementStatus::ClaimAuthorized;
        let authorization_id = authorization.authorization_id.clone();
        self.counters.slh_dsa_authorizations += 1;
        self.slh_dsa_authorizations
    ...
    fn compute_roots(&self) -> Roots {
        ...
        let slh_dsa_authorization_root = record_root(
            "slh-dsa-authorizations",
            self.slh_dsa_authorizations
                .values()
                .map(SlhDsaClaimAuthorization::roots_only_record)
                .collect(),
        );
        ...
        let public_record_root = value_root(
            ...
            &json!({
                ...
                "slh_dsa_authorization_root": slh_dsa_authorization_root,
        ...
        let state_root = domain_hash(
            "PRIVATE-L2-PQ-CONFIDENTIAL-SLH-DSA-EXIT-RECEIPT-REINSURANCE-CLAIM-REDEMPTION-SETTLEMENT-STATE",
            &[
                ...
                HashPart::Str(&slh_dsa_authorization_root),
...
pub fn public_record(state: &State) -> Value {
    json!({
        ...
        "slh_dsa_settlement_suite": SLH_DSA_SETTLEMENT_SUITE,
        ...
        "redacted_private_records": {
            ...
            "slh_dsa_authorizations": state.roots.slh_dsa_authorization_root,
...
pub fn deterministic_id(domain: &str, parts: &[HashPart<'_>]) -> String {
    domain_hash(
        &format!("PRIVATE-L2-PQ-CONFIDENTIAL-SLH-DSA-EXIT-RECEIPT-REINSURANCE-CLAIM-REDEMPTION-SETTLEMENT-{domain}-ID"),
        parts,
        24,
    )
}
...
pub fn sample_root(label: &str, index: u64) -> String {
    domain_hash(
        "PRIVATE-L2-PQ-CONFIDENTIAL-SLH-DSA-EXIT-RECEIPT-REINSURANCE-CLAIM-REDEMPTION-SETTLEMENT-DEVNET-SAMPLE",
...
pub fn value_root(domain: &str, value: &Value) -> String {
    domain_hash(
        &format!("PRIVATE-L2-PQ-CONFIDENTIAL-SLH-DSA-EXIT-RECEIPT-REINSURANCE-CLAIM-REDEMPTION-SETTLEMENT-{domain}"),
        &[HashPart::Json(value)],
        32,
    )
}
