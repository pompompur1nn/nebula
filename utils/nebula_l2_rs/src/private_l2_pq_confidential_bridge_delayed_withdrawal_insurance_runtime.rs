...
pub type PrivateL2PqConfidentialBridgeDelayedWithdrawalInsuranceRuntimeResult<T> = Result<T>;
pub type Runtime = State;

pub const PROTOCOL_VERSION: &str =
    "nebula-private-l2-pq-confidential-bridge-delayed-withdrawal-insurance-runtime-v1";
pub const PRIVATE_L2_PQ_CONFIDENTIAL_BRIDGE_DELAYED_WITHDRAWAL_INSURANCE_RUNTIME_PROTOCOL_VERSION:
    &str = PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const PQ_INSURANCE_SUITE: &str = "ML-DSA-87+ML-KEM-1024+SLH-DSA-SHAKE-256f";
pub const POLICY_SUITE: &str = "confidential-monero-bridge-delayed-withdrawal-policy-root-v1";
pub const CLAIM_SUITE: &str = "confidential-delayed-withdrawal-insurance-claim-root-v1";
pub const REBATE_SUITE: &str = "delayed-withdrawal-insurance-low-fee-rebate-root-v1";
pub const REDACTION_SUITE: &str = "operator-safe-delayed-withdrawal-insurance-redaction-root-v1";
pub const DEFAULT_FEE_ASSET_ID: &str = "piconero-devnet";
pub const DEFAULT_COVERAGE_ASSET_ID: &str = "xmr-bridge-withdrawal-insurance-note-devnet";
pub const DEFAULT_MIN_PRIVACY_SET_SIZE: u64 = 65_536;
pub const DEFAULT_TARGET_PRIVACY_SET_SIZE: u64 = 262_144;
pub const DEFAULT_MIN_PQ_SECURITY_BITS: u16 = 256;
pub const DEFAULT_COVERAGE_WINDOW_SLOTS: u64 = 720;
pub const DEFAULT_CLAIM_WINDOW_SLOTS: u64 = 144;
pub const DEFAULT_MAX_PREMIUM_BPS: u64 = 24;
pub const DEFAULT_TARGET_REBATE_BPS: u64 = 8;
pub const DEFAULT_MIN_POOL_RESERVE_MICRO_UNITS: u64 = 120_000_000;
...
pub const DEFAULT_MAX_PAYOUT_BPS: u64 = 5_000;
...
pub const DEVNET_EPOCH: u64 = 7_616;
pub const DEVNET_SLOT: u64 = 173;
...
pub enum CoverageScope {
    ...
    FastExitFallback,
    EmergencyExit,
    ReorgDelayedWithdrawal,
    WatchtowerDispute,
    LiquidityBackstopDelay,
    AtomicSwapTimeout,
    ReserveProofDelay,
}
...
impl CoverageScope {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::StandardWithdrawal => "standard_withdrawal",
            Self::FastExitFallback => "fast_exit_fallback",
            Self::EmergencyExit => "emergency_exit",
            Self::ReorgDelayedWithdrawal => "reorg_delayed_withdrawal",
            Self::WatchtowerDispute => "watchtower_dispute",
            Self::LiquidityBackstopDelay => "liquidity_backstop_delay",
            Self::AtomicSwapTimeout => "atomic_swap_timeout",
            Self::ReserveProofDelay => "reserve_proof_delay",
        }
    }

    pub fn risk_weight(self) -> u64 {
        match self {
            Self::EmergencyExit => 10,
            Self::ReorgDelayedWithdrawal => 9,
            Self::WatchtowerDispute => 8,
            Self::ReserveProofDelay => 7,
            Self::LiquidityBackstopDelay => 6,
            Self::FastExitFallback => 5,
            Self::AtomicSwapTimeout => 5,
            Self::StandardWithdrawal => 3,
        }
    }
}
...
pub enum DelayCause {
    ...
    WatcherDisagreement,
    ReserveProofLag,
    LiquidityRouteExhausted,
    FeeSpikeBackpressure,
    EmergencyExitCongestion,
    AtomicSwapCounterpartyTimeout,
    PrivacyBudgetCooldown,
}
...
impl DelayCause {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::WatcherDisagreement => "watcher_disagreement",
            Self::ReserveProofLag => "reserve_proof_lag",
            Self::LiquidityRouteExhausted => "liquidity_route_exhausted",
            Self::FeeSpikeBackpressure => "fee_spike_backpressure",
            Self::EmergencyExitCongestion => "emergency_exit_congestion",
            Self::AtomicSwapCounterpartyTimeout => "atomic_swap_counterparty_timeout",
            Self::PrivacyBudgetCooldown => "privacy_budget_cooldown",
        }
    }
}
...
pub enum AttestationKind {
    PqInsurerSignatureVerified,
    DelayWindowObserved,
    WithdrawalCommitmentOpened,
...
impl AttestationKind {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::PqInsurerSignatureVerified => "pq_insurer_signature_verified",
            Self::DelayWindowObserved => "delay_window_observed",
            Self::WithdrawalCommitmentOpened => "withdrawal_commitment_opened",
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            pq_insurance_suite: PQ_INSURANCE_SUITE.to_string(),
            policy_suite: POLICY_SUITE.to_string(),
            ...
            coverage_window_slots: DEFAULT_COVERAGE_WINDOW_SLOTS,
...
pub struct WithdrawalPolicy {
    ...
    pub insured_withdrawal_root: String,
    pub claimant_commitment: String,
    pub withdrawal_hint_root: String,
...
pub struct DelayClaim {
    ...
    pub delay_commitment_root: String,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct UnderwritePolicyReceipt {
    pub policy_id: String,
    pub expires_slot: u64,
    pub premium_micro_units: u64,
    pub state_root: String,
...
pub struct State {
    ...
    pub policies: BTreeMap<String, WithdrawalPolicy>,
    pub claims: BTreeMap<String, DelayClaim>,
    pub attestations: BTreeMap<String, ClaimAttestation>,
...
impl State {
    ...
    pub fn open_pool(&mut self, request: OpenPoolRequest) -> Result<OpenPoolReceipt> {
        ensure_capacity(self.pools.len(), MAX_POOLS, "insurance pools")?;
        ...
        if self.pools.contains_key(&pool_id) {
            return Err(format!("insurance pool {pool_id} already exists"));
        }
    ...
    pub fn underwrite_policy(
        &mut self,
        request: UnderwritePolicyRequest,
    ) -> Result<UnderwritePolicyReceipt> {
        ensure_capacity(self.policies.len(), MAX_POLICIES, "policies")?;
        ensure_non_empty(&request.pool_id, "pool_id")?;
        ensure_non_empty(&request.insured_withdrawal_root, "insured_withdrawal_root")?;
        ensure_non_empty(&request.claimant_commitment, "claimant_commitment")?;
        ensure_non_empty(&request.withdrawal_hint_root, "withdrawal_hint_root")?;
        ensure_bps(request.premium_bps, "premium_bps")?;
        ...
        if !matches!(pool.status, PoolStatus::Open | PoolStatus::Throttled) {
            return Err("insurance pool is not accepting policies".to_string());
        }
        if request.coverage_micro_units > pool.available_reserve_micro_units {
            return Err("coverage exceeds available pool reserve".to_string());
        }
        ...
        let policy_id = stable_id(
            ...
            &[
                ...
                HashPart::Str(&request.insured_withdrawal_root),
        ...
        let expires_slot = request
            .issued_slot
            .saturating_add(self.config.coverage_window_slots);
        let policy = WithdrawalPolicy {
            policy_id: policy_id.clone(),
            pool_id: request.pool_id.clone(),
            status: PolicyStatus::Active,
            insured_withdrawal_root: request.insured_withdrawal_root,
            claimant_commitment: request.claimant_commitment,
            withdrawal_hint_root: request.withdrawal_hint_root,
    ...
    pub fn submit_claim(&mut self, request: SubmitClaimRequest) -> Result<SubmitClaimReceipt> {
        ensure_capacity(self.claims.len(), MAX_CLAIMS, "claims")?;
        ...
        ensure_non_empty(&request.delay_commitment_root, "delay_commitment_root")?;
        ensure_bps(request.requested_payout_bps, "requested_payout_bps")?;
        ensure_bps(request.requested_rebate_bps, "requested_rebate_bps")?;
        if request.requested_payout_bps > self.config.max_payout_bps {
            return Err("requested_payout_bps exceeds configured maximum".to_string());
        }
        ...
        let claim = DelayClaim {
            ...
            delay_commitment_root: request.delay_commitment_root,
    ...
    pub fn settle_claim(&mut self, request: SettleClaimRequest) -> Result<SettleClaimReceipt> {
        ...
        if payout_micro_units > pool.reserve_micro_units {
            return Err("payout exceeds pool reserve".to_string());
        }
    ...
    pub fn issue_rebate(&mut self, request: IssueRebateRequest) -> Result<IssueRebateReceipt> {
        ...
        if request.fee_rebate_bps > self.config.max_premium_bps {
    ...
    pub fn refresh_roots(&mut self) {
        ...
        self.roots.state_root = merkle_root(
            "bridge-delayed-withdrawal-insurance:state",
            &[
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "pq_insurance_suite": self.config.pq_insurance_suite,
            "policy_suite": self.config.policy_suite,
            ...
            "coverage_window_slots": self.config.coverage_window_slots,
            ...
            "policies": self.policies.values().map(|policy| json!({
                ...
                "withdrawal_hint_root": policy.withdrawal_hint_root,
            ...
            "claims": self.claims.values().map(|claim| json!({
                ...
                "delay_commitment_root": claim.delay_commitment_root,
            ...
            "payouts": self.payouts.values().map(|payout| json!({
                "payout_id": payout.payout_id,
                "claim_id": payout.claim_id,
                "policy_id": payout.policy_id,
                "pool_id": payout.pool_id,
                "decision": payout.decision,
                "payout_bps": payout.payout_bps,
                "payout_micro_units": payout.payout_micro_units,
                "reserve_remainder_micro_units": payout.reserve_remainder_micro_units,
                "settled_slot": payout.settled_slot,
            })).collect::<Vec<_>>(),
            "rebates": self.rebates.values().map(|rebate| json!({
                "rebate_id": rebate.rebate_id,
                "claim_id": rebate.claim_id,
...
pub fn devnet() -> State {
    ...
    let pool = state
        .open_pool(OpenPoolRequest {
            scope: CoverageScope::ReorgDelayedWithdrawal,
            ...
            reserve_micro_units: 260_000_000,
            premium_bps: 12,
            payout_cap_bps: 2_500,
            opened_slot: DEVNET_SLOT,
        })
        .expect("devnet delayed-withdrawal insurance pool opened");
    let policy = state
        .underwrite_policy(UnderwritePolicyRequest {
            pool_id: pool.pool_id.clone(),
            insured_withdrawal_root: sample_hash("withdrawal", 1),
            claimant_commitment: sample_hash("claimant", 1),
            withdrawal_hint_root: sample_hash("withdrawal-hint", 1),
            coverage_micro_units: 48_000_000,
            premium_bps: 10,
            privacy_set_size: DEFAULT_TARGET_PRIVACY_SET_SIZE,
            issued_slot: DEVNET_SLOT + 1,
        })
        .expect("devnet delayed-withdrawal policy underwritten");
    let claim = state
        .submit_claim(SubmitClaimRequest {
            policy_id: policy.policy_id.clone(),
            cause: DelayCause::WatcherDisagreement,
            sealed_evidence_root: sample_hash("sealed-evidence", 1),
            redacted_evidence_root: sample_hash("redacted-evidence", 1),
            delay_commitment_root: sample_hash("delay", 1),
            requested_payout_bps: 1_400,
            requested_rebate_bps: DEFAULT_TARGET_REBATE_BPS,
            submitted_slot: DEVNET_SLOT + 12,
        })
        .expect("devnet delay claim submitted");
    state
        ...
        .expect("devnet delay claim attested");
    state
        .settle_claim(SettleClaimRequest {
            ...
            settled_slot: DEVNET_SLOT + 24,
        })
        .expect("devnet delay claim settled");
    state
        .issue_rebate(IssueRebateRequest {
            ...
            amount_micro_units: 980,
            fee_rebate_bps: DEFAULT_TARGET_REBATE_BPS,
            issued_slot: DEVNET_SLOT + 25,
            expires_slot: DEVNET_SLOT + DEFAULT_COVERAGE_WINDOW_SLOTS,
        })
    ...
    state
        .publish_redaction_budget(RedactionBudgetRequest {
            ...
            public_fields: [
                ...
                "pool_id",
                "cause",
                "requested_payout_bps",
            ]
            .into_iter()
            .map(str::to_string)
            .collect(),
            redacted_fields: [
                "insured_withdrawal_root",
                "claimant_commitment",
...
pub fn demo() -> State {
    ...
    state
        .open_pool(OpenPoolRequest {
            scope: CoverageScope::EmergencyExit,
        ...
        .expect("demo delayed-withdrawal insurance pool opened");
    state.refresh_roots();
    state
}
...
fn stable_id(domain: &str, parts: &[HashPart<'_>]) -> String {
    domain_hash(
        &format!("bridge-delayed-withdrawal-insurance:{domain}:id"),
        parts,
        24,
    )
}
...
fn empty_root(domain: &str) -> String {
    merkle_root(
        &format!("bridge-delayed-withdrawal-insurance:{domain}"),
        &[],
    )
}
...
fn sample_hash(label: &str, index: u64) -> String {
    domain_hash(
        "bridge-delayed-withdrawal-insurance:devnet-sample",
