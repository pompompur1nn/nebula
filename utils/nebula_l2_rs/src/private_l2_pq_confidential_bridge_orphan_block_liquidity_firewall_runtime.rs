...
pub type PrivateL2PqConfidentialBridgeOrphanBlockLiquidityFirewallRuntimeResult<T> = Result<T>;
pub type Runtime = State;

pub const PROTOCOL_VERSION: &str =
    "nebula-private-l2-pq-confidential-bridge-orphan-block-liquidity-firewall-runtime-v1";
pub const PRIVATE_L2_PQ_CONFIDENTIAL_BRIDGE_ORPHAN_BLOCK_LIQUIDITY_FIREWALL_RUNTIME_PROTOCOL_VERSION:
    &str = PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const PQ_ORPHAN_BLOCK_LIQUIDITY_FIREWALL_SUITE: &str =
    "ML-DSA-87+ML-KEM-1024+SLH-DSA-SHAKE-256f";
pub const ORPHAN_BLOCK_LIQUIDITY_FIREWALL_SUITE: &str =
    "monero-bridge-orphan-block-liquidity-firewall-root-v1";
pub const REBATE_SUITE: &str = "orphan-block-liquidity-firewall-low-fee-rebate-root-v1";
pub const REDACTION_SUITE: &str = "operator-safe-orphan-block-liquidity-firewall-redaction-root-v1";
...
pub const DEFAULT_ORPHAN_BLOCK_WINDOW_SLOTS: u64 = 720;
...
impl OrphanBlockScope {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::EmergencyOrphanBlockLiquidityFirewall => {
                "emergency_orphan_block_liquidity_firewall"
            }
...
#[serde(rename_all = "snake_case")]
pub enum SignalKind {
    OrphanBlockLinkagePressure,
    OrphanBlockRelayFragmentation,
    OrphanBlockRelaySkew,
    OrphanBlockGraphLinkagePressure,
    OrphanBlockTimingCorrelationPressure,
    ReserveMismatch,
    FeeSpikeOrphanBlockFirewallClaimPressure,
    WatchtowerEmergency,
}
...
pub enum AttestationVerdict {
    ConfirmOrphanBlockPressure,
    ConfirmPrivacyRisk,
    RequireOrphanBlockCoverage,
    RequireOrphanBlockCoverageGuard,
    RequireQuarantine,
    ClearFalsePositive,
}
...
#[derive(Clone, Copy, Debug, Deserialize, Eq, Ord, PartialEq, PartialOrd, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum OrphanBlockAction {
    Observe,
    QueueOrphanBlockFirewallClaimReview,
    SettleOrphanBlock,
    RotateOrphanBlockFirewallClaimClaimSettlementCohort,
...
pub struct Config {
    ...
    pub orphan_block_liquidity_firewall_suite: String,
    ...
    pub orphan_block_window_slots: u64,
    pub signal_window_slots: u64,
    pub max_claim_settlement_fee_bps: u64,
    pub target_rebate_bps: u64,
    pub min_pre_orphan_block_reserve_micro_units: u64,
    pub min_orphan_block_lane_claim_reserve_micro_units: u64,
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            pq_claim_settlement_insurance_suite: PQ_ORPHAN_BLOCK_LIQUIDITY_FIREWALL_SUITE
                .to_string(),
            orphan_block_liquidity_firewall_suite: ORPHAN_BLOCK_LIQUIDITY_FIREWALL_SUITE
                .to_string(),
            ...
            orphan_block_window_slots: DEFAULT_ORPHAN_BLOCK_WINDOW_SLOTS,
            signal_window_slots: DEFAULT_SIGNAL_WINDOW_SLOTS,
            max_claim_settlement_fee_bps: DEFAULT_MAX_ROUTE_FEE_BPS,
            target_rebate_bps: DEFAULT_TARGET_REBATE_BPS,
            min_pre_orphan_block_reserve_micro_units: DEFAULT_MIN_SOURCE_LIQUIDITY_MICRO_UNITS,
            min_orphan_block_lane_claim_reserve_micro_units:
...
pub struct Counters {
    ...
    pub orphan_block_filters_settled: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct OrphanBlockPolicy {
    pub policy_id: String,
    pub scope: OrphanBlockScope,
    pub owner_commitment: String,
    pub policy_commitment_root: String,
    pub claim_source_orphan_block_set_root: String,
    pub claim_target_orphan_block_set_root: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct OrphanBlockLiquidityFirewallCohort {
    ...
    pub pre_orphan_block_reserve_micro_units: u64,
    pub orphan_block_lane_claim_reserve_micro_units: u64,
...
pub struct OrphanBlockSignal {
    ...
    pub orphan_block_pressure_root: String,
    pub reporter_commitment: String,
    pub pre_orphan_block_reserve_delta_micro_units: i128,
    pub orphan_block_lane_claim_reserve_delta_micro_units: i128,
    pub privacy_set_size: u64,
    pub orphan_block_risk_score_bps: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct PqOrphanBlockFirewallAttestation {
    ...
    pub observed_orphan_block_risk_score_bps: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct OrphanBlockDecision {
    pub decision_id: String,
    pub signal_id: String,
    pub cohort_id: String,
    pub policy_id: String,
    pub action: OrphanBlockAction,
    pub decision_root: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct LowFeeOrphanBlockFirewallRebate {
...
pub struct State {
    ...
    pub roots: Roots,
    pub policies: BTreeMap<String, OrphanBlockPolicy>,
    pub cohorts: BTreeMap<String, OrphanBlockLiquidityFirewallCohort>,
    pub signals: BTreeMap<String, OrphanBlockSignal>,
    pub attestations: BTreeMap<String, PqOrphanBlockFirewallAttestation>,
    pub decisions: BTreeMap<String, OrphanBlockDecision>,
    pub rebates: BTreeMap<String, LowFeeOrphanBlockFirewallRebate>,
...
impl State {
    ...
    pub fn devnet() -> Self {
        ...
        let withdrawal_policy = state
            .register_policy(RegisterPolicyRequest {
                scope: OrphanBlockScope::WithdrawalSubaddressLane,
                owner_commitment: "owner:dandelion-orphan-block-liquidity-firewall-council-a"
                    .to_string(),
                policy_commitment_root: devnet_root(
                    "policy",
                    "withdrawal-dandelion-orphan-block-liquidity-firewall",
                ),
                claim_source_orphan_block_set_root: devnet_root(
                    "source-orphan_block-relay-set",
                    "withdrawal-alpha",
                ),
                claim_target_orphan_block_set_root: devnet_root(
                    "target-orphan_block-relay-set",
                    "withdrawal-alpha",
                ),
                ...
                min_pre_orphan_block_reserve_micro_units: 160_000_000,
                min_orphan_block_lane_claim_reserve_micro_units: 72_000_000,
                max_claim_settlement_fee_bps: 16,
                min_attestation_quorum_bps: 7_200,
                signal_window_slots: 96,
                orphan_block_window_slots: 720,
                redaction_profile: "withdrawal-dandelion-orphan-block-liquidity-firewall-profile"
                    .to_string(),
            })
            .expect("devnet withdrawal policy");
        let fast_dandelion_exit_policy = state
            .register_policy(RegisterPolicyRequest {
                scope: OrphanBlockScope::FastDandelionOrphanBlockFirewallClaimLane,
                owner_commitment: "owner:fast-dandelion-decoy-insurance-orphan_claim_path-council-b".to_string(),
                policy_commitment_root: devnet_root(
                    "policy",
                    "fast-dandelion-decoy-insurance-dandelion-orphan-block-liquidity-firewall",
                ),
                claim_source_orphan_block_set_root: devnet_root(
                    "source-orphan_block-relay-set",
                ...
                min_pre_orphan_block_reserve_micro_units: 144_000_000,
                min_orphan_block_lane_claim_reserve_micro_units: 64_000_000,
                max_claim_settlement_fee_bps: 18,
                min_attestation_quorum_bps: 7_000,
                signal_window_slots: 80,
                orphan_block_window_slots: 640,
                redaction_profile:
                    "fast-dandelion-decoy-insurance-dandelion-orphan-block-liquidity-firewall-profile"
                        .to_string(),
            })
            .expect("devnet fast orphan_block policy");
        let withdrawal_cohort = state
            .register_cohort(RegisterCohortRequest {
                ...
                pre_orphan_block_reserve_micro_units: 216_000_000,
                orphan_block_lane_claim_reserve_micro_units: 96_000_000,
        ...
        let fast_dandelion_exit_cohort = state
            .register_cohort(RegisterCohortRequest {
                ...
                pre_orphan_block_reserve_micro_units: 168_000_000,
                orphan_block_lane_claim_reserve_micro_units: 76_000_000,
                privacy_set_size: 393_216,
                claim_settlement_fee_bps: 13,
                public_hint_root: devnet_root("hint", "cohort-beta"),
            })
            .expect("devnet fast orphan_block cohort");
        let signal = state
            .submit_signal(SubmitSignalRequest {
                cohort_id: withdrawal_cohort,
                kind: SignalKind::OrphanBlockLinkagePressure,
                ...
                orphan_block_pressure_root: devnet_root("pressure", "withdrawal-cluster-pressure"),
                reporter_commitment: "reporter:dandelion-orphan_claim_path-watchtower-11"
                    .to_string(),
                pre_orphan_block_reserve_delta_micro_units: -48_000_000,
                orphan_block_lane_claim_reserve_delta_micro_units: 24_000_000,
                privacy_set_size: 524_288,
                orphan_block_risk_score_bps: 7_300,
        ...
        state
            .submit_attestation(SubmitAttestationRequest {
                ...
                verdict: AttestationVerdict::RequireOrphanBlockCoverageGuard,
                attestor_commitment: "attestor:pq-dandelion-orphan-block-liquidity-firewall-a"
                    .to_string(),
                pq_signature_root: devnet_root("ml-dsa-signature", "withdrawal-orphan_block-a"),
                pq_public_key_root: devnet_root("ml-kem-public-key", "withdrawal-orphan_block-a"),
                transcript_root: devnet_root("transcript", "withdrawal-orphan_block-a"),
                quorum_weight_bps: 7_500,
                pq_security_bits: 256,
                observed_orphan_block_risk_score_bps: 7_500,
        ...
        state
            .submit_attestation(SubmitAttestationRequest {
                signal_id: signal.clone(),
                verdict: AttestationVerdict::ConfirmOrphanBlockPressure,
                attestor_commitment: "attestor:pq-dandelion-orphan-block-liquidity-firewall-b"
                    .to_string(),
                pq_signature_root: devnet_root("slh-dsa-signature", "withdrawal-orphan_block-b"),
                pq_public_key_root: devnet_root("ml-kem-public-key", "withdrawal-orphan_block-b"),
                transcript_root: devnet_root("transcript", "withdrawal-orphan_block-b"),
                quorum_weight_bps: 8_700,
                pq_security_bits: 256,
                observed_orphan_block_risk_score_bps: 7_800,
                observed_privacy_set_size: 524_288,
        ...
        let decision = state
            .settle_decision(SettleDecisionRequest {
                signal_id: signal,
                action: OrphanBlockAction::SettleOrphanBlock,
                decision_root: devnet_root("decision", "withdrawal-liquidity-orphan_block"),
                replacement_claim_orphan_claim_path_root: devnet_root(
                    "replacement-orphan_claim_path",
                    "withdrawal-orphan_block",
                ),
            ...
            .expect("devnet orphan_block decision");
        state
            .publish_rebate(PublishRebateRequest {
                decision_id: decision,
                recipient_commitment:
                    "recipient:dandelion-orphan-block-liquidity-firewall-fee-sponsor".to_string(),
                rebate_micro_units: 22_000,
                fee_saved_micro_units: 88_000,
                rebate_bps: 8,
                settlement_slot: DEVNET_SLOT + 6,
                rebate_root: devnet_root("rebate", "withdrawal-orphan_block"),
            })
    ...
    pub fn register_policy(&mut self, request: RegisterPolicyRequest) -> Result<String> {
        ensure_capacity(
            self.policies.len(),
            MAX_POLICIES,
            "Dandelion orphan_block-phase privacy guard policies",
        )?;
        ...
        ensure_non_empty(
            &request.claim_source_orphan_block_set_root,
            "claim_source_orphan_block_set_root",
        )?;
        ensure_non_empty(
            &request.claim_target_orphan_block_set_root,
            "claim_target_orphan_block_set_root",
        )?;
        ensure_non_empty(
        ...
        if request.min_pre_orphan_block_reserve_micro_units
            < self.config.min_pre_orphan_block_reserve_micro_units
        {
            return Err("policy source liquidity below runtime floor".to_string());
        }
        if request.min_orphan_block_lane_claim_reserve_micro_units
            < self.config.min_orphan_block_lane_claim_reserve_micro_units
        {
            return Err("policy target liquidity below runtime floor".to_string());
        }
        let policy_id = stable_id(
            "dandelion-orphan-block-liquidity-firewall-policy",
            &[
                HashPart::Str(request.scope.as_str()),
                HashPart::Str(&request.owner_commitment),
                HashPart::Str(&request.policy_commitment_root),
                HashPart::Str(&request.claim_source_orphan_block_set_root),
            ],
        );
        if self.policies.contains_key(&policy_id) {
            return Err(format!(
                "Dandelion orphan_block-phase privacy guard policy {policy_id} already exists"
            ));
        }
        self.policies.insert(
            ...
            OrphanBlockPolicy {
                ...
                claim_source_orphan_block_set_root: request.claim_source_orphan_block_set_root,
                claim_target_orphan_block_set_root: request.claim_target_orphan_block_set_root,
                claim_rotation_allowlist_root: request.claim_rotation_allowlist_root,
                min_privacy_set_size: request.min_privacy_set_size,
                min_pre_orphan_block_reserve_micro_units: request
                    .min_pre_orphan_block_reserve_micro_units,
                min_orphan_block_lane_claim_reserve_micro_units: request
                    .min_orphan_block_lane_claim_reserve_micro_units,
                ...
                orphan_block_window_slots: request.orphan_block_window_slots,
                active: true,
    ...
    pub fn register_cohort(&mut self, request: RegisterCohortRequest) -> Result<String> {
        ...
        let policy = self.policies.get(&request.policy_id).ok_or_else(|| {
            format!(
                "unknown Dandelion orphan_block-phase privacy guard policy {}",
                request.policy_id
            )
        })?;
        ...
        if request.pre_orphan_block_reserve_micro_units
            < policy.min_pre_orphan_block_reserve_micro_units
        {
            return Err("cohort source liquidity below policy floor".to_string());
        }
        if request.orphan_block_lane_claim_reserve_micro_units
            < policy.min_orphan_block_lane_claim_reserve_micro_units
        {
        ...
        let cohort_id = stable_id(
            "dandelion-orphan-block-liquidity-firewall-cohort",
            &[
        ...
        if self.cohorts.contains_key(&cohort_id) {
            return Err(format!(
                "Dandelion orphan_block-phase privacy guard cohort {cohort_id} already exists"
            ));
        }
        self.cohorts.insert(
            cohort_id.clone(),
            OrphanBlockLiquidityFirewallCohort {
                ...
                pre_orphan_block_reserve_micro_units: request.pre_orphan_block_reserve_micro_units,
                orphan_block_lane_claim_reserve_micro_units: request
                    .orphan_block_lane_claim_reserve_micro_units,
    ...
    pub fn submit_signal(&mut self, request: SubmitSignalRequest) -> Result<String> {
        ensure_capacity(
            self.signals.len(),
            MAX_SIGNALS,
            "Dandelion orphan_block-phase privacy guard signals",
        )?;
        let cohort = self
            .cohorts
            ...
            .ok_or_else(|| {
                format!(
                    "unknown Dandelion orphan_block-phase privacy guard cohort {}",
                    request.cohort_id
                )
            })?
            .clone();
        ...
        ensure_non_empty(
            &request.orphan_block_pressure_root,
            "orphan_block_pressure_root",
        )?;
        ensure_non_empty(&request.reporter_commitment, "reporter_commitment")?;
        ensure_bps(
            request.orphan_block_risk_score_bps,
            "orphan_block_risk_score_bps",
        )?;
        if request.privacy_set_size < policy.min_privacy_set_size {
            return Err("signal privacy set below policy floor".to_string());
        }
        let signal_id = stable_id(
            "dandelion-orphan-block-liquidity-firewall-signal",
            &[
        ...
        if self.signals.contains_key(&signal_id) {
            return Err(format!(
                "Dandelion orphan_block-phase privacy guard signal {signal_id} already exists"
            ));
        }
        ...
        self.signals.insert(
            ...
            OrphanBlockSignal {
                ...
                observed_claim_orphan_claim_path_root: request
                    .observed_claim_orphan_claim_path_root,
                orphan_block_pressure_root: request.orphan_block_pressure_root,
                reporter_commitment: request.reporter_commitment,
                pre_orphan_block_reserve_delta_micro_units: request
                    .pre_orphan_block_reserve_delta_micro_units,
                orphan_block_lane_claim_reserve_delta_micro_units: request
                    .orphan_block_lane_claim_reserve_delta_micro_units,
                privacy_set_size: request.privacy_set_size,
                orphan_block_risk_score_bps: request.orphan_block_risk_score_bps,
    ...
    pub fn submit_attestation(&mut self, request: SubmitAttestationRequest) -> Result<String> {
        ensure_capacity(
            ...
            MAX_ATTESTATIONS,
            "Dandelion orphan_block-phase privacy guard attestations",
        )?;
        let signal = self
            .signals
            ...
            .ok_or_else(|| {
                format!(
                    "unknown Dandelion orphan_block-phase privacy guard signal {}",
                    request.signal_id
                )
            })?
            .clone();
        ...
        ensure_bps(
            request.observed_orphan_block_risk_score_bps,
            "observed_orphan_block_risk_score_bps",
        )?;
        ...
        let attestation_id = stable_id(
            "dandelion-orphan-block-liquidity-firewall-pq-attestation",
            &[
        ...
        if self.attestations.contains_key(&attestation_id) {
            return Err(format!(
                "Dandelion orphan_block-phase privacy guard attestation {attestation_id} already exists"
            ));
        }
        self.attestations.insert(
            attestation_id.clone(),
            PqOrphanBlockFirewallAttestation {
                ...
                observed_orphan_block_risk_score_bps: request.observed_orphan_block_risk_score_bps,
    ...
    pub fn settle_decision(&mut self, request: SettleDecisionRequest) -> Result<String> {
        ensure_capacity(
            self.decisions.len(),
            MAX_DECISIONS,
            "Dandelion orphan_block-phase privacy guard decisions",
        )?;
        let signal = self
            .signals
            .get(&request.signal_id)
        ...
        let privacy_filter_deadline_slot = match request.action {
            OrphanBlockAction::QueueOrphanBlockFirewallClaimReview
            | OrphanBlockAction::SettleOrphanBlock
            | OrphanBlockAction::RotateOrphanBlockFirewallClaimClaimSettlementCohort => request
                .decided_at_slot
                .saturating_add(policy.orphan_block_window_slots),
            OrphanBlockAction::QuarantineOrphanClaimPath | OrphanBlockAction::EmergencyPause => {
                request
                    .decided_at_slot
                    .saturating_add(policy.orphan_block_window_slots / 2)
            }
            _ => 0,
        };
        let decision_id = stable_id(
            "dandelion-orphan-block-liquidity-firewall-decision",
            &[
                HashPart::Str(&request.signal_id),
        ...
        if self.decisions.contains_key(&decision_id) {
            return Err(format!(
                "Dandelion orphan_block-phase privacy guard decision {decision_id} already exists"
            ));
        }
        ...
        if let Some(signal) = self.signals.get_mut(&request.signal_id) {
            signal.status = match request.action {
                ...
                OrphanBlockAction::SettleOrphanBlock
                | OrphanBlockAction::RotateOrphanBlockFirewallClaimClaimSettlementCohort => {
                    SignalStatus::OrphanBlockFirewalling
                }
                OrphanBlockAction::QuarantineOrphanClaimPath
                | OrphanBlockAction::EmergencyPause => SignalStatus::Quarantined,
                OrphanBlockAction::Release => SignalStatus::Settled,
                OrphanBlockAction::Reject => SignalStatus::Rejected,
                OrphanBlockAction::Expire => SignalStatus::Expired,
                OrphanBlockAction::Observe => SignalStatus::Attested,
            };
        }
        if let Some(cohort) = self.cohorts.get_mut(&signal.cohort_id) {
            match request.action {
                ...
                OrphanBlockAction::SettleOrphanBlock => {
                    cohort.status = CohortStatus::OrphanBlockFirewalling;
                    cohort.pre_orphan_block_reserve_micro_units = cohort
                        .pre_orphan_block_reserve_micro_units
                        .saturating_sub(request.privacy_filtered_micro_units);
                    cohort.orphan_block_lane_claim_reserve_micro_units = cohort
                        .orphan_block_lane_claim_reserve_micro_units
                        .saturating_add(request.privacy_filtered_micro_units);
                    cohort.privacy_filter_deadline_slot = privacy_filter_deadline_slot;
                    self.counters.orphan_block_filters_settled =
                        self.counters.orphan_block_filters_settled.saturating_add(1);
                ...
                OrphanBlockAction::EmergencyPause => {
    ...
    pub fn publish_rebate(&mut self, request: PublishRebateRequest) -> Result<String> {
        ensure_capacity(
            self.rebates.len(),
            MAX_REBATES,
            "Dandelion orphan_block-phase privacy guard rebates",
        )?;
        let decision = self.decisions.get(&request.decision_id).ok_or_else(|| {
            format!(
                "unknown Dandelion orphan_block-phase privacy guard decision {}",
                request.decision_id
            )
        })?;
        ...
        let rebate_id = stable_id(
            "dandelion-orphan-block-liquidity-firewall-low-fee-rebate",
            &[
        ...
        if self.rebates.contains_key(&rebate_id) {
            return Err(format!(
                "Dandelion orphan_block-phase privacy guard rebate {rebate_id} already exists"
            ));
        }
        self.rebates.insert(
            rebate_id.clone(),
            LowFeeOrphanBlockFirewallRebate {
    ...
    pub fn publish_redaction_budget(
        ...
        ensure_capacity(
            ...
            "Dandelion orphan_block-phase privacy guard redaction budgets",
        )?;
        ...
        let budget_id = stable_id(
            "dandelion-orphan-block-liquidity-firewall-redaction-budget",
            &[
                HashPart::Str(request.scope.as_str()),
                HashPart::Str(&request.cohort_id),
        ...
        if self.redaction_budgets.contains_key(&budget_id) {
            return Err(format!(
                "Dandelion orphan_block-phase privacy guard redaction budget {budget_id} already exists"
            ));
        }
    ...
    pub fn publish_operator_summary(
        ...
        ensure_capacity(
            ...
            "Dandelion orphan_block-phase privacy guard operator summaries",
        )?;
        self.refresh_live_counts();
        ...
        let summary_id = stable_id(
            "dandelion-orphan-block-liquidity-firewall-operator-summary",
            &[
                HashPart::U64(epoch),
                HashPart::U64(slot),
    ...
    fn refresh_roots(&mut self) {
        ...
        self.roots.state_root = merkle_root(
            "bridge-dandelion-orphan-block-liquidity-firewall:state",
            &[
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "orphan_block_liquidity_firewall_suite": self.config.orphan_block_liquidity_firewall_suite,
            ...
            "orphan_block_window_slots": self.config.orphan_block_window_slots,
            ...
            "policies": self.policies.values().map(|policy| json!({
                ...
                "min_pre_orphan_block_reserve_micro_units": policy.min_pre_orphan_block_reserve_micro_units,
                "min_orphan_block_lane_claim_reserve_micro_units": policy.min_orphan_block_lane_claim_reserve_micro_units,
            ...
            "cohorts": self.cohorts.values().map(|cohort| json!({
                ...
                "pre_orphan_block_reserve_micro_units": cohort.pre_orphan_block_reserve_micro_units,
                "orphan_block_lane_claim_reserve_micro_units": cohort.orphan_block_lane_claim_reserve_micro_units,
            ...
            "signals": self.signals.values().map(|signal| json!({
                ...
                "orphan_block_risk_score_bps": signal.orphan_block_risk_score_bps,
...
pub fn demo() -> State {
    ...
    let signal = state
        .submit_signal(SubmitSignalRequest {
            cohort_id,
            kind: SignalKind::OrphanBlockGraphLinkagePressure,
            signal_commitment_root: devnet_root("signal", "demo-viewtag-pressure"),
            observed_claim_orphan_claim_path_root: devnet_root(
                "observed-orphan_claim_path",
                "demo-viewtag-pressure",
            ),
            orphan_block_pressure_root: devnet_root("pressure", "demo-viewtag-pressure"),
            reporter_commitment: "reporter:demo-subaddress-watchtower".to_string(),
            pre_orphan_block_reserve_delta_micro_units: -10_000_000,
            orphan_block_lane_claim_reserve_delta_micro_units: 8_000_000,
            privacy_set_size: 524_288,
            orphan_block_risk_score_bps: 6_200,
    ...
    state
        .submit_attestation(SubmitAttestationRequest {
            signal_id: signal,
            verdict: AttestationVerdict::RequireOrphanBlockCoverage,
            attestor_commitment: "attestor:demo-dandelion-orphan-block-liquidity-firewall"
                .to_string(),
            ...
            pq_security_bits: 256,
            observed_orphan_block_risk_score_bps: 6_400,
            observed_privacy_set_size: 524_288,
...
fn object_root<T: Serialize>(domain: &str, value: &T) -> String {
    ...
    domain_hash(
        &format!("bridge-dandelion-orphan-block-liquidity-firewall:{domain}"),
        &[HashPart::Json(&value)],
        32,
    )
}
...
fn devnet_root(domain: &str, label: &str) -> String {
    domain_hash(
        &format!("bridge-dandelion-orphan-block-liquidity-firewall:devnet:{domain}"),
