...
pub type MoneroL2PqPrivateClsagDecoyFeeCacheRuntimeResult<T> = Result<T>;
pub type Runtime = State;

pub const MONERO_L2_PQ_PRIVATE_CLSAG_DECOY_FEE_CACHE_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-monero-l2-pq-private-clsag-decoy-fee-cache-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    MONERO_L2_PQ_PRIVATE_CLSAG_DECOY_FEE_CACHE_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const CLSAG_DECOY_COHORT_SCHEME: &str = "monero-clsag-decoy-cohort-fee-cache-root-v1";
pub const FEE_CACHE_SHARD_SCHEME: &str = "low-fee-private-clsag-decoy-fee-cache-shard-root-v1";
pub const RING_MEMBER_FRESHNESS_SCHEME: &str = "monero-ring-member-freshness-window-root-v1";
pub const PQ_CACHE_ATTESTATION_SCHEME: &str =
    "ML-DSA-87+SLH-DSA-SHAKE-256f-clsag-decoy-fee-cache-attestation-v1";
pub const WALLET_SCAN_HINT_SCHEME: &str = "view-key-safe-clsag-decoy-wallet-scan-hint-root-v1";
pub const CACHE_INVALIDATION_SCHEME: &str =
    "privacy-preserving-clsag-decoy-cache-invalidation-root-v1";
pub const LOW_FEE_REBATE_SCHEME: &str = "low-fee-private-clsag-decoy-cache-rebate-root-v1";
pub const PRIVACY_REDACTION_BUDGET_SCHEME: &str = "clsag-decoy-fee-cache-redaction-budget-root-v1";
pub const OPERATOR_SUMMARY_SCHEME: &str = "redacted-clsag-decoy-fee-cache-operator-summary-root-v1";
pub const PUBLIC_RECORD_SCHEME: &str = "deterministic-public-clsag-decoy-fee-cache-record-v1";
...
pub const DEFAULT_FRESHNESS_WINDOW_BLOCKS: u64 = 720;
...
pub const DEFAULT_MAX_FRESHNESS_WINDOWS: usize = 4_194_304;
...
impl InvalidationReason {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::StaleRingMember => "stale_ring_member",
...
pub struct Config {
    ...
    pub max_freshness_windows: usize,
...
impl Config {
    pub fn devnet() -> Self {
        Self {
            ...
            max_freshness_windows: DEFAULT_MAX_FRESHNESS_WINDOWS,
    ...
    pub fn validate(&self) -> Result<()> {
            ...
            "fee policy must keep CLSAG decoy cache usage low-fee",
        )?;
        ensure(
...
impl Counters {
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "freshness_members": self.freshness_members,
...
impl ClsagDecoyCohort {
    pub fn new(
        ...
        let freshness_window_id = scoped_id("freshness", &cohort_id);
        Self {
            ...
            output_bucket_start: monero_height_opened
                .saturating_sub(DEFAULT_FRESHNESS_WINDOW_BLOCKS),
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "freshness_window_id": self.freshness_window_id,
    ...
    pub fn state_root(&self) -> String {
        root_from_record(CLSAG_DECOY_COHORT_SCHEME, &self.public_record())
    }
}
...
impl FeeCacheShard {
    pub fn new(shard_id: impl Into<String>, epoch: u64, monero_height_start: u64) -> Self {
        ...
        Self {
            ...
            freshness_root: empty_root("shard-freshness"),
    ...
    pub fn reserve_units(&mut self, cohort_id: impl Into<String>, units: u64) -> Result<()> {
        ensure(self.status.active(), "fee cache shard is not active")?;
        ensure(
            self.reserved_units.saturating_add(units) <= self.capacity_units,
            "fee cache shard capacity exceeded",
        )?;
        ...
        if self.reserved_units.saturating_mul(MAX_BPS) / self.capacity_units >= 9_000 {
            self.status = FeeShardStatus::Saturated;
        } else if self.reserved_units > 0 {
            self.status = FeeShardStatus::Warm;
        }
        Ok(())
    }

    pub fn invalidate(&mut self, invalidation_root: impl Into<String>) {
        self.status = FeeShardStatus::Invalidated;
    ...
    pub fn state_root(&self) -> String {
        root_from_record(FEE_CACHE_SHARD_SCHEME, &self.public_record())
    }
}

#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct RingMemberFreshness {
...
impl RingMemberFreshness {
    pub fn new(freshness_id: impl Into<String>, cohort_id: impl Into<String>, height: u64) -> Self {
        let freshness_id = freshness_id.into();
        let cohort_id = cohort_id.into();
        Self {
            ...
            monero_height_expires: height + DEFAULT_FRESHNESS_WINDOW_BLOCKS,
            ...
            key_image_nullifier_root: empty_root("freshness-key-image-nullifiers"),
            freshness_proof_root: empty_root("freshness-proofs"),
            entropy_bits: DEFAULT_TARGET_PQ_SECURITY_BITS,
        }
    }

    pub fn stale_at(&self, height: u64) -> bool {
        height >= self.monero_height_expires || self.status == FreshnessStatus::Stale
    }
    ...
    pub fn state_root(&self) -> String {
        root_from_record(RING_MEMBER_FRESHNESS_SCHEME, &self.public_record())
    }
}
...
impl PqCacheAttestation {
    pub fn new(
        ...
        Self {
            ...
            freshness_root: empty_root("pq-cache-freshness"),
...
pub struct State {
    ...
    pub monero_height: u64,
    pub epoch: u64,
    pub cohorts: BTreeMap<String, ClsagDecoyCohort>,
    pub shards: BTreeMap<String, FeeCacheShard>,
    pub freshness_windows: BTreeMap<String, RingMemberFreshness>,
...
impl State {
    ...
    pub fn open_cohort(&mut self, cohort: ClsagDecoyCohort) -> Result<String> {
        ...
        self.emit_public_record("clsag_decoy_cohort_opened", &cohort_id, self.monero_height)?;
        self.refresh_roots();
        Ok(cohort_id)
    }

    pub fn record_freshness(&mut self, freshness: RingMemberFreshness) -> Result<String> {
        ensure(
            self.freshness_windows.len() < self.config.max_freshness_windows,
            "too many ring member freshness windows",
        )?;
        ensure(
            self.cohorts.contains_key(&freshness.cohort_id),
            "freshness cohort does not exist",
        )?;
        let freshness_id = freshness.freshness_id.clone();
        self.counters.freshness_windows = self.counters.freshness_windows.saturating_add(1);
        self.counters.freshness_members = self
            .counters
            .freshness_members
            .saturating_add(freshness.member_count);
        self.freshness_windows
            .insert(freshness_id.clone(), freshness);
        self.emit_public_record(
            "ring_member_freshness_recorded",
            &freshness_id,
            self.monero_height,
        )?;
        self.refresh_roots();
    ...
    pub fn expire_stale_freshness(&mut self, height: u64) -> Result<Vec<String>> {
        let mut expired = Vec::new();
        for freshness in self.freshness_windows.values_mut() {
            if freshness.stale_at(height) && freshness.status != FreshnessStatus::Stale {
                freshness.status = FreshnessStatus::Stale;
                expired.push(freshness.freshness_id.clone());
            }
        }
        if !expired.is_empty() {
            self.emit_public_record(
                "ring_member_freshness_expired",
                &root_from_strings("expired-freshness", expired.iter()),
                height,
            )?;
            self.refresh_roots();
        }
...
pub fn devnet() -> State {
    ...
    .expect("devnet CLSAG decoy fee cache config is valid");

    let shard = FeeCacheShard::new(
        "clsag-fee-shard-devnet-0",
        DEVNET_EPOCH,
        DEVNET_MONERO_HEIGHT,
    );
    let shard_id = state.open_shard(shard).expect("devnet shard opens");

    let cohort = ClsagDecoyCohort::new(
        "clsag-decoy-cohort-devnet-0",
        shard_id.clone(),
    ...
    let freshness = RingMemberFreshness::new(
        scoped_id("freshness", &cohort_id),
        cohort_id.clone(),
        DEVNET_MONERO_HEIGHT,
    );
    state
        .record_freshness(freshness)
        .expect("devnet freshness records");
...
pub fn demo_record() -> Value {
    ...
    json!({
        "runtime": "monero_l2_pq_private_clsag_decoy_fee_cache_runtime",
        "protocol_version": PROTOCOL_VERSION,
        "priorities": {
...
pub fn monero_l2_pq_private_clsag_decoy_fee_cache_runtime_public_record(state: &State) -> Value {
    state.public_record()
}

pub fn monero_l2_pq_private_clsag_decoy_fee_cache_runtime_state_root(state: &State) -> String {
    state.state_root()
}
