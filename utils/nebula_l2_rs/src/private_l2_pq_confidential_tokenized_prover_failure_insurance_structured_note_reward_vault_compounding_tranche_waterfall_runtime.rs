...
pub type PrivateL2PqConfidentialTokenizedProverFailureInsuranceStructuredNoteRewardVaultCompoundingTrancheWaterfallRuntimeResult<
    T,
> = Result<T>;
pub type Runtime = State;

pub const PRIVATE_L2_PQ_CONFIDENTIAL_TOKENIZED_PROVER_FAILURE_INSURANCE_STRUCTURED_NOTE_REWARD_VAULT_COMPOUNDING_TRANCHE_WATERFALL_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-private-l2-pq-confidential-tokenized-prover-failure-insurance-structured-note-reward-vault-compounding-tranche-waterfall-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_PQ_CONFIDENTIAL_TOKENIZED_PROVER_FAILURE_INSURANCE_STRUCTURED_NOTE_REWARD_VAULT_COMPOUNDING_TRANCHE_WATERFALL_RUNTIME_PROTOCOL_VERSION;
...
pub const STATE_ROOT_SUITE: &str =
    "private-l2-pq-confidential-tokenized-prover-failure-insurance-structured-note-reward-vault-compounding-tranche-waterfall-state-root-v1";
pub const PAYLOAD_ROOT_SUITE: &str =
    "private-l2-pq-confidential-tokenized-prover-failure-insurance-structured-note-reward-vault-compounding-tranche-waterfall-payload-root-v1";
pub const DEVNET_REPLAY_DOMAIN: &str =
    "nebula-private-l2-pq-prover-failure-insurance-structured-note-reward-vault-compounding-tranche-waterfall-devnet";
pub const DEVNET_RUNTIME_ID: &str =
    "private-l2-pq-prover-failure-insurance-structured-note-reward-vault-compounding-tranche-waterfall-devnet";
...
pub const DEVNET_NOTE_TOKEN_ID: &str =
    "tpfi-note-reward-vault-compounding-tranche-waterfall-devnet";
...
impl State {
    ...
    pub fn redeem_senior_note(
        &mut self,
        input: NoteRedemptionInput,
    ) -> Result<NoteRedemptionEntry> {
        self.redeem_tranche_note(input, NoteSeniority::Senior)
    }

    pub fn redeem_junior_note(
        &mut self,
        input: NoteRedemptionInput,
    ) -> Result<NoteRedemptionEntry> {
        self.redeem_tranche_note(input, NoteSeniority::Junior)
    }

    pub fn redeem_note(&mut self, input: NoteRedemptionInput) -> Result<NoteRedemptionEntry> {
        self.redeem_note_inner(input)
    }

    fn redeem_tranche_note(
        &mut self,
        input: NoteRedemptionInput,
        seniority: NoteSeniority,
    ) -> Result<NoteRedemptionEntry> {
        let subscription = self
            .compounding_subscriptions
            .get(&input.subscription_id)
            .ok_or_else(|| "unknown compounding subscription".to_string())?;
        let tranche_found = self.reward_tranches.values().any(|tranche| {
            tranche.campaign_id == subscription.campaign_id
                && tranche.vault_id == subscription.vault_id
                && tranche.seniority == seniority
        });
        require(
            tranche_found,
            "subscription does not map to requested tranche seniority",
        )?;
        self.redeem_note_inner(input)
    }

    fn redeem_note_inner(&mut self, input: NoteRedemptionInput) -> Result<NoteRedemptionEntry> {
        ensure_capacity(
            "note_redemptions",
