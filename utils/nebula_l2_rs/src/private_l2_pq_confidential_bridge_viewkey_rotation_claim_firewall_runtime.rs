...
pub type PrivateL2PqConfidentialBridgeViewkeyRotationClaimFirewallRuntimeResult<T> = Result<T>;
pub type Runtime = State;

pub const PROTOCOL_VERSION: &str =
    "nebula-private-l2-pq-confidential-bridge-viewkey-rotation-claim-firewall-runtime-v1";
pub const PRIVATE_L2_PQ_CONFIDENTIAL_BRIDGE_VIEWKEY_ROTATION_CLAIM_FIREWALL_RUNTIME_PROTOCOL_VERSION:
    &str = PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const PQ_VIEWKEY_ROTATION_CLAIM_FIREWALL_SUITE: &str =
    "ML-DSA-87+ML-KEM-1024+SLH-DSA-SHAKE-256f";
pub const VIEWKEY_ROTATION_CLAIM_FIREWALL_SUITE: &str =
    "monero-bridge-viewkey-rotation-claim-firewall-root-v1";
pub const REBATE_SUITE: &str = "viewkey-rotation-claim-firewall-low-fee-rebate-root-v1";
pub const REDACTION_SUITE: &str = "operator-safe-viewkey-rotation-claim-firewall-redaction-root-v1";
...
pub const DEFAULT_VIEWKEY_ROTATION_CLAIM_WINDOW_SLOTS: u64 = 720;
...
pub enum ViewkeyRotationClaimScope {
    DepositSubaddressLane,
    WithdrawalSubaddressLane,
    FastDandelionViewkeyRotationFirewallClaimLane,
    MerchantPaymentSubaddressLane,
    AtomicSwapSubaddressLane,
    ReserveMirror,
    EmergencyViewkeyRotationClaimFirewall,
    WatchtowerRecovery,
}

impl ViewkeyRotationClaimScope {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::DepositSubaddressLane => "deposit_subaddress_lane",
            Self::WithdrawalSubaddressLane => "withdrawal_subaddress_lane",
            Self::FastDandelionViewkeyRotationFirewallClaimLane => {
                "fast_dandelion_viewkey_claim_lane"
            }
            ...
            Self::EmergencyViewkeyRotationClaimFirewall => {
                "emergency_viewkey_rotation_claim_firewall"
            }
...
#[serde(rename_all = "snake_case")]
pub enum SignalKind {
    ViewkeyRotationLinkagePressure,
    ViewkeyRelayFragmentation,
    ViewkeyRelaySkew,
    ViewkeyRotationGraphLinkagePressure,
    ViewkeyTimingCorrelationPressure,
    ReserveMismatch,
    FeeSpikeViewkeyRotationFirewallClaimPressure,
    WatchtowerEmergency,
}
...
pub enum AttestationVerdict {
    ConfirmViewkeyRotationClaimPressure,
    ConfirmPrivacyRisk,
    RequireViewkeyRotationClaimCoverage,
    RequireViewkeyRotationClaimCoverageGuard,
    RequireQuarantine,
    ClearFalsePositive,
}
...
#[derive(Clone, Copy, Debug, Deserialize, Eq, Ord, PartialEq, PartialOrd, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum ViewkeyRotationClaimAction {
    Observe,
    QueueViewkeyRotationFirewallClaimReview,
    SettleViewkeyRotationClaim,
    RotateViewkeyRotationFirewallClaimClaimSettlementCohort,
    QuarantineRotationPath,
    EmergencyPause,
    Release,
    Reject,
    Expire,
...
pub struct Config {
    ...
    pub viewkey_rotation_claim_firewall_suite: String,
    ...
    pub viewkey_rotation_claim_window_slots: u64,
    pub signal_window_slots: u64,
    pub max_claim_settlement_fee_bps: u64,
    pub target_rebate_bps: u64,
    pub min_pre_rotation_claim_reserve_micro_units: u64,
    pub min_viewkey_lane_claim_reserve_micro_units: u64,
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            pq_claim_settlement_insurance_suite: PQ_VIEWKEY_ROTATION_CLAIM_FIREWALL_SUITE
                .to_string(),
            viewkey_rotation_claim_firewall_suite: VIEWKEY_ROTATION_CLAIM_FIREWALL_SUITE
                .to_string(),
            ...
            viewkey_rotation_claim_window_slots: DEFAULT_VIEWKEY_ROTATION_CLAIM_WINDOW_SLOTS,
            signal_window_slots: DEFAULT_SIGNAL_WINDOW_SLOTS,
            max_claim_settlement_fee_bps: DEFAULT_MAX_ROUTE_FEE_BPS,
            target_rebate_bps: DEFAULT_TARGET_REBATE_BPS,
            min_pre_rotation_claim_reserve_micro_units: DEFAULT_MIN_SOURCE_LIQUIDITY_MICRO_UNITS,
            min_viewkey_lane_claim_reserve_micro_units: DEFAULT_MIN_TARGET_LIQUIDITY_MICRO_UNITS,
...
pub struct Counters {
    ...
    pub viewkey_filters_settled: u64,
    pub cohorts_rotated: u64,
    pub claim_rotation_paths_quarantined: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct ViewkeyRotationClaimPolicy {
    pub policy_id: String,
    pub scope: ViewkeyRotationClaimScope,
    pub owner_commitment: String,
    pub policy_commitment_root: String,
    pub claim_source_viewkey_rotation_set_root: String,
    pub claim_target_viewkey_rotation_set_root: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct ViewkeyRotationClaimFirewallCohort {
    ...
    pub pre_rotation_claim_reserve_micro_units: u64,
    pub viewkey_lane_claim_reserve_micro_units: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct ViewkeyRotationClaimSignal {
    ...
    pub observed_claim_rotation_path_root: String,
    pub viewkey_rotation_pressure_root: String,
    pub reporter_commitment: String,
    pub pre_rotation_claim_reserve_delta_micro_units: i128,
    pub viewkey_lane_claim_reserve_delta_micro_units: i128,
    pub privacy_set_size: u64,
    pub viewkey_rotation_risk_score_bps: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct PqViewkeyClaimFirewallAttestation {
    ...
    pub observed_viewkey_rotation_risk_score_bps: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct ViewkeyRotationClaimDecision {
    pub decision_id: String,
    pub signal_id: String,
    pub cohort_id: String,
    pub policy_id: String,
    pub action: ViewkeyRotationClaimAction,
    pub decision_root: String,
    pub replacement_claim_rotation_path_root: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct LowFeeViewkeyClaimFirewallRebate {
...
pub struct State {
    ...
    pub roots: Roots,
    pub policies: BTreeMap<String, ViewkeyRotationClaimPolicy>,
    pub cohorts: BTreeMap<String, ViewkeyRotationClaimFirewallCohort>,
    pub signals: BTreeMap<String, ViewkeyRotationClaimSignal>,
    pub attestations: BTreeMap<String, PqViewkeyClaimFirewallAttestation>,
    pub decisions: BTreeMap<String, ViewkeyRotationClaimDecision>,
    pub rebates: BTreeMap<String, LowFeeViewkeyClaimFirewallRebate>,
...
impl State {
    ...
    pub fn devnet() -> Self {
        ...
        let withdrawal_policy = state
            .register_policy(RegisterPolicyRequest {
                scope: ViewkeyRotationClaimScope::WithdrawalSubaddressLane,
                owner_commitment: "owner:dandelion-viewkey-rotation-claim-firewall-council-a"
                    .to_string(),
                policy_commitment_root: devnet_root(
                    "policy",
                    "withdrawal-dandelion-viewkey-rotation-claim-firewall",
                ),
                claim_source_viewkey_rotation_set_root: devnet_root(
                    "source-viewkey-relay-set",
                    "withdrawal-alpha",
                ),
                claim_target_viewkey_rotation_set_root: devnet_root(
                    "target-viewkey-relay-set",
                    "withdrawal-alpha",
                ),
                claim_rotation_allowlist_root: devnet_root(
                    "rotation_path-allowlist",
                    "withdrawal-alpha",
                ),
                min_privacy_set_size: 524_288,
                min_pre_rotation_claim_reserve_micro_units: 160_000_000,
                min_viewkey_lane_claim_reserve_micro_units: 72_000_000,
                max_claim_settlement_fee_bps: 16,
                min_attestation_quorum_bps: 7_200,
                signal_window_slots: 96,
                viewkey_rotation_claim_window_slots: 720,
                redaction_profile: "withdrawal-dandelion-viewkey-rotation-claim-firewall-profile"
                    .to_string(),
            })
            .expect("devnet withdrawal policy");
        let fast_dandelion_exit_policy = state
            .register_policy(RegisterPolicyRequest {
                scope: ViewkeyRotationClaimScope::FastDandelionViewkeyRotationFirewallClaimLane,
                owner_commitment: "owner:fast-dandelion-decoy-insurance-rotation_path-council-b".to_string(),
                policy_commitment_root: devnet_root(
                    "policy",
                    "fast-dandelion-decoy-insurance-dandelion-viewkey-rotation-claim-firewall",
                ),
                claim_source_viewkey_rotation_set_root: devnet_root(
                    "source-viewkey-relay-set",
                ...
                min_pre_rotation_claim_reserve_micro_units: 144_000_000,
                min_viewkey_lane_claim_reserve_micro_units: 64_000_000,
                max_claim_settlement_fee_bps: 18,
                min_attestation_quorum_bps: 7_000,
                signal_window_slots: 80,
                viewkey_rotation_claim_window_slots: 640,
                redaction_profile:
                    "fast-dandelion-decoy-insurance-dandelion-viewkey-rotation-claim-firewall-profile"
                        .to_string(),
            })
            .expect("devnet fast viewkey_rotation_claim policy");
        let withdrawal_cohort = state
            .register_cohort(RegisterCohortRequest {
                ...
                pre_rotation_claim_reserve_micro_units: 216_000_000,
                viewkey_lane_claim_reserve_micro_units: 96_000_000,
        ...
        let fast_dandelion_exit_cohort = state
            .register_cohort(RegisterCohortRequest {
                ...
                bridge_lane_id: "fast-dandelion-decoy-insurance-rotation_path-beta".to_string(),
                claim_source_viewkey_rotation_set_root: devnet_root(
                    "source-viewkey-relay-set",
                    "cohort-beta",
                ),
                ...
                pre_rotation_claim_reserve_micro_units: 168_000_000,
                viewkey_lane_claim_reserve_micro_units: 76_000_000,
                privacy_set_size: 393_216,
                claim_settlement_fee_bps: 13,
                public_hint_root: devnet_root("hint", "cohort-beta"),
            })
            .expect("devnet fast viewkey_rotation_claim cohort");
        let signal = state
            .submit_signal(SubmitSignalRequest {
                cohort_id: withdrawal_cohort,
                kind: SignalKind::ViewkeyRotationLinkagePressure,
                signal_commitment_root: devnet_root("signal", "withdrawal-cluster-pressure"),
                observed_claim_rotation_path_root: devnet_root(
                    "observed-rotation_path",
                    "withdrawal-cluster-pressure",
                ),
                viewkey_rotation_pressure_root: devnet_root(
                    "pressure",
                    "withdrawal-cluster-pressure",
                ),
                reporter_commitment: "reporter:dandelion-rotation_path-watchtower-11".to_string(),
                pre_rotation_claim_reserve_delta_micro_units: -48_000_000,
                viewkey_lane_claim_reserve_delta_micro_units: 24_000_000,
                privacy_set_size: 524_288,
                viewkey_rotation_risk_score_bps: 7_300,
        ...
        state
            .submit_attestation(SubmitAttestationRequest {
                ...
                verdict: AttestationVerdict::RequireViewkeyRotationClaimCoverageGuard,
                attestor_commitment: "attestor:pq-dandelion-viewkey-rotation-claim-firewall-a"
                    .to_string(),
                pq_signature_root: devnet_root(
                    "ml-dsa-signature",
                    "withdrawal-viewkey_rotation_claim-a",
                ),
                pq_public_key_root: devnet_root(
                    "ml-kem-public-key",
                    "withdrawal-viewkey_rotation_claim-a",
                ),
                transcript_root: devnet_root("transcript", "withdrawal-viewkey_rotation_claim-a"),
                quorum_weight_bps: 7_500,
                pq_security_bits: 256,
                observed_viewkey_rotation_risk_score_bps: 7_500,
        ...
        state
            .submit_attestation(SubmitAttestationRequest {
                signal_id: signal.clone(),
                verdict: AttestationVerdict::ConfirmViewkeyRotationClaimPressure,
                attestor_commitment: "attestor:pq-dandelion-viewkey-rotation-claim-firewall-b"
                    .to_string(),
                pq_signature_root: devnet_root(
                    "slh-dsa-signature",
                    "withdrawal-viewkey_rotation_claim-b",
                ),
                pq_public_key_root: devnet_root(
                    "ml-kem-public-key",
                    "withdrawal-viewkey_rotation_claim-b",
                ),
                transcript_root: devnet_root("transcript", "withdrawal-viewkey_rotation_claim-b"),
                quorum_weight_bps: 8_700,
                pq_security_bits: 256,
                observed_viewkey_rotation_risk_score_bps: 7_800,
                observed_privacy_set_size: 524_288,
        ...
        let decision = state
            .settle_decision(SettleDecisionRequest {
                signal_id: signal,
                action: ViewkeyRotationClaimAction::SettleViewkeyRotationClaim,
                decision_root: devnet_root(
                    "decision",
                    "withdrawal-liquidity-viewkey_rotation_claim",
                ),
                replacement_claim_rotation_path_root: devnet_root(
                    "replacement-rotation_path",
                    "withdrawal-viewkey_rotation_claim",
                ),
            ...
            .expect("devnet viewkey_rotation_claim decision");
        state
            .publish_rebate(PublishRebateRequest {
                decision_id: decision,
                recipient_commitment:
                    "recipient:dandelion-viewkey-rotation-claim-firewall-fee-sponsor".to_string(),
                rebate_micro_units: 22_000,
                fee_saved_micro_units: 88_000,
                rebate_bps: 8,
                settlement_slot: DEVNET_SLOT + 6,
                rebate_root: devnet_root("rebate", "withdrawal-viewkey_rotation_claim"),
            })
    ...
    pub fn register_policy(&mut self, request: RegisterPolicyRequest) -> Result<String> {
        ensure_capacity(
            self.policies.len(),
            MAX_POLICIES,
            "Dandelion viewkey-phase privacy guard policies",
        )?;
        ...
        ensure_non_empty(
            &request.claim_source_viewkey_rotation_set_root,
            "claim_source_viewkey_rotation_set_root",
        )?;
        ensure_non_empty(
            &request.claim_target_viewkey_rotation_set_root,
            "claim_target_viewkey_rotation_set_root",
        )?;
        ensure_non_empty(
        ...
        if request.min_pre_rotation_claim_reserve_micro_units
            < self.config.min_pre_rotation_claim_reserve_micro_units
        {
            return Err("policy source liquidity below runtime floor".to_string());
        }
        if request.min_viewkey_lane_claim_reserve_micro_units
            < self.config.min_viewkey_lane_claim_reserve_micro_units
        {
            return Err("policy target liquidity below runtime floor".to_string());
        }
        let policy_id = stable_id(
            "dandelion-viewkey-rotation-claim-firewall-policy",
            &[
                HashPart::Str(request.scope.as_str()),
                HashPart::Str(&request.owner_commitment),
                HashPart::Str(&request.policy_commitment_root),
                HashPart::Str(&request.claim_source_viewkey_rotation_set_root),
            ],
        );
        if self.policies.contains_key(&policy_id) {
            return Err(format!(
                "Dandelion viewkey-phase privacy guard policy {policy_id} already exists"
            ));
        }
        self.policies.insert(
            policy_id.clone(),
            ViewkeyRotationClaimPolicy {
                policy_id: policy_id.clone(),
                scope: request.scope,
                owner_commitment: request.owner_commitment,
                policy_commitment_root: request.policy_commitment_root,
                claim_source_viewkey_rotation_set_root: request
                    .claim_source_viewkey_rotation_set_root,
                claim_target_viewkey_rotation_set_root: request
                    .claim_target_viewkey_rotation_set_root,
                claim_rotation_allowlist_root: request.claim_rotation_allowlist_root,
                min_privacy_set_size: request.min_privacy_set_size,
                min_pre_rotation_claim_reserve_micro_units: request
                    .min_pre_rotation_claim_reserve_micro_units,
                min_viewkey_lane_claim_reserve_micro_units: request
                    .min_viewkey_lane_claim_reserve_micro_units,
                ...
                viewkey_rotation_claim_window_slots: request.viewkey_rotation_claim_window_slots,
                active: true,
    ...
    pub fn register_cohort(&mut self, request: RegisterCohortRequest) -> Result<String> {
        ensure_capacity(
            self.cohorts.len(),
            MAX_COHORTS,
            "Dandelion rotation_path privacy cohorts",
        )?;
        let policy = self.policies.get(&request.policy_id).ok_or_else(|| {
            format!(
                "unknown Dandelion viewkey-phase privacy guard policy {}",
                request.policy_id
            )
        })?;
        ...
        if request.claim_settlement_fee_bps > policy.max_claim_settlement_fee_bps {
            return Err("cohort rotation_path fee exceeds policy cap".to_string());
        }
        if request.pre_rotation_claim_reserve_micro_units
            < policy.min_pre_rotation_claim_reserve_micro_units
        {
            return Err("cohort source liquidity below policy floor".to_string());
        }
        if request.viewkey_lane_claim_reserve_micro_units
            < policy.min_viewkey_lane_claim_reserve_micro_units
        {
        ...
        let cohort_id = stable_id(
            "dandelion-viewkey-rotation-claim-firewall-cohort",
            &[
        ...
        if self.cohorts.contains_key(&cohort_id) {
            return Err(format!(
                "Dandelion viewkey-phase privacy guard cohort {cohort_id} already exists"
            ));
        }
        self.cohorts.insert(
            cohort_id.clone(),
            ViewkeyRotationClaimFirewallCohort {
                ...
                pre_rotation_claim_reserve_micro_units: request
                    .pre_rotation_claim_reserve_micro_units,
                viewkey_lane_claim_reserve_micro_units: request
                    .viewkey_lane_claim_reserve_micro_units,
    ...
    pub fn submit_signal(&mut self, request: SubmitSignalRequest) -> Result<String> {
        ensure_capacity(
            self.signals.len(),
            MAX_SIGNALS,
            "Dandelion viewkey-phase privacy guard signals",
        )?;
        let cohort = self
            .cohorts
            ...
            .ok_or_else(|| {
                format!(
                    "unknown Dandelion viewkey-phase privacy guard cohort {}",
                    request.cohort_id
                )
            })?
            .clone();
        ...
        ensure_non_empty(
            &request.observed_claim_rotation_path_root,
            "observed_claim_rotation_path_root",
        )?;
        ensure_non_empty(
            &request.viewkey_rotation_pressure_root,
            "viewkey_rotation_pressure_root",
        )?;
        ensure_non_empty(&request.reporter_commitment, "reporter_commitment")?;
        ensure_bps(
            request.viewkey_rotation_risk_score_bps,
            "viewkey_rotation_risk_score_bps",
        )?;
        if request.privacy_set_size < policy.min_privacy_set_size {
            return Err("signal privacy set below policy floor".to_string());
        }
        let signal_id = stable_id(
            "dandelion-viewkey-rotation-claim-firewall-signal",
            &[
        ...
        if self.signals.contains_key(&signal_id) {
            return Err(format!(
                "Dandelion viewkey-phase privacy guard signal {signal_id} already exists"
            ));
        }
        ...
        self.signals.insert(
            ...
            ViewkeyRotationClaimSignal {
                ...
                observed_claim_rotation_path_root: request.observed_claim_rotation_path_root,
                viewkey_rotation_pressure_root: request.viewkey_rotation_pressure_root,
                reporter_commitment: request.reporter_commitment,
                pre_rotation_claim_reserve_delta_micro_units: request
                    .pre_rotation_claim_reserve_delta_micro_units,
                viewkey_lane_claim_reserve_delta_micro_units: request
                    .viewkey_lane_claim_reserve_delta_micro_units,
                privacy_set_size: request.privacy_set_size,
                viewkey_rotation_risk_score_bps: request.viewkey_rotation_risk_score_bps,
    ...
    pub fn submit_attestation(&mut self, request: SubmitAttestationRequest) -> Result<String> {
        ensure_capacity(
            ...
            MAX_ATTESTATIONS,
            "Dandelion viewkey-phase privacy guard attestations",
        )?;
        let signal = self
            .signals
            ...
            .ok_or_else(|| {
                format!(
                    "unknown Dandelion viewkey-phase privacy guard signal {}",
                    request.signal_id
                )
            })?
            .clone();
        ...
        ensure_bps(
            request.observed_viewkey_rotation_risk_score_bps,
            "observed_viewkey_rotation_risk_score_bps",
        )?;
        ...
        let attestation_id = stable_id(
            "dandelion-viewkey-rotation-claim-firewall-pq-attestation",
            &[
        ...
        if self.attestations.contains_key(&attestation_id) {
            return Err(format!(
                "Dandelion viewkey-phase privacy guard attestation {attestation_id} already exists"
            ));
        }
        self.attestations.insert(
            attestation_id.clone(),
            PqViewkeyClaimFirewallAttestation {
                ...
                observed_viewkey_rotation_risk_score_bps: request
                    .observed_viewkey_rotation_risk_score_bps,
    ...
    pub fn settle_decision(&mut self, request: SettleDecisionRequest) -> Result<String> {
        ensure_capacity(
            self.decisions.len(),
            MAX_DECISIONS,
            "Dandelion viewkey-phase privacy guard decisions",
        )?;
        let signal = self
            .signals
            .get(&request.signal_id)
        ...
        ensure_non_empty(
            &request.replacement_claim_rotation_path_root,
            "replacement_claim_rotation_path_root",
        )?;
        ensure_bps(request.rebate_bps, "rebate_bps")?;
        let privacy_filter_deadline_slot = match request.action {
            ViewkeyRotationClaimAction::QueueViewkeyRotationFirewallClaimReview
            | ViewkeyRotationClaimAction::SettleViewkeyRotationClaim
            | ViewkeyRotationClaimAction::RotateViewkeyRotationFirewallClaimClaimSettlementCohort => {
                request
                    .decided_at_slot
                    .saturating_add(policy.viewkey_rotation_claim_window_slots)
            }
            ViewkeyRotationClaimAction::QuarantineRotationPath
            | ViewkeyRotationClaimAction::EmergencyPause => request
                .decided_at_slot
                .saturating_add(policy.viewkey_rotation_claim_window_slots / 2),
            _ => 0,
        };
        let decision_id = stable_id(
            "dandelion-viewkey-rotation-claim-firewall-decision",
            &[
                HashPart::Str(&request.signal_id),
        ...
        if self.decisions.contains_key(&decision_id) {
            return Err(format!(
                "Dandelion viewkey-phase privacy guard decision {decision_id} already exists"
            ));
        }
        self.decisions.insert(
            decision_id.clone(),
            ViewkeyRotationClaimDecision {
                ...
                replacement_claim_rotation_path_root: request.replacement_claim_rotation_path_root,
        ...
        if let Some(signal) = self.signals.get_mut(&request.signal_id) {
            signal.status = match request.action {
                ViewkeyRotationClaimAction::QueueViewkeyRotationFirewallClaimReview => SignalStatus::RotationPathQueued,
                ViewkeyRotationClaimAction::SettleViewkeyRotationClaim
                | ViewkeyRotationClaimAction::RotateViewkeyRotationFirewallClaimClaimSettlementCohort => {
                    SignalStatus::RotationFirewalling
                }
                ViewkeyRotationClaimAction::QuarantineRotationPath
                | ViewkeyRotationClaimAction::EmergencyPause => SignalStatus::Quarantined,
                ViewkeyRotationClaimAction::Release => SignalStatus::Settled,
                ViewkeyRotationClaimAction::Reject => SignalStatus::Rejected,
                ViewkeyRotationClaimAction::Expire => SignalStatus::Expired,
                ViewkeyRotationClaimAction::Observe => SignalStatus::Attested,
            };
        }
        if let Some(cohort) = self.cohorts.get_mut(&signal.cohort_id) {
            match request.action {
                ViewkeyRotationClaimAction::QueueViewkeyRotationFirewallClaimReview => {
                    cohort.status = CohortStatus::RotationPathPending;
                    cohort.privacy_filter_deadline_slot = privacy_filter_deadline_slot;
                }
                ViewkeyRotationClaimAction::SettleViewkeyRotationClaim => {
                    cohort.status = CohortStatus::RotationFirewalling;
                    cohort.pre_rotation_claim_reserve_micro_units = cohort
                        .pre_rotation_claim_reserve_micro_units
                        .saturating_sub(request.privacy_filtered_micro_units);
                    cohort.viewkey_lane_claim_reserve_micro_units = cohort
                        .viewkey_lane_claim_reserve_micro_units
                        .saturating_add(request.privacy_filtered_micro_units);
                    cohort.privacy_filter_deadline_slot = privacy_filter_deadline_slot;
                    self.counters.viewkey_filters_settled =
                        self.counters.viewkey_filters_settled.saturating_add(1);
                ...
                ViewkeyRotationClaimAction::QuarantineRotationPath => {
                    ...
                    cohort.privacy_filter_deadline_slot = privacy_filter_deadline_slot;
                    self.counters.claim_rotation_paths_quarantined =
                        self.counters.claim_rotation_paths_quarantined.saturating_add(1);
                }
                ViewkeyRotationClaimAction::EmergencyPause => {
                    cohort.status = CohortStatus::Paused;
                    cohort.privacy_filter_deadline_slot = privacy_filter_deadline_slot;
                    self.counters.claim_rotation_paths_quarantined =
                        self.counters.claim_rotation_paths_quarantined.saturating_add(1);
                }
                ViewkeyRotationClaimAction::Release => {
    ...
    pub fn publish_rebate(&mut self, request: PublishRebateRequest) -> Result<String> {
        ensure_capacity(
            self.rebates.len(),
            MAX_REBATES,
            "Dandelion viewkey-phase privacy guard rebates",
        )?;
        let decision = self.decisions.get(&request.decision_id).ok_or_else(|| {
            format!(
                "unknown Dandelion viewkey-phase privacy guard decision {}",
                request.decision_id
            )
        })?;
        ...
        let rebate_id = stable_id(
            "dandelion-viewkey-rotation-claim-firewall-low-fee-rebate",
            &[
        ...
        if self.rebates.contains_key(&rebate_id) {
            return Err(format!(
                "Dandelion viewkey-phase privacy guard rebate {rebate_id} already exists"
            ));
        }
        self.rebates.insert(
            rebate_id.clone(),
            LowFeeViewkeyClaimFirewallRebate {
    ...
    pub fn publish_redaction_budget(
        ...
        ensure_capacity(
            ...
            "Dandelion viewkey-phase privacy guard redaction budgets",
        )?;
        ...
        let budget_id = stable_id(
            "dandelion-viewkey-rotation-claim-firewall-redaction-budget",
            &[
                HashPart::Str(request.scope.as_str()),
                HashPart::Str(&request.cohort_id),
        ...
        if self.redaction_budgets.contains_key(&budget_id) {
            return Err(format!(
                "Dandelion viewkey-phase privacy guard redaction budget {budget_id} already exists"
            ));
        }
    ...
    pub fn publish_operator_summary(
        ...
        ensure_capacity(
            ...
            "Dandelion viewkey-phase privacy guard operator summaries",
        )?;
        self.refresh_live_counts();
        ...
        let summary_id = stable_id(
            "dandelion-viewkey-rotation-claim-firewall-operator-summary",
            &[
                HashPart::U64(epoch),
                HashPart::U64(slot),
            ...
            .filter(|signal| {
                matches!(
                    ...
                    SignalStatus::Submitted
                        ...
                        | SignalStatus::RotationPathQueued
                        | SignalStatus::RotationFirewalling
    ...
    fn refresh_roots(&mut self) {
        ...
        self.roots.state_root = merkle_root(
            "bridge-dandelion-viewkey-rotation-claim-firewall:state",
            &[
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "viewkey_rotation_claim_firewall_suite": self.config.viewkey_rotation_claim_firewall_suite,
            ...
            "viewkey_rotation_claim_window_slots": self.config.viewkey_rotation_claim_window_slots,
            ...
            "policies": self.policies.values().map(|policy| json!({
                ...
                "min_pre_rotation_claim_reserve_micro_units": policy.min_pre_rotation_claim_reserve_micro_units,
                "min_viewkey_lane_claim_reserve_micro_units": policy.min_viewkey_lane_claim_reserve_micro_units,
            ...
            "cohorts": self.cohorts.values().map(|cohort| json!({
                ...
                "pre_rotation_claim_reserve_micro_units": cohort.pre_rotation_claim_reserve_micro_units,
                "viewkey_lane_claim_reserve_micro_units": cohort.viewkey_lane_claim_reserve_micro_units,
            ...
            "signals": self.signals.values().map(|signal| json!({
                ...
                "viewkey_rotation_risk_score_bps": signal.viewkey_rotation_risk_score_bps,
...
pub fn demo() -> State {
    ...
    let signal = state
        .submit_signal(SubmitSignalRequest {
            cohort_id,
            kind: SignalKind::ViewkeyRotationGraphLinkagePressure,
            signal_commitment_root: devnet_root("signal", "demo-viewtag-pressure"),
            observed_claim_rotation_path_root: devnet_root(
                "observed-rotation_path",
                "demo-viewtag-pressure",
            ),
            viewkey_rotation_pressure_root: devnet_root("pressure", "demo-viewtag-pressure"),
            reporter_commitment: "reporter:demo-subaddress-watchtower".to_string(),
            pre_rotation_claim_reserve_delta_micro_units: -10_000_000,
            viewkey_lane_claim_reserve_delta_micro_units: 8_000_000,
            privacy_set_size: 524_288,
            viewkey_rotation_risk_score_bps: 6_200,
    ...
    state
        .submit_attestation(SubmitAttestationRequest {
            signal_id: signal,
            verdict: AttestationVerdict::RequireViewkeyRotationClaimCoverage,
            attestor_commitment: "attestor:demo-dandelion-viewkey-rotation-claim-firewall"
                .to_string(),
            ...
            pq_security_bits: 256,
            observed_viewkey_rotation_risk_score_bps: 6_400,
            observed_privacy_set_size: 524_288,
...
fn object_root<T: Serialize>(domain: &str, value: &T) -> String {
    ...
    domain_hash(
        &format!("bridge-dandelion-viewkey-rotation-claim-firewall:{domain}"),
        &[HashPart::Json(&value)],
        32,
    )
}
...
fn devnet_root(domain: &str, label: &str) -> String {
    domain_hash(
        &format!("bridge-dandelion-viewkey-rotation-claim-firewall:devnet:{domain}"),
