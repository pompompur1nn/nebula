...
pub type PrivateL2PqConfidentialContractSealedTransitionMerkleQueueRuntimeResult<T> =
    std::result::Result<T, String>;
pub type Result<T> = PrivateL2PqConfidentialContractSealedTransitionMerkleQueueRuntimeResult<T>;
pub type Runtime = State;

pub const PRIVATE_L2_PQ_CONFIDENTIAL_CONTRACT_SEALED_TRANSITION_MERKLE_QUEUE_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-private-l2-pq-confidential-contract-sealed-transition-merkle-queue-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_PQ_CONFIDENTIAL_CONTRACT_SEALED_TRANSITION_MERKLE_QUEUE_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const SEALED_TRANSITION_MERKLE_QUEUE_SUITE: &str =
    "confidential-contract-sealed-transition-merkle-queue-v1";
pub const PQ_TRANSITION_PROOF_SUITE: &str =
    "ML-DSA-87+SLH-DSA-SHAKE-256f-sealed-transition-merkle-proof-v1";
...
pub const LOW_FEE_BATCH_SUITE: &str = "low-fee-sealed-transition-merkle-batch-execution-v1";
pub const DETERMINISTIC_MERKLE_QUEUE_SUITE: &str =
    "deterministic-confidential-sealed-transition-merkle-queue-root-v1";
pub const CONTRACT_QUEUE_SCHEME: &str = "sealed-transition-contract-merkle-queue-root-v1";
pub const QUEUE_LEAF_SCHEME: &str = "sealed-transition-merkle-queue-leaf-root-v1";
pub const PQ_PROOF_SCHEME: &str = "pq-authenticated-sealed-transition-proof-root-v1";
pub const ROLLBACK_ROOT_SCHEME: &str = "sealed-transition-merkle-rollback-root-v1";
pub const COMPENSATION_ROOT_SCHEME: &str = "sealed-transition-merkle-compensation-root-v1";
pub const EXECUTION_BATCH_SCHEME: &str = "low-fee-sealed-transition-merkle-batch-root-v1";
pub const DETERMINISTIC_TRACE_SCHEME: &str =
    "sealed-transition-merkle-queue-deterministic-trace-root-v1";
pub const FIXTURE_SCHEME: &str = "sealed-transition-merkle-queue-devnet-fixture-root-v1";
pub const DEVNET_L2_NETWORK: &str = "nebula-private-l2-devnet";
pub const DEVNET_FEE_ASSET_ID: &str = "piconero-devnet";
pub const DEVNET_HEIGHT: u64 = 5_285_120;
pub const DEVNET_EPOCH: u64 = 10_324;
pub const DEFAULT_QUEUE_DEPTH: u8 = 32;
...
pub const DEFAULT_MAX_QUEUE_LEAVES: usize = 65_536;
pub const DEFAULT_MAX_TRANSITIONS_PER_BATCH: usize = 2_048;
pub const DEFAULT_MAX_CONTRACT_QUEUES: usize = 512;
...
pub const DEFAULT_BASE_QUEUE_MICRO_FEE: u64 = 6;
...
impl QueueLeafStatus {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::Sealed => "sealed",
            Self::MerkleQueued => "merkle_queued",
    ...
    pub fn pending(self) -> bool {
        matches!(
            self,
            Self::Sealed | Self::MerkleQueued | Self::PqProofAttached | Self::BatchReady
        )
    }
}
...
pub struct Config {
    ...
    pub sealed_transition_merkle_queue_suite: String,
...
impl Config {
    pub fn devnet() -> Self {
        Self {
            ...
            sealed_transition_merkle_queue_suite: SEALED_TRANSITION_MERKLE_QUEUE_SUITE.to_string(),
            ...
            deterministic_merkle_queue_suite: DETERMINISTIC_MERKLE_QUEUE_SUITE.to_string(),
            ...
            max_queue_leaves: DEFAULT_MAX_QUEUE_LEAVES,
            max_transitions_per_batch: DEFAULT_MAX_TRANSITIONS_PER_BATCH,
            max_contract_queues: DEFAULT_MAX_CONTRACT_QUEUES,
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.max_transitions_per_batch > self.max_queue_leaves {
            return Err("batch transition limit exceeds queue leaf limit".to_string());
        }
        Ok(())
    }
}
...
pub struct Counters {
    pub contract_queues: u64,
    pub queue_leaves: u64,
    pub pending_queue_leaves: u64,
...
#[derive(Clone, Debug, Deserialize, PartialEq, Eq, Serialize)]
pub struct Roots {
    pub contract_queue_root: String,
    pub queue_leaf_root: String,
...
impl Default for Roots {
    fn default() -> Self {
        Self {
            contract_queue_root: merkle_root(CONTRACT_QUEUE_SCHEME, &[]),
            queue_leaf_root: merkle_root(QUEUE_LEAF_SCHEME, &[]),
...
#[derive(Clone, Debug, Deserialize, PartialEq, Eq, Serialize)]
pub struct ContractMerkleQueue {
    pub queue_id: String,
    ...
    pub merkle_queue_root: String,
    pub next_leaf_index: u64,
    pub priority_weight: u64,
    pub enabled: bool,
}

impl ContractMerkleQueue {
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "merkle_queue_root": self.merkle_queue_root,
...
#[derive(Clone, Debug, Deserialize, PartialEq, Eq, Serialize)]
pub struct SealedTransitionLeaf {
    pub leaf_id: String,
    pub queue_id: String,
    pub contract_id: String,
    pub leaf_index: u64,
    ...
    pub status: QueueLeafStatus,
    pub submitted_height: u64,
    pub expires_height: u64,
}

impl SealedTransitionLeaf {
    pub fn public_record(&self) -> Value {
        json!({
...
pub struct State {
    ...
    pub roots: Roots,
    pub contract_queues: BTreeMap<String, ContractMerkleQueue>,
    pub queue_leaves: BTreeMap<String, SealedTransitionLeaf>,
...
impl State {
    pub fn devnet() -> Self {
        Self {
            ...
            roots: Roots::default(),
            contract_queues: BTreeMap::new(),
            queue_leaves: BTreeMap::new(),
    ...
    pub fn demo() -> Self {
        ...
        let queue_id = contract_queue_id(transition_kind, "contract-private-router", 0);
        let leaf_id =
            sealed_transition_leaf_id(&queue_id, "caller-commitment-alice", 0, "vault-route-call");
        let proof_id = pq_transition_proof_id(&leaf_id, "devnet-merkle-transition-proof", 0);
        let rollback_id = rollback_root_id(&leaf_id, "vault-route-rollback", 0);
        let compensation_id = compensation_root_id(&leaf_id, "vault-route-compensation", 0);
        let operator_id = operator_id("devnet-merkle-queue-operator-alpha", DEVNET_EPOCH);
        let batch_id = execution_batch_id(&operator_id, DEVNET_EPOCH, DEVNET_HEIGHT + 2);
        let trace_id = deterministic_trace_id(&batch_id, &queue_id, &leaf_id, &proof_id);

        state.contract_queues.insert(
            queue_id.clone(),
            ContractMerkleQueue {
                ...
                queue_policy_root: deterministic_root(
                    "queue-policy",
                    "low-fee-merkle-cross-call",
                    0,
                ),
                merkle_queue_root: root_for_values(QUEUE_LEAF_SCHEME, &[json!(leaf_id.clone())]),
                next_leaf_index: 1,
                priority_weight: transition_kind.priority_weight(),
                enabled: true,
            },
        );
        state.queue_leaves.insert(
            leaf_id.clone(),
            SealedTransitionLeaf {
                leaf_id: leaf_id.clone(),
                queue_id: queue_id.clone(),
                contract_id: "contract-private-router".to_string(),
                leaf_index: 0,
                caller_commitment: "caller-commitment-alice".to_string(),
                sealed_transition_root: deterministic_root(
                    "sealed-transition",
                    "vault-route-call",
                    0,
                ),
                encrypted_witness_root: deterministic_root("encrypted-witness", "vault-route", 0),
                merkle_path_root: deterministic_root("merkle-path", "vault-route-leaf-0", 0),
                ...
                status: QueueLeafStatus::Executed,
        ...
        state.pq_transition_proofs.insert(
            ...
            PqTransitionProof {
                ...
                pq_signature_root: deterministic_root("pq-signature", "merkle-queue-proof", 0),
                transcript_root: deterministic_root("pq-transcript", "merkle-queue-proof", 0),
                merkle_inclusion_root: deterministic_root(
                    "merkle-inclusion",
                    "vault-route-leaf-0",
                    0,
                ),
        ...
        state.execution_batches.insert(
            ...
            ExecutionBatch {
                ...
                operator_id,
                epoch: DEVNET_EPOCH,
                queue_root: root_for_values(CONTRACT_QUEUE_SCHEME, &[json!(queue_id.clone())]),
                leaf_root: root_for_values(QUEUE_LEAF_SCHEME, &[json!(leaf_id.clone())]),
                proof_root: root_for_values(PQ_PROOF_SCHEME, &[json!(proof_id.clone())]),
        ...
        state.deterministic_traces.insert(
            ...
            DeterministicTrace {
                ...
                leaf_id,
                proof_id,
                queue_position_root: deterministic_root("queue-position", "vault-route-leaf-0", 0),
        ...
        let fixture_id = fixture_id("sealed-transition-merkle-queue-demo", 1);
        state.devnet_fixtures.insert(
            fixture_id.clone(),
            DevnetFixture {
                fixture_id,
                label: "sealed-transition-merkle-queue-demo".to_string(),
    ...
    pub fn public_record_without_state_root(&self) -> Value {
        json!({
            "kind": "private_l2_pq_confidential_contract_sealed_transition_merkle_queue_runtime_state",
            "protocol_version": PROTOCOL_VERSION,
            "schema_version": SCHEMA_VERSION,
            "hash_suite": HASH_SUITE,
            "sealed_transition_merkle_queue_suite": SEALED_TRANSITION_MERKLE_QUEUE_SUITE,
            ...
            "deterministic_merkle_queue_suite": DETERMINISTIC_MERKLE_QUEUE_SUITE,
            "config": self.config,
            "counters": self.counters,
            "roots": self.roots,
            "contract_queues": self.contract_queues.values().map(ContractMerkleQueue::public_record).collect::<Vec<_>>(),
            "queue_leaves": self.queue_leaves.values().map(SealedTransitionLeaf::public_record).collect::<Vec<_>>(),
    ...
    pub fn recompute_counters(&mut self) {
        self.counters = Counters {
            contract_queues: self.contract_queues.len() as u64,
            queue_leaves: self.queue_leaves.len() as u64,
            pending_queue_leaves: self
                .queue_leaves
                .values()
                .filter(|leaf| leaf.status.pending())
                .count() as u64,
    ...
    pub fn recompute_roots(&mut self) {
        self.roots = Roots {
            contract_queue_root: record_root(
                CONTRACT_QUEUE_SCHEME,
                self.contract_queues
                    .values()
                    .map(ContractMerkleQueue::public_record)
                    .collect::<Vec<_>>()
                    .as_slice(),
            ),
            queue_leaf_root: record_root(
                QUEUE_LEAF_SCHEME,
                self.queue_leaves
                    .values()
                    .map(SealedTransitionLeaf::public_record)
                    .collect::<Vec<_>>()
                    .as_slice(),
            ),
...
pub fn contract_queue_id(kind: TransitionKind, contract_id: &str, nonce: u64) -> String {
    domain_hash(
        "PRIVATE-L2-PQ-CONFIDENTIAL-CONTRACT-SEALED-TRANSITION-MERKLE-QUEUE:QUEUE-ID",
        &[
...
pub fn sealed_transition_leaf_id(
    queue_id: &str,
    ...
    domain_hash(
        "PRIVATE-L2-PQ-CONFIDENTIAL-CONTRACT-SEALED-TRANSITION-MERKLE-QUEUE:LEAF-ID",
        &[
            HashPart::Str(PROTOCOL_VERSION),
            HashPart::Str(queue_id),
            HashPart::Str(caller_commitment),
            HashPart::U64(leaf_index),
...
pub fn pq_transition_proof_id(leaf_id: &str, proof_label: &str, nonce: u64) -> String {
    domain_hash(
        "PRIVATE-L2-PQ-CONFIDENTIAL-CONTRACT-SEALED-TRANSITION-MERKLE-QUEUE:PQ-PROOF-ID",
        &[
            HashPart::Str(PROTOCOL_VERSION),
...
pub fn rollback_root_id(leaf_id: &str, root_label: &str, nonce: u64) -> String {
    domain_hash(
        "PRIVATE-L2-PQ-CONFIDENTIAL-CONTRACT-SEALED-TRANSITION-MERKLE-QUEUE:ROLLBACK-ROOT-ID",
        &[
            HashPart::Str(PROTOCOL_VERSION),
            HashPart::Str(leaf_id),
...
pub fn compensation_root_id(leaf_id: &str, root_label: &str, nonce: u64) -> String {
    domain_hash(
        "PRIVATE-L2-PQ-CONFIDENTIAL-CONTRACT-SEALED-TRANSITION-MERKLE-QUEUE:COMPENSATION-ROOT-ID",
        &[
            HashPart::Str(PROTOCOL_VERSION),
            HashPart::Str(leaf_id),
            HashPart::Str(root_label),
...
pub fn operator_id(operator_commitment: &str, epoch: u64) -> String {
    domain_hash(
        "PRIVATE-L2-PQ-CONFIDENTIAL-CONTRACT-SEALED-TRANSITION-MERKLE-QUEUE:OPERATOR-ID",
        &[
            HashPart::Str(PROTOCOL_VERSION),
...
pub fn execution_batch_id(operator_id: &str, epoch: u64, height: u64) -> String {
    domain_hash(
        "PRIVATE-L2-PQ-CONFIDENTIAL-CONTRACT-SEALED-TRANSITION-MERKLE-QUEUE:BATCH-ID",
        &[
            HashPart::Str(PROTOCOL_VERSION),
...
pub fn deterministic_trace_id(
    ...
    domain_hash(
        "PRIVATE-L2-PQ-CONFIDENTIAL-CONTRACT-SEALED-TRANSITION-MERKLE-QUEUE:TRACE-ID",
        &[
            HashPart::Str(PROTOCOL_VERSION),
...
pub fn fixture_id(label: &str, nonce: u64) -> String {
    domain_hash(
        "PRIVATE-L2-PQ-CONFIDENTIAL-CONTRACT-SEALED-TRANSITION-MERKLE-QUEUE:FIXTURE-ID",
        &[
            HashPart::Str(PROTOCOL_VERSION),
            HashPart::Str(label),
            HashPart::U64(nonce),
...
pub fn deterministic_root(label: &str, subject: &str, nonce: u64) -> String {
    domain_hash(
        "PRIVATE-L2-PQ-CONFIDENTIAL-CONTRACT-SEALED-TRANSITION-MERKLE-QUEUE:DETERMINISTIC-ROOT",
        &[
            HashPart::Str(PROTOCOL_VERSION),
            HashPart::Str(label),
...
pub fn state_root_from_record(record: &Value) -> String {
    domain_hash(
        "PRIVATE-L2-PQ-CONFIDENTIAL-CONTRACT-SEALED-TRANSITION-MERKLE-QUEUE:STATE-ROOT",
