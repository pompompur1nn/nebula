...
pub type PrivateL2FastPqConfidentialContractPrefetchLaneRuntimeResult<T> = Result<T>;
pub type Runtime = State;

pub const PROTOCOL_VERSION: &str =
    "nebula-private-l2-fast-pq-confidential-contract-prefetch-lane-runtime-v1";
pub const PRIVATE_L2_FAST_PQ_CONFIDENTIAL_CONTRACT_PREFETCH_LANE_PROTOCOL: &str = PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const PQ_ATTESTATION_SUITE: &str = "ML-DSA-87+SLH-DSA-SHAKE-256f-committee-attestation-v1";
pub const PQ_ENVELOPE_SUITE: &str = "ML-KEM-1024-threshold-contract_prefetch-envelope-v1";
pub const PRECONFIRMATION_PROOF_SUITE: &str =
    "zk-fast-confidential-contract_prefetch-preconfirmation-proof-v1";
pub const RECURSIVE_RECEIPT_SUITE: &str = "nova-pq-private-contract_prefetch-recursive-receipt-v1";
pub const NULLIFIER_FENCE_SUITE: &str = "monero-l2-private-contract_prefetch-nullifier-fence-v1";
pub const MEV_CHALLENGE_SUITE: &str = "confidential-contract_prefetch-mev-challenge-evidence-v1";
pub const SPONSOR_REBATE_SUITE: &str = "low-fee-private-contract_prefetch-sponsor-rebate-v1";
...
pub enum ContractPrefetchLaneKind {
    MoneroSwap,
    ConfidentialToken,
    DefiIntent,
    SmartContractCall,
...
impl Config {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        {
            return Err("confidential contract_prefetch capacities must be positive".to_string());
        }
        if self.min_pq_security_bits < 128 {
            return Err("confidential contract_prefetch PQ security floor is too low".to_string());
        }
        if self.min_privacy_set_size == 0
            || self.target_privacy_set_size < self.min_privacy_set_size
        {
            return Err("confidential contract_prefetch privacy set policy is invalid".to_string());
        }
        if self.slot_width_ms == 0
            || self.target_preconfirmation_ms == 0
            || self.max_preconfirmation_ms < self.target_preconfirmation_ms
        {
            return Err("confidential contract_prefetch latency policy is invalid".to_string());
        }
        if self.envelope_ttl_slots == 0
            || self.proof_ttl_slots == 0
            || self.challenge_window_slots == 0
        {
            return Err("confidential contract_prefetch TTL windows must be positive".to_string());
        }
        if self.quorum_weight_bps == 0
            || self.supermajority_weight_bps < self.quorum_weight_bps
            || self.supermajority_weight_bps > MAX_BPS
        {
            return Err("confidential contract_prefetch quorum policy is invalid".to_string());
        }
        ...
        {
            return Err(
                "confidential contract_prefetch fee or slashing BPS is invalid".to_string(),
            );
        }
        if self.fee_asset_id.trim().is_empty() {
            return Err("confidential contract_prefetch fee asset id is required".to_string());
        }
        Ok(())
    }
...
impl LanePolicy {
    pub fn for_kind(kind: ContractPrefetchLaneKind, config: &Config) -> Self {
        Self {
            max_envelopes_per_slot: match kind {
                ContractPrefetchLaneKind::EmergencyUnwind => 512,
                ContractPrefetchLaneKind::OracleUpdate => 1_024,
                ContractPrefetchLaneKind::SmartContractCall => 2_048,
                _ => 4_096,
            },
            min_privacy_set_size: config.min_privacy_set_size.max(
                if matches!(kind, ContractPrefetchLaneKind::MoneroSwap) {
                    32_768
                } else {
            ...
            require_sponsor: !matches!(kind, ContractPrefetchLaneKind::OracleUpdate),
            allow_contract_calls: matches!(
                kind,
                ContractPrefetchLaneKind::SmartContractCall
                    | ContractPrefetchLaneKind::DefiIntent
                    | ContractPrefetchLaneKind::PerpetualMargin
                    | ContractPrefetchLaneKind::LendingLiquidation
            ),
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct ContractPrefetchLane {
    pub lane_id: String,
    pub kind: ContractPrefetchLaneKind,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct PrivateContractPrefetchEnvelope {
...
impl PrivateContractPrefetchEnvelope {
    pub fn public_record(&self) -> Value {
        json!({
...
pub struct PreconfirmationProof {
    ...
    pub attestation_root: String,
    pub nullifier_root: String,
    pub encrypted_contract_prefetch_root: String,
...
impl PreconfirmationProof {
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "attestation_root": self.attestation_root,
            "nullifier_root": self.nullifier_root,
            "encrypted_contract_prefetch_root": self.encrypted_contract_prefetch_root,
...
pub struct State {
    ...
    pub lanes: BTreeMap<String, ContractPrefetchLane>,
    pub slots: BTreeMap<String, LatencySlot>,
    pub envelopes: BTreeMap<String, PrivateContractPrefetchEnvelope>,
...
impl State {
    ...
    pub fn devnet() -> Result<Self> {
        ...
        let lane_id = state.open_lane(
            ContractPrefetchLaneKind::DefiIntent,
            "devnet-fast-contract_prefetch-operator",
    ...
    pub fn advance_clock(
        ...
        if height < self.current_height || slot < self.current_slot {
            return Err("confidential contract_prefetch clock cannot move backwards".to_string());
        }
    ...
    pub fn open_lane(
        &mut self,
        kind: ContractPrefetchLaneKind,
        operator_commitment: &str,
        committee_id: &str,
        policy: Option<LanePolicy>,
    ) -> Result<String> {
        if self.lanes.len() >= self.config.max_lanes {
            return Err("confidential contract_prefetch lane capacity reached".to_string());
        }
        ...
        if self.slashed_committees.contains(committee_id) {
            return Err("slashed committee cannot open a new contract_prefetch lane".to_string());
        }
        ...
        if self.lanes.contains_key(&lane_id) {
            return Err("confidential contract_prefetch lane id collision".to_string());
        }
        let lane = ContractPrefetchLane {
    ...
    pub fn open_latency_slot(
        ...
        if self.slots.len() >= self.config.max_active_slots {
            return Err("confidential contract_prefetch active slot capacity reached".to_string());
        }
        let lane = self
            .lanes
            .get_mut(lane_id)
            .ok_or_else(|| "confidential contract_prefetch lane not found".to_string())?;
        if !lane.status.accepts_envelopes() {
            return Err("confidential contract_prefetch lane is not accepting slots".to_string());
        }
        if slot_index < lane.last_slot {
            return Err("confidential contract_prefetch slot index is stale".to_string());
        }
        let slot_id = latency_slot_id(lane_id, slot_index, opens_at_ms, self.counters.slots_opened);
        if self.slots.contains_key(&slot_id) {
            return Err("confidential contract_prefetch slot id collision".to_string());
        }
    ...
    pub fn submit_envelope(&mut self, request: SubmitEnvelope) -> Result<String> {
        if self.envelopes.len() >= self.config.max_envelopes {
            return Err("confidential contract_prefetch envelope capacity reached".to_string());
        }
        ...
        let lane = self
            ...
            .ok_or_else(|| "confidential contract_prefetch lane not found".to_string())?;
        if !lane.status.accepts_envelopes() {
            return Err("confidential contract_prefetch lane is closed to envelopes".to_string());
        }
        if request.privacy_set_size < lane.policy.min_privacy_set_size {
            return Err(
                "confidential contract_prefetch envelope privacy set below lane floor".to_string(),
            );
        }
        if request.max_fee_bps > lane.policy.max_user_fee_bps {
            return Err("confidential contract_prefetch envelope fee exceeds lane cap".to_string());
        }
        if lane.policy.require_sponsor && request.sponsor_id.is_none() {
            return Err("confidential contract_prefetch envelope requires a sponsor".to_string());
        }
        if self.nullifier_fences.contains_key(&request.nullifier) {
            return Err("confidential contract_prefetch nullifier already fenced".to_string());
        }
        ...
        let envelope = PrivateContractPrefetchEnvelope {
            envelope_id: envelope_id.clone(),
            lane_id: request.lane_id.clone(),
    ...
    pub fn assign_envelope_to_slot(&mut self, envelope_id: &str, slot_id: &str) -> Result<()> {
        let slot = self
            .slots
            .get_mut(slot_id)
            .ok_or_else(|| "confidential contract_prefetch slot not found".to_string())?;
        if !slot.status.accepts_attestation() {
            return Err("confidential contract_prefetch slot cannot accept envelopes".to_string());
        }
        let lane = self
            .lanes
            .get(&slot.lane_id)
        ...
        if slot.envelope_ids.len() >= lane.policy.max_envelopes_per_slot {
            return Err(
                "confidential contract_prefetch slot envelope capacity reached".to_string(),
            );
        }
        let envelope = self
            .envelopes
            .get_mut(envelope_id)
            .ok_or_else(|| "confidential contract_prefetch envelope not found".to_string())?;
        if envelope.lane_id != slot.lane_id {
            return Err(
                "confidential contract_prefetch envelope and slot lane mismatch".to_string(),
            );
        }
        if !envelope.status.live() || envelope.expires_slot < self.current_slot {
            return Err("confidential contract_prefetch envelope is not assignable".to_string());
        }
        ...
        slot.target_root = merkle_root(
            "private-l2-fast-pq-confidential-contract_prefetch-slot-target",
            &slot
                .envelope_ids
                .iter()
                .map(|id| json!(id))
    ...
    pub fn attest_slot(&mut self, request: AttestSlot) -> Result<String> {
        if self.attestations.len() >= self.config.max_attestations {
            return Err("confidential contract_prefetch attestation capacity reached".to_string());
        }
        request.validate(&self.config)?;
        let slot = self
            .slots
        ...
        if !slot.status.accepts_attestation() {
            return Err(
                "confidential contract_prefetch slot no longer accepts attestations".to_string(),
            );
        }
        let lane = self
            .lanes
        ...
        if lane.committee_id != request.committee_id {
            return Err(
                "confidential contract_prefetch attestation committee mismatch".to_string(),
            );
        }
        for envelope_id in &request.envelope_ids {
            if !slot.envelope_ids.iter().any(|id| id == envelope_id) {
                return Err(
                    "confidential contract_prefetch attestation references envelope outside slot"
                        .to_string(),
                );
            }
        }
    ...
    pub fn seal_preconfirmation(
        ...
        if self.proofs.len() >= self.config.max_proofs {
            return Err("confidential contract_prefetch proof capacity reached".to_string());
        }
        ...
        if slot.include_weight_bps < self.config.quorum_weight_bps {
            return Err(
                "confidential contract_prefetch slot lacks preconfirmation quorum".to_string(),
            );
        }
        if slot.reject_weight_bps >= self.config.quorum_weight_bps {
            return Err("confidential contract_prefetch slot has rejection quorum".to_string());
        }
        ...
        let proof = PreconfirmationProof {
            ...
            attestation_root: merkle_root(
                "private-l2-fast-pq-confidential-contract_prefetch-attestation-set",
                &attestation_leaves,
            ),
            nullifier_root: merkle_root(
                "private-l2-fast-pq-confidential-contract_prefetch-nullifier-set",
                &nullifier_leaves,
            ),
            encrypted_contract_prefetch_root: merkle_root(
                "private-l2-fast-pq-confidential-contract_prefetch-encrypted-set",
                &encrypted_leaves,
            ),
    ...
    pub fn reserve_sponsor_rebate(
        ...
        if self.rebates.len() >= self.config.max_rebates {
            return Err("confidential contract_prefetch rebate capacity reached".to_string());
        }
        ...
        if reserved_micro_units == 0 || rebate_bps == 0 || rebate_bps > self.config.max_rebate_bps {
            return Err("confidential contract_prefetch rebate quote is invalid".to_string());
        }
        let envelope = self
            .envelopes
            .get(envelope_id)
        ...
        if let Some(expected_sponsor) = &envelope.sponsor_id {
            if expected_sponsor != sponsor_id {
                return Err("confidential contract_prefetch sponsor mismatch".to_string());
            }
        }
    ...
    pub fn publish_receipt(
        ...
        if self.receipts.len() >= self.config.max_receipts {
            return Err("confidential contract_prefetch receipt capacity reached".to_string());
        }
        ...
        let proof = self
            .proofs
            .get_mut(proof_id)
            .ok_or_else(|| "confidential contract_prefetch proof not found".to_string())?;
        if !matches!(proof.status, ProofStatus::Sealed) {
            return Err("confidential contract_prefetch proof is not receiptable".to_string());
        }
        if proof.expires_slot < self.current_slot {
            proof.status = ProofStatus::Expired;
            return Err("confidential contract_prefetch proof expired before receipt".to_string());
        }
    ...
    pub fn challenge_preconfirmation(&mut self, request: ChallengeRequest) -> Result<String> {
        if self.challenges.len() >= self.config.max_challenges {
            return Err("confidential contract_prefetch challenge capacity reached".to_string());
        }
        request.validate()?;
        let proof = self.proofs.get(&request.target_proof_id).ok_or_else(|| {
            "confidential contract_prefetch challenged proof not found".to_string()
        })?;
        if self.current_slot > proof.sealed_slot + self.config.challenge_window_slots {
            return Err("confidential contract_prefetch challenge window is closed".to_string());
        }
    ...
    pub fn resolve_challenge(&mut self, challenge_id: &str, accept: bool) -> Result<()> {
        let (proof_id, slot_id, slash_micro_units) = {
            let challenge = self
                ...
                .ok_or_else(|| "confidential contract_prefetch challenge not found".to_string())?;
            if !matches!(challenge.status, ChallengeStatus::Filed) {
                return Err("confidential contract_prefetch challenge already resolved".to_string());
            }
    ...
    pub fn finalize_receipt(&mut self, receipt_id: &str) -> Result<()> {
        let receipt = self
            .receipts
            .get_mut(receipt_id)
            .ok_or_else(|| "confidential contract_prefetch receipt not found".to_string())?;
        if !matches!(
            receipt.status,
            ReceiptStatus::Published | ReceiptStatus::RecursiveAggregated
        ) {
            return Err("confidential contract_prefetch receipt cannot be finalized".to_string());
        }
    ...
    pub fn state_root(&self) -> String {
        domain_hash(
            "private-l2-fast-pq-confidential-contract-prefetch-lane-state-root",
            &[
    ...
    pub fn refresh_roots(&mut self) {
        self.roots.lane_root = merkle_root(
            "private-l2-fast-pq-confidential-contract_prefetch-lanes",
            &self
                .lanes
                .values()
        ...
        self.roots.slot_root = merkle_root(
            "private-l2-fast-pq-confidential-contract_prefetch-slots",
            &self
                .slots
                .values()
                .map(|v| v.public_record())
        ...
        self.roots.envelope_root = merkle_root(
            "private-l2-fast-pq-confidential-contract_prefetch-envelopes",
            &self
                .envelopes
                .values()
                .map(|v| v.public_record())
        ...
        self.roots.attestation_root = merkle_root(
            "private-l2-fast-pq-confidential-contract_prefetch-attestations",
            &self
                .attestations
                .values()
                .map(|v| v.public_record())
        ...
        self.roots.proof_root = merkle_root(
            "private-l2-fast-pq-confidential-contract_prefetch-proofs",
            &self
                .proofs
                .values()
                .map(|v| v.public_record())
        ...
        self.roots.rebate_root = merkle_root(
            "private-l2-fast-pq-confidential-contract_prefetch-rebates",
            &self
                .rebates
                .values()
                .map(|v| v.public_record())
        ...
        self.roots.nullifier_root = merkle_root(
            "private-l2-fast-pq-confidential-contract_prefetch-nullifiers",
            &self
                .nullifier_fences
                .values()
                .map(|v| v.public_record())
        ...
        self.roots.challenge_root = merkle_root(
            "private-l2-fast-pq-confidential-contract_prefetch-challenges",
            &self
                .challenges
                .values()
                .map(|v| v.public_record())
        ...
        self.roots.receipt_root = merkle_root(
            "private-l2-fast-pq-confidential-contract_prefetch-receipts",
            &self
                .receipts
                .values()
                .map(|v| v.public_record())
        ...
        self.roots.public_record_root = merkle_root(
            "private-l2-fast-pq-confidential-contract_prefetch-public-records",
            &self
                .public_records
                .values()
                .map(|v| v.public_record())
    ...
    fn emit_public_record(&mut self, record_kind: &str, payload: Value) -> Result<String> {
        if self.public_records.len() >= self.config.max_public_records {
            return Err(
                "confidential contract_prefetch public record capacity reached".to_string(),
            );
        }
...
impl SubmitEnvelope {
    pub fn validate(&self, config: &Config) -> Result<()> {
        ...
        if self.max_fee_bps > config.max_user_fee_bps {
            return Err(
                "confidential contract_prefetch envelope fee exceeds runtime cap".to_string(),
            );
        }
        if self.privacy_set_size < config.min_privacy_set_size {
            return Err(
                "confidential contract_prefetch envelope privacy set too small".to_string(),
            );
        }
        if self.pq_security_bits < config.min_pq_security_bits {
            return Err("confidential contract_prefetch envelope PQ security too low".to_string());
        }
        Ok(())
    }
}
...
impl AttestSlot {
    pub fn validate(&self, config: &Config) -> Result<()> {
        ...
        if self.envelope_ids.is_empty() {
            return Err(
                "confidential contract_prefetch attestation requires envelopes".to_string(),
            );
        }
        if self.weight_bps == 0 || self.weight_bps > MAX_BPS {
            return Err("confidential contract_prefetch attestation weight is invalid".to_string());
        }
        if self.latency_ms > config.max_preconfirmation_ms {
            return Err(
                "confidential contract_prefetch attestation latency exceeds maximum".to_string(),
            );
        }
        Ok(())
    }
...
impl ChallengeRequest {
    pub fn validate(&self) -> Result<()> {
        ...
        if self.claimed_slash_micro_units == 0 {
            return Err(
                "confidential contract_prefetch challenge slash amount must be positive"
                    .to_string(),
            );
        }
        Ok(())
...
pub fn lane_id(
    ...
    domain_hash(
        "private-l2-fast-pq-confidential-contract_prefetch-lane-id",
        &[
            HashPart::Str(CHAIN_ID),
            HashPart::Str(PROTOCOL_VERSION),
...
pub fn latency_slot_id(lane_id: &str, slot_index: u64, opens_at_ms: u64, nonce: u64) -> String {
    domain_hash(
        "private-l2-fast-pq-confidential-contract_prefetch-latency-slot-id",
        &[
            HashPart::Str(CHAIN_ID),
            HashPart::Str(PROTOCOL_VERSION),
...
pub fn envelope_id(request: &SubmitEnvelope, height: u64, slot: u64, nonce: u64) -> String {
    domain_hash(
        "private-l2-fast-pq-confidential-contract_prefetch-envelope-id",
        &[
            HashPart::Str(CHAIN_ID),
            HashPart::Str(PROTOCOL_VERSION),
...
pub fn nullifier_fence_id(nullifier: &str, replay_fence: &str, envelope_id: &str) -> String {
    domain_hash(
        "private-l2-fast-pq-confidential-contract_prefetch-nullifier-fence-id",
        &[
            HashPart::Str(CHAIN_ID),
...
pub fn attestation_id(request: &AttestSlot, height: u64, slot: u64, nonce: u64) -> String {
    let envelope_root = merkle_root(
        "private-l2-fast-pq-confidential-contract_prefetch-attestation-envelope-ids",
        &request
            .envelope_ids
            .iter()
            .map(|id| json!(id))
    ...
    domain_hash(
        "private-l2-fast-pq-confidential-contract_prefetch-attestation-id",
        &[
            HashPart::Str(CHAIN_ID),
...
pub fn preconfirmation_proof_id(
    ...
    domain_hash(
        "private-l2-fast-pq-confidential-contract_prefetch-preconfirmation-proof-id",
        &[
            HashPart::Str(CHAIN_ID),
...
pub fn sponsor_rebate_id(
    ...
    domain_hash(
        "private-l2-fast-pq-confidential-contract_prefetch-sponsor-rebate-id",
        &[
            HashPart::Str(CHAIN_ID),
...
pub fn recursive_receipt_id(
    ...
    domain_hash(
        "private-l2-fast-pq-confidential-contract_prefetch-recursive-receipt-id",
        &[
            HashPart::Str(CHAIN_ID),
...
pub fn mev_challenge_id(request: &ChallengeRequest, height: u64, slot: u64, nonce: u64) -> String {
    domain_hash(
        "private-l2-fast-pq-confidential-contract_prefetch-mev-challenge-id",
        &[
            HashPart::Str(CHAIN_ID),
...
pub fn public_record_id(
    ...
    domain_hash(
        "private-l2-fast-pq-confidential-contract_prefetch-public-record-id",
        &[
            HashPart::Str(CHAIN_ID),
            HashPart::Str(PROTOCOL_VERSION),
...
pub fn root_from_record(domain: &str, record: &Value) -> String {
    domain_hash(
        "private-l2-fast-pq-confidential-contract_prefetch-record-root",
        &[
            HashPart::Str(CHAIN_ID),
            HashPart::Str(PROTOCOL_VERSION),
            HashPart::Str(domain),
...
pub fn empty_root(label: &str) -> String {
    merkle_root(
        &format!("private-l2-fast-pq-confidential-contract_prefetch-{label}"),
        &[],
    )
}
...
fn require_nonempty(label: &str, value: &str) -> Result<()> {
    if value.trim().is_empty() {
        Err(format!(
            "confidential contract_prefetch {label} is required"
        ))
    } else {
        Ok(())
    }
