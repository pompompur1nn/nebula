...
pub type PrivateL2PqConfidentialTokenizedProverFailureInsuranceStructuredNoteLiquidityPoolRuntimeResult<
    T,
> = Result<T>;
pub type Runtime = State;

pub const PRIVATE_L2_PQ_CONFIDENTIAL_TOKENIZED_PROVER_FAILURE_INSURANCE_STRUCTURED_NOTE_LIQUIDITY_POOL_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-private-l2-pq-confidential-tokenized-prover-failure-insurance-structured-note-liquidity-pool-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_PQ_CONFIDENTIAL_TOKENIZED_PROVER_FAILURE_INSURANCE_STRUCTURED_NOTE_LIQUIDITY_POOL_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const PQ_LIQUIDITY_POOL_AUTHORIZATION_SUITE: &str =
    "ML-KEM-1024+ML-DSA-87+SLH-DSA-SHAKE-256f-prover-failure-note-liquidity-pool-auth-v1";
pub const POOL_VENUE_SUITE: &str =
    "private-prover-failure-structured-note-liquidity-pool-venue-root-v1";
pub const ELIGIBLE_NOTE_POOL_SUITE: &str =
    "confidential-tokenized-prover-failure-structured-note-eligible-pool-root-v1";
pub const SEALED_LISTING_SUITE: &str =
    "sealed-confidential-prover-failure-structured-note-liquidity-pool-listing-root-v1";
pub const PRIVATE_RFQ_SUITE: &str =
    "private-prover-failure-structured-note-liquidity-pool-rfq-root-v1";
pub const ORDER_COMMITMENT_SUITE: &str =
    "pq-private-prover-failure-structured-note-liquidity-pool-order-commitment-root-v1";
pub const MATCH_INTENT_SUITE: &str =
    "confidential-prover-failure-structured-note-liquidity-pool-match-intent-root-v1";
pub const TRADE_FILL_SUITE: &str =
    "speedy-prover-failure-structured-note-liquidity-pool-trade-fill-root-v1";
pub const ATOMIC_SETTLEMENT_SUITE: &str =
    "low-fee-prover-failure-structured-note-liquidity-pool-atomic-settlement-root-v1";
pub const LIQUIDITY_LANE_SUITE: &str =
    "defi-prover-failure-structured-note-liquidity-pool-liquidity-lane-root-v1";
pub const PRICE_BAND_SUITE: &str =
    "privacy-preserving-prover-failure-structured-note-liquidity-pool-price-band-root-v1";
pub const PQ_ATTESTATION_SUITE: &str =
    "pq-prover-failure-structured-note-liquidity-pool-attestation-root-v1";
pub const FEE_REBATE_SUITE: &str =
    "low-fee-prover-failure-structured-note-liquidity-pool-fee-rebate-root-v1";
pub const NULLIFIER_SUITE: &str =
    "anti-replay-prover-failure-structured-note-liquidity-pool-nullifier-root-v1";
pub const PUBLIC_RECORD_SUITE: &str =
    "roots-only-prover-failure-insurance-structured-note-liquidity-pool-public-record-v1";
pub const STATE_ROOT_SUITE: &str =
    "private-l2-pq-confidential-tokenized-prover-failure-insurance-structured-note-liquidity-pool-state-root-v1";
pub const PAYLOAD_ROOT_SUITE: &str =
    "private-l2-pq-confidential-tokenized-prover-failure-insurance-structured-note-liquidity-pool-payload-root-v1";
pub const DEVNET_REPLAY_DOMAIN: &str =
    "nebula-private-l2-pq-prover-failure-insurance-structured-note-liquidity-pool-devnet";
pub const DEVNET_RUNTIME_ID: &str =
    "private-l2-pq-prover-failure-insurance-structured-note-liquidity-pool-devnet";
...
pub const DEVNET_NOTE_TOKEN_ID: &str = "tpfi-note-liquidity-pool-devnet";
...
pub const DEFAULT_POOL_MAKER_QUORUM: u16 = 3;
...
pub const DEFAULT_MAX_NOTE_POOLS: usize = 262_144;
...
impl NoteSeniority {
    pub fn liquidity_pool_margin_bps(self) -> u64 {
        match self {
...
pub struct Config {
    ...
    pub pq_liquidity_pool_authorization_suite: String,
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            pq_liquidity_pool_authorization_suite: PQ_LIQUIDITY_POOL_AUTHORIZATION_SUITE
                .to_string(),
            ...
            pool_maker_quorum: DEFAULT_POOL_MAKER_QUORUM,
            ...
            max_note_pools: DEFAULT_MAX_NOTE_POOLS,
...
impl Config {
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "pq_liquidity_pool_authorization_suite": self.pq_liquidity_pool_authorization_suite,
            ...
            "pool_maker_quorum": self.pool_maker_quorum,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct LiquidityPoolVenueInput {
    pub venue_id: String,
    pub kind: LiquidityPoolKind,
    pub operator_commitment: String,
    pub eligible_note_pool_root: String,
...
impl LiquidityPoolVenue {
    pub fn public_record(&self) -> Value {
        roots_safe_record("pool_venue", &self.venue_id, &self.venue_commitment_root)
    }
}

#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct EligibleNotePoolInput {
    pub pool_id: String,
    pub venue_id: String,
...
impl EligibleNotePool {
    pub fn public_record(&self) -> Value {
        roots_safe_record(
            "eligible_note_pool",
            &self.pool_id,
            &self.pool_commitment_root,
        )
    }
}
...
pub struct State {
    ...
    pub roots: Roots,
    pub venues: BTreeMap<String, LiquidityPoolVenue>,
    pub note_pools: BTreeMap<String, EligibleNotePool>,
    ...
    pub pq_attestations: BTreeMap<String, PqPoolAttestation>,
...
impl State {
    ...
    pub fn devnet() -> Self {
        ...
        state.seed_devnet().expect("valid liquidity pool devnet");
        state
    }

    pub fn add_venue(&mut self, input: LiquidityPoolVenueInput) -> Result<LiquidityPoolVenue> {
        ...
        let root = payload_root("pool_venue", &public_record_for(&input));
        let venue = LiquidityPoolVenue {
            venue_id: input.venue_id,
            kind: input.kind,
            status: VenueStatus::Active,
            operator_commitment: input.operator_commitment,
            eligible_note_pool_root: input.eligible_note_pool_root,
    ...
    pub fn add_note_pool(&mut self, input: EligibleNotePoolInput) -> Result<EligibleNotePool> {
        ensure_capacity(
            "note_pools",
            self.note_pools.len(),
            self.config.max_note_pools,
        )?;
        ...
        self.require_privacy_and_pq(input.privacy_set_size, input.pq_security_bits)?;
        let root = payload_root("eligible_note_pool", &public_record_for(&input));
        let pool = EligibleNotePool {
            ...
            privacy_set_size: input.privacy_set_size,
            pq_security_bits: input.pq_security_bits,
            pool_commitment_root: root,
        };
        insert_unique(&mut self.note_pools, pool.pool_id.clone(), pool.clone())?;
        self.publish_roots_only(format!("note_pool:{}", pool.pool_id), pool.public_record())?;
        self.refresh_roots();
        Ok(pool)
    }
    ...
    pub fn add_listing(&mut self, input: ListingInput) -> Result<SealedListing> {
        ...
        require(
            self.note_pools.contains_key(&input.pool_id),
            "unknown note pool",
        )?;
        require(
    ...
    pub fn add_pq_attestation(&mut self, attestation: PqPoolAttestation) -> Result<()> {
        require(
    ...
    pub fn compute_roots(&self) -> Roots {
        ...
        Roots {
            ...
            venue_root: map_public_root(
                POOL_VENUE_SUITE,
                &self.venues,
                LiquidityPoolVenue::public_record,
            ),
            note_pool_root: map_public_root(
                ELIGIBLE_NOTE_POOL_SUITE,
                &self.note_pools,
                EligibleNotePool::public_record,
            ),
            ...
            pq_attestation_root: map_public_root(
                ...
                PqPoolAttestation::public_record,
            ),
    ...
    fn current_counters(&self) -> Counters {
        Counters {
            ...
            note_pools: self.note_pools.len() as u64,
    ...
    fn seed_devnet(&mut self) -> Result<()> {
        let venue = self.add_venue(LiquidityPoolVenueInput {
            venue_id: "pf-note-liquidity-pool-clob-devnet".to_string(),
            kind: LiquidityPoolKind::ContinuousLimitOrderBook,
            operator_commitment: demo_root("operator", "liquidity-pool"),
            eligible_note_pool_root: demo_root("eligible_pool_set", "autocall-senior"),
            quote_asset_root: demo_root("quote_asset", DEVNET_QUOTE_ASSET_ID),
            fee_vault_root: demo_root("fee_vault", "liquidity-pool"),
        ...
        let pool = self.add_note_pool(EligibleNotePoolInput {
            pool_id: "pf-note-liquidity-pool-autocall-senior-devnet".to_string(),
        ...
        self.add_pq_attestation(PqPoolAttestation {
            attestation_id: "pq-liquidity-pool-attestation-devnet-0".to_string(),
            venue_id: venue.venue_id.clone(),
            subject_root: demo_root("attestation_subject", "venue-open"),
            committee_root: demo_root("committee", "liquidity-pool"),
        ...
        self.add_settlement(SettlementInput {
            settlement_id: "pf-note-liquidity-pool-settlement-devnet-0".to_string(),
            venue_id: venue.venue_id.clone(),
        ...
        self.add_fee_rebate_epoch(FeeRebateEpoch {
            rebate_epoch_id: "pf-note-liquidity-pool-rebate-devnet-0".to_string(),
            venue_id: venue.venue_id,
            maker_set_root: demo_root("maker_set", "rebate-0"),
            volume_root: demo_root("volume", "rebate-0"),
            fee_pool_root: demo_root("fee_pool", "rebate-0"),
