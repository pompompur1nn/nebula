...
pub type PrivateL2FastPqConfidentialWitnessDerivativeLatencyBankruptcyBackstopRuntimeResult<T> =
    Result<T>;
pub type Runtime = State;

pub const PRIVATE_L2_FAST_PQ_CONFIDENTIAL_WITNESS_DERIVATIVE_LATENCY_BANKRUPTCY_BACKSTOP_RUNTIME_PROTOCOL_VERSION: &str = "nebula-private-l2-fast-pq-confidential-witness-derivative-latency-bankruptcy-backstop-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_FAST_PQ_CONFIDENTIAL_WITNESS_DERIVATIVE_LATENCY_BANKRUPTCY_BACKSTOP_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "shake256-domain-separated-canonical-json-v1";
pub const PQ_BACKSTOP_ATTESTATION_SUITE: &str =
    "ml-dsa-87+slh-dsa-shake-256f-latency-bankruptcy-backstop-resolution-root-v1";
pub const BACKSTOP_POSITION_SUITE: &str =
    "ml-kem-1024-sealed-defi-derivative-bankruptcy-backstop-position-root-v1";
pub const SOCIALIZED_LOSS_SUITE: &str =
    "roots-only-socialized-loss-resolution-derivative-bankruptcy-backstop-v1";
pub const PRIVACY_BOUNDARY: &str =
    "roots_only_no_plaintext_witnesses_addresses_keys_backstop_terms_positions_losses_or_insurance_fund_members";
pub const PUBLIC_RECORD_SUITE: &str =
    "roots-only-private-l2-fast-pq-confidential-derivative-latency-bankruptcy-backstop-record-v1";
...
pub const DEFAULT_BACKSTOP_CYCLE_MS: u64 = 30;
...
pub const DEFAULT_BACKSTOP_DISCOUNT_BPS: u64 = 650;
...
const D_CONFIG: &str = "PL2-FAST-PQ-CONF-WITNESS-DERIV-LAT-BANKRUPTCY-BACKSTOP:CONFIG";
const D_COUNTERS: &str = "PL2-FAST-PQ-CONF-WITNESS-DERIV-LAT-BANKRUPTCY-BACKSTOP:COUNTERS";
const D_ROOTS: &str = "PL2-FAST-PQ-CONF-WITNESS-DERIV-LAT-BANKRUPTCY-BACKSTOP:ROOTS";
const D_STATE: &str = "PL2-FAST-PQ-CONF-WITNESS-DERIV-LAT-BANKRUPTCY-BACKSTOP:STATE";
const D_POOLS: &str = "PL2-FAST-PQ-CONF-WITNESS-DERIV-LAT-BANKRUPTCY-BACKSTOP:POOLS";
const D_CLAIMS: &str = "PL2-FAST-PQ-CONF-WITNESS-DERIV-LAT-BANKRUPTCY-BACKSTOP:CLAIMS";
const D_CASES: &str = "PL2-FAST-PQ-CONF-WITNESS-DERIV-LAT-BANKRUPTCY-BACKSTOP:CASES";
const D_LOSS_EPOCHS: &str =
    "PL2-FAST-PQ-CONF-WITNESS-DERIV-LAT-BANKRUPTCY-BACKSTOP:SOCIALIZED-LOSS-EPOCHS";
const D_BACKSTOP_DRAWS: &str =
    "PL2-FAST-PQ-CONF-WITNESS-DERIV-LAT-BANKRUPTCY-BACKSTOP:BACKSTOP-DRAWS";
const D_ATTESTATIONS: &str = "PL2-FAST-PQ-CONF-WITNESS-DERIV-LAT-BANKRUPTCY-BACKSTOP:ATTESTATIONS";
const D_RECEIPTS: &str = "PL2-FAST-PQ-CONF-WITNESS-DERIV-LAT-BANKRUPTCY-BACKSTOP:RECEIPTS";
const D_NULLIFIERS: &str = "PL2-FAST-PQ-CONF-WITNESS-DERIV-LAT-BANKRUPTCY-BACKSTOP:NULLIFIERS";
const D_PUBLIC: &str = "PL2-FAST-PQ-CONF-WITNESS-DERIV-LAT-BANKRUPTCY-BACKSTOP:PUBLIC";
const D_DEVNET: &str = "PL2-FAST-PQ-CONF-WITNESS-DERIV-LAT-BANKRUPTCY-BACKSTOP:DEVNET";
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            pq_backstop_attestation_suite: PQ_BACKSTOP_ATTESTATION_SUITE.to_string(),
            backstop_position_suite: BACKSTOP_POSITION_SUITE.to_string(),
            socialized_loss_suite: SOCIALIZED_LOSS_SUITE.to_string(),
            roots_only_public_records: true,
            min_pq_security_bits: DEFAULT_MIN_PQ_SECURITY_BITS,
            min_privacy_set_size: DEFAULT_MIN_PRIVACY_SET_SIZE,
            target_privacy_set_size: DEFAULT_TARGET_PRIVACY_SET_SIZE,
            backstop_cycle_ms: DEFAULT_BACKSTOP_CYCLE_MS,
            ...
            backstop_discount_bps: DEFAULT_BACKSTOP_DISCOUNT_BPS,
...
impl Config {
    pub fn validate(&self) -> Result<()> {
        ...
        ensure_bps("backstop_discount_bps", self.backstop_discount_bps)?;
        ...
        if self.backstop_cycle_ms == 0 || self.max_latency_ms == 0 {
            return Err("backstop cycle and max latency must be non-zero".to_string());
        }
        if !self.roots_only_public_records {
            return Err(
                "bankruptcy backstop runtime requires roots-only public records".to_string(),
            );
        }
        Ok(())
    }
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "pq_backstop_attestation_suite": self.pq_backstop_attestation_suite,
            "backstop_position_suite": self.backstop_position_suite,
            ...
            "backstop_cycle_ms": self.backstop_cycle_ms,
...
pub struct Roots {
    ...
    pub backstop_resolutions_root: String,
...
#[derive(Clone, Debug, Deserialize, PartialEq, Eq, Serialize)]
pub struct RegisterBackstopPoolInput {
    pub pool_id: String,
    pub derivative_class: DerivativeClass,
    pub bankruptcy_mode: BackstopMode,
...
impl BackstopPool {
    pub fn from_input(input: RegisterBackstopPoolInput) -> Self {
        Self {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Eq, Serialize)]
pub struct OpenBackstopClaimInput {
    pub claim_id: String,
    pub pool_id: String,
    pub side: ClaimSide,
...
pub struct BackstopClaim {
    ...
    pub expires_slot: u64,
    pub created_at_height: u64,
    pub backstop_resolution_id: Option<String>,
    pub loss_epoch_id: Option<String>,
}

impl BackstopClaim {
    pub fn from_input(input: OpenBackstopClaimInput) -> Self {
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "backstop_resolution_id": self.backstop_resolution_id,
...
#[derive(Clone, Debug, Deserialize, PartialEq, Eq, Serialize)]
pub struct OpenBackstopResolutionInput {
    pub case_id: String,
    pub pool_id: String,
...
#[derive(Clone, Debug, Deserialize, PartialEq, Eq, Serialize)]
pub struct PqBackstopAttestationInput {
...
#[derive(Clone, Debug, Deserialize, PartialEq, Eq, Serialize)]
pub struct PqBackstopAttestation {
    pub attestation_id: String,
    pub subject_id: String,
    pub subject_root: String,
...
pub struct State {
    ...
    pub claims: BTreeMap<String, BackstopClaim>,
    pub backstop_resolutions: BTreeMap<String, BackstopResolution>,
    pub socialized_loss_epochs: BTreeMap<String, SocializedLossEpoch>,
    pub backstop_draws: BTreeMap<String, BackstopDraw>,
    pub pq_attestations: BTreeMap<String, PqBackstopAttestation>,
...
impl State {
    pub fn new(config: Config) -> Self {
        let mut state = Self {
            ...
            pools: BTreeMap::new(),
            claims: BTreeMap::new(),
            backstop_resolutions: BTreeMap::new(),
    ...
    pub fn register_pool(&mut self, input: RegisterBackstopPoolInput) -> Result<()> {
        ...
        let pool = BackstopPool::from_input(input);
    ...
    pub fn open_claim(&mut self, input: OpenBackstopClaimInput) -> Result<()> {
        ...
        if !pool.status.accepts_claims() {
            return Err("pool does not accept backstop claims".to_string());
        }
        ...
        let claim = BackstopClaim::from_input(input);
    ...
    pub fn open_backstop_resolution(&mut self, input: OpenBackstopResolutionInput) -> Result<()> {
        ensure_capacity(
            self.backstop_resolutions.len(),
            self.config.max_cases,
            "backstop resolutions",
        )?;
        ensure_unique(
            &self.backstop_resolutions,
            "backstop resolution",
            &input.case_id,
        )?;
        ...
        if input.creditor_approval_bps < self.config.min_creditor_approval_bps {
            return Err("creditor approval below configured backstop floor".to_string());
        }
        ...
        if input.observed_latency_ms > pool_snapshot.max_latency_ms {
            return Err("backstop resolution exceeds pool latency bound".to_string());
        }
        ...
        for claim_id in &input.claim_ids {
            ...
            if claim.pool_id != input.pool_id || !claim.status.live() {
                return Err(format!("claim `{claim_id}` is not backstop-nettable"));
            }
            gross_claim_micros =
        ...
        if deficit_micros == 0 {
            return Err("backstop resolution requires a positive confidential deficit".to_string());
        }
        ...
        let fee_savings_micros =
            fee_savings_for_resolution(deficit_micros, self.config.backstop_discount_bps);
        ...
        for claim_id in &input.claim_ids {
            if let Some(claim) = self.claims.get_mut(claim_id) {
                ...
                claim.backstop_resolution_id = Some(input.case_id.clone());
            }
        }
        ...
        self.backstop_resolutions.insert(case.case_id.clone(), case);
        self.refresh_roots();
        Ok(())
    }
    ...
    pub fn request_backstop_draw(&mut self, input: RequestBackstopDrawInput) -> Result<()> {
        ...
        let case = self
            .backstop_resolutions
            .get(&input.case_id)
            .ok_or_else(|| format!("case `{}` not found", input.case_id))?;
        if case.pool_id != input.pool_id {
            return Err("backstop draw pool does not match backstop resolution".to_string());
        }
        let pool = self
            .pools
            .get_mut(&input.pool_id)
    ...
    pub fn allocate_socialized_loss(&mut self, input: AllocateSocializedLossInput) -> Result<()> {
        ...
        if !pool.status.can_resolve() {
            return Err("pool is not in a backstop resolution state".to_string());
        }
        ...
        for case_id in &input.case_ids {
            if let Some(case) = self.backstop_resolutions.get_mut(case_id) {
    ...
    pub fn attest_backstop_resolution(&mut self, input: PqBackstopAttestationInput) -> Result<()> {
        ensure_capacity(
        ...
        let attestation = PqBackstopAttestation {
    ...
    pub fn settle_case(&mut self, case_id: &str, height: u64, epoch: u64) -> Result<()> {
        ...
        for claim in self
            .claims
            .values_mut()
            .filter(|claim| claim.backstop_resolution_id.as_deref() == Some(case_id))
        {
    ...
    pub fn refresh_roots(&mut self) {
        ...
        self.roots.backstop_resolutions_root = map_root(D_CASES, &self.backstop_resolutions);
        ...
        self.roots.state_root = domain_hash(
            ...
            &[
                ...
                HashPart::Str(self.roots.backstop_resolutions_root.as_str()),
    ...
    fn roots_only_record(&self) -> PublicRecord {
        PublicRecord {
            ...
            backstop_resolutions_root: self.roots.backstop_resolutions_root.clone(),
    ...
    fn mark_subject_attested(&mut self, subject_id: &str) {
        if let Some(case) = self.backstop_resolutions.get_mut(subject_id) {
...
pub fn devnet() -> State {
    ...
    state
        .register_pool(RegisterBackstopPoolInput {
            pool_id: pool_id.clone(),
            derivative_class: DerivativeClass::BankruptcyBackstop,
            bankruptcy_mode: BackstopMode::SocializedLoss,
        ...
        .expect("devnet backstop pool must register");
    state
        .open_claim(OpenBackstopClaimInput {
    ...
    state
        .open_backstop_resolution(OpenBackstopResolutionInput {
            case_id: "devnet-backstop-resolution-0".to_string(),
            pool_id: pool_id.clone(),
            claim_ids: vec![
        ...
        .expect("devnet backstop resolution must open");
    state
        .attest_backstop_resolution(PqBackstopAttestationInput {
            attestation_id: "devnet-backstop-resolution-attestation-0".to_string(),
            subject_id: "devnet-backstop-resolution-0".to_string(),
    ...
    state
        .allocate_socialized_loss(AllocateSocializedLossInput {
            ...
            pool_id,
            case_ids: vec!["devnet-backstop-resolution-0".to_string()],
    ...
    state
        .attest_backstop_resolution(PqBackstopAttestationInput {
            attestation_id: "devnet-backstop-loss-attestation-0".to_string(),
    ...
    state
        .settle_case(
            "devnet-backstop-resolution-0",
            DEVNET_HEIGHT + 7,
            DEVNET_EPOCH + 3,
        )
        .expect("devnet backstop resolution must settle");
    state.refresh_roots();
    state
}
...
fn bankruptcy_fee(gross: u128, deficit: u128, config: &Config) -> u64 {
    let fee_bps = config
        .taker_fee_bps
        .saturating_sub(config.backstop_discount_bps.min(config.taker_fee_bps))
