...
pub type PrivateL2PqConfidentialForsExitReceiptReinsuranceClaimSettlementRuntimeResult<T> =
    Result<T>;
pub type Runtime = State;

pub const PRIVATE_L2_PQ_CONFIDENTIAL_FORS_EXIT_RECEIPT_REINSURANCE_CLAIM_SETTLEMENT_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-private-l2-pq-confidential-fors-exit-receipt-reinsurance-claim-settlement-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_PQ_CONFIDENTIAL_FORS_EXIT_RECEIPT_REINSURANCE_CLAIM_SETTLEMENT_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const FORS_EXIT_RECEIPT_REINSURANCE_CLAIM_SETTLEMENT_SUITE: &str =
    "fors-hash-based-exit-receipt-reinsurance-claim-settlement-v1";
...
pub const PQ_WITNESS_BUNDLE_SUITE: &str = "fors-pq-reinsurance-claim-witness-bundle-v1";
...
pub const LOW_FEE_BATCH_ADJUDICATION_SUITE: &str =
    "low-fee-fors-exit-receipt-reinsurance-claim-adjudication-batch-v1";
...
pub const ROOTS_ONLY_PUBLIC_RECORD_SUITE: &str =
    "roots-only-fors-exit-receipt-reinsurance-claim-public-record-v1";
pub const DEVNET_HEIGHT: u64 = 8_736_000;
pub const DEVNET_EPOCH: u64 = 36_400;
pub const DEVNET_SLOT: u64 = 512;
pub const DEFAULT_MIN_PQ_SECURITY_BITS: u16 = 256;
pub const DEFAULT_FORS_TREE_HEIGHT: u8 = 14;
pub const DEFAULT_FORS_TREE_COUNT: u16 = 36;
pub const DEFAULT_FORS_MESSAGE_DIGEST_BYTES: u16 = 64;
pub const DEFAULT_FORS_AUTH_PATH_CAP: u16 = 504;
...
impl ClaimFailureKind {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ForsLeafReuse => "fors_leaf_reuse",
...
pub struct Config {
    ...
    pub hash_based_fors_attestations_required: bool,
...
impl Config {
    pub fn devnet() -> Self {
        Self {
            ...
            fors_claim_suite: FORS_EXIT_RECEIPT_REINSURANCE_CLAIM_SETTLEMENT_SUITE.to_string(),
            ...
            fors_tree_height: DEFAULT_FORS_TREE_HEIGHT,
            fors_tree_count: DEFAULT_FORS_TREE_COUNT,
            ...
            hash_based_fors_attestations_required: true,
    ...
    pub fn validate(&self) -> Result<()> {
        if self.min_pq_security_bits < DEFAULT_MIN_PQ_SECURITY_BITS {
            return Err("pq security bits below FORS reinsurance claim minimum".to_string());
        }
        if self.fors_tree_height < 8 || self.fors_tree_height > 20 || self.fors_tree_count == 0 {
            return Err("invalid FORS claim hypertree parameter schedule".to_string());
        }
        if self.fors_message_digest_bytes < 32 || self.fors_message_digest_bytes > 96 {
            return Err("unsupported FORS message digest size".to_string());
        }
        if self.fors_auth_path_cap == 0 || self.nullifier_buckets == 0 {
            return Err("FORS auth path cap and nullifier buckets must be positive".to_string());
        }
...
impl Roots {
    pub fn empty() -> Self {
        Self {
            sealed_claim_root: merkle_root("fors-reinsurance-claim-settlement:sealed:empty", &[]),
            coupon_root: merkle_root("fors-reinsurance-claim-settlement:coupons:empty", &[]),
            witness_bundle_root: merkle_root(
                "fors-reinsurance-claim-settlement:witnesses:empty",
                &[],
            ),
            nullifier_root: merkle_root("fors-reinsurance-claim-settlement:nullifiers:empty", &[]),
            adjudication_batch_root: merkle_root(
                "fors-reinsurance-claim-settlement:batches:empty",
                &[],
            ),
            privacy_redaction_root: merkle_root(
                "fors-reinsurance-claim-settlement:redactions:empty",
                &[],
            ),
            receipt_status_root: merkle_root(
                "fors-reinsurance-claim-settlement:receipts:empty",
                &[],
            ),
            public_record_root: merkle_root("fors-reinsurance-claim-settlement:public:empty", &[]),
        }
    }
}

#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct ForsClaimAttestation {
    pub attestation_id: String,
    pub fors_public_root: String,
    pub fors_leaf_index: u32,
    pub fors_tree_epoch: u64,
    pub fors_tree_commitment: String,
...
impl ForsClaimAttestation {
    pub fn new(
        fors_public_root: impl Into<String>,
        fors_leaf_index: u32,
        fors_tree_epoch: u64,
        fors_tree_commitment: impl Into<String>,
        auth_path_root: impl Into<String>,
        message_digest: impl Into<String>,
        prior_leaf_cursor: u32,
        pq_security_bits: u16,
    ) -> Self {
        let fors_public_root = fors_public_root.into();
        let fors_tree_commitment = fors_tree_commitment.into();
        let auth_path_root = auth_path_root.into();
        let message_digest = message_digest.into();
        let next_leaf_cursor = fors_leaf_index.saturating_add(1);
        let leaf_state_hash = domain_hash(
            "fors-reinsurance-claim-settlement:leaf-state",
            &[
                HashPart::Str(&fors_public_root),
                HashPart::U64(u64::from(fors_leaf_index)),
                HashPart::U64(fors_tree_epoch),
                HashPart::Str(&fors_tree_commitment),
        ...
        let attestation_id = domain_hash(
            "fors-reinsurance-claim-settlement:attestation-id",
            &[
                HashPart::Str(&fors_public_root),
                HashPart::U64(u64::from(fors_leaf_index)),
                HashPart::U64(fors_tree_epoch),
    ...
    pub fn validate(&self, config: &Config) -> Result<()> {
        if self.pq_security_bits < config.min_pq_security_bits {
            return Err("FORS claim attestation below pq security floor".to_string());
        }
        if self.fors_leaf_index >= u32::from(config.fors_auth_path_cap) {
            return Err("FORS claim leaf index outside configured auth path cap".to_string());
        }
        if self.next_leaf_cursor <= self.prior_leaf_cursor {
            return Err("FORS claim leaf cursor did not advance".to_string());
        }
        if self.next_leaf_cursor != self.fors_leaf_index.saturating_add(1) {
            return Err("FORS claim leaf cursor inconsistent with leaf index".to_string());
        }
        require_hash_like("fors_public_root", &self.fors_public_root)?;
        require_hash_like("fors_tree_commitment", &self.fors_tree_commitment)?;
    ...
    pub fn public_commitment(&self) -> Value {
        json!({
            ...
            "fors_public_root": self.fors_public_root,
            "fors_tree_epoch": self.fors_tree_epoch,
...
impl SealedExitReceiptClaim {
    pub fn new(input: SealClaimInput, config: &Config) -> Result<Self> {
        ...
        let claim_id = domain_hash(
            "fors-reinsurance-claim-settlement:sealed-claim-id",
            &[
...
impl ReinsuranceClaimCoupon {
    pub fn new(input: IssueCouponInput, config: &Config) -> Result<Self> {
        ...
        let coupon_id = domain_hash(
            "fors-reinsurance-claim-settlement:coupon-id",
            &[
...
impl PqWitnessBundle {
    pub fn new(input: SubmitWitnessInput, config: &Config) -> Result<Self> {
        ...
        let witness_id = domain_hash(
            "fors-reinsurance-claim-settlement:witness-id",
            &[
                HashPart::Str(&input.claim_id),
    ...
    pub fn bundle_root(&self) -> String {
        domain_hash(
            "fors-reinsurance-claim-settlement:witness-bundle-root",
            &[
                ...
                HashPart::Str(&self.fors_auth_path_root),
    ...
    pub fn leaf(&self) -> Value {
        json!({
            ...
            "fors_auth_path_root": self.fors_auth_path_root,
...
impl ClaimNullifier {
    pub fn new(input: SpendNullifierInput, config: &Config) -> Result<Self> {
        ...
        let nullifier_id = domain_hash(
            "fors-reinsurance-claim-settlement:nullifier-id",
            &[
                HashPart::Str(&input.claim_id),
                HashPart::Str(&input.receipt_commitment),
        ...
        let bucket_hash = domain_hash(
            "fors-reinsurance-claim-settlement:nullifier-bucket",
...
impl PrivacyRedactionRoot {
    pub fn new(input: RedactionRootInput, config: &Config) -> Result<Self> {
        ...
        let redaction_root_id = domain_hash(
            "fors-reinsurance-claim-settlement:redaction-root-id",
            &[
...
impl LowFeeClaimAdjudicationBatch {
    pub fn new(input: AdjudicateBatchInput, state: &State) -> Result<Self> {
        ...
        let settlement_root = domain_hash(
            "fors-reinsurance-claim-settlement:batch-settlement-root",
            &[
        ...
        let batch_id = domain_hash(
            "fors-reinsurance-claim-settlement:batch-id",
            &[
                HashPart::Json(&json!(input.claim_ids)),
...
pub struct SealClaimInput {
    ...
    pub claim_open_slot: u64,
    pub risk: ClaimRisk,
    pub attestation: ForsClaimAttestation,
...
impl SubmitWitnessInput {
    pub fn validate(&self, _config: &Config) -> Result<()> {
        ...
        require_hash_like("fors_auth_path_root", &self.fors_auth_path_root)?;
...
pub struct State {
    ...
    pub attestations: BTreeMap<String, ForsClaimAttestation>,
    ...
    pub used_fors_leaves: BTreeSet<String>,
...
impl State {
    ...
    pub fn devnet() -> Self {
        ...
        Self::new(config, DEVNET_HEIGHT, DEVNET_EPOCH, DEVNET_SLOT)
            .expect("devnet FORS reinsurance claim config is valid")
    }
    ...
    pub fn seal_claim(&mut self, input: SealClaimInput) -> Result<String> {
        ...
        let leaf_key = fors_leaf_key(&attestation);
        if self.used_fors_leaves.contains(&leaf_key) {
            self.claim_failures.insert(
                attestation.attestation_id.clone(),
                ClaimFailureKind::ForsLeafReuse,
            );
            return Err("FORS leaf already used for reinsurance claim".to_string());
        }
        ...
        self.used_fors_leaves.insert(leaf_key);
        self.attestations
    ...
    pub fn refresh_roots(&mut self) {
        ...
        let receipts = self
            ...
            .collect::<Vec<_>>();
        self.roots = Roots {
            sealed_claim_root: merkle_root(
                "fors-reinsurance-claim-settlement:sealed",
                &sealed_claims,
            ),
            coupon_root: merkle_root("fors-reinsurance-claim-settlement:coupons", &coupons),
            witness_bundle_root: merkle_root(
                "fors-reinsurance-claim-settlement:witnesses",
                &witnesses,
            ),
            nullifier_root: merkle_root(
                "fors-reinsurance-claim-settlement:nullifiers",
                &nullifiers,
            ),
            adjudication_batch_root: merkle_root(
                "fors-reinsurance-claim-settlement:batches",
                &batches,
            ),
            privacy_redaction_root: merkle_root(
                "fors-reinsurance-claim-settlement:redactions",
                &redactions,
            ),
            receipt_status_root: merkle_root(
                "fors-reinsurance-claim-settlement:receipts",
                &receipts,
            ),
            public_record_root: merkle_root(
                "fors-reinsurance-claim-settlement:public-record",
    ...
    pub fn state_root(&self) -> String {
        ...
        domain_hash(
            "fors-reinsurance-claim-settlement:state-root",
    ...
    fn public_record_without_public_root(&self) -> Value {
        json!({
            ...
            "hash_suite": self.config.hash_suite,
            "suites": {
                "fors_claim": self.config.fors_claim_suite,
...
fn fors_leaf_key(attestation: &ForsClaimAttestation) -> String {
    domain_hash(
        "fors-reinsurance-claim-settlement:leaf-key",
        &[
            HashPart::Str(&attestation.fors_public_root),
            HashPart::U64(attestation.fors_tree_epoch),
            HashPart::U64(u64::from(attestation.fors_leaf_index)),
        ],
        32,
    )
}
