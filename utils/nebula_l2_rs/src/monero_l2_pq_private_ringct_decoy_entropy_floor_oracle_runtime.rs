...
pub type Result<T> = MoneroL2PqPrivateRingctDecoyEntropyFloorOracleRuntimeResult<T>;
pub type MoneroL2PqPrivateRingctDecoyEntropyFloorOracleRuntimeResult<T> =
    std::result::Result<T, String>;
pub type Runtime = State;

pub const MONERO_L2_PQ_PRIVATE_RINGCT_DECOY_ENTROPY_FLOOR_ORACLE_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-monero-l2-pq-private-ringct-decoy-entropy-floor-oracle-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    MONERO_L2_PQ_PRIVATE_RINGCT_DECOY_ENTROPY_FLOOR_ORACLE_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const ENTROPY_FLOOR_OBSERVATION_SCHEME: &str = "ringct-decoy-entropy-floor-observation-root-v1";
pub const MINIMUM_ENTROPY_BAND_SCHEME: &str = "ringct-minimum-decoy-entropy-band-root-v1";
pub const OUTPUT_AGE_DIVERSITY_SCORE_SCHEME: &str = "ringct-output-age-diversity-score-root-v1";
pub const PQ_MIGRATION_SAFETY_SCORE_SCHEME: &str =
    "ML-DSA-87+ML-KEM-1024+SLH-DSA-SHAKE-256f-ringct-decoy-entropy-floor-safety-root-v1";
pub const LOW_FEE_SCAN_BATCH_SCHEME: &str =
    "low-fee-private-ringct-decoy-entropy-floor-scan-batch-root-v1";
pub const PUBLIC_RECORD_SCHEME: &str =
    "deterministic-public-ringct-decoy-entropy-floor-oracle-record-v1";
...
#[derive(Clone, Copy, Debug, Deserialize, Eq, Ord, PartialEq, PartialOrd, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum MinimumEntropyBandKind {
    AncientAnchor,
    ColdHistorical,
    WarmRecent,
    HotRecent,
...
impl MinimumEntropyBandKind {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::LiquidityBridge => "liquidity_bridge",
...
impl Config {
    ...
    pub fn validate(&self) -> Result<()> {
            ...
            "minimum RingCT ring size is too low",
        )?;
        ensure(
            self.target_ring_size >= self.min_ring_size,
            "target RingCT ring size must cover minimum ring size",
        )?;
        ensure(
            ...
            "target anonymity set must cover RingCT privacy floor",
        )?;
        ensure(
            self.target_entropy_bands >= self.min_entropy_bands,
            "target entropy bands must cover minimum entropy bands",
        )?;
        ensure(
    ...
    pub fn state_root(&self) -> String {
        root_from_record(
            "ringct-decoy-entropy-floor-oracle-config",
            &self.public_record(),
        )
    }
}
...
impl Counters {
    ...
    pub fn state_root(&self) -> String {
        root_from_record(
            "ringct-decoy-entropy-floor-oracle-counters",
            &self.public_record(),
        )
    }
}
...
pub struct Roots {
    ...
    pub minimum_entropy_bands_root: String,
...
impl Roots {
    pub fn empty() -> Self {
        Self {
            entropy_floor_observations_root: empty_root(ENTROPY_FLOOR_OBSERVATION_SCHEME),
            minimum_entropy_bands_root: empty_root(MINIMUM_ENTROPY_BAND_SCHEME),
            ...
            deterministic_state_root: empty_root("ringct-decoy-entropy-floor-oracle-state"),
        }
    }
...
pub struct EntropyFloorObservation {
    ...
    pub ringct_distribution_skew_bps: u64,
    pub redacted_ringct_sample_root: String,
...
impl EntropyFloorObservation {
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "ringct_distribution_skew_bps": self.ringct_distribution_skew_bps,
            "redacted_ringct_sample_root": self.redacted_ringct_sample_root,
    ...
    pub fn state_root(&self) -> String {
        root_from_record(ENTROPY_FLOOR_OBSERVATION_SCHEME, &self.public_record())
    }
}

#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct MinimumEntropyBand {
    pub band_id: String,
    pub observation_id: String,
    pub band_kind: MinimumEntropyBandKind,
    pub min_output_age_blocks: u64,
    pub max_output_age_blocks: u64,
    pub minimum_entropy_millibits: u64,
    pub floor_contribution_bps: u64,
    pub band_weight_bps: u64,
    pub sampled_outputs: u64,
    pub unique_commitment_buckets: u64,
    pub redacted_output_age_bucket_root: String,
    pub ringct_decoy_sampler_root: String,
    pub minimum_entropy_witness_root: String,
    pub status: EntropyFloorStatus,
}

impl MinimumEntropyBand {
    pub fn public_record(&self) -> Value {
        json!({
            "band_id": self.band_id,
            "observation_id": self.observation_id,
            "band_kind": self.band_kind.as_str(),
            "min_output_age_blocks": self.min_output_age_blocks,
            "max_output_age_blocks": self.max_output_age_blocks,
            "minimum_entropy_millibits": self.minimum_entropy_millibits,
            "floor_contribution_bps": self.floor_contribution_bps,
            "band_weight_bps": self.band_weight_bps,
            "sampled_outputs": self.sampled_outputs,
            "unique_commitment_buckets": self.unique_commitment_buckets,
            "redacted_output_age_bucket_root": self.redacted_output_age_bucket_root,
            "ringct_decoy_sampler_root": self.ringct_decoy_sampler_root,
            "minimum_entropy_witness_root": self.minimum_entropy_witness_root,
            "status": self.status,
        })
    }

    pub fn state_root(&self) -> String {
        root_from_record(MINIMUM_ENTROPY_BAND_SCHEME, &self.public_record())
    }
}
...
pub struct PqMigrationSafetyScore {
    ...
    pub ringct_to_pq_binding_root: String,
...
pub struct LowFeeScanBatch {
    ...
    pub rebate_bps: u64,
    pub batched_ringct_scan_root: String,
...
pub struct State {
    ...
    pub minimum_entropy_bands: BTreeMap<String, MinimumEntropyBand>,
...
impl State {
    pub fn new(config: Config, l2_height: u64, monero_height: u64, epoch: u64) -> Result<Self> {
        ...
        let mut state = Self {
            ...
            minimum_entropy_bands: BTreeMap::new(),
    ...
    pub fn public_record(&self) -> Value {
        json!({
            "runtime": "monero_l2_pq_private_ringct_decoy_entropy_floor_oracle_runtime",
    ...
    pub fn insert_entropy_floor_observation(
        ...
        observation: EntropyFloorObservation,
            ...
            observation.ringct_distribution_skew_bps <= self.config.max_age_bucket_skew_bps,
            "RingCT decoy distribution skew exceeds cap",
        )?;
        ensure(
            observation.expires_at_monero_height > self.monero_height,
            "RingCT entropy floor observation must expire in the future",
        )?;
    ...
    pub fn insert_minimum_entropy_band(&mut self, band: MinimumEntropyBand) -> Result<()> {
        ensure(
            self.entropy_floor_observations
                .contains_key(&band.observation_id),
            "minimum entropy band references unknown observation",
        )?;
        ensure(
            band.max_output_age_blocks >= band.min_output_age_blocks,
            "max output age must cover min output age",
        )?;
        ensure(
            band.minimum_entropy_millibits >= self.config.min_min_entropy_millibits,
            "minimum entropy band is below configured min-entropy floor",
        )?;
        ensure(
            band.floor_contribution_bps <= MAX_BPS,
            "minimum entropy band contribution exceeds max bps",
        )?;
        ensure(
            band.band_weight_bps <= MAX_BPS,
            "minimum entropy band weight exceeds max bps",
        )?;
        ensure(
            band.sampled_outputs >= self.config.min_ring_size as u64,
            "minimum entropy band sampled outputs are below ring floor",
        )?;
        ensure(
            band.unique_commitment_buckets > 0
                && band.unique_commitment_buckets <= band.sampled_outputs,
            "minimum entropy band unique buckets must be within sampled outputs",
        )?;
        self.minimum_entropy_bands
    ...
    pub fn refresh_roots(&mut self) {
        ...
        self.counters.minimum_entropy_bands = self.minimum_entropy_bands.len() as u64;
        ...
        self.roots.entropy_floor_observations_root = map_root(
            ENTROPY_FLOOR_OBSERVATION_SCHEME,
            self.entropy_floor_observations
                .iter()
                .map(|(id, observation)| (id.as_str(), observation.state_root())),
        );
        self.roots.minimum_entropy_bands_root = map_root(
            MINIMUM_ENTROPY_BAND_SCHEME,
            self.minimum_entropy_bands
                .iter()
    ...
    fn state_root_without_cached_roots(&self) -> String {
        root_from_parts(
            "ringct-decoy-entropy-floor-oracle-state",
            &[
                ...
                HashPart::Str(&self.roots.minimum_entropy_bands_root),
...
impl Default for State {
    fn default() -> Self {
        ...
        .expect("default RingCT decoy entropy floor oracle config is valid")
    }
}

pub fn devnet() -> State {
    let mut state = State::default();
    let observation_id = "ringct-decoy-entropy-floor-oracle-devnet-0".to_string();
    let diversity_id = "ringct-output-age-diversity-devnet-0".to_string();
    ...
    state
        .insert_entropy_floor_observation(EntropyFloorObservation {
            ...
            ringct_distribution_skew_bps: 240,
            redacted_ringct_sample_root: root_from_parts(
                "devnet-redacted-ringct-decoy-entropy-sample",
                &[HashPart::Str(&observation_id)],
            ),
            output_commitment_set_root: root_from_parts(
                "devnet-ringct-output-commitment-set",
                &[HashPart::Str(&observation_id)],
            ),
            key_image_domain_root: root_from_parts(
                "devnet-ringct-key-image-domain",
                &[HashPart::Str(&observation_id)],
            ),
            oracle_attestation_root: root_from_parts(
                "devnet-ringct-decoy-entropy-floor-attestation",
                &[HashPart::Str(&observation_id)],
            ),
            expires_at_monero_height: DEVNET_MONERO_HEIGHT + DEFAULT_ORACLE_TTL_BLOCKS,
            status: EntropyFloorStatus::Observed,
        })
        .expect("devnet RingCT entropy floor observation inserts");

    for (index, band_kind, min_age, max_age, weight, contribution) in [
        (
            0_u64,
            MinimumEntropyBandKind::AncientAnchor,
            1_036_800_u64,
            2_073_600_u64,
            1_400_u64,
            1_370_u64,
        ...
        (
            1,
            MinimumEntropyBandKind::ColdHistorical,
            172_800,
            1_036_800,
            2_600,
            2_560,
        ...
        (
            2,
            MinimumEntropyBandKind::WarmRecent,
            14_400,
            172_800,
            3_100,
            3_020,
        ...
        (
            3,
            MinimumEntropyBandKind::HotRecent,
            0,
            14_400,
            1_200,
            1_150,
        ...
        (
            4,
            MinimumEntropyBandKind::LiquidityBridge,
            28_800,
            259_200,
            900,
            880,
        ...
        (
            5,
            MinimumEntropyBandKind::SwapSettlement,
            7_200,
            86_400,
            800,
            790,
        ...
        let band_id = format!("ringct-minimum-entropy-band-devnet-{index}");
        state
            .insert_minimum_entropy_band(MinimumEntropyBand {
                band_id: band_id.clone(),
                observation_id: observation_id.clone(),
                band_kind,
                min_output_age_blocks: min_age,
                max_output_age_blocks: max_age,
                minimum_entropy_millibits: DEFAULT_TARGET_MIN_ENTROPY_MILLIBITS - (index * 25_000),
                floor_contribution_bps: contribution,
                band_weight_bps: weight,
                sampled_outputs: DEFAULT_TARGET_LOW_FEE_BATCH_OUTPUTS + (index * 768),
                unique_commitment_buckets: 768 + (index * 48),
                redacted_output_age_bucket_root: root_from_parts(
                    "devnet-redacted-ringct-output-age-bucket",
                    &[HashPart::Str(&band_id)],
                ),
                ringct_decoy_sampler_root: root_from_parts(
                    "devnet-ringct-decoy-sampler",
                    &[HashPart::Str(&band_id)],
                ),
                minimum_entropy_witness_root: root_from_parts(
                    "devnet-ringct-minimum-entropy-witness",
                    &[HashPart::Str(&band_id)],
                ),
                status: EntropyFloorStatus::FloorBanded,
            })
            .expect("devnet RingCT minimum entropy band inserts");
    }
    ...
    state
        .insert_output_age_diversity_score(OutputAgeDiversityScore {
            ...
            redacted_age_histogram_root: root_from_parts(
                "devnet-redacted-ringct-output-age-histogram",
                &[HashPart::Str(&diversity_id)],
            ),
            age_diversity_proof_root: root_from_parts(
                "devnet-ringct-output-age-diversity-proof",
                &[HashPart::Str(&diversity_id)],
            ),
            residual_privacy_root: root_from_parts(
                "devnet-ringct-output-age-diversity-residual-privacy",
                &[HashPart::Str(&diversity_id)],
            ),
            status: EntropyFloorStatus::AgeDiversityScored,
        })
        .expect("devnet RingCT output age diversity score inserts");

    state
        .insert_pq_migration_safety_score(PqMigrationSafetyScore {
            safety_id: "ringct-decoy-entropy-floor-pq-safety-devnet-0".to_string(),
            observation_id: observation_id.clone(),
            ...
            ringct_to_pq_binding_root: root_from_parts(
                "devnet-ringct-to-pq-decoy-entropy-binding",
                &[HashPart::Str(&observation_id)],
            ),
            entropy_floor_guard_root: root_from_parts(
                "devnet-ringct-decoy-entropy-floor-pq-guard",
                &[HashPart::Str(&observation_id)],
            ),
            output_age_guard_root: root_from_parts(
                "devnet-ringct-output-age-diversity-pq-guard",
                &[HashPart::Str(&observation_id)],
            ),
            attestation_root: root_from_parts(
                "devnet-ringct-decoy-entropy-floor-pq-attestation",
                &[HashPart::Str(&observation_id)],
            ),
            status: EntropyFloorStatus::PqSafe,
        })
        .expect("devnet RingCT PQ migration safety inserts");

    state
        .insert_low_fee_scan_batch(LowFeeScanBatch {
            batch_id: "ringct-decoy-entropy-floor-low-fee-scan-batch-devnet-0".to_string(),
            observation_id,
            diversity_id,
            fee_asset_id: DEVNET_FEE_ASSET_ID.to_string(),
            ...
            batched_ringct_scan_root: root_from_parts(
                "devnet-ringct-batched-low-fee-wallet-scan",
                &[HashPart::Str("0")],
            ),
            scan_sponsor_receipt_root: root_from_parts(
                "devnet-ringct-low-fee-scan-sponsor-receipt",
                &[HashPart::Str("0")],
            ),
            privacy_budget_root: root_from_parts(
                "devnet-ringct-low-fee-scan-privacy-budget",
                &[HashPart::Str("0")],
            ),
            status: EntropyFloorStatus::BatchEligible,
        })
        .expect("devnet RingCT low-fee scan batch inserts");
...
fn root_from_parts(domain: &str, parts: &[HashPart<'_>]) -> String {
    domain_hash(
        &format!("RINGCT-DECOY-ENTROPY-FLOOR-ORACLE-{domain}"),
        parts,
        32,
    )
}
