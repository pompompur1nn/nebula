...
pub type Result<T> = PrivateL2PqConfidentialSlhDsaWithdrawalQuorumRolloverRuntimeResult<T>;
pub type PrivateL2PqConfidentialSlhDsaWithdrawalQuorumRolloverRuntimeResult<T> =
    std::result::Result<T, String>;
pub type Runtime = State;

pub const PRIVATE_L2_PQ_CONFIDENTIAL_SLH_DSA_WITHDRAWAL_QUORUM_ROLLOVER_RUNTIME_PROTOCOL_VERSION:
    &str = "nebula-private-l2-pq-confidential-slh-dsa-withdrawal-quorum-rollover-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_PQ_CONFIDENTIAL_SLH_DSA_WITHDRAWAL_QUORUM_ROLLOVER_RUNTIME_PROTOCOL_VERSION;
...
pub const PQ_SIGNATURE_SUITE: &str = "SLH-DSA-SHAKE-256f-withdrawal-quorum-rollover-v1";
pub const PQ_HYBRID_ATTESTATION_SUITE: &str =
    "SLH-DSA-SHAKE-256f+ML-DSA-87-rollover-attestation-v1";
pub const BOND_COMMITMENT_SCHEME: &str = "confidential-signer-bond-commitment-v1";
pub const QUARANTINE_SCHEME: &str = "legacy-slh-dsa-signer-quarantine-root-v1";
pub const CHALLENGE_SCHEME: &str = "withdrawal-quorum-rollover-challenge-window-root-v1";
pub const REBATE_SCHEME: &str = "low-fee-withdrawal-rollover-fee-credit-rebate-root-v1";
pub const REDACTION_SCHEME: &str = "operator-safe-rollover-redaction-budget-root-v1";
...
pub const DEFAULT_SIGNER_BOND_FLOOR_PICONERO: u64 = 25_000_000_000;
...
pub const DEFAULT_MAX_BONDS: usize = 4_194_304;
...
pub struct Config {
    ...
    pub signer_bond_floor_piconero: u64,
...
impl Config {
    pub fn devnet() -> Self {
        Self {
            ...
            signer_bond_floor_piconero: DEFAULT_SIGNER_BOND_FLOOR_PICONERO,
...
pub struct SlhDsaSignerSet {
    ...
    pub ml_dsa_hybrid_key_set_root: String,
...
impl SlhDsaSignerSet {
    pub fn root(&self) -> String {
        object_root("slh-dsa-signer-set", self)
    }
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "ml_dsa_hybrid_key_set_root": self.ml_dsa_hybrid_key_set_root,
...
impl SlhDsaSigner {
    pub fn root(&self) -> String {
        object_root("slh-dsa-signer", self)
    }
}
...
impl LegacySignerQuarantine {
    pub fn root(&self) -> String {
        object_root("legacy-signer-quarantine", self)
    }
}
...
pub struct RolloverReadiness {
    ...
    pub bond_coverage_piconero: u64,
    pub privacy_set_size: u64,
    pub pq_security_bits: u16,
    pub ready: bool,
}
...
pub struct State {
    ...
    pub roots: Roots,
    pub signer_sets: BTreeMap<String, SlhDsaSignerSet>,
    pub signers: BTreeMap<String, SlhDsaSigner>,
    pub withdrawal_epochs: BTreeMap<String, WithdrawalQuorumEpoch>,
    pub rollover_attestations: BTreeMap<String, RolloverAttestation>,
    pub signer_bonds: BTreeMap<String, SignerBond>,
...
impl State {
    ...
    pub fn register_signer_set(&mut self, request: RegisterSignerSetRequest) -> Result<String> {
        ...
        let signer_set_id = self.counters.allocate("slh-dsa-signer-set");
        let signer_set = SlhDsaSignerSet {
            ...
            ml_dsa_hybrid_key_set_root: request.ml_dsa_hybrid_key_set_root,
    ...
    pub fn register_signer(&mut self, request: RegisterSignerRequest) -> Result<String> {
        ...
        if request.bond_amount_piconero < self.config.signer_bond_floor_piconero {
            return Err("signer bond amount below configured floor".to_string());
        }
        if !self.signer_sets.contains_key(&request.signer_set_id) {
            return Err(format!("unknown signer set {}", request.signer_set_id));
        }

        let signer_id = self.counters.allocate("slh-dsa-signer");
        let bond_id = self.counters.allocate("signer-bond");
        ...
        self.signer_bonds.insert(bond_id, bond);
        self.counters.signers = self.counters.signers.saturating_add(1);
        self.counters.signer_bonds = self.counters.signer_bonds.saturating_add(1);
        self.refresh_roots();
        Ok(signer_id)
    }
    ...
    pub fn rollover_readiness(&self, epoch_id: &str) -> Result<RolloverReadiness> {
        ...
        let quarantined_legacy_signer_count = self
            ...
            .count() as u64;
        let bond_coverage_piconero = self
            .signer_bonds
            .values()
            .filter(|bond| {
                bond.signer_set_id == epoch.next_signer_set_id
                    && matches!(bond.status, BondStatus::Locked | BondStatus::Active)
            })
        ...
        let ready = counted_rollover_weight_bps >= epoch.rollover_quorum_bps
            ...
            && bond_coverage_piconero
                >= self
                    .config
                    .signer_bond_floor_piconero
    ...
    pub fn bridge_safety_posture(&self) -> BridgeSafetyPosture {
        ...
        let locked_bond_piconero = self
            .signer_bonds
            .values()
            .filter(|bond| bond.status == BondStatus::Locked)
            .map(|bond| bond.amount_piconero)
            .fold(0u64, u64::saturating_add);
        let slashed_bond_piconero = self
            .signer_bonds
            .values()
            .filter(|bond| bond.status == BondStatus::Slashed)
            .map(|bond| bond.amount_piconero)
            .fold(0u64, u64::saturating_add);
        BridgeSafetyPosture {
    ...
    pub fn public_record_without_state_root_with_roots(&self, roots: &Roots) -> Value {
        json!({
            ...
            "roots": {
                ...
                "signer_bond_root": roots.signer_bond_root,
            ...
            "signer_bonds": public_values(&self.signer_bonds),
    ...
    fn refresh_roots(&mut self) {
        ...
        self.roots.signer_bond_root = map_root("signer-bonds", &self.signer_bonds);
        ...
        self.roots.safety_root = stable_root(
            ...
            &json!({
                "bonds": self.roots.signer_bond_root,
...
pub fn devnet() -> State {
    ...
    let active_set = state
        .register_signer_set(RegisterSignerSetRequest {
            ...
            slh_dsa_public_key_set_root: sample_root("active-slh-dsa-key-set"),
            ml_dsa_hybrid_key_set_root: sample_root("active-ml-dsa-key-set"),
    ...
    let next_set = state
        .register_signer_set(RegisterSignerSetRequest {
            ...
            signer_count: 19,
            active_weight_bps: MAX_BPS,
            slh_dsa_public_key_set_root: sample_root("next-slh-dsa-key-set"),
            ml_dsa_hybrid_key_set_root: sample_root("next-ml-dsa-key-set"),
    ...
    for index in 0..6 {
        let signer_id = state
            .register_signer(RegisterSignerRequest {
                ...
                bond_commitment_root: sample_root(&format!("active-bond-{index}")),
                reserve_proof_root: sample_root(&format!("active-reserve-{index}")),
                bond_amount_piconero: DEFAULT_SIGNER_BOND_FLOOR_PICONERO + (index * 1_000_000),
    ...
    for index in 0..6 {
        let signer_id = state
            .register_signer(RegisterSignerRequest {
                ...
                bond_commitment_root: sample_root(&format!("next-bond-{index}")),
...
pub fn demo() -> State {
    ...
    let _ = state.open_challenge(OpenChallengeRequest {
        ...
        slash_amount_piconero: DEFAULT_SIGNER_BOND_FLOOR_PICONERO / 10,
    });
    state
}
