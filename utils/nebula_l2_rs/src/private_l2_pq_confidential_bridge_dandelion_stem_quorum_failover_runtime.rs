...
pub type PrivateL2PqConfidentialBridgeDandelionStemQuorumFailoverRuntimeResult<T> = Result<T>;
pub type Runtime = State;

pub const PROTOCOL_VERSION: &str =
    "nebula-private-l2-pq-confidential-bridge-dandelion-stem-quorum-failover-runtime-v1";
pub const PRIVATE_L2_PQ_CONFIDENTIAL_BRIDGE_DANDELION_STEM_QUORUM_FAILOVER_RUNTIME_PROTOCOL_VERSION:
    &str = PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const PQ_FIREWALL_SUITE: &str = "ML-DSA-87+ML-KEM-1024+SLH-DSA-SHAKE-256f";
pub const FAILOVER_SUITE: &str = "monero-bridge-dandelion-stem-quorum-failover-root-v1";
pub const REBATE_SUITE: &str = "dandelion-stem-quorum-failover-low-fee-rebate-root-v1";
pub const REDACTION_SUITE: &str = "operator-safe-dandelion-stem-quorum-failover-redaction-root-v1";
...
pub const DEFAULT_FAILOVER_WINDOW_SLOTS: u64 = 720;
...
impl StemQuorumFailoverScope {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::EmergencyStemFailover => "emergency_stem_quorum_failover",
...
#[serde(rename_all = "snake_case")]
pub enum SignalKind {
    StemQuorumDropPressure,
    FailoverLiquidityFragmentation,
    StemRelaySkew,
    FluffFailoverLinkagePressure,
    StemTimingQuorumPressure,
    ReserveMismatch,
    FeeSpikeFailoverFailure,
    WatchtowerEmergency,
}
...
pub enum AttestationVerdict {
    ConfirmStemQuorumFailoverNeed,
    ConfirmPrivacyRisk,
    RequireDandelionStemRotation,
    RequireStemQuorumFailover,
    RequireQuarantine,
    ClearFalsePositive,
}
...
#[derive(Clone, Copy, Debug, Deserialize, Eq, Ord, PartialEq, PartialOrd, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum StemQuorumFailoverAction {
    Observe,
    QueueStemFailover,
    ApplyStemQuorumFailover,
    RotateDandelionStemCohort,
    QuarantineRoute,
    EmergencyPause,
    Release,
    Reject,
...
pub struct Config {
    ...
    pub failover_window_slots: u64,
    pub signal_window_slots: u64,
    pub max_route_fee_bps: u64,
    pub target_rebate_bps: u64,
    pub min_stem_stem_quorum_failover_liquidity_micro_units: u64,
    pub min_fluff_stem_quorum_failover_liquidity_micro_units: u64,
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            failover_suite: FAILOVER_SUITE.to_string(),
            ...
            failover_window_slots: DEFAULT_FAILOVER_WINDOW_SLOTS,
            signal_window_slots: DEFAULT_SIGNAL_WINDOW_SLOTS,
            max_route_fee_bps: DEFAULT_MAX_ROUTE_FEE_BPS,
            target_rebate_bps: DEFAULT_TARGET_REBATE_BPS,
            min_stem_stem_quorum_failover_liquidity_micro_units:
                DEFAULT_MIN_SOURCE_LIQUIDITY_MICRO_UNITS,
            min_fluff_stem_quorum_failover_liquidity_micro_units:
...
pub struct Counters {
    ...
    pub total_failover_routed_micro_units: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct StemQuorumFailoverPolicy {
    pub policy_id: String,
    pub scope: StemQuorumFailoverScope,
    pub owner_commitment: String,
    pub policy_commitment_root: String,
    pub active_stem_relay_set_root: String,
    pub fallback_stem_relay_set_root: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct DandelionStemQuorumFailoverCohort {
    ...
    pub quorum_failover_commitment_root: String,
    pub stem_stem_quorum_failover_liquidity_micro_units: u64,
    pub fluff_stem_quorum_failover_liquidity_micro_units: u64,
    pub privacy_set_size: u64,
    pub route_fee_bps: u64,
    pub active_signal_count: u64,
    pub failover_deadline_slot: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct StemQuorumFailoverSignal {
    ...
    pub stem_quorum_pressure_root: String,
    pub reporter_commitment: String,
    pub stem_stem_quorum_failover_liquidity_delta_micro_units: i128,
    pub fluff_stem_quorum_failover_liquidity_delta_micro_units: i128,
    pub privacy_set_size: u64,
    pub quorum_failover_risk_score_bps: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct PqStemQuorumFailoverAttestation {
    ...
    pub observed_quorum_failover_risk_score_bps: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct StemQuorumFailoverDecision {
    pub decision_id: String,
    pub signal_id: String,
    pub cohort_id: String,
    pub policy_id: String,
    pub action: StemQuorumFailoverAction,
    pub decision_root: String,
    pub replacement_route_root: String,
    pub failover_routed_micro_units: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct LowFeeStemQuorumFailoverRebate {
...
pub struct State {
    ...
    pub roots: Roots,
    pub policies: BTreeMap<String, StemQuorumFailoverPolicy>,
    pub cohorts: BTreeMap<String, DandelionStemQuorumFailoverCohort>,
    pub signals: BTreeMap<String, StemQuorumFailoverSignal>,
    pub attestations: BTreeMap<String, PqStemQuorumFailoverAttestation>,
    pub decisions: BTreeMap<String, StemQuorumFailoverDecision>,
    pub rebates: BTreeMap<String, LowFeeStemQuorumFailoverRebate>,
...
impl State {
    ...
    pub fn devnet() -> Self {
        ...
        let withdrawal_policy = state
            .register_policy(RegisterPolicyRequest {
                scope: StemQuorumFailoverScope::WithdrawalSubaddressLane,
                owner_commitment: "owner:dandelion-stem-quorum-failover-council-a".to_string(),
                policy_commitment_root: devnet_root(
                    "policy",
                    "withdrawal-dandelion-stem-quorum-failover",
                ),
                active_stem_relay_set_root: devnet_root(
                    "active-stem-relay-set",
                    "withdrawal-alpha",
                ),
                fallback_stem_relay_set_root: devnet_root(
                    "fallback-stem-relay-set",
                    "withdrawal-alpha",
                ),
                route_allowlist_root: devnet_root("route-allowlist", "withdrawal-alpha"),
                min_privacy_set_size: 524_288,
                min_stem_stem_quorum_failover_liquidity_micro_units: 160_000_000,
                min_fluff_stem_quorum_failover_liquidity_micro_units: 72_000_000,
                max_route_fee_bps: 16,
                min_attestation_quorum_bps: 7_200,
                signal_window_slots: 96,
                failover_window_slots: 720,
                redaction_profile: "withdrawal-dandelion-stem-quorum-failover-profile".to_string(),
            })
            .expect("devnet withdrawal policy");
        let fast_dandelion_exit_policy = state
            .register_policy(RegisterPolicyRequest {
                scope: StemQuorumFailoverScope::FastDandelionStemFailoverLane,
                owner_commitment: "owner:fast-dandelion-stem-failover-route-council-b".to_string(),
                policy_commitment_root: devnet_root(
                    "policy",
                    "fast-dandelion-stem-failover-dandelion-stem-quorum-failover",
                ),
                active_stem_relay_set_root: devnet_root(
                    "active-stem-relay-set",
                    "fast-dandelion-stem-failover-beta",
                ),
                fallback_stem_relay_set_root: devnet_root(
                    "fallback-stem-relay-set",
                    "fast-dandelion-stem-failover-beta",
                ),
                route_allowlist_root: devnet_root(
                    "route-allowlist",
                    "fast-dandelion-stem-failover-beta",
                ),
                min_privacy_set_size: 393_216,
                min_stem_stem_quorum_failover_liquidity_micro_units: 144_000_000,
                min_fluff_stem_quorum_failover_liquidity_micro_units: 64_000_000,
                max_route_fee_bps: 18,
                min_attestation_quorum_bps: 7_000,
                signal_window_slots: 80,
                failover_window_slots: 640,
                redaction_profile:
                    "fast-dandelion-stem-failover-dandelion-stem-quorum-failover-profile"
                        .to_string(),
            })
            .expect("devnet fast stem_quorum_failover policy");
        let withdrawal_cohort = state
            .register_cohort(RegisterCohortRequest {
                policy_id: withdrawal_policy,
                bridge_lane_id: "withdrawal-stem-quorum-alpha".to_string(),
                active_stem_relay_set_root: devnet_root("active-stem-relay-set", "cohort-alpha"),
                fallback_stem_relay_set_root: devnet_root(
                    "fallback-stem-relay-set",
                    "cohort-alpha",
                ),
                output_set_root: devnet_root("outputs", "cohort-alpha"),
                quorum_failover_commitment_root: devnet_root("liquidity", "cohort-alpha"),
                stem_stem_quorum_failover_liquidity_micro_units: 216_000_000,
                fluff_stem_quorum_failover_liquidity_micro_units: 96_000_000,
        ...
        let fast_dandelion_exit_cohort = state
            .register_cohort(RegisterCohortRequest {
                ...
                bridge_lane_id: "fast-dandelion-stem-failover-route-beta".to_string(),
                active_stem_relay_set_root: devnet_root("active-stem-relay-set", "cohort-beta"),
                fallback_stem_relay_set_root: devnet_root("fallback-stem-relay-set", "cohort-beta"),
                output_set_root: devnet_root("outputs", "cohort-beta"),
                quorum_failover_commitment_root: devnet_root("liquidity", "cohort-beta"),
                stem_stem_quorum_failover_liquidity_micro_units: 168_000_000,
                fluff_stem_quorum_failover_liquidity_micro_units: 76_000_000,
                privacy_set_size: 393_216,
                route_fee_bps: 13,
                public_hint_root: devnet_root("hint", "cohort-beta"),
            })
            .expect("devnet fast stem_quorum_failover cohort");
        let signal = state
            .submit_signal(SubmitSignalRequest {
                cohort_id: withdrawal_cohort,
                kind: SignalKind::StemQuorumDropPressure,
                ...
                stem_quorum_pressure_root: devnet_root("pressure", "withdrawal-cluster-pressure"),
                reporter_commitment: "reporter:dandelion-route-watchtower-11".to_string(),
                stem_stem_quorum_failover_liquidity_delta_micro_units: -48_000_000,
                fluff_stem_quorum_failover_liquidity_delta_micro_units: 24_000_000,
                privacy_set_size: 524_288,
                quorum_failover_risk_score_bps: 7_300,
        ...
        state
            .submit_attestation(SubmitAttestationRequest {
                ...
                verdict: AttestationVerdict::RequireStemQuorumFailover,
                attestor_commitment: "attestor:pq-dandelion-stem-quorum-failover-a".to_string(),
                pq_signature_root: devnet_root(
                    "ml-dsa-signature",
                    "withdrawal-stem_quorum_failover-a",
                ),
                pq_public_key_root: devnet_root(
                    "ml-kem-public-key",
                    "withdrawal-stem_quorum_failover-a",
                ),
                transcript_root: devnet_root("transcript", "withdrawal-stem_quorum_failover-a"),
                quorum_weight_bps: 7_500,
                pq_security_bits: 256,
                observed_quorum_failover_risk_score_bps: 7_500,
        ...
        state
            .submit_attestation(SubmitAttestationRequest {
                signal_id: signal.clone(),
                verdict: AttestationVerdict::ConfirmStemQuorumFailoverNeed,
                attestor_commitment: "attestor:pq-dandelion-stem-quorum-failover-b".to_string(),
                pq_signature_root: devnet_root(
                    "slh-dsa-signature",
                    "withdrawal-stem_quorum_failover-b",
                ),
                pq_public_key_root: devnet_root(
                    "ml-kem-public-key",
                    "withdrawal-stem_quorum_failover-b",
                ),
                transcript_root: devnet_root("transcript", "withdrawal-stem_quorum_failover-b"),
                quorum_weight_bps: 8_700,
                pq_security_bits: 256,
                observed_quorum_failover_risk_score_bps: 7_800,
                observed_privacy_set_size: 524_288,
        ...
        let decision = state
            .settle_decision(SettleDecisionRequest {
                signal_id: signal,
                action: StemQuorumFailoverAction::ApplyStemQuorumFailover,
                decision_root: devnet_root("decision", "withdrawal-liquidity-stem_quorum_failover"),
                replacement_route_root: devnet_root(
                    "replacement-route",
                    "withdrawal-stem_quorum_failover",
                ),
                failover_routed_micro_units: 32_000_000,
                shielded_output_count: 4_096,
                rebate_bps: 8,
                decided_at_slot: DEVNET_SLOT + 5,
            })
            .expect("devnet stem_quorum_failover decision");
        state
            .publish_rebate(PublishRebateRequest {
                decision_id: decision,
                recipient_commitment: "recipient:dandelion-stem-quorum-failover-fee-sponsor"
                    .to_string(),
                rebate_micro_units: 22_000,
                fee_saved_micro_units: 88_000,
                rebate_bps: 8,
                settlement_slot: DEVNET_SLOT + 6,
                rebate_root: devnet_root("rebate", "withdrawal-stem_quorum_failover"),
            })
    ...
    pub fn register_policy(&mut self, request: RegisterPolicyRequest) -> Result<String> {
        ensure_capacity(
            self.policies.len(),
            MAX_POLICIES,
            "Dandelion stem quorum failover policies",
        )?;
        ...
        ensure_non_empty(
            &request.active_stem_relay_set_root,
            "active_stem_relay_set_root",
        )?;
        ensure_non_empty(
            &request.fallback_stem_relay_set_root,
            "fallback_stem_relay_set_root",
        )?;
        ...
        if request.min_stem_stem_quorum_failover_liquidity_micro_units
            < self
                .config
                .min_stem_stem_quorum_failover_liquidity_micro_units
        {
        ...
        if request.min_fluff_stem_quorum_failover_liquidity_micro_units
            < self
                .config
                .min_fluff_stem_quorum_failover_liquidity_micro_units
        {
        ...
        let policy_id = stable_id(
            "dandelion-stem-quorum-failover-policy",
            &[
                HashPart::Str(request.scope.as_str()),
                HashPart::Str(&request.owner_commitment),
                HashPart::Str(&request.policy_commitment_root),
                HashPart::Str(&request.active_stem_relay_set_root),
            ],
        );
        if self.policies.contains_key(&policy_id) {
            return Err(format!(
                "Dandelion stem quorum failover policy {policy_id} already exists"
            ));
        }
        self.policies.insert(
            policy_id.clone(),
            StemQuorumFailoverPolicy {
                policy_id: policy_id.clone(),
                scope: request.scope,
                owner_commitment: request.owner_commitment,
                policy_commitment_root: request.policy_commitment_root,
                active_stem_relay_set_root: request.active_stem_relay_set_root,
                fallback_stem_relay_set_root: request.fallback_stem_relay_set_root,
                route_allowlist_root: request.route_allowlist_root,
                min_privacy_set_size: request.min_privacy_set_size,
                min_stem_stem_quorum_failover_liquidity_micro_units: request
                    .min_stem_stem_quorum_failover_liquidity_micro_units,
                min_fluff_stem_quorum_failover_liquidity_micro_units: request
                    .min_fluff_stem_quorum_failover_liquidity_micro_units,
                ...
                failover_window_slots: request.failover_window_slots,
                active: true,
    ...
    pub fn register_cohort(&mut self, request: RegisterCohortRequest) -> Result<String> {
        ...
        let policy = self.policies.get(&request.policy_id).ok_or_else(|| {
            format!(
                "unknown Dandelion stem quorum failover policy {}",
                request.policy_id
            )
        })?;
        ...
        ensure_non_empty(
            &request.quorum_failover_commitment_root,
            "quorum_failover_commitment_root",
        )?;
        ...
        if request.stem_stem_quorum_failover_liquidity_micro_units
            < policy.min_stem_stem_quorum_failover_liquidity_micro_units
        {
            return Err("cohort source liquidity below policy floor".to_string());
        }
        if request.fluff_stem_quorum_failover_liquidity_micro_units
            < policy.min_fluff_stem_quorum_failover_liquidity_micro_units
        {
        ...
        let cohort_id = stable_id(
            "dandelion-stem-quorum-failover-cohort",
            &[
                HashPart::Str(&request.policy_id),
                HashPart::Str(&request.bridge_lane_id),
                HashPart::Str(&request.active_stem_relay_set_root),
                HashPart::Str(&request.quorum_failover_commitment_root),
            ],
        );
        if self.cohorts.contains_key(&cohort_id) {
            return Err(format!(
                "Dandelion stem quorum failover cohort {cohort_id} already exists"
            ));
        }
        self.cohorts.insert(
            cohort_id.clone(),
            DandelionStemQuorumFailoverCohort {
                ...
                quorum_failover_commitment_root: request.quorum_failover_commitment_root,
                stem_stem_quorum_failover_liquidity_micro_units: request
                    .stem_stem_quorum_failover_liquidity_micro_units,
                fluff_stem_quorum_failover_liquidity_micro_units: request
                    .fluff_stem_quorum_failover_liquidity_micro_units,
                privacy_set_size: request.privacy_set_size,
                route_fee_bps: request.route_fee_bps,
                active_signal_count: 0,
                failover_deadline_slot: 0,
    ...
    pub fn submit_signal(&mut self, request: SubmitSignalRequest) -> Result<String> {
        ensure_capacity(
            self.signals.len(),
            MAX_SIGNALS,
            "Dandelion stem quorum failover signals",
        )?;
        let cohort = self
            .cohorts
            ...
            .ok_or_else(|| {
                format!(
                    "unknown Dandelion stem quorum failover cohort {}",
                    request.cohort_id
                )
            })?
            .clone();
        ...
        ensure_non_empty(
            &request.stem_quorum_pressure_root,
            "stem_quorum_pressure_root",
        )?;
        ensure_non_empty(&request.reporter_commitment, "reporter_commitment")?;
        ensure_bps(
            request.quorum_failover_risk_score_bps,
            "quorum_failover_risk_score_bps",
        )?;
        if request.privacy_set_size < policy.min_privacy_set_size {
            return Err("signal privacy set below policy floor".to_string());
        }
        let signal_id = stable_id(
            "dandelion-stem-quorum-failover-signal",
            &[
        ...
        if self.signals.contains_key(&signal_id) {
            return Err(format!(
                "Dandelion stem quorum failover signal {signal_id} already exists"
            ));
        }
        ...
        self.signals.insert(
            ...
            StemQuorumFailoverSignal {
                ...
                stem_quorum_pressure_root: request.stem_quorum_pressure_root,
                reporter_commitment: request.reporter_commitment,
                stem_stem_quorum_failover_liquidity_delta_micro_units: request
                    .stem_stem_quorum_failover_liquidity_delta_micro_units,
                fluff_stem_quorum_failover_liquidity_delta_micro_units: request
                    .fluff_stem_quorum_failover_liquidity_delta_micro_units,
                privacy_set_size: request.privacy_set_size,
                quorum_failover_risk_score_bps: request.quorum_failover_risk_score_bps,
    ...
    pub fn submit_attestation(&mut self, request: SubmitAttestationRequest) -> Result<String> {
        ensure_capacity(
            ...
            MAX_ATTESTATIONS,
            "Dandelion stem quorum failover attestations",
        )?;
        let signal = self
            .signals
            ...
            .ok_or_else(|| {
                format!(
                    "unknown Dandelion stem quorum failover signal {}",
                    request.signal_id
                )
            })?
            .clone();
        ...
        ensure_bps(
            request.observed_quorum_failover_risk_score_bps,
            "observed_quorum_failover_risk_score_bps",
        )?;
        ...
        let attestation_id = stable_id(
            "dandelion-stem-quorum-failover-pq-attestation",
            &[
        ...
        if self.attestations.contains_key(&attestation_id) {
            return Err(format!(
                "Dandelion stem quorum failover attestation {attestation_id} already exists"
            ));
        }
        self.attestations.insert(
            attestation_id.clone(),
            PqStemQuorumFailoverAttestation {
                ...
                observed_quorum_failover_risk_score_bps: request
                    .observed_quorum_failover_risk_score_bps,
    ...
    pub fn settle_decision(&mut self, request: SettleDecisionRequest) -> Result<String> {
        ensure_capacity(
            self.decisions.len(),
            MAX_DECISIONS,
            "Dandelion stem quorum failover decisions",
        )?;
        let signal = self
            .signals
            .get(&request.signal_id)
        ...
        let failover_deadline_slot = match request.action {
            StemQuorumFailoverAction::QueueStemFailover
            | StemQuorumFailoverAction::ApplyStemQuorumFailover
            | StemQuorumFailoverAction::RotateDandelionStemCohort => request
                .decided_at_slot
                .saturating_add(policy.failover_window_slots),
            StemQuorumFailoverAction::QuarantineRoute
            | StemQuorumFailoverAction::EmergencyPause => request
                .decided_at_slot
                .saturating_add(policy.failover_window_slots / 2),
            _ => 0,
        };
        let decision_id = stable_id(
            "dandelion-stem-quorum-failover-decision",
            &[
                HashPart::Str(&request.signal_id),
        ...
        if self.decisions.contains_key(&decision_id) {
            return Err(format!(
                "Dandelion stem quorum failover decision {decision_id} already exists"
            ));
        }
        self.decisions.insert(
            decision_id.clone(),
            StemQuorumFailoverDecision {
                ...
                failover_routed_micro_units: request.failover_routed_micro_units,
                shielded_output_count: request.shielded_output_count,
                failover_deadline_slot,
        ...
        if let Some(signal) = self.signals.get_mut(&request.signal_id) {
            signal.status = match request.action {
                StemQuorumFailoverAction::QueueStemFailover => SignalStatus::RouteQueued,
                StemQuorumFailoverAction::ApplyStemQuorumFailover
                | StemQuorumFailoverAction::RotateDandelionStemCohort => SignalStatus::Routing,
                StemQuorumFailoverAction::QuarantineRoute
                | StemQuorumFailoverAction::EmergencyPause => SignalStatus::Quarantined,
                StemQuorumFailoverAction::Release => SignalStatus::Settled,
                StemQuorumFailoverAction::Reject => SignalStatus::Rejected,
                StemQuorumFailoverAction::Expire => SignalStatus::Expired,
                StemQuorumFailoverAction::Observe => SignalStatus::Attested,
            };
        }
        if let Some(cohort) = self.cohorts.get_mut(&signal.cohort_id) {
            match request.action {
                StemQuorumFailoverAction::QueueStemFailover => {
                    ...
                    cohort.failover_deadline_slot = failover_deadline_slot;
                }
                StemQuorumFailoverAction::ApplyStemQuorumFailover => {
                    cohort.status = CohortStatus::Routing;
                    cohort.stem_stem_quorum_failover_liquidity_micro_units = cohort
                        .stem_stem_quorum_failover_liquidity_micro_units
                        .saturating_sub(request.failover_routed_micro_units);
                    cohort.fluff_stem_quorum_failover_liquidity_micro_units = cohort
                        .fluff_stem_quorum_failover_liquidity_micro_units
                        .saturating_add(request.failover_routed_micro_units);
                    cohort.failover_deadline_slot = failover_deadline_slot;
                    self.counters.failovers_settled =
                        self.counters.failovers_settled.saturating_add(1);
                    self.counters.total_failover_routed_micro_units = self
                        .counters
                        .total_failover_routed_micro_units
                        .saturating_add(request.failover_routed_micro_units);
                }
                StemQuorumFailoverAction::RotateDandelionStemCohort => {
                ...
                StemQuorumFailoverAction::EmergencyPause => {
                    cohort.status = CohortStatus::Paused;
                    cohort.failover_deadline_slot = failover_deadline_slot;
                    self.counters.routes_quarantined =
                        self.counters.routes_quarantined.saturating_add(1);
                }
                StemQuorumFailoverAction::Release => {
                    cohort.status = CohortStatus::Settled;
                    cohort.failover_deadline_slot = 0;
    ...
    pub fn publish_rebate(&mut self, request: PublishRebateRequest) -> Result<String> {
        ensure_capacity(
            self.rebates.len(),
            MAX_REBATES,
            "Dandelion stem quorum failover rebates",
        )?;
        let decision = self.decisions.get(&request.decision_id).ok_or_else(|| {
            format!(
                "unknown Dandelion stem quorum failover decision {}",
                request.decision_id
            )
        })?;
        ...
        let rebate_id = stable_id(
            "dandelion-stem-quorum-failover-low-fee-rebate",
            &[
        ...
        if self.rebates.contains_key(&rebate_id) {
            return Err(format!(
                "Dandelion stem quorum failover rebate {rebate_id} already exists"
            ));
        }
        self.rebates.insert(
            rebate_id.clone(),
            LowFeeStemQuorumFailoverRebate {
    ...
    pub fn publish_redaction_budget(
        ...
        ensure_capacity(
            ...
            "Dandelion stem quorum failover redaction budgets",
        )?;
        ...
        let budget_id = stable_id(
            "dandelion-stem-quorum-failover-redaction-budget",
            &[
                HashPart::Str(request.scope.as_str()),
                HashPart::Str(&request.cohort_id),
        ...
        if self.redaction_budgets.contains_key(&budget_id) {
            return Err(format!(
                "Dandelion stem quorum failover redaction budget {budget_id} already exists"
            ));
        }
    ...
    pub fn publish_operator_summary(
        ...
        ensure_capacity(
            ...
            "Dandelion stem quorum failover operator summaries",
        )?;
        self.refresh_live_counts();
        ...
        let summary_id = stable_id(
            "dandelion-stem-quorum-failover-operator-summary",
            &[
                HashPart::U64(epoch),
                HashPart::U64(slot),
        ...
        self.operator_summaries.insert(
            ...
            OperatorSummary {
                ...
                total_failover_routed_micro_units: self.counters.total_failover_routed_micro_units,
    ...
    fn refresh_roots(&mut self) {
        ...
        self.roots.state_root = merkle_root(
            "bridge-dandelion-stem-quorum-failover:state",
            &[
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "failover_suite": self.config.failover_suite,
            ...
            "failover_window_slots": self.config.failover_window_slots,
            ...
            "policies": self.policies.values().map(|policy| json!({
                ...
                "min_stem_stem_quorum_failover_liquidity_micro_units": policy.min_stem_stem_quorum_failover_liquidity_micro_units,
                "min_fluff_stem_quorum_failover_liquidity_micro_units": policy.min_fluff_stem_quorum_failover_liquidity_micro_units,
            ...
            "cohorts": self.cohorts.values().map(|cohort| json!({
                ...
                "stem_stem_quorum_failover_liquidity_micro_units": cohort.stem_stem_quorum_failover_liquidity_micro_units,
                "fluff_stem_quorum_failover_liquidity_micro_units": cohort.fluff_stem_quorum_failover_liquidity_micro_units,
                "privacy_set_size": cohort.privacy_set_size,
                "route_fee_bps": cohort.route_fee_bps,
                "active_signal_count": cohort.active_signal_count,
                "failover_deadline_slot": cohort.failover_deadline_slot,
            ...
            "signals": self.signals.values().map(|signal| json!({
                ...
                "quorum_failover_risk_score_bps": signal.quorum_failover_risk_score_bps,
            ...
            "decisions": self.decisions.values().map(|decision| json!({
                ...
                "failover_routed_micro_units": decision.failover_routed_micro_units,
                "shielded_output_count": decision.shielded_output_count,
                "failover_deadline_slot": decision.failover_deadline_slot,
...
pub fn demo() -> State {
    ...
    let signal = state
        .submit_signal(SubmitSignalRequest {
            cohort_id,
            kind: SignalKind::FluffFailoverLinkagePressure,
            ...
            stem_quorum_pressure_root: devnet_root("pressure", "demo-viewtag-pressure"),
            reporter_commitment: "reporter:demo-subaddress-watchtower".to_string(),
            stem_stem_quorum_failover_liquidity_delta_micro_units: -10_000_000,
            fluff_stem_quorum_failover_liquidity_delta_micro_units: 8_000_000,
            privacy_set_size: 524_288,
            quorum_failover_risk_score_bps: 6_200,
    ...
    state
        .submit_attestation(SubmitAttestationRequest {
            signal_id: signal,
            verdict: AttestationVerdict::RequireDandelionStemRotation,
            attestor_commitment: "attestor:demo-dandelion-stem-quorum-failover".to_string(),
            ...
            pq_security_bits: 256,
            observed_quorum_failover_risk_score_bps: 6_400,
            observed_privacy_set_size: 524_288,
...
fn object_root<T: Serialize>(domain: &str, value: &T) -> String {
    ...
    domain_hash(
        &format!("bridge-dandelion-stem-quorum-failover:{domain}"),
        &[HashPart::Json(&value)],
        32,
    )
}
...
fn devnet_root(domain: &str, label: &str) -> String {
    domain_hash(
        &format!("bridge-dandelion-stem-quorum-failover:devnet:{domain}"),
