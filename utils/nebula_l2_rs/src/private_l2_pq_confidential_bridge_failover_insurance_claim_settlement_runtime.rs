...
pub type PrivateL2PqConfidentialBridgeFailoverInsuranceClaimSettlementRuntimeResult<T> = Result<T>;
pub type Runtime = State;

pub const PROTOCOL_VERSION: &str =
    "nebula-private-l2-pq-confidential-bridge-failover-insurance-claim-settlement-runtime-v1";
pub const PRIVATE_L2_PQ_CONFIDENTIAL_BRIDGE_FAILOVER_INSURANCE_CLAIM_SETTLEMENT_RUNTIME_PROTOCOL_VERSION:
    &str = PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const PQ_CLAIM_SETTLEMENT_INSURANCE_SUITE: &str = "ML-DSA-87+ML-KEM-1024+SLH-DSA-SHAKE-256f";
pub const FAILOVER_INSURANCE_CLAIM_SETTLEMENT_SUITE: &str =
    "monero-bridge-failover-insurance-claim-settlement-root-v1";
pub const REBATE_SUITE: &str = "failover-insurance-claim-settlement-low-fee-rebate-root-v1";
pub const REDACTION_SUITE: &str =
    "operator-safe-failover-insurance-claim-settlement-redaction-root-v1";
...
pub const DEFAULT_FAILOVER_INSURANCE_CLAIM_WINDOW_SLOTS: u64 = 720;
...
pub enum FailoverInsuranceClaimScope {
    DepositSubaddressLane,
    WithdrawalSubaddressLane,
    FastDandelionFailoverClaimLane,
    MerchantPaymentSubaddressLane,
    AtomicSwapSubaddressLane,
    ReserveMirror,
    EmergencyFailoverInsuranceClaimSettlement,
    WatchtowerRecovery,
}

impl FailoverInsuranceClaimScope {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::DepositSubaddressLane => "deposit_subaddress_lane",
            Self::WithdrawalSubaddressLane => "withdrawal_subaddress_lane",
            Self::FastDandelionFailoverClaimLane => "fast_dandelion_failover_claim_lane",
            ...
            Self::EmergencyFailoverInsuranceClaimSettlement => {
                "emergency_failover_insurance_claim_settlement"
            }
...
pub enum AttestationVerdict {
    ConfirmFailoverInsuranceClaimPressure,
    ConfirmPrivacyRisk,
    RequireFailoverInsuranceClaimCoverage,
    RequireFailoverInsuranceClaimCoverageGuard,
    RequireQuarantine,
    ClearFalsePositive,
}
...
#[derive(Clone, Copy, Debug, Deserialize, Eq, Ord, PartialEq, PartialOrd, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum FailoverInsuranceClaimAction {
    Observe,
    QueueFailoverClaimReview,
    SettleFailoverInsuranceClaim,
    RotateFailoverClaimClaimSettlementCohort,
    QuarantineRoute,
    EmergencyPause,
    Release,
    Reject,
...
pub struct Config {
    ...
    pub failover_insurance_claim_settlement_suite: String,
    ...
    pub failover_insurance_claim_window_slots: u64,
    pub signal_window_slots: u64,
    pub max_claim_settlement_fee_bps: u64,
    pub target_rebate_bps: u64,
    pub min_stem_lane_claim_reserve_micro_units: u64,
    pub min_fluff_lane_claim_reserve_micro_units: u64,
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            pq_claim_settlement_insurance_suite: PQ_CLAIM_SETTLEMENT_INSURANCE_SUITE.to_string(),
            failover_insurance_claim_settlement_suite: FAILOVER_INSURANCE_CLAIM_SETTLEMENT_SUITE
                .to_string(),
            ...
            failover_insurance_claim_window_slots: DEFAULT_FAILOVER_INSURANCE_CLAIM_WINDOW_SLOTS,
            signal_window_slots: DEFAULT_SIGNAL_WINDOW_SLOTS,
            max_claim_settlement_fee_bps: DEFAULT_MAX_ROUTE_FEE_BPS,
            target_rebate_bps: DEFAULT_TARGET_REBATE_BPS,
            min_stem_lane_claim_reserve_micro_units: DEFAULT_MIN_SOURCE_LIQUIDITY_MICRO_UNITS,
            min_fluff_lane_claim_reserve_micro_units: DEFAULT_MIN_TARGET_LIQUIDITY_MICRO_UNITS,
...
pub struct Counters {
    ...
    pub claim_routes_quarantined: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct FailoverInsuranceClaimPolicy {
    pub policy_id: String,
    pub scope: FailoverInsuranceClaimScope,
    pub owner_commitment: String,
    pub policy_commitment_root: String,
    pub claim_source_relay_set_root: String,
    pub claim_target_relay_set_root: String,
    pub claim_route_allowlist_root: String,
    pub min_privacy_set_size: u64,
    pub min_stem_lane_claim_reserve_micro_units: u64,
    pub min_fluff_lane_claim_reserve_micro_units: u64,
    pub max_claim_settlement_fee_bps: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct FailoverInsuranceClaimSettlementCohort {
    ...
    pub stem_lane_claim_reserve_micro_units: u64,
    pub fluff_lane_claim_reserve_micro_units: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct FailoverInsuranceClaimSignal {
    ...
    pub observed_claim_route_root: String,
    pub fluff_linkage_pressure_root: String,
    pub reporter_commitment: String,
    pub stem_lane_claim_reserve_delta_micro_units: i128,
    pub fluff_lane_claim_reserve_delta_micro_units: i128,
    pub privacy_set_size: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct PqFailoverClaimSettlementAttestation {
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct FailoverInsuranceClaimDecision {
    pub decision_id: String,
    pub signal_id: String,
    pub cohort_id: String,
    pub policy_id: String,
    pub action: FailoverInsuranceClaimAction,
    pub decision_root: String,
    pub replacement_claim_route_root: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct LowFeeFailoverClaimSettlementRebate {
...
pub struct State {
    ...
    pub roots: Roots,
    pub policies: BTreeMap<String, FailoverInsuranceClaimPolicy>,
    pub cohorts: BTreeMap<String, FailoverInsuranceClaimSettlementCohort>,
    pub signals: BTreeMap<String, FailoverInsuranceClaimSignal>,
    pub attestations: BTreeMap<String, PqFailoverClaimSettlementAttestation>,
    pub decisions: BTreeMap<String, FailoverInsuranceClaimDecision>,
    pub rebates: BTreeMap<String, LowFeeFailoverClaimSettlementRebate>,
...
impl State {
    ...
    pub fn devnet() -> Self {
        ...
        let withdrawal_policy = state
            .register_policy(RegisterPolicyRequest {
                scope: FailoverInsuranceClaimScope::WithdrawalSubaddressLane,
                owner_commitment: "owner:dandelion-failover-insurance-claim-settlement-council-a"
                    .to_string(),
                policy_commitment_root: devnet_root(
                    "policy",
                    "withdrawal-dandelion-failover-insurance-claim-settlement",
                ),
                claim_source_relay_set_root: devnet_root(
                    "source-fluff-relay-set",
                    "withdrawal-alpha",
                ),
                claim_target_relay_set_root: devnet_root(
                    "target-fluff-relay-set",
                    "withdrawal-alpha",
                ),
                claim_route_allowlist_root: devnet_root("route-allowlist", "withdrawal-alpha"),
                min_privacy_set_size: 524_288,
                min_stem_lane_claim_reserve_micro_units: 160_000_000,
                min_fluff_lane_claim_reserve_micro_units: 72_000_000,
                max_claim_settlement_fee_bps: 16,
                min_attestation_quorum_bps: 7_200,
                signal_window_slots: 96,
                failover_insurance_claim_window_slots: 720,
                redaction_profile:
                    "withdrawal-dandelion-failover-insurance-claim-settlement-profile".to_string(),
            })
            .expect("devnet withdrawal policy");
        let fast_dandelion_exit_policy = state
            .register_policy(RegisterPolicyRequest {
                scope: FailoverInsuranceClaimScope::FastDandelionFailoverClaimLane,
                owner_commitment: "owner:fast-dandelion-decoy-insurance-route-council-b".to_string(),
                policy_commitment_root: devnet_root(
                    "policy",
                    "fast-dandelion-decoy-insurance-dandelion-failover-insurance-claim-settlement",
                ),
                claim_source_relay_set_root: devnet_root(
                    "source-fluff-relay-set",
                ...
                claim_route_allowlist_root: devnet_root(
                    "route-allowlist",
                    "fast-dandelion-decoy-insurance-beta",
                ),
                min_privacy_set_size: 393_216,
                min_stem_lane_claim_reserve_micro_units: 144_000_000,
                min_fluff_lane_claim_reserve_micro_units: 64_000_000,
                max_claim_settlement_fee_bps: 18,
                min_attestation_quorum_bps: 7_000,
                signal_window_slots: 80,
                failover_insurance_claim_window_slots: 640,
                redaction_profile:
                    "fast-dandelion-decoy-insurance-dandelion-failover-insurance-claim-settlement-profile"
                        .to_string(),
            })
            .expect("devnet fast failover_insurance_claim policy");
        let withdrawal_cohort = state
            .register_cohort(RegisterCohortRequest {
                ...
                claim_source_relay_set_root: devnet_root("source-fluff-relay-set", "cohort-alpha"),
                claim_target_relay_set_root: devnet_root("target-fluff-relay-set", "cohort-alpha"),
                output_set_root: devnet_root("outputs", "cohort-alpha"),
                decoy_insurance_commitment_root: devnet_root("liquidity", "cohort-alpha"),
                stem_lane_claim_reserve_micro_units: 216_000_000,
                fluff_lane_claim_reserve_micro_units: 96_000_000,
        ...
        let fast_dandelion_exit_cohort = state
            .register_cohort(RegisterCohortRequest {
                ...
                claim_source_relay_set_root: devnet_root("source-fluff-relay-set", "cohort-beta"),
                claim_target_relay_set_root: devnet_root("target-fluff-relay-set", "cohort-beta"),
                output_set_root: devnet_root("outputs", "cohort-beta"),
                decoy_insurance_commitment_root: devnet_root("liquidity", "cohort-beta"),
                stem_lane_claim_reserve_micro_units: 168_000_000,
                fluff_lane_claim_reserve_micro_units: 76_000_000,
                privacy_set_size: 393_216,
                claim_settlement_fee_bps: 13,
                public_hint_root: devnet_root("hint", "cohort-beta"),
            })
            .expect("devnet fast failover_insurance_claim cohort");
        let signal = state
            .submit_signal(SubmitSignalRequest {
                ...
                observed_claim_route_root: devnet_root(
                    "observed-route",
                ...
                stem_lane_claim_reserve_delta_micro_units: -48_000_000,
                fluff_lane_claim_reserve_delta_micro_units: 24_000_000,
                privacy_set_size: 524_288,
        ...
        state
            .submit_attestation(SubmitAttestationRequest {
                ...
                verdict: AttestationVerdict::RequireFailoverInsuranceClaimCoverageGuard,
                attestor_commitment: "attestor:pq-dandelion-failover-insurance-claim-settlement-a"
                    .to_string(),
                pq_signature_root: devnet_root(
                    "ml-dsa-signature",
                    "withdrawal-failover_insurance_claim-a",
                ),
                pq_public_key_root: devnet_root(
                    "ml-kem-public-key",
                    "withdrawal-failover_insurance_claim-a",
                ),
                transcript_root: devnet_root("transcript", "withdrawal-failover_insurance_claim-a"),
        ...
        state
            .submit_attestation(SubmitAttestationRequest {
                signal_id: signal.clone(),
                verdict: AttestationVerdict::ConfirmFailoverInsuranceClaimPressure,
                attestor_commitment: "attestor:pq-dandelion-failover-insurance-claim-settlement-b"
                    .to_string(),
                pq_signature_root: devnet_root(
                    "slh-dsa-signature",
                    "withdrawal-failover_insurance_claim-b",
                ),
                pq_public_key_root: devnet_root(
                    "ml-kem-public-key",
                    "withdrawal-failover_insurance_claim-b",
                ),
                transcript_root: devnet_root("transcript", "withdrawal-failover_insurance_claim-b"),
        ...
        let decision = state
            .settle_decision(SettleDecisionRequest {
                signal_id: signal,
                action: FailoverInsuranceClaimAction::SettleFailoverInsuranceClaim,
                decision_root: devnet_root(
                    "decision",
                    "withdrawal-liquidity-failover_insurance_claim",
                ),
                replacement_claim_route_root: devnet_root(
                    "replacement-route",
                    "withdrawal-failover_insurance_claim",
                ),
            ...
            .expect("devnet failover_insurance_claim decision");
        state
            .publish_rebate(PublishRebateRequest {
                decision_id: decision,
                recipient_commitment:
                    "recipient:dandelion-failover-insurance-claim-settlement-fee-sponsor"
                        .to_string(),
                rebate_micro_units: 22_000,
                fee_saved_micro_units: 88_000,
                rebate_bps: 8,
                settlement_slot: DEVNET_SLOT + 6,
                rebate_root: devnet_root("rebate", "withdrawal-failover_insurance_claim"),
            })
    ...
    pub fn register_policy(&mut self, request: RegisterPolicyRequest) -> Result<String> {
        ...
        ensure_non_empty(
            &request.claim_source_relay_set_root,
            "claim_source_relay_set_root",
        )?;
        ensure_non_empty(
            &request.claim_target_relay_set_root,
            "claim_target_relay_set_root",
        )?;
        ensure_non_empty(
            &request.claim_route_allowlist_root,
            "claim_route_allowlist_root",
        )?;
        ...
        if request.min_stem_lane_claim_reserve_micro_units
            < self.config.min_stem_lane_claim_reserve_micro_units
        {
            return Err("policy source liquidity below runtime floor".to_string());
        }
        if request.min_fluff_lane_claim_reserve_micro_units
            < self.config.min_fluff_lane_claim_reserve_micro_units
        {
            return Err("policy target liquidity below runtime floor".to_string());
        }
        let policy_id = stable_id(
            "dandelion-failover-insurance-claim-settlement-policy",
            &[
                HashPart::Str(request.scope.as_str()),
                HashPart::Str(&request.owner_commitment),
                HashPart::Str(&request.policy_commitment_root),
                HashPart::Str(&request.claim_source_relay_set_root),
            ],
        );
        ...
        self.policies.insert(
            policy_id.clone(),
            FailoverInsuranceClaimPolicy {
                policy_id: policy_id.clone(),
                scope: request.scope,
                owner_commitment: request.owner_commitment,
                policy_commitment_root: request.policy_commitment_root,
                claim_source_relay_set_root: request.claim_source_relay_set_root,
                claim_target_relay_set_root: request.claim_target_relay_set_root,
                claim_route_allowlist_root: request.claim_route_allowlist_root,
                min_privacy_set_size: request.min_privacy_set_size,
                min_stem_lane_claim_reserve_micro_units: request
                    .min_stem_lane_claim_reserve_micro_units,
                min_fluff_lane_claim_reserve_micro_units: request
                    .min_fluff_lane_claim_reserve_micro_units,
                ...
                failover_insurance_claim_window_slots: request
                    .failover_insurance_claim_window_slots,
                active: true,
    ...
    pub fn register_cohort(&mut self, request: RegisterCohortRequest) -> Result<String> {
        ...
        if request.stem_lane_claim_reserve_micro_units
            < policy.min_stem_lane_claim_reserve_micro_units
        {
            return Err("cohort source liquidity below policy floor".to_string());
        }
        if request.fluff_lane_claim_reserve_micro_units
            < policy.min_fluff_lane_claim_reserve_micro_units
        {
        ...
        let cohort_id = stable_id(
            "dandelion-failover-insurance-claim-settlement-cohort",
            &[
        ...
        self.cohorts.insert(
            cohort_id.clone(),
            FailoverInsuranceClaimSettlementCohort {
                ...
                stem_lane_claim_reserve_micro_units: request.stem_lane_claim_reserve_micro_units,
                fluff_lane_claim_reserve_micro_units: request.fluff_lane_claim_reserve_micro_units,
    ...
    pub fn submit_signal(&mut self, request: SubmitSignalRequest) -> Result<String> {
        ...
        ensure_non_empty(
            &request.observed_claim_route_root,
            "observed_claim_route_root",
        )?;
        ensure_non_empty(
        ...
        let signal_id = stable_id(
            "dandelion-failover-insurance-claim-settlement-signal",
            &[
        ...
        self.signals.insert(
            ...
            FailoverInsuranceClaimSignal {
                ...
                observed_claim_route_root: request.observed_claim_route_root,
                fluff_linkage_pressure_root: request.fluff_linkage_pressure_root,
                reporter_commitment: request.reporter_commitment,
                stem_lane_claim_reserve_delta_micro_units: request
                    .stem_lane_claim_reserve_delta_micro_units,
                fluff_lane_claim_reserve_delta_micro_units: request
                    .fluff_lane_claim_reserve_delta_micro_units,
                privacy_set_size: request.privacy_set_size,
    ...
    pub fn submit_attestation(&mut self, request: SubmitAttestationRequest) -> Result<String> {
        ...
        let attestation_id = stable_id(
            "dandelion-failover-insurance-claim-settlement-pq-attestation",
            &[
        ...
        self.attestations.insert(
            ...
            PqFailoverClaimSettlementAttestation {
    ...
    pub fn settle_decision(&mut self, request: SettleDecisionRequest) -> Result<String> {
        ...
        ensure_non_empty(
            &request.replacement_claim_route_root,
            "replacement_claim_route_root",
        )?;
        ensure_bps(request.rebate_bps, "rebate_bps")?;
        let privacy_filter_deadline_slot = match request.action {
            FailoverInsuranceClaimAction::QueueFailoverClaimReview
            | FailoverInsuranceClaimAction::SettleFailoverInsuranceClaim
            | FailoverInsuranceClaimAction::RotateFailoverClaimClaimSettlementCohort => request
                .decided_at_slot
                .saturating_add(policy.failover_insurance_claim_window_slots),
            FailoverInsuranceClaimAction::QuarantineRoute
            | FailoverInsuranceClaimAction::EmergencyPause => request
                .decided_at_slot
                .saturating_add(policy.failover_insurance_claim_window_slots / 2),
            _ => 0,
        };
        let decision_id = stable_id(
            "dandelion-failover-insurance-claim-settlement-decision",
            &[
                HashPart::Str(&request.signal_id),
        ...
        self.decisions.insert(
            decision_id.clone(),
            FailoverInsuranceClaimDecision {
                ...
                replacement_claim_route_root: request.replacement_claim_route_root,
        ...
        if let Some(signal) = self.signals.get_mut(&request.signal_id) {
            signal.status = match request.action {
                FailoverInsuranceClaimAction::QueueFailoverClaimReview => SignalStatus::RouteQueued,
                FailoverInsuranceClaimAction::SettleFailoverInsuranceClaim
                | FailoverInsuranceClaimAction::RotateFailoverClaimClaimSettlementCohort => {
                    SignalStatus::Routing
                }
                FailoverInsuranceClaimAction::QuarantineRoute
                | FailoverInsuranceClaimAction::EmergencyPause => SignalStatus::Quarantined,
                FailoverInsuranceClaimAction::Release => SignalStatus::Settled,
                FailoverInsuranceClaimAction::Reject => SignalStatus::Rejected,
                FailoverInsuranceClaimAction::Expire => SignalStatus::Expired,
                FailoverInsuranceClaimAction::Observe => SignalStatus::Attested,
            };
        }
        if let Some(cohort) = self.cohorts.get_mut(&signal.cohort_id) {
            match request.action {
                ...
                FailoverInsuranceClaimAction::SettleFailoverInsuranceClaim => {
                    ...
                    cohort.stem_lane_claim_reserve_micro_units = cohort
                        .stem_lane_claim_reserve_micro_units
                        .saturating_sub(request.privacy_filtered_micro_units);
                    cohort.fluff_lane_claim_reserve_micro_units = cohort
                        .fluff_lane_claim_reserve_micro_units
                ...
                FailoverInsuranceClaimAction::QuarantineRoute => {
                    ...
                    cohort.privacy_filter_deadline_slot = privacy_filter_deadline_slot;
                    self.counters.claim_routes_quarantined =
                        self.counters.claim_routes_quarantined.saturating_add(1);
                }
                FailoverInsuranceClaimAction::EmergencyPause => {
                    cohort.status = CohortStatus::Paused;
                    cohort.privacy_filter_deadline_slot = privacy_filter_deadline_slot;
                    self.counters.claim_routes_quarantined =
                        self.counters.claim_routes_quarantined.saturating_add(1);
                }
                FailoverInsuranceClaimAction::Release => {
    ...
    pub fn publish_rebate(&mut self, request: PublishRebateRequest) -> Result<String> {
        ...
        let rebate_id = stable_id(
            "dandelion-failover-insurance-claim-settlement-low-fee-rebate",
            &[
        ...
        self.rebates.insert(
            rebate_id.clone(),
            LowFeeFailoverClaimSettlementRebate {
    ...
    pub fn publish_redaction_budget(
        ...
        let budget_id = stable_id(
            "dandelion-failover-insurance-claim-settlement-redaction-budget",
            &[
                HashPart::Str(request.scope.as_str()),
                HashPart::Str(&request.cohort_id),
    ...
    pub fn publish_operator_summary(
        ...
        let summary_id = stable_id(
            "dandelion-failover-insurance-claim-settlement-operator-summary",
            &[
                HashPart::U64(epoch),
                HashPart::U64(slot),
    ...
    fn refresh_roots(&mut self) {
        ...
        self.roots.state_root = merkle_root(
            "bridge-dandelion-failover-insurance-claim-settlement:state",
            &[
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "failover_insurance_claim_settlement_suite": self.config.failover_insurance_claim_settlement_suite,
            ...
            "failover_insurance_claim_window_slots": self.config.failover_insurance_claim_window_slots,
            ...
            "policies": self.policies.values().map(|policy| json!({
                ...
                "min_stem_lane_claim_reserve_micro_units": policy.min_stem_lane_claim_reserve_micro_units,
                "min_fluff_lane_claim_reserve_micro_units": policy.min_fluff_lane_claim_reserve_micro_units,
            ...
            "cohorts": self.cohorts.values().map(|cohort| json!({
                ...
                "stem_lane_claim_reserve_micro_units": cohort.stem_lane_claim_reserve_micro_units,
                "fluff_lane_claim_reserve_micro_units": cohort.fluff_lane_claim_reserve_micro_units,
...
pub fn demo() -> State {
    ...
    let signal = state
        .submit_signal(SubmitSignalRequest {
            ...
            observed_claim_route_root: devnet_root("observed-route", "demo-viewtag-pressure"),
            ...
            stem_lane_claim_reserve_delta_micro_units: -10_000_000,
            fluff_lane_claim_reserve_delta_micro_units: 8_000_000,
            privacy_set_size: 524_288,
    ...
    state
        .submit_attestation(SubmitAttestationRequest {
            signal_id: signal,
            verdict: AttestationVerdict::RequireFailoverInsuranceClaimCoverage,
            attestor_commitment: "attestor:demo-dandelion-failover-insurance-claim-settlement"
                .to_string(),
...
fn object_root<T: Serialize>(domain: &str, value: &T) -> String {
    ...
    domain_hash(
        &format!("bridge-dandelion-failover-insurance-claim-settlement:{domain}"),
        &[HashPart::Json(&value)],
        32,
    )
}
...
fn devnet_root(domain: &str, label: &str) -> String {
    domain_hash(
        &format!("bridge-dandelion-failover-insurance-claim-settlement:devnet:{domain}"),
