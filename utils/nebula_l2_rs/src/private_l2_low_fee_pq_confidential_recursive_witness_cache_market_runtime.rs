...
pub type PrivateL2LowFeePqConfidentialRecursiveWitnessCacheMarketRuntimeResult<T> = Result<T>;
pub type Runtime = State;

pub const PRIVATE_L2_LOW_FEE_PQ_CONFIDENTIAL_RECURSIVE_WITNESS_CACHE_MARKET_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-private-l2-low-fee-pq-confidential-recursive-witness-cache-market-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_LOW_FEE_PQ_CONFIDENTIAL_RECURSIVE_WITNESS_CACHE_MARKET_RUNTIME_PROTOCOL_VERSION;
...
pub const PQ_AUTH_SUITE: &str =
    "ML-KEM-1024+ML-DSA-87+SLH-DSA-SHAKE-256f-recursive-witness-cache-market-v1";
...
pub enum WitnessCacheWorkloadKind {
    TransferBatch,
    ContractExecution,
    DefiSettlement,
    TokenNetting,
...
impl Config {
    ...
    pub fn public_record(&self) -> Value {
        json!({
            "kind": "private_l2_low_fee_pq_confidential_recursive_witness_cache_market_config",
...
impl Counters {
    pub fn public_record(&self) -> Value {
        json!({
            "kind": "private_l2_low_fee_pq_confidential_recursive_witness_cache_market_counters",
...
impl Roots {
    pub fn public_record(&self) -> Value {
        json!({
            "kind": "private_l2_low_fee_pq_confidential_recursive_witness_cache_market_roots",
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct WitnessCacheWorkloadCommitmentRequest {
    pub owner_commitment: String,
    pub kind: WitnessCacheWorkloadKind,
    pub workload_root: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct WitnessCacheWorkloadCommitment {
...
impl WitnessCacheWorkloadCommitment {
    pub fn from_request(
        config: &Config,
        request: WitnessCacheWorkloadCommitmentRequest,
    ) -> Result<Self> {
    ...
    pub fn public_record(&self) -> Value {
        json!({
            "kind": "pq_recursive_witness_cache_workload_commitment",
            "chain_id": CHAIN_ID,
            "protocol_version": PROTOCOL_VERSION,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct WitnessCacheBidRequest {
    pub workload_id: String,
...
impl WitnessCacheBid {
    pub fn from_request(config: &Config, request: WitnessCacheBidRequest) -> Result<Self> {
        ...
        let bid_id = witness_cache_bid_id(&request);
        Ok(Self {
            bid_id,
            ...
            score: witness_cache_bid_score(request.bid_fee_micro_units, request.max_latency_ms),
    ...
    pub fn public_record(&self) -> Value {
        json!({
            "kind": "sealed_recursive_witness_cache_bid",
            "chain_id": CHAIN_ID,
            "protocol_version": PROTOCOL_VERSION,
            "bid_id": self.bid_id,
            "workload_id": self.workload_id,
...
pub struct State {
    ...
    pub roots: Roots,
    pub workloads: BTreeMap<String, WitnessCacheWorkloadCommitment>,
    pub bids: BTreeMap<String, WitnessCacheBid>,
...
impl State {
    ...
    pub fn submit_workload(
        &mut self,
        request: WitnessCacheWorkloadCommitmentRequest,
    ) -> Result<WitnessCacheWorkloadCommitment> {
        ...
        let workload = WitnessCacheWorkloadCommitment::from_request(&self.config, request)?;
    ...
    pub fn post_bid(&mut self, request: WitnessCacheBidRequest) -> Result<WitnessCacheBid> {
        ensure_capacity("bids", self.bids.len(), self.config.max_bids)?;
        let bid = WitnessCacheBid::from_request(&self.config, request)?;
        let workload = self
            .workloads
            .get_mut(&bid.workload_id)
            .ok_or_else(|| format!("unknown workload_id {}", bid.workload_id))?;
        if !workload.status.accepts_bid() {
            return Err("workload is not accepting witness cache bids".to_string());
        }
    ...
    pub fn public_record_without_state_root(&self) -> Value {
        json!({
            "kind": "private_l2_low_fee_pq_confidential_recursive_witness_cache_market_runtime",
            "chain_id": CHAIN_ID,
            "protocol_version": PROTOCOL_VERSION,
            "schema_version": SCHEMA_VERSION,
...
pub fn private_l2_low_fee_pq_confidential_recursive_witness_cache_market_runtime_state_root(
    state: &State,
) -> String {
    state.state_root()
}

pub fn private_l2_low_fee_pq_confidential_recursive_witness_cache_market_runtime_public_record(
    state: &State,
) -> Value {
    state.public_record()
}

pub fn workload_id(request: &WitnessCacheWorkloadCommitmentRequest) -> String {
    domain_hash(
...
pub fn witness_cache_bid_id(request: &WitnessCacheBidRequest) -> String {
    domain_hash(
...
pub fn witness_cache_bid_score(bid_fee_micro_units: u64, max_latency_ms: u64) -> u64 {
