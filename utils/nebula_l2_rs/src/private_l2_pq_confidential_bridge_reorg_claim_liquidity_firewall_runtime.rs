...
pub type PrivateL2PqConfidentialBridgeReorgClaimLiquidityFirewallRuntimeResult<T> = Result<T>;
pub type Runtime = State;

pub const PROTOCOL_VERSION: &str =
    "nebula-private-l2-pq-confidential-bridge-reorg-claim-liquidity-firewall-runtime-v1";
pub const PRIVATE_L2_PQ_CONFIDENTIAL_BRIDGE_REORG_CLAIM_LIQUIDITY_FIREWALL_RUNTIME_PROTOCOL_VERSION:
    &str = PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const PQ_REORG_CLAIM_LIQUIDITY_FIREWALL_SUITE: &str =
    "ML-DSA-87+ML-KEM-1024+SLH-DSA-SHAKE-256f";
pub const REORG_CLAIM_LIQUIDITY_FIREWALL_SUITE: &str =
    "monero-bridge-reorg-claim-liquidity-firewall-root-v1";
pub const REBATE_SUITE: &str = "reorg-claim-liquidity-firewall-low-fee-rebate-root-v1";
pub const REDACTION_SUITE: &str = "operator-safe-reorg-claim-liquidity-firewall-redaction-root-v1";
...
pub const DEFAULT_REORG_CLAIM_WINDOW_SLOTS: u64 = 720;
...
impl ReorgClaimScope {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::EmergencyReorgClaimLiquidityFirewall => {
                "emergency_reorg_claim_liquidity_firewall"
            }
...
pub enum CohortStatus {
    Active,
    ReorgClaimPathPending,
    ReorgLiquidityFirewalling,
    Quarantined,
    Rotating,
    Paused,
    Settled,
...
#[serde(rename_all = "snake_case")]
pub enum SignalKind {
    ReorgLiquidityLinkagePressure,
    ReorgRelayFragmentation,
    ReorgRelaySkew,
    ReorgLiquidityGraphLinkagePressure,
    ReorgTimingCorrelationPressure,
    ReserveMismatch,
    FeeSpikeReorgLiquidityFirewallClaimPressure,
    WatchtowerEmergency,
}
...
pub enum AttestationVerdict {
    ConfirmReorgClaimPressure,
    ConfirmPrivacyRisk,
    RequireReorgClaimCoverage,
    RequireReorgClaimCoverageGuard,
    RequireQuarantine,
    ClearFalsePositive,
}
...
#[derive(Clone, Copy, Debug, Deserialize, Eq, Ord, PartialEq, PartialOrd, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum ReorgClaimAction {
    Observe,
    QueueReorgLiquidityFirewallClaimReview,
    SettleReorgClaim,
    RotateReorgLiquidityFirewallClaimClaimSettlementCohort,
    QuarantineReorgClaimPath,
    EmergencyPause,
    Release,
    Reject,
    Expire,
...
pub struct Config {
    ...
    pub reorg_claim_liquidity_firewall_suite: String,
    ...
    pub reorg_claim_window_slots: u64,
    pub signal_window_slots: u64,
    pub max_claim_settlement_fee_bps: u64,
    pub target_rebate_bps: u64,
    pub min_pre_reorg_claim_reserve_micro_units: u64,
    pub min_reorg_lane_claim_reserve_micro_units: u64,
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            pq_claim_settlement_insurance_suite: PQ_REORG_CLAIM_LIQUIDITY_FIREWALL_SUITE
                .to_string(),
            reorg_claim_liquidity_firewall_suite: REORG_CLAIM_LIQUIDITY_FIREWALL_SUITE.to_string(),
            ...
            reorg_claim_window_slots: DEFAULT_REORG_CLAIM_WINDOW_SLOTS,
            signal_window_slots: DEFAULT_SIGNAL_WINDOW_SLOTS,
            max_claim_settlement_fee_bps: DEFAULT_MAX_ROUTE_FEE_BPS,
            target_rebate_bps: DEFAULT_TARGET_REBATE_BPS,
            min_pre_reorg_claim_reserve_micro_units: DEFAULT_MIN_SOURCE_LIQUIDITY_MICRO_UNITS,
            min_reorg_lane_claim_reserve_micro_units: DEFAULT_MIN_TARGET_LIQUIDITY_MICRO_UNITS,
...
pub struct Counters {
    ...
    pub reorg_filters_settled: u64,
    pub cohorts_rotated: u64,
    pub claim_reorg_claim_paths_quarantined: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct ReorgClaimPolicy {
    pub policy_id: String,
    pub scope: ReorgClaimScope,
    pub owner_commitment: String,
    pub policy_commitment_root: String,
    pub claim_source_reorg_liquidity_set_root: String,
    pub claim_target_reorg_liquidity_set_root: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct ReorgClaimLiquidityFirewallCohort {
    ...
    pub pre_reorg_claim_reserve_micro_units: u64,
    pub reorg_lane_claim_reserve_micro_units: u64,
...
pub struct ReorgClaimSignal {
    ...
    pub observed_claim_reorg_claim_path_root: String,
    pub reorg_liquidity_pressure_root: String,
    pub reporter_commitment: String,
    pub pre_reorg_claim_reserve_delta_micro_units: i128,
    pub reorg_lane_claim_reserve_delta_micro_units: i128,
    pub privacy_set_size: u64,
    pub reorg_liquidity_risk_score_bps: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct PqReorgClaimFirewallAttestation {
    ...
    pub observed_reorg_liquidity_risk_score_bps: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct ReorgClaimDecision {
    pub decision_id: String,
    pub signal_id: String,
    pub cohort_id: String,
    pub policy_id: String,
    pub action: ReorgClaimAction,
    pub decision_root: String,
    pub replacement_claim_reorg_claim_path_root: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct LowFeeReorgClaimFirewallRebate {
...
pub struct State {
    ...
    pub roots: Roots,
    pub policies: BTreeMap<String, ReorgClaimPolicy>,
    pub cohorts: BTreeMap<String, ReorgClaimLiquidityFirewallCohort>,
    pub signals: BTreeMap<String, ReorgClaimSignal>,
    pub attestations: BTreeMap<String, PqReorgClaimFirewallAttestation>,
    pub decisions: BTreeMap<String, ReorgClaimDecision>,
    pub rebates: BTreeMap<String, LowFeeReorgClaimFirewallRebate>,
...
impl State {
    ...
    pub fn devnet() -> Self {
        ...
        let withdrawal_policy = state
            .register_policy(RegisterPolicyRequest {
                scope: ReorgClaimScope::WithdrawalSubaddressLane,
                owner_commitment: "owner:dandelion-reorg-claim-liquidity-firewall-council-a"
                    .to_string(),
                policy_commitment_root: devnet_root(
                    "policy",
                    "withdrawal-dandelion-reorg-claim-liquidity-firewall",
                ),
                claim_source_reorg_liquidity_set_root: devnet_root(
                    "source-reorg-relay-set",
                    "withdrawal-alpha",
                ),
                claim_target_reorg_liquidity_set_root: devnet_root(
                    "target-reorg-relay-set",
                    "withdrawal-alpha",
                ),
                claim_rotation_allowlist_root: devnet_root(
                    "reorg_claim_path-allowlist",
                    "withdrawal-alpha",
                ),
                min_privacy_set_size: 524_288,
                min_pre_reorg_claim_reserve_micro_units: 160_000_000,
                min_reorg_lane_claim_reserve_micro_units: 72_000_000,
                max_claim_settlement_fee_bps: 16,
                min_attestation_quorum_bps: 7_200,
                signal_window_slots: 96,
                reorg_claim_window_slots: 720,
                redaction_profile: "withdrawal-dandelion-reorg-claim-liquidity-firewall-profile"
                    .to_string(),
            })
            .expect("devnet withdrawal policy");
        let fast_dandelion_exit_policy = state
            .register_policy(RegisterPolicyRequest {
                scope: ReorgClaimScope::FastDandelionReorgLiquidityFirewallClaimLane,
                owner_commitment: "owner:fast-dandelion-decoy-insurance-reorg_claim_path-council-b".to_string(),
                policy_commitment_root: devnet_root(
                    "policy",
                    "fast-dandelion-decoy-insurance-dandelion-reorg-claim-liquidity-firewall",
                ),
                claim_source_reorg_liquidity_set_root: devnet_root(
                    "source-reorg-relay-set",
                ...
                min_pre_reorg_claim_reserve_micro_units: 144_000_000,
                min_reorg_lane_claim_reserve_micro_units: 64_000_000,
                max_claim_settlement_fee_bps: 18,
                min_attestation_quorum_bps: 7_000,
                signal_window_slots: 80,
                reorg_claim_window_slots: 640,
                redaction_profile:
                    "fast-dandelion-decoy-insurance-dandelion-reorg-claim-liquidity-firewall-profile"
                        .to_string(),
            })
            .expect("devnet fast reorg_claim policy");
        let withdrawal_cohort = state
            .register_cohort(RegisterCohortRequest {
                ...
                pre_reorg_claim_reserve_micro_units: 216_000_000,
                reorg_lane_claim_reserve_micro_units: 96_000_000,
        ...
        let fast_dandelion_exit_cohort = state
            .register_cohort(RegisterCohortRequest {
                ...
                bridge_lane_id: "fast-dandelion-decoy-insurance-reorg_claim_path-beta".to_string(),
                claim_source_reorg_liquidity_set_root: devnet_root(
                    "source-reorg-relay-set",
                    "cohort-beta",
                ),
                ...
                pre_reorg_claim_reserve_micro_units: 168_000_000,
                reorg_lane_claim_reserve_micro_units: 76_000_000,
                privacy_set_size: 393_216,
                claim_settlement_fee_bps: 13,
                public_hint_root: devnet_root("hint", "cohort-beta"),
            })
            .expect("devnet fast reorg_claim cohort");
        let signal = state
            .submit_signal(SubmitSignalRequest {
                cohort_id: withdrawal_cohort,
                kind: SignalKind::ReorgLiquidityLinkagePressure,
                signal_commitment_root: devnet_root("signal", "withdrawal-cluster-pressure"),
                observed_claim_reorg_claim_path_root: devnet_root(
                    "observed-reorg_claim_path",
                    "withdrawal-cluster-pressure",
                ),
                reorg_liquidity_pressure_root: devnet_root(
                    "pressure",
                    "withdrawal-cluster-pressure",
                ),
                reporter_commitment: "reporter:dandelion-reorg_claim_path-watchtower-11"
                    .to_string(),
                pre_reorg_claim_reserve_delta_micro_units: -48_000_000,
                reorg_lane_claim_reserve_delta_micro_units: 24_000_000,
                privacy_set_size: 524_288,
                reorg_liquidity_risk_score_bps: 7_300,
        ...
        state
            .submit_attestation(SubmitAttestationRequest {
                ...
                verdict: AttestationVerdict::RequireReorgClaimCoverageGuard,
                attestor_commitment: "attestor:pq-dandelion-reorg-claim-liquidity-firewall-a"
                    .to_string(),
                pq_signature_root: devnet_root("ml-dsa-signature", "withdrawal-reorg_claim-a"),
                pq_public_key_root: devnet_root("ml-kem-public-key", "withdrawal-reorg_claim-a"),
                transcript_root: devnet_root("transcript", "withdrawal-reorg_claim-a"),
                quorum_weight_bps: 7_500,
                pq_security_bits: 256,
                observed_reorg_liquidity_risk_score_bps: 7_500,
        ...
        state
            .submit_attestation(SubmitAttestationRequest {
                signal_id: signal.clone(),
                verdict: AttestationVerdict::ConfirmReorgClaimPressure,
                attestor_commitment: "attestor:pq-dandelion-reorg-claim-liquidity-firewall-b"
                    .to_string(),
                pq_signature_root: devnet_root("slh-dsa-signature", "withdrawal-reorg_claim-b"),
                pq_public_key_root: devnet_root("ml-kem-public-key", "withdrawal-reorg_claim-b"),
                transcript_root: devnet_root("transcript", "withdrawal-reorg_claim-b"),
                quorum_weight_bps: 8_700,
                pq_security_bits: 256,
                observed_reorg_liquidity_risk_score_bps: 7_800,
                observed_privacy_set_size: 524_288,
        ...
        let decision = state
            .settle_decision(SettleDecisionRequest {
                signal_id: signal,
                action: ReorgClaimAction::SettleReorgClaim,
                decision_root: devnet_root("decision", "withdrawal-liquidity-reorg_claim"),
                replacement_claim_reorg_claim_path_root: devnet_root(
                    "replacement-reorg_claim_path",
                    "withdrawal-reorg_claim",
                ),
            ...
            .expect("devnet reorg_claim decision");
        state
            .publish_rebate(PublishRebateRequest {
                decision_id: decision,
                recipient_commitment:
                    "recipient:dandelion-reorg-claim-liquidity-firewall-fee-sponsor".to_string(),
                rebate_micro_units: 22_000,
                fee_saved_micro_units: 88_000,
                rebate_bps: 8,
                settlement_slot: DEVNET_SLOT + 6,
                rebate_root: devnet_root("rebate", "withdrawal-reorg_claim"),
            })
    ...
    pub fn register_policy(&mut self, request: RegisterPolicyRequest) -> Result<String> {
        ensure_capacity(
            self.policies.len(),
            MAX_POLICIES,
            "Dandelion reorg-phase privacy guard policies",
        )?;
        ...
        ensure_non_empty(
            &request.claim_source_reorg_liquidity_set_root,
            "claim_source_reorg_liquidity_set_root",
        )?;
        ensure_non_empty(
            &request.claim_target_reorg_liquidity_set_root,
            "claim_target_reorg_liquidity_set_root",
        )?;
        ensure_non_empty(
        ...
        if request.min_pre_reorg_claim_reserve_micro_units
            < self.config.min_pre_reorg_claim_reserve_micro_units
        {
            return Err("policy source liquidity below runtime floor".to_string());
        }
        if request.min_reorg_lane_claim_reserve_micro_units
            < self.config.min_reorg_lane_claim_reserve_micro_units
        {
            return Err("policy target liquidity below runtime floor".to_string());
        }
        let policy_id = stable_id(
            "dandelion-reorg-claim-liquidity-firewall-policy",
            &[
                HashPart::Str(request.scope.as_str()),
                HashPart::Str(&request.owner_commitment),
                HashPart::Str(&request.policy_commitment_root),
                HashPart::Str(&request.claim_source_reorg_liquidity_set_root),
            ],
        );
        if self.policies.contains_key(&policy_id) {
            return Err(format!(
                "Dandelion reorg-phase privacy guard policy {policy_id} already exists"
            ));
        }
        self.policies.insert(
            ...
            ReorgClaimPolicy {
                ...
                claim_source_reorg_liquidity_set_root: request
                    .claim_source_reorg_liquidity_set_root,
                claim_target_reorg_liquidity_set_root: request
                    .claim_target_reorg_liquidity_set_root,
                claim_rotation_allowlist_root: request.claim_rotation_allowlist_root,
                min_privacy_set_size: request.min_privacy_set_size,
                min_pre_reorg_claim_reserve_micro_units: request
                    .min_pre_reorg_claim_reserve_micro_units,
                min_reorg_lane_claim_reserve_micro_units: request
                    .min_reorg_lane_claim_reserve_micro_units,
                ...
                reorg_claim_window_slots: request.reorg_claim_window_slots,
                active: true,
    ...
    pub fn register_cohort(&mut self, request: RegisterCohortRequest) -> Result<String> {
        ensure_capacity(
            self.cohorts.len(),
            MAX_COHORTS,
            "Dandelion reorg_claim_path privacy cohorts",
        )?;
        let policy = self.policies.get(&request.policy_id).ok_or_else(|| {
            format!(
                "unknown Dandelion reorg-phase privacy guard policy {}",
                request.policy_id
            )
        })?;
        ...
        if request.claim_settlement_fee_bps > policy.max_claim_settlement_fee_bps {
            return Err("cohort reorg_claim_path fee exceeds policy cap".to_string());
        }
        if request.pre_reorg_claim_reserve_micro_units
            < policy.min_pre_reorg_claim_reserve_micro_units
        {
            return Err("cohort source liquidity below policy floor".to_string());
        }
        if request.reorg_lane_claim_reserve_micro_units
            < policy.min_reorg_lane_claim_reserve_micro_units
        {
        ...
        let cohort_id = stable_id(
            "dandelion-reorg-claim-liquidity-firewall-cohort",
            &[
        ...
        if self.cohorts.contains_key(&cohort_id) {
            return Err(format!(
                "Dandelion reorg-phase privacy guard cohort {cohort_id} already exists"
            ));
        }
        self.cohorts.insert(
            cohort_id.clone(),
            ReorgClaimLiquidityFirewallCohort {
                ...
                pre_reorg_claim_reserve_micro_units: request.pre_reorg_claim_reserve_micro_units,
                reorg_lane_claim_reserve_micro_units: request.reorg_lane_claim_reserve_micro_units,
    ...
    pub fn submit_signal(&mut self, request: SubmitSignalRequest) -> Result<String> {
        ensure_capacity(
            self.signals.len(),
            MAX_SIGNALS,
            "Dandelion reorg-phase privacy guard signals",
        )?;
        let cohort = self
            .cohorts
            ...
            .ok_or_else(|| {
                format!(
                    "unknown Dandelion reorg-phase privacy guard cohort {}",
                    request.cohort_id
                )
            })?
            .clone();
        ...
        ensure_non_empty(
            &request.observed_claim_reorg_claim_path_root,
            "observed_claim_reorg_claim_path_root",
        )?;
        ensure_non_empty(
            &request.reorg_liquidity_pressure_root,
            "reorg_liquidity_pressure_root",
        )?;
        ensure_non_empty(&request.reporter_commitment, "reporter_commitment")?;
        ensure_bps(
            request.reorg_liquidity_risk_score_bps,
            "reorg_liquidity_risk_score_bps",
        )?;
        if request.privacy_set_size < policy.min_privacy_set_size {
            return Err("signal privacy set below policy floor".to_string());
        }
        let signal_id = stable_id(
            "dandelion-reorg-claim-liquidity-firewall-signal",
            &[
        ...
        if self.signals.contains_key(&signal_id) {
            return Err(format!(
                "Dandelion reorg-phase privacy guard signal {signal_id} already exists"
            ));
        }
        ...
        self.signals.insert(
            ...
            ReorgClaimSignal {
                ...
                observed_claim_reorg_claim_path_root: request.observed_claim_reorg_claim_path_root,
                reorg_liquidity_pressure_root: request.reorg_liquidity_pressure_root,
                reporter_commitment: request.reporter_commitment,
                pre_reorg_claim_reserve_delta_micro_units: request
                    .pre_reorg_claim_reserve_delta_micro_units,
                reorg_lane_claim_reserve_delta_micro_units: request
                    .reorg_lane_claim_reserve_delta_micro_units,
                privacy_set_size: request.privacy_set_size,
                reorg_liquidity_risk_score_bps: request.reorg_liquidity_risk_score_bps,
    ...
    pub fn submit_attestation(&mut self, request: SubmitAttestationRequest) -> Result<String> {
        ensure_capacity(
            ...
            MAX_ATTESTATIONS,
            "Dandelion reorg-phase privacy guard attestations",
        )?;
        let signal = self
            .signals
            ...
            .ok_or_else(|| {
                format!(
                    "unknown Dandelion reorg-phase privacy guard signal {}",
                    request.signal_id
                )
            })?
            .clone();
        ...
        ensure_bps(
            request.observed_reorg_liquidity_risk_score_bps,
            "observed_reorg_liquidity_risk_score_bps",
        )?;
        ...
        let attestation_id = stable_id(
            "dandelion-reorg-claim-liquidity-firewall-pq-attestation",
            &[
        ...
        if self.attestations.contains_key(&attestation_id) {
            return Err(format!(
                "Dandelion reorg-phase privacy guard attestation {attestation_id} already exists"
            ));
        }
        self.attestations.insert(
            attestation_id.clone(),
            PqReorgClaimFirewallAttestation {
                ...
                observed_reorg_liquidity_risk_score_bps: request
                    .observed_reorg_liquidity_risk_score_bps,
    ...
    pub fn settle_decision(&mut self, request: SettleDecisionRequest) -> Result<String> {
        ensure_capacity(
            self.decisions.len(),
            MAX_DECISIONS,
            "Dandelion reorg-phase privacy guard decisions",
        )?;
        let signal = self
            .signals
            .get(&request.signal_id)
        ...
        ensure_non_empty(
            &request.replacement_claim_reorg_claim_path_root,
            "replacement_claim_reorg_claim_path_root",
        )?;
        ensure_bps(request.rebate_bps, "rebate_bps")?;
        let privacy_filter_deadline_slot = match request.action {
            ReorgClaimAction::QueueReorgLiquidityFirewallClaimReview
            | ReorgClaimAction::SettleReorgClaim
            | ReorgClaimAction::RotateReorgLiquidityFirewallClaimClaimSettlementCohort => request
                .decided_at_slot
                .saturating_add(policy.reorg_claim_window_slots),
            ReorgClaimAction::QuarantineReorgClaimPath | ReorgClaimAction::EmergencyPause => {
                request
                    .decided_at_slot
                    .saturating_add(policy.reorg_claim_window_slots / 2)
            }
            _ => 0,
        };
        let decision_id = stable_id(
            "dandelion-reorg-claim-liquidity-firewall-decision",
            &[
                HashPart::Str(&request.signal_id),
        ...
        if self.decisions.contains_key(&decision_id) {
            return Err(format!(
                "Dandelion reorg-phase privacy guard decision {decision_id} already exists"
            ));
        }
        self.decisions.insert(
            ...
            ReorgClaimDecision {
                ...
                replacement_claim_reorg_claim_path_root: request
                    .replacement_claim_reorg_claim_path_root,
        ...
        if let Some(signal) = self.signals.get_mut(&request.signal_id) {
            signal.status = match request.action {
                ReorgClaimAction::QueueReorgLiquidityFirewallClaimReview => {
                    SignalStatus::ReorgClaimPathQueued
                }
                ReorgClaimAction::SettleReorgClaim
                | ReorgClaimAction::RotateReorgLiquidityFirewallClaimClaimSettlementCohort => {
                    SignalStatus::ReorgLiquidityFirewalling
                }
                ReorgClaimAction::QuarantineReorgClaimPath | ReorgClaimAction::EmergencyPause => {
                    SignalStatus::Quarantined
                }
                ReorgClaimAction::Release => SignalStatus::Settled,
                ReorgClaimAction::Reject => SignalStatus::Rejected,
                ReorgClaimAction::Expire => SignalStatus::Expired,
                ReorgClaimAction::Observe => SignalStatus::Attested,
            };
        }
        if let Some(cohort) = self.cohorts.get_mut(&signal.cohort_id) {
            match request.action {
                ReorgClaimAction::QueueReorgLiquidityFirewallClaimReview => {
                    cohort.status = CohortStatus::ReorgClaimPathPending;
                    cohort.privacy_filter_deadline_slot = privacy_filter_deadline_slot;
                }
                ReorgClaimAction::SettleReorgClaim => {
                    cohort.status = CohortStatus::ReorgLiquidityFirewalling;
                    cohort.pre_reorg_claim_reserve_micro_units = cohort
                        .pre_reorg_claim_reserve_micro_units
                        .saturating_sub(request.privacy_filtered_micro_units);
                    cohort.reorg_lane_claim_reserve_micro_units = cohort
                        .reorg_lane_claim_reserve_micro_units
                        .saturating_add(request.privacy_filtered_micro_units);
                    cohort.privacy_filter_deadline_slot = privacy_filter_deadline_slot;
                    self.counters.reorg_filters_settled =
                        self.counters.reorg_filters_settled.saturating_add(1);
                ...
                ReorgClaimAction::QuarantineReorgClaimPath => {
                    cohort.status = CohortStatus::Quarantined;
                    cohort.privacy_filter_deadline_slot = privacy_filter_deadline_slot;
                    self.counters.claim_reorg_claim_paths_quarantined = self
                        .counters
                        .claim_reorg_claim_paths_quarantined
                        .saturating_add(1);
                }
                ReorgClaimAction::EmergencyPause => {
    ...
    pub fn publish_rebate(&mut self, request: PublishRebateRequest) -> Result<String> {
        ensure_capacity(
            self.rebates.len(),
            MAX_REBATES,
            "Dandelion reorg-phase privacy guard rebates",
        )?;
        let decision = self.decisions.get(&request.decision_id).ok_or_else(|| {
            format!(
                "unknown Dandelion reorg-phase privacy guard decision {}",
                request.decision_id
            )
        })?;
        ...
        let rebate_id = stable_id(
            "dandelion-reorg-claim-liquidity-firewall-low-fee-rebate",
            &[
        ...
        if self.rebates.contains_key(&rebate_id) {
            return Err(format!(
                "Dandelion reorg-phase privacy guard rebate {rebate_id} already exists"
            ));
        }
        self.rebates.insert(
            rebate_id.clone(),
            LowFeeReorgClaimFirewallRebate {
    ...
    pub fn publish_redaction_budget(
        ...
        ensure_capacity(
            ...
            "Dandelion reorg-phase privacy guard redaction budgets",
        )?;
        ...
        let budget_id = stable_id(
            "dandelion-reorg-claim-liquidity-firewall-redaction-budget",
            &[
                HashPart::Str(request.scope.as_str()),
                HashPart::Str(&request.cohort_id),
        ...
        if self.redaction_budgets.contains_key(&budget_id) {
            return Err(format!(
                "Dandelion reorg-phase privacy guard redaction budget {budget_id} already exists"
            ));
        }
    ...
    pub fn publish_operator_summary(
        ...
        ensure_capacity(
            ...
            "Dandelion reorg-phase privacy guard operator summaries",
        )?;
        self.refresh_live_counts();
        ...
        let summary_id = stable_id(
            "dandelion-reorg-claim-liquidity-firewall-operator-summary",
            &[
                HashPart::U64(epoch),
                HashPart::U64(slot),
    ...
    fn refresh_roots(&mut self) {
        ...
        self.roots.state_root = merkle_root(
            "bridge-dandelion-reorg-claim-liquidity-firewall:state",
            &[
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "reorg_claim_liquidity_firewall_suite": self.config.reorg_claim_liquidity_firewall_suite,
            ...
            "reorg_claim_window_slots": self.config.reorg_claim_window_slots,
            ...
            "policies": self.policies.values().map(|policy| json!({
                ...
                "min_pre_reorg_claim_reserve_micro_units": policy.min_pre_reorg_claim_reserve_micro_units,
                "min_reorg_lane_claim_reserve_micro_units": policy.min_reorg_lane_claim_reserve_micro_units,
            ...
            "cohorts": self.cohorts.values().map(|cohort| json!({
                ...
                "pre_reorg_claim_reserve_micro_units": cohort.pre_reorg_claim_reserve_micro_units,
                "reorg_lane_claim_reserve_micro_units": cohort.reorg_lane_claim_reserve_micro_units,
            ...
            "signals": self.signals.values().map(|signal| json!({
                ...
                "reorg_liquidity_risk_score_bps": signal.reorg_liquidity_risk_score_bps,
...
pub fn demo() -> State {
    ...
    let signal = state
        .submit_signal(SubmitSignalRequest {
            cohort_id,
            kind: SignalKind::ReorgLiquidityGraphLinkagePressure,
            signal_commitment_root: devnet_root("signal", "demo-viewtag-pressure"),
            observed_claim_reorg_claim_path_root: devnet_root(
                "observed-reorg_claim_path",
                "demo-viewtag-pressure",
            ),
            reorg_liquidity_pressure_root: devnet_root("pressure", "demo-viewtag-pressure"),
            reporter_commitment: "reporter:demo-subaddress-watchtower".to_string(),
            pre_reorg_claim_reserve_delta_micro_units: -10_000_000,
            reorg_lane_claim_reserve_delta_micro_units: 8_000_000,
            privacy_set_size: 524_288,
            reorg_liquidity_risk_score_bps: 6_200,
    ...
    state
        .submit_attestation(SubmitAttestationRequest {
            signal_id: signal,
            verdict: AttestationVerdict::RequireReorgClaimCoverage,
            attestor_commitment: "attestor:demo-dandelion-reorg-claim-liquidity-firewall"
                .to_string(),
            ...
            pq_security_bits: 256,
            observed_reorg_liquidity_risk_score_bps: 6_400,
            observed_privacy_set_size: 524_288,
...
fn object_root<T: Serialize>(domain: &str, value: &T) -> String {
    ...
    domain_hash(
        &format!("bridge-dandelion-reorg-claim-liquidity-firewall:{domain}"),
        &[HashPart::Json(&value)],
        32,
    )
}
...
fn devnet_root(domain: &str, label: &str) -> String {
    domain_hash(
        &format!("bridge-dandelion-reorg-claim-liquidity-firewall:devnet:{domain}"),
