...
pub type PrivateL2LowFeePqConfidentialCrossBatchFeeSmoothingVaultRuntimeResult<T> = Result<T>;
pub type Runtime = State;

pub const PRIVATE_L2_LOW_FEE_PQ_CONFIDENTIAL_CROSS_BATCH_FEE_SMOOTHING_VAULT_RUNTIME_PROTOCOL_VERSION: &str =
    "nebula-private-l2-low-fee-pq-confidential-cross-batch-fee-smoothing-vault-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_LOW_FEE_PQ_CONFIDENTIAL_CROSS_BATCH_FEE_SMOOTHING_VAULT_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const CONFIDENTIAL_BUCKET_SUITE: &str =
    "ml-kem-1024-sealed-cross-batch-fee-smoothing-buckets-v1";
pub const PQ_VERIFIER_ELIGIBILITY_SUITE: &str =
    "ml-dsa-87+slh-dsa-shake-256f-cross-batch-fee-smoothing-eligibility-v1";
pub const FEE_SMOOTHING_SUITE: &str = "low-fee-confidential-cross-batch-da-proof-fee-smoothing-v1";
pub const PRIVACY_BOUNDARY: &str =
    "roots_only_no_bucket_plaintexts_batch_payloads_view_keys_addresses_or_secret_keys";
pub const DEVNET_L2_HEIGHT: u64 = 4_580_000;
pub const DEVNET_EPOCH: u64 = 14_656;
pub const DEFAULT_FEE_ASSET_ID: &str = "piconero-devnet";
pub const DEFAULT_SMOOTHING_ASSET_ID: &str = "cross-batch-fee-smoothing-credit-devnet";
pub const DEFAULT_MIN_PQ_SECURITY_BITS: u16 = 256;
pub const DEFAULT_MIN_PRIVACY_SET_SIZE: u64 = 131_072;
pub const DEFAULT_TARGET_PRIVACY_SET_SIZE: u64 = 1_048_576;
pub const DEFAULT_BASE_PROOF_FEE_MICRO_UNITS: u64 = 4_500;
pub const DEFAULT_BASE_DA_FEE_MICRO_UNITS: u64 = 6_500;
pub const DEFAULT_TARGET_USER_FEE_BPS: u64 = 12;
pub const DEFAULT_MAX_USER_FEE_BPS: u64 = 25;
pub const DEFAULT_MAX_SMOOTHING_BPS: u64 = 90;
pub const DEFAULT_BUCKET_TTL_SLOTS: u64 = 8_192;
pub const DEFAULT_ELIGIBILITY_TTL_SLOTS: u64 = 2_048;
pub const DEFAULT_SMOOTHING_WINDOW_BATCHES: u64 = 16;
pub const MAX_BPS: u64 = 10_000;

const D_STATE: &str = "PL2-LOW-FEE-PQ-CONF-CROSS-BATCH-FEE-SMOOTHING-VAULT:STATE";
const D_CONFIG: &str = "PL2-LOW-FEE-PQ-CONF-CROSS-BATCH-FEE-SMOOTHING-VAULT:CONFIG";
const D_COUNTERS: &str = "PL2-LOW-FEE-PQ-CONF-CROSS-BATCH-FEE-SMOOTHING-VAULT:COUNTERS";
const D_ROOTS: &str = "PL2-LOW-FEE-PQ-CONF-CROSS-BATCH-FEE-SMOOTHING-VAULT:ROOTS";
const D_VAULTS: &str = "PL2-LOW-FEE-PQ-CONF-CROSS-BATCH-FEE-SMOOTHING-VAULT:VAULTS";
const D_BUCKETS: &str = "PL2-LOW-FEE-PQ-CONF-CROSS-BATCH-FEE-SMOOTHING-VAULT:BUCKETS";
const D_ELIGIBILITY: &str = "PL2-LOW-FEE-PQ-CONF-CROSS-BATCH-FEE-SMOOTHING-VAULT:ELIGIBILITY";
const D_CLAIMS: &str = "PL2-LOW-FEE-PQ-CONF-CROSS-BATCH-FEE-SMOOTHING-VAULT:CLAIMS";
const D_SETTLEMENTS: &str = "PL2-LOW-FEE-PQ-CONF-CROSS-BATCH-FEE-SMOOTHING-VAULT:SETTLEMENTS";
const D_NULLIFIERS: &str = "PL2-LOW-FEE-PQ-CONF-CROSS-BATCH-FEE-SMOOTHING-VAULT:NULLIFIERS";
const D_EVENTS: &str = "PL2-LOW-FEE-PQ-CONF-CROSS-BATCH-FEE-SMOOTHING-VAULT:EVENTS";

#[derive(Clone, Copy, Debug, Deserialize, Eq, Ord, PartialEq, PartialOrd, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum SmoothingLane {
    MicroBatch,
    StandardBatch,
    ProofHeavy,
    DaHeavy,
...
impl VaultStatus {
    ...
    pub fn accepts_claims(self) -> bool {
        matches!(self, Self::Open | Self::Smoothing | Self::Draining)
    }
}
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            smoothing_asset_id: DEFAULT_SMOOTHING_ASSET_ID.to_string(),
            ...
            max_smoothing_bps: DEFAULT_MAX_SMOOTHING_BPS,
...
impl Config {
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "confidential_bucket_suite": CONFIDENTIAL_BUCKET_SUITE,
            "pq_verifier_eligibility_suite": PQ_VERIFIER_ELIGIBILITY_SUITE,
            "fee_smoothing_suite": FEE_SMOOTHING_SUITE,
            "privacy_boundary": PRIVACY_BOUNDARY,
            "fee_asset_id": self.fee_asset_id,
            "smoothing_asset_id": self.smoothing_asset_id,
            ...
            "max_smoothing_bps": self.max_smoothing_bps,
...
pub struct Counters {
    ...
    pub batch_fee_claims: u64,
    pub smoothed_fee_settlements: u64,
    pub nullifiers: u64,
    pub public_events: u64,
    pub deposited_smoothing_micro_units: u64,
    pub reserved_smoothing_micro_units: u64,
    pub settled_smoothing_micro_units: u64,
...
pub struct Roots {
    ...
    pub batch_fee_claim_root: String,
    pub smoothed_fee_settlement_root: String,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct SmoothingVault {
    pub vault_id: String,
    pub sponsor_commitment: String,
    pub lane: SmoothingLane,
    pub sealed_vault_root: String,
    pub total_smoothing_micro_units: u64,
    pub available_smoothing_micro_units: u64,
...
impl SmoothingVault {
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "total_smoothing_micro_units": self.total_smoothing_micro_units,
            "available_smoothing_micro_units": self.available_smoothing_micro_units,
...
pub struct BatchFeeClaim {
    ...
    pub da_bytes: u64,
    pub requested_smoothing_micro_units: u64,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct SmoothedFeeSettlement {
    ...
    pub smoothing_credit_micro_units: u64,
    pub smoothing_window_batches: u64,
    pub slot: u64,
}

impl SmoothedFeeSettlement {
    pub fn public_record(&self) -> Value {
        json!(self)
    }
}
...
pub struct State {
    ...
    pub roots: Roots,
    pub vaults: BTreeMap<String, SmoothingVault>,
    ...
    pub batch_fee_claims: BTreeMap<String, BatchFeeClaim>,
    pub smoothed_fee_settlements: BTreeMap<String, SmoothedFeeSettlement>,
...
impl Default for State {
    fn default() -> Self {
        let mut state = Self {
            ...
            batch_fee_claims: BTreeMap::new(),
            smoothed_fee_settlements: BTreeMap::new(),
...
impl State {
    ...
    pub fn open_vault(
        ...
        smoothing_micro_units: u64,
        privacy_set_size: u64,
        opened_slot: u64,
    ) -> Result<String> {
        if smoothing_micro_units == 0 {
            return Err("smoothing vault requires positive fee smoothing liquidity".to_string());
        }
        if privacy_set_size < self.config.min_privacy_set_size {
            return Err("smoothing vault privacy set below configured floor".to_string());
        }
        ...
        let vault = SmoothingVault {
            vault_id: vault_id.clone(),
            sponsor_commitment,
            lane,
            sealed_vault_root,
            total_smoothing_micro_units: smoothing_micro_units,
            available_smoothing_micro_units: smoothing_micro_units,
            privacy_set_size,
            opened_slot,
        ...
        self.counters.deposited_smoothing_micro_units = self
            .counters
            .deposited_smoothing_micro_units
            .saturating_add(smoothing_micro_units);
        self.record_event("cross_batch_fee_smoothing_vault_opened", &vault_id);
    ...
    pub fn commit_confidential_fee_bucket(
        ...
        if !self.vaults.contains_key(&vault_id) {
            return Err("unknown smoothing vault".to_string());
        }
        if capacity_micro_units == 0 {
            return Err("confidential fee bucket requires positive capacity".to_string());
        }
        if min_batch_index > max_batch_index {
            return Err("confidential fee bucket batch range is invalid".to_string());
        }
        ...
        self.record_event("confidential_fee_bucket_committed", &bucket_id);
        self.recompute_roots();
        Ok(bucket_id)
    }
    ...
    pub fn record_pq_verifier_eligibility(
        ...
        let vault_id = vault_id.into();
        if !self.vaults.contains_key(&vault_id) {
            return Err("unknown smoothing vault for PQ verifier eligibility".to_string());
        }
        ...
        self.record_event("pq_verifier_eligibility_recorded", &eligibility_id);
        self.recompute_roots();
        Ok(eligibility_id)
    }

    pub fn claim_batch_fee_smoothing(
        &mut self,
        vault_id: impl Into<String>,
        bucket_id: impl Into<String>,
        eligibility_id: impl Into<String>,
        batch_commitment_root: impl Into<String>,
        claim_nullifier: impl Into<String>,
        batch_index: u64,
        proof_work_units: u64,
        da_bytes: u64,
        requested_smoothing_micro_units: u64,
        slot: u64,
    ) -> Result<String> {
        let vault_id = vault_id.into();
        ...
        if eligibility.vault_id != vault_id {
            return Err("PQ verifier eligibility is not bound to smoothing vault".to_string());
        }
        ...
        if bucket.vault_id != vault_id {
            return Err("confidential fee bucket is not bound to smoothing vault".to_string());
        }
        if batch_index < bucket.min_batch_index || batch_index > bucket.max_batch_index {
            return Err("batch index is outside confidential fee bucket range".to_string());
        }
        if requested_smoothing_micro_units == 0
            || requested_smoothing_micro_units > bucket.capacity_micro_units
        {
            return Err("requested smoothing exceeds confidential bucket capacity".to_string());
        }
        if proof_work_units == 0 || da_bytes == 0 {
            return Err("batch fee smoothing claim requires proof work and DA bytes".to_string());
        }
        let vault = self
            .vaults
            .get_mut(&vault_id)
            .ok_or_else(|| "unknown smoothing vault".to_string())?;
        if !vault.status.accepts_claims() {
            return Err("smoothing vault is not accepting claims".to_string());
        }
        if slot > vault.expires_slot {
            vault.status = VaultStatus::Expired;
            return Err("smoothing vault has expired".to_string());
        }
        if requested_smoothing_micro_units > vault.available_smoothing_micro_units {
            return Err("insufficient smoothing vault liquidity".to_string());
        }
        let gross_fee = estimated_fee(&self.config, vault.lane, proof_work_units, da_bytes);
        let max_smoothing = gross_fee
            .saturating_mul(self.config.max_smoothing_bps)
            .saturating_div(MAX_BPS);
        if requested_smoothing_micro_units > max_smoothing {
            return Err("requested smoothing exceeds configured gross fee bound".to_string());
        }
        let max_user_fee_micro_units = gross_fee
            .saturating_mul(self.config.max_user_fee_bps)
            .saturating_div(MAX_BPS);
        vault.available_smoothing_micro_units = vault
            .available_smoothing_micro_units
            .saturating_sub(requested_smoothing_micro_units);
        vault.status = if vault.available_smoothing_micro_units == 0 {
            VaultStatus::Exhausted
        } else {
            VaultStatus::Smoothing
        };
        let batch_commitment_root = batch_commitment_root.into();
        ...
        let claim = BatchFeeClaim {
            ...
            proof_work_units,
            da_bytes,
            requested_smoothing_micro_units,
            max_user_fee_micro_units,
            slot,
        };
        self.nullifiers.insert(claim_nullifier);
        self.counters.reserved_smoothing_micro_units = self
            .counters
            .reserved_smoothing_micro_units
            .saturating_add(requested_smoothing_micro_units);
        self.batch_fee_claims.insert(claim_id.clone(), claim);
        self.record_event("batch_fee_smoothing_claimed", &claim_id);
        self.recompute_roots();
        Ok(claim_id)
    }

    pub fn settle_smoothed_fee(
        &mut self,
        ...
        let claim = self
            .batch_fee_claims
            .get(&claim_id)
            .ok_or_else(|| "unknown batch fee smoothing claim".to_string())?;
        let vault = self
            .vaults
            .get(&claim.vault_id)
            .ok_or_else(|| "unknown smoothing vault for settlement".to_string())?;
        let proof_fee = smoothed_proof_fee(&self.config, vault.lane, claim.proof_work_units);
        let da_fee = smoothed_da_fee(&self.config, vault.lane, claim.da_bytes);
        ...
        if user_fee_micro_units > claim.max_user_fee_micro_units {
            return Err("user fee exceeds low-fee smoothing cap".to_string());
        }
        let smoothing_credit = gross_fee
            .saturating_sub(user_fee_micro_units.max(target_user_fee))
            .min(claim.requested_smoothing_micro_units);
        let settlement_id = deterministic_id(
            "smoothed-fee-settlement",
            &[&claim_id, &settlement_nullifier, &slot.to_string()],
        );
        let settlement = SmoothedFeeSettlement {
            ...
            smoothing_credit_micro_units: smoothing_credit,
            smoothing_window_batches: self.config.smoothing_window_batches,
            slot,
        };
        self.nullifiers.insert(settlement_nullifier);
        self.counters.settled_smoothing_micro_units = self
            .counters
            .settled_smoothing_micro_units
            .saturating_add(smoothing_credit);
        ...
        if let Some(vault) = self.vaults.get_mut(&settlement.vault_id) {
            if vault.status == VaultStatus::Smoothing {
                vault.status = VaultStatus::Draining;
            }
        }
        self.smoothed_fee_settlements
            .insert(settlement_id.clone(), settlement);
        self.record_event("smoothed_fee_settled", &settlement_id);
        self.recompute_roots();
        Ok(settlement_id)
    }
    ...
    pub fn recompute_roots(&mut self) {
        ...
        self.counters.batch_fee_claims = self.batch_fee_claims.len() as u64;
        self.counters.smoothed_fee_settlements = self.smoothed_fee_settlements.len() as u64;
        ...
        self.roots.vault_root = map_root(D_VAULTS, &self.vaults, SmoothingVault::public_record);
        ...
        self.roots.batch_fee_claim_root = map_root(
            D_CLAIMS,
            &self.batch_fee_claims,
            BatchFeeClaim::public_record,
        );
        self.roots.smoothed_fee_settlement_root = map_root(
            D_SETTLEMENTS,
            &self.smoothed_fee_settlements,
            SmoothedFeeSettlement::public_record,
        );
    ...
    fn public_record_without_state_root(&self) -> Value {
        json!({
            ...
            "vaults": public_map(&self.vaults, SmoothingVault::public_record),
            ...
            "batch_fee_claims": public_map(&self.batch_fee_claims, BatchFeeClaim::public_record),
            "smoothed_fee_settlements": public_map(&self.smoothed_fee_settlements, SmoothedFeeSettlement::public_record),
...
pub fn devnet() -> State {
    ...
    let vault = state
        .open_vault(
            "sponsor_commitment:devnet-cross-batch-fee-smoothing-vault",
            SmoothingLane::StandardBatch,
            demo_root("sealed-smoothing-vault"),
            40_000_000,
            DEFAULT_TARGET_PRIVACY_SET_SIZE,
            DEVNET_EPOCH,
        )
        .expect("devnet smoothing vault opens");
    state
        .commit_confidential_fee_bucket(
            vault.clone(),
            demo_root("encrypted-fee-bucket"),
            demo_root("fee-bucket-commitments"),
            "nullifier:devnet-fee-bucket",
            20_000_000,
            0,
            DEFAULT_SMOOTHING_WINDOW_BATCHES - 1,
            DEVNET_EPOCH,
        )
        .expect("devnet confidential fee bucket commits");
    state
        .record_pq_verifier_eligibility(
            vault,
            "verifier_commitment:devnet-cross-batch-verifier",
            "pq_key_commitment:devnet-cross-batch-verifier-ml-dsa-87",
            "pq_signature_commitment:devnet-cross-batch-verifier",
...
pub fn demo() -> State {
    ...
    let vault_id = state
        ...
        .next()
        .cloned()
        .expect("demo has smoothing vault");
    let bucket_id = state
        .confidential_fee_buckets
        .keys()
        .next()
    ...
    let claim = state
        .claim_batch_fee_smoothing(
            vault_id,
            bucket_id,
            eligibility_id,
            demo_root("batch-commitment"),
            "nullifier:devnet-batch-fee-smoothing-claim",
            7,
            11,
            262_144,
            18_000,
        ...
        .expect("demo batch fee smoothing claim succeeds");
    state
        .settle_smoothed_fee(
            claim,
            "nullifier:devnet-smoothed-fee-settlement",
            13,
            DEVNET_EPOCH + 6,
        )
        .expect("demo smoothed fee settlement succeeds");
    state.recompute_roots();
    state
}
...
fn deterministic_leaf(domain: &str, parts: &[&str]) -> String {
    let hash_parts = parts
        ...
        .collect::<Vec<_>>();
    domain_hash(
        &format!("PRIVATE-L2-LOW-FEE-PQ-CONFIDENTIAL-CROSS-BATCH-FEE-SMOOTHING-VAULT:{domain}"),
        &hash_parts,
        32,
    )
}
...
fn smoothed_proof_fee(config: &Config, lane: SmoothingLane, proof_work_units: u64) -> u64 {
    config
...
fn smoothed_da_fee(config: &Config, lane: SmoothingLane, da_bytes: u64) -> u64 {
...
fn redacted_commitment(commitment: &str) -> String {
    ...
    let digest = domain_hash(
        "PRIVATE-L2-LOW-FEE-PQ-CONFIDENTIAL-CROSS-BATCH-FEE-SMOOTHING-VAULT:REDACTED-COMMITMENT",
