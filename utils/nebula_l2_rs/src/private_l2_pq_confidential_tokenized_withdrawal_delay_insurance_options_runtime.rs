...
pub type PrivateL2PqConfidentialTokenizedWithdrawalDelayInsuranceOptionsRuntimeResult<T> =
    Result<T>;
pub type Runtime = State;

pub const PRIVATE_L2_PQ_CONFIDENTIAL_TOKENIZED_WITHDRAWAL_DELAY_INSURANCE_OPTIONS_RUNTIME_PROTOCOL_VERSION:
    &str = "nebula-private-l2-pq-confidential-tokenized-withdrawal-delay-insurance-options-runtime-v1";
pub const PROTOCOL_VERSION: &str =
    PRIVATE_L2_PQ_CONFIDENTIAL_TOKENIZED_WITHDRAWAL_DELAY_INSURANCE_OPTIONS_RUNTIME_PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const PQ_CLAIM_COUPON_SUITE: &str =
    "ML-KEM-1024+ML-DSA-87+SLH-DSA-SHAKE-256f-withdrawal-delay-claim-coupon-v1";
pub const SEALED_TRANCHE_SUITE: &str =
    "sealed-confidential-withdrawal-delay-option-tranche-root-v1";
...
pub const DELAYED_EXIT_SETTLEMENT_SUITE: &str = "delayed-exit-payout-settlement-root-v1";
pub const COLLATERAL_ROOT_SUITE: &str = "confidential-withdrawal-delay-collateral-root-v1";
pub const LOW_FEE_BATCH_SUITE: &str = "low-fee-exercise-batch-root-v1";
pub const ANTI_REPLAY_NULLIFIER_SUITE: &str = "anti-replay-withdrawal-delay-nullifier-root-v1";
pub const PUBLIC_RECORD_SUITE: &str =
    "roots-only-withdrawal-delay-insurance-options-public-record-v1";
pub const STATE_ROOT_SUITE: &str = "withdrawal-delay-insurance-options-state-root-v1";
pub const DEVNET_REPLAY_DOMAIN: &str =
    "nebula-private-l2-pq-withdrawal-delay-insurance-options-devnet";
pub const DEVNET_MARKET_ID: &str = "private-l2-pq-withdrawal-delay-insurance-options-devnet";
...
pub const DEFAULT_MIN_DELAY_BLOCKS: u64 = 12;
pub const DEFAULT_MAX_DELAY_BLOCKS: u64 = 2_880;
...
impl OptionKind {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::DelayPut => "delay_put",
            Self::DelayCall => "delay_call",
            Self::BinaryDelay => "binary_delay",
            Self::QueueJumpRebate => "queue_jump_rebate",
            Self::LiquidityBackstop => "liquidity_backstop",
            Self::CatastrophicDelay => "catastrophic_delay",
        }
    }
}
...
impl Config {
    pub fn devnet() -> Self {
        Self {
            ...
            min_delay_blocks: DEFAULT_MIN_DELAY_BLOCKS,
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.min_delay_blocks > self.max_delay_blocks {
            return Err("min_delay_blocks exceeds max_delay_blocks".to_string());
        }
...
pub struct PqClaimCoupon {
    ...
    pub delay_evidence_root: String,
...
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct DelayedExitPayoutSettlement {
    pub settlement_id: String,
    pub coupon_id: String,
    pub batch_id: String,
    pub settlement_nullifier: String,
    pub exit_queue_root: String,
    pub delay_attestation_root: String,
...
impl DelayedExitPayoutSettlement {
    pub fn public_record(&self) -> Value {
        public_record_for(self)
    }
    pub fn commitment_root(&self) -> String {
        root_from_record("DelayedExitPayoutSettlement", &self.public_record())
    }
}
...
pub struct State {
    ...
    pub settlements: BTreeMap<String, DelayedExitPayoutSettlement>,
...
impl State {
    ...
    pub fn devnet() -> Self {
        ...
        let collateral = CollateralSnapshot {
            ...
            vault_commitment: "vault:withdrawal-delay-insurance:devnet".to_string(),
        ...
        let curve = PremiumCurveCommitment {
            ...
            tranche_id: "devnet-delay-put-tranche".to_string(),
            ...
            delay_bucket_root: root_from_parts("devnet-delay", &["12", "144", "720"]),
            premium_asset_id: DEVNET_PREMIUM_ASSET_ID.to_string(),
        ...
        let tranche = SealTrancheInput {
            series_commitment: "series:withdrawal-delay:devnet".to_string(),
            option_kind: OptionKind::DelayPut,
            exercise_style: ExerciseStyle::Bermudan,
            notional_commitment: commitment("notional", 100),
            strike_delay_blocks: 144,
            max_delay_blocks: 720,
            ...
            terms_root: root_from_parts("devnet-terms", &["delay-put"]),
    ...
    pub fn seal_option_tranche(&mut self, input: SealTrancheInput) -> Result<SealedOptionTranche> {
        ...
        let record = SealedOptionTranche {
            ...
            strike_delay_blocks: input.strike_delay_blocks,
            max_delay_blocks: input.max_delay_blocks,
    ...
    pub fn admit_pq_claim_coupon(&mut self, input: AdmitCouponInput) -> Result<PqClaimCoupon> {
        ...
        let record = PqClaimCoupon {
            ...
            delay_evidence_root: input.delay_evidence_root,
    ...
    pub fn open_low_fee_batch(
        ...
        let record = LowFeeExerciseBatch {
            ...
            settlement_root: merkle_root(DELAYED_EXIT_SETTLEMENT_SUITE, &[]),
            fee_pool_commitment,
            rebate_root,
    ...
    pub fn queue_delayed_exit_settlement(
        &mut self,
        input: SettlementInput,
    ) -> Result<DelayedExitPayoutSettlement> {
    ...
    pub fn settle_delayed_exit(&mut self, settlement_id: &str) -> Result<()> {
        let settlement = self
            .settlements
    ...
    fn validate_tranche_input(&self, input: &SealTrancheInput) -> Result<()> {
        ...
        if input.strike_delay_blocks < self.config.min_delay_blocks
            || input.max_delay_blocks > self.config.max_delay_blocks
            || input.strike_delay_blocks > input.max_delay_blocks
        {
            return Err("delay bounds invalid".to_string());
        }
    ...
    fn validate_settlement_input(&self, input: &SettlementInput) -> Result<()> {
        ...
        if input.settle_after_l2_height < self.config.l2_height + self.config.min_delay_blocks {
            return Err("settlement delay too short".to_string());
        }
    ...
    fn compute_roots(&self) -> Roots {
        ...
        let settlement_root = map_root(DELAYED_EXIT_SETTLEMENT_SUITE, &self.settlements);
...
impl RuntimePublicRecord for DelayedExitPayoutSettlement {
    fn public_record(&self) -> Value {
        DelayedExitPayoutSettlement::public_record(self)
    }
}
...
pub fn surface_markers() -> Vec<RuntimeSurfaceMarker> {
    vec![
        ...
        RuntimeSurfaceMarker {
            name: "delayed_exit_payout_settlement".to_string(),
            category: "withdrawal_delay".to_string(),
            root_suite: DELAYED_EXIT_SETTLEMENT_SUITE.to_string(),
            privacy_note: "commitments-and-roots-only".to_string(),
        },
        RuntimeSurfaceMarker {
...
pub fn withdrawal_delay_policy_checkpoint_001(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_001";
    json!({
        "marker": marker,
        ...
        "coupon_root": state.roots().coupon_root,
        "settlement_root": state.roots().settlement_root,
        "nullifier_root": state.roots().nullifier_root,
    })
}

pub fn withdrawal_delay_policy_checkpoint_002(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_002";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_003(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_003";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_004(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_004";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_005(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_005";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_006(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_006";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_007(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_007";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_008(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_008";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_009(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_009";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_010(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_010";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_011(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_011";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_012(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_012";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_013(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_013";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_014(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_014";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_015(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_015";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_016(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_016";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_017(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_017";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_018(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_018";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_019(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_019";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_020(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_020";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_021(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_021";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_022(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_022";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_023(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_023";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_024(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_024";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_025(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_025";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_026(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_026";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
...
pub fn withdrawal_delay_policy_checkpoint_027(state: &State) -> Value {
    let marker = "withdrawal_delay_policy_checkpoint_027";
    json!({
        "marker": marker,
        "protocol_version": PROTOCOL_VERSION,
        "roots_only": true,
