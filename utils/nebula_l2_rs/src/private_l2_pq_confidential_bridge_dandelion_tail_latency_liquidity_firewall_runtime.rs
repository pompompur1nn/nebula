...
pub type PrivateL2PqConfidentialBridgeDandelionTailLatencyLiquidityFirewallRuntimeResult<T> =
    Result<T>;
pub type Runtime = State;

pub const PROTOCOL_VERSION: &str =
    "nebula-private-l2-pq-confidential-bridge-dandelion-tail-latency-liquidity-firewall-runtime-v1";
pub const PRIVATE_L2_PQ_CONFIDENTIAL_BRIDGE_DANDELION_TAIL_LATENCY_LIQUIDITY_FIREWALL_RUNTIME_PROTOCOL_VERSION:
    &str = PROTOCOL_VERSION;
pub const SCHEMA_VERSION: u64 = 1;
pub const HASH_SUITE: &str = "SHAKE256-domain-separated-canonical-json";
pub const PQ_DANDELION_TAIL_LATENCY_LIQUIDITY_FIREWALL_SUITE: &str =
    "ML-DSA-87+ML-KEM-1024+SLH-DSA-SHAKE-256f";
pub const DANDELION_TAIL_LATENCY_LIQUIDITY_FIREWALL_SUITE: &str =
    "monero-bridge-dandelion-tail-latency-liquidity-firewall-root-v1";
pub const REBATE_SUITE: &str = "dandelion-tail-latency-liquidity-firewall-low-fee-rebate-root-v1";
pub const REDACTION_SUITE: &str =
    "operator-safe-dandelion-tail-latency-liquidity-firewall-redaction-root-v1";
...
pub const DEFAULT_DANDELION_TAIL_LATENCY_WINDOW_SLOTS: u64 = 720;
...
pub enum DandelionTailLatencyScope {
    DepositSubaddressLane,
    WithdrawalSubaddressLane,
    FastDandelionTailLatencyFirewallClaimLane,
    MerchantPaymentSubaddressLane,
    AtomicSwapSubaddressLane,
    ReserveMirror,
    EmergencyDandelionTailLatencyLiquidityFirewall,
    WatchtowerRecovery,
}

impl DandelionTailLatencyScope {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::DepositSubaddressLane => "deposit_subaddress_lane",
            Self::WithdrawalSubaddressLane => "withdrawal_subaddress_lane",
            Self::FastDandelionTailLatencyFirewallClaimLane => "fast_dandelion_tail_latency_lane",
            ...
            Self::EmergencyDandelionTailLatencyLiquidityFirewall => {
                "emergency_dandelion_tail_latency_liquidity_firewall"
            }
...
pub enum CohortStatus {
    Active,
    TailLatencyPathPending,
    DandelionTailLatencyFirewalling,
    Quarantined,
    Rotating,
    Paused,
    Settled,
...
#[serde(rename_all = "snake_case")]
pub enum SignalKind {
    DandelionTailLatencyLinkagePressure,
    DandelionTailLatencyPathFragmentation,
    DandelionTailLatencyPathSkew,
    DandelionTailLatencyGraphLinkagePressure,
    DandelionTailLatencyTimingCorrelationPressure,
    ReserveMismatch,
    FeeSpikeDandelionTailLatencyFirewallClaimPressure,
    WatchtowerEmergency,
}
...
pub enum AttestationVerdict {
    ConfirmDandelionTailLatencyPressure,
    ConfirmPrivacyRisk,
    RequireDandelionTailLatencyCoverage,
    RequireDandelionTailLatencyCoverageGuard,
    RequireQuarantine,
    ClearFalsePositive,
}
...
#[derive(Clone, Copy, Debug, Deserialize, Eq, Ord, PartialEq, PartialOrd, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum DandelionTailLatencyAction {
    Observe,
    QueueDandelionTailLatencyFirewallClaimReview,
    SettleDandelionTailLatency,
    RotateDandelionTailLatencyFirewallClaimClaimSettlementCohort,
    QuarantineTailLatencyPath,
    EmergencyPause,
    Release,
    Reject,
    Expire,
...
pub struct Config {
    ...
    pub dandelion_tail_latency_liquidity_firewall_suite: String,
    ...
    pub dandelion_tail_latency_window_slots: u64,
    pub signal_window_slots: u64,
    pub max_claim_settlement_fee_bps: u64,
    pub target_rebate_bps: u64,
    pub min_pre_dandelion_tail_latency_reserve_micro_units: u64,
    pub min_dandelion_tail_latency_lane_claim_reserve_micro_units: u64,
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            pq_claim_settlement_insurance_suite: PQ_DANDELION_TAIL_LATENCY_LIQUIDITY_FIREWALL_SUITE
                .to_string(),
            dandelion_tail_latency_liquidity_firewall_suite:
                DANDELION_TAIL_LATENCY_LIQUIDITY_FIREWALL_SUITE.to_string(),
            ...
            dandelion_tail_latency_window_slots: DEFAULT_DANDELION_TAIL_LATENCY_WINDOW_SLOTS,
            signal_window_slots: DEFAULT_SIGNAL_WINDOW_SLOTS,
            max_claim_settlement_fee_bps: DEFAULT_MAX_ROUTE_FEE_BPS,
            target_rebate_bps: DEFAULT_TARGET_REBATE_BPS,
            min_pre_dandelion_tail_latency_reserve_micro_units:
                DEFAULT_MIN_SOURCE_LIQUIDITY_MICRO_UNITS,
            min_dandelion_tail_latency_lane_claim_reserve_micro_units:
...
pub struct Counters {
    ...
    pub dandelion_tail_latency_filters_settled: u64,
    pub cohorts_rotated: u64,
    pub claim_tail_latency_paths_quarantined: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct DandelionTailLatencyPolicy {
    pub policy_id: String,
    pub scope: DandelionTailLatencyScope,
    pub owner_commitment: String,
    pub policy_commitment_root: String,
    pub claim_source_dandelion_tail_latency_set_root: String,
    pub claim_target_dandelion_tail_latency_set_root: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct DandelionTailLatencyLiquidityFirewallCohort {
    ...
    pub pre_dandelion_tail_latency_reserve_micro_units: u64,
    pub dandelion_tail_latency_lane_claim_reserve_micro_units: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct DandelionTailLatencySignal {
    ...
    pub observed_claim_tail_latency_path_root: String,
    pub dandelion_tail_latency_pressure_root: String,
    pub reporter_commitment: String,
    pub pre_dandelion_tail_latency_reserve_delta_micro_units: i128,
    pub dandelion_tail_latency_lane_claim_reserve_delta_micro_units: i128,
    pub privacy_set_size: u64,
    pub dandelion_tail_latency_risk_score_bps: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct PqDandelionTailLatencyFirewallAttestation {
    ...
    pub observed_dandelion_tail_latency_risk_score_bps: u64,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct DandelionTailLatencyDecision {
    pub decision_id: String,
    pub signal_id: String,
    pub cohort_id: String,
    pub policy_id: String,
    pub action: DandelionTailLatencyAction,
    pub decision_root: String,
    pub replacement_claim_tail_latency_path_root: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct LowFeeDandelionTailLatencyFirewallRebate {
...
pub struct State {
    ...
    pub roots: Roots,
    pub policies: BTreeMap<String, DandelionTailLatencyPolicy>,
    pub cohorts: BTreeMap<String, DandelionTailLatencyLiquidityFirewallCohort>,
    pub signals: BTreeMap<String, DandelionTailLatencySignal>,
    pub attestations: BTreeMap<String, PqDandelionTailLatencyFirewallAttestation>,
    pub decisions: BTreeMap<String, DandelionTailLatencyDecision>,
    pub rebates: BTreeMap<String, LowFeeDandelionTailLatencyFirewallRebate>,
...
impl State {
    ...
    pub fn devnet() -> Self {
        ...
        let withdrawal_policy = state
            .register_policy(RegisterPolicyRequest {
                scope: DandelionTailLatencyScope::WithdrawalSubaddressLane,
                owner_commitment: "owner:dandelion-tail-latency-liquidity-firewall-council-a"
                    .to_string(),
                policy_commitment_root: devnet_root(
                    "policy",
                    "withdrawal-dandelion-tail-latency-liquidity-firewall",
                ),
                claim_source_dandelion_tail_latency_set_root: devnet_root(
                    "source-dandelion_tail_tail-latency-set",
                    "withdrawal-alpha",
                ),
                claim_target_dandelion_tail_latency_set_root: devnet_root(
                    "target-dandelion_tail_tail-latency-set",
                    "withdrawal-alpha",
                ),
                claim_rotation_allowlist_root: devnet_root(
                    "tail_latency_path-allowlist",
                    "withdrawal-alpha",
                ),
                min_privacy_set_size: 524_288,
                min_pre_dandelion_tail_latency_reserve_micro_units: 160_000_000,
                min_dandelion_tail_latency_lane_claim_reserve_micro_units: 72_000_000,
                max_claim_settlement_fee_bps: 16,
                min_attestation_quorum_bps: 7_200,
                signal_window_slots: 96,
                dandelion_tail_latency_window_slots: 720,
                redaction_profile: "withdrawal-dandelion-tail-latency-liquidity-firewall-profile"
                    .to_string(),
            })
            .expect("devnet withdrawal policy");
        let fast_dandelion_exit_policy = state
            .register_policy(RegisterPolicyRequest {
                scope: DandelionTailLatencyScope::FastDandelionTailLatencyFirewallClaimLane,
                owner_commitment: "owner:fast-dandelion-decoy-insurance-tail_latency_path-council-b".to_string(),
                policy_commitment_root: devnet_root(
                    "policy",
                    "fast-dandelion-decoy-insurance-dandelion-tail-latency-liquidity-firewall",
                ),
                claim_source_dandelion_tail_latency_set_root: devnet_root(
                    "source-dandelion_tail_tail-latency-set",
                ...
                min_pre_dandelion_tail_latency_reserve_micro_units: 144_000_000,
                min_dandelion_tail_latency_lane_claim_reserve_micro_units: 64_000_000,
                max_claim_settlement_fee_bps: 18,
                min_attestation_quorum_bps: 7_000,
                signal_window_slots: 80,
                dandelion_tail_latency_window_slots: 640,
                redaction_profile:
                    "fast-dandelion-decoy-insurance-dandelion-tail-latency-liquidity-firewall-profile"
                        .to_string(),
            })
            .expect("devnet fast dandelion_tail_latency policy");
        let withdrawal_cohort = state
            .register_cohort(RegisterCohortRequest {
                ...
                pre_dandelion_tail_latency_reserve_micro_units: 216_000_000,
                dandelion_tail_latency_lane_claim_reserve_micro_units: 96_000_000,
        ...
        let fast_dandelion_exit_cohort = state
            .register_cohort(RegisterCohortRequest {
                ...
                bridge_lane_id: "fast-dandelion-decoy-insurance-tail_latency_path-beta".to_string(),
                claim_source_dandelion_tail_latency_set_root: devnet_root(
                    "source-dandelion_tail_tail-latency-set",
                    "cohort-beta",
                ),
                ...
                pre_dandelion_tail_latency_reserve_micro_units: 168_000_000,
                dandelion_tail_latency_lane_claim_reserve_micro_units: 76_000_000,
                privacy_set_size: 393_216,
                claim_settlement_fee_bps: 13,
                public_hint_root: devnet_root("hint", "cohort-beta"),
            })
            .expect("devnet fast dandelion_tail_latency cohort");
        let signal = state
            .submit_signal(SubmitSignalRequest {
                cohort_id: withdrawal_cohort,
                kind: SignalKind::DandelionTailLatencyLinkagePressure,
                signal_commitment_root: devnet_root("signal", "withdrawal-cluster-pressure"),
                observed_claim_tail_latency_path_root: devnet_root(
                    "observed-tail_latency_path",
                    "withdrawal-cluster-pressure",
                ),
                dandelion_tail_latency_pressure_root: devnet_root(
                    "pressure",
                    "withdrawal-cluster-pressure",
                ),
                reporter_commitment: "reporter:dandelion-tail_latency_path-watchtower-11"
                    .to_string(),
                pre_dandelion_tail_latency_reserve_delta_micro_units: -48_000_000,
                dandelion_tail_latency_lane_claim_reserve_delta_micro_units: 24_000_000,
                privacy_set_size: 524_288,
                dandelion_tail_latency_risk_score_bps: 7_300,
        ...
        state
            .submit_attestation(SubmitAttestationRequest {
                ...
                verdict: AttestationVerdict::RequireDandelionTailLatencyCoverageGuard,
                attestor_commitment: "attestor:pq-dandelion-tail-latency-liquidity-firewall-a"
                    .to_string(),
                pq_signature_root: devnet_root(
                    "ml-dsa-signature",
                    "withdrawal-dandelion_tail_latency-a",
                ),
                pq_public_key_root: devnet_root(
                    "ml-kem-public-key",
                    "withdrawal-dandelion_tail_latency-a",
                ),
                transcript_root: devnet_root("transcript", "withdrawal-dandelion_tail_latency-a"),
                quorum_weight_bps: 7_500,
                pq_security_bits: 256,
                observed_dandelion_tail_latency_risk_score_bps: 7_500,
        ...
        state
            .submit_attestation(SubmitAttestationRequest {
                signal_id: signal.clone(),
                verdict: AttestationVerdict::ConfirmDandelionTailLatencyPressure,
                attestor_commitment: "attestor:pq-dandelion-tail-latency-liquidity-firewall-b"
                    .to_string(),
                pq_signature_root: devnet_root(
                    "slh-dsa-signature",
                    "withdrawal-dandelion_tail_latency-b",
                ),
                pq_public_key_root: devnet_root(
                    "ml-kem-public-key",
                    "withdrawal-dandelion_tail_latency-b",
                ),
                transcript_root: devnet_root("transcript", "withdrawal-dandelion_tail_latency-b"),
                quorum_weight_bps: 8_700,
                pq_security_bits: 256,
                observed_dandelion_tail_latency_risk_score_bps: 7_800,
                observed_privacy_set_size: 524_288,
        ...
        let decision = state
            .settle_decision(SettleDecisionRequest {
                signal_id: signal,
                action: DandelionTailLatencyAction::SettleDandelionTailLatency,
                decision_root: devnet_root(
                    "decision",
                    "withdrawal-liquidity-dandelion_tail_latency",
                ),
                replacement_claim_tail_latency_path_root: devnet_root(
                    "replacement-tail_latency_path",
                    "withdrawal-dandelion_tail_latency",
                ),
            ...
            .expect("devnet dandelion_tail_latency decision");
        state
            .publish_rebate(PublishRebateRequest {
                decision_id: decision,
                recipient_commitment:
                    "recipient:dandelion-tail-latency-liquidity-firewall-fee-sponsor".to_string(),
                rebate_micro_units: 22_000,
                fee_saved_micro_units: 88_000,
                rebate_bps: 8,
                settlement_slot: DEVNET_SLOT + 6,
                rebate_root: devnet_root("rebate", "withdrawal-dandelion_tail_latency"),
            })
    ...
    pub fn register_policy(&mut self, request: RegisterPolicyRequest) -> Result<String> {
        ensure_capacity(
            self.policies.len(),
            MAX_POLICIES,
            "Dandelion dandelion_tail_latency-phase privacy guard policies",
        )?;
        ...
        ensure_non_empty(
            &request.claim_source_dandelion_tail_latency_set_root,
            "claim_source_dandelion_tail_latency_set_root",
        )?;
        ensure_non_empty(
            &request.claim_target_dandelion_tail_latency_set_root,
            "claim_target_dandelion_tail_latency_set_root",
        )?;
        ensure_non_empty(
        ...
        if request.min_pre_dandelion_tail_latency_reserve_micro_units
            < self
                .config
                .min_pre_dandelion_tail_latency_reserve_micro_units
        {
        ...
        if request.min_dandelion_tail_latency_lane_claim_reserve_micro_units
            < self
                .config
                .min_dandelion_tail_latency_lane_claim_reserve_micro_units
        {
        ...
        let policy_id = stable_id(
            "dandelion-tail-latency-liquidity-firewall-policy",
            &[
                HashPart::Str(request.scope.as_str()),
                HashPart::Str(&request.owner_commitment),
                HashPart::Str(&request.policy_commitment_root),
                HashPart::Str(&request.claim_source_dandelion_tail_latency_set_root),
            ],
        );
        if self.policies.contains_key(&policy_id) {
            return Err(format!(
                "Dandelion dandelion_tail_latency-phase privacy guard policy {policy_id} already exists"
            ));
        }
        self.policies.insert(
            policy_id.clone(),
            DandelionTailLatencyPolicy {
                policy_id: policy_id.clone(),
                scope: request.scope,
                owner_commitment: request.owner_commitment,
                policy_commitment_root: request.policy_commitment_root,
                claim_source_dandelion_tail_latency_set_root: request
                    .claim_source_dandelion_tail_latency_set_root,
                claim_target_dandelion_tail_latency_set_root: request
                    .claim_target_dandelion_tail_latency_set_root,
                claim_rotation_allowlist_root: request.claim_rotation_allowlist_root,
                min_privacy_set_size: request.min_privacy_set_size,
                min_pre_dandelion_tail_latency_reserve_micro_units: request
                    .min_pre_dandelion_tail_latency_reserve_micro_units,
                min_dandelion_tail_latency_lane_claim_reserve_micro_units: request
                    .min_dandelion_tail_latency_lane_claim_reserve_micro_units,
                ...
                dandelion_tail_latency_window_slots: request.dandelion_tail_latency_window_slots,
                active: true,
    ...
    pub fn register_cohort(&mut self, request: RegisterCohortRequest) -> Result<String> {
        ensure_capacity(
            self.cohorts.len(),
            MAX_COHORTS,
            "Dandelion tail_latency_path privacy cohorts",
        )?;
        let policy = self.policies.get(&request.policy_id).ok_or_else(|| {
            format!(
                "unknown Dandelion dandelion_tail_latency-phase privacy guard policy {}",
                request.policy_id
            )
        })?;
        ...
        if request.claim_settlement_fee_bps > policy.max_claim_settlement_fee_bps {
            return Err("cohort tail_latency_path fee exceeds policy cap".to_string());
        }
        if request.pre_dandelion_tail_latency_reserve_micro_units
            < policy.min_pre_dandelion_tail_latency_reserve_micro_units
        {
            return Err("cohort source liquidity below policy floor".to_string());
        }
        if request.dandelion_tail_latency_lane_claim_reserve_micro_units
            < policy.min_dandelion_tail_latency_lane_claim_reserve_micro_units
        {
        ...
        let cohort_id = stable_id(
            "dandelion-tail-latency-liquidity-firewall-cohort",
            &[
        ...
        if self.cohorts.contains_key(&cohort_id) {
            return Err(format!(
                "Dandelion dandelion_tail_latency-phase privacy guard cohort {cohort_id} already exists"
            ));
        }
        self.cohorts.insert(
            cohort_id.clone(),
            DandelionTailLatencyLiquidityFirewallCohort {
                ...
                pre_dandelion_tail_latency_reserve_micro_units: request
                    .pre_dandelion_tail_latency_reserve_micro_units,
                dandelion_tail_latency_lane_claim_reserve_micro_units: request
                    .dandelion_tail_latency_lane_claim_reserve_micro_units,
    ...
    pub fn submit_signal(&mut self, request: SubmitSignalRequest) -> Result<String> {
        ensure_capacity(
            self.signals.len(),
            MAX_SIGNALS,
            "Dandelion dandelion_tail_latency-phase privacy guard signals",
        )?;
        let cohort = self
            .cohorts
            ...
            .ok_or_else(|| {
                format!(
                    "unknown Dandelion dandelion_tail_latency-phase privacy guard cohort {}",
                    request.cohort_id
                )
            })?
            .clone();
        ...
        ensure_non_empty(
            &request.observed_claim_tail_latency_path_root,
            "observed_claim_tail_latency_path_root",
        )?;
        ensure_non_empty(
            &request.dandelion_tail_latency_pressure_root,
            "dandelion_tail_latency_pressure_root",
        )?;
        ensure_non_empty(&request.reporter_commitment, "reporter_commitment")?;
        ensure_bps(
            request.dandelion_tail_latency_risk_score_bps,
            "dandelion_tail_latency_risk_score_bps",
        )?;
        if request.privacy_set_size < policy.min_privacy_set_size {
            return Err("signal privacy set below policy floor".to_string());
        }
        let signal_id = stable_id(
            "dandelion-tail-latency-liquidity-firewall-signal",
            &[
        ...
        if self.signals.contains_key(&signal_id) {
            return Err(format!(
                "Dandelion dandelion_tail_latency-phase privacy guard signal {signal_id} already exists"
            ));
        }
        ...
        self.signals.insert(
            ...
            DandelionTailLatencySignal {
                ...
                observed_claim_tail_latency_path_root: request
                    .observed_claim_tail_latency_path_root,
                dandelion_tail_latency_pressure_root: request.dandelion_tail_latency_pressure_root,
                reporter_commitment: request.reporter_commitment,
                pre_dandelion_tail_latency_reserve_delta_micro_units: request
                    .pre_dandelion_tail_latency_reserve_delta_micro_units,
                dandelion_tail_latency_lane_claim_reserve_delta_micro_units: request
                    .dandelion_tail_latency_lane_claim_reserve_delta_micro_units,
                privacy_set_size: request.privacy_set_size,
                dandelion_tail_latency_risk_score_bps: request
                    .dandelion_tail_latency_risk_score_bps,
    ...
    pub fn submit_attestation(&mut self, request: SubmitAttestationRequest) -> Result<String> {
        ensure_capacity(
            ...
            MAX_ATTESTATIONS,
            "Dandelion dandelion_tail_latency-phase privacy guard attestations",
        )?;
        let signal = self
            .signals
            ...
            .ok_or_else(|| {
                format!(
                    "unknown Dandelion dandelion_tail_latency-phase privacy guard signal {}",
                    request.signal_id
                )
            })?
            .clone();
        ...
        ensure_bps(
            request.observed_dandelion_tail_latency_risk_score_bps,
            "observed_dandelion_tail_latency_risk_score_bps",
        )?;
        ...
        let attestation_id = stable_id(
            "dandelion-tail-latency-liquidity-firewall-pq-attestation",
            &[
        ...
        if self.attestations.contains_key(&attestation_id) {
            return Err(format!(
                "Dandelion dandelion_tail_latency-phase privacy guard attestation {attestation_id} already exists"
            ));
        }
        self.attestations.insert(
            attestation_id.clone(),
            PqDandelionTailLatencyFirewallAttestation {
                ...
                observed_dandelion_tail_latency_risk_score_bps: request
                    .observed_dandelion_tail_latency_risk_score_bps,
    ...
    pub fn settle_decision(&mut self, request: SettleDecisionRequest) -> Result<String> {
        ensure_capacity(
            self.decisions.len(),
            MAX_DECISIONS,
            "Dandelion dandelion_tail_latency-phase privacy guard decisions",
        )?;
        let signal = self
            .signals
            .get(&request.signal_id)
        ...
        ensure_non_empty(
            &request.replacement_claim_tail_latency_path_root,
            "replacement_claim_tail_latency_path_root",
        )?;
        ensure_bps(request.rebate_bps, "rebate_bps")?;
        let privacy_filter_deadline_slot = match request.action {
            DandelionTailLatencyAction::QueueDandelionTailLatencyFirewallClaimReview
            | DandelionTailLatencyAction::SettleDandelionTailLatency
            | DandelionTailLatencyAction::RotateDandelionTailLatencyFirewallClaimClaimSettlementCohort => request
                .decided_at_slot
                .saturating_add(policy.dandelion_tail_latency_window_slots),
            DandelionTailLatencyAction::QuarantineTailLatencyPath | DandelionTailLatencyAction::EmergencyPause => {
                request
                    .decided_at_slot
                    .saturating_add(policy.dandelion_tail_latency_window_slots / 2)
            }
            _ => 0,
        };
        let decision_id = stable_id(
            "dandelion-tail-latency-liquidity-firewall-decision",
            &[
                HashPart::Str(&request.signal_id),
        ...
        if self.decisions.contains_key(&decision_id) {
            return Err(format!(
                "Dandelion dandelion_tail_latency-phase privacy guard decision {decision_id} already exists"
            ));
        }
        self.decisions.insert(
            decision_id.clone(),
            DandelionTailLatencyDecision {
                ...
                replacement_claim_tail_latency_path_root: request
                    .replacement_claim_tail_latency_path_root,
        ...
        if let Some(signal) = self.signals.get_mut(&request.signal_id) {
            signal.status = match request.action {
                DandelionTailLatencyAction::QueueDandelionTailLatencyFirewallClaimReview => {
                    SignalStatus::TailLatencyPathQueued
                }
                DandelionTailLatencyAction::SettleDandelionTailLatency
                | DandelionTailLatencyAction::RotateDandelionTailLatencyFirewallClaimClaimSettlementCohort => {
                    SignalStatus::DandelionTailLatencyFirewalling
                }
                DandelionTailLatencyAction::QuarantineTailLatencyPath
                | DandelionTailLatencyAction::EmergencyPause => SignalStatus::Quarantined,
                DandelionTailLatencyAction::Release => SignalStatus::Settled,
                DandelionTailLatencyAction::Reject => SignalStatus::Rejected,
                DandelionTailLatencyAction::Expire => SignalStatus::Expired,
                DandelionTailLatencyAction::Observe => SignalStatus::Attested,
            };
        }
        if let Some(cohort) = self.cohorts.get_mut(&signal.cohort_id) {
            match request.action {
                DandelionTailLatencyAction::QueueDandelionTailLatencyFirewallClaimReview => {
                    cohort.status = CohortStatus::TailLatencyPathPending;
                    cohort.privacy_filter_deadline_slot = privacy_filter_deadline_slot;
                }
                DandelionTailLatencyAction::SettleDandelionTailLatency => {
                    cohort.status = CohortStatus::DandelionTailLatencyFirewalling;
                    cohort.pre_dandelion_tail_latency_reserve_micro_units = cohort
                        .pre_dandelion_tail_latency_reserve_micro_units
                        .saturating_sub(request.privacy_filtered_micro_units);
                    cohort.dandelion_tail_latency_lane_claim_reserve_micro_units = cohort
                        .dandelion_tail_latency_lane_claim_reserve_micro_units
                        .saturating_add(request.privacy_filtered_micro_units);
                    cohort.privacy_filter_deadline_slot = privacy_filter_deadline_slot;
                    self.counters.dandelion_tail_latency_filters_settled =
                        self.counters.dandelion_tail_latency_filters_settled.saturating_add(1);
                ...
                DandelionTailLatencyAction::QuarantineTailLatencyPath => {
                    ...
                    cohort.privacy_filter_deadline_slot = privacy_filter_deadline_slot;
                    self.counters.claim_tail_latency_paths_quarantined = self
                        .counters
                        .claim_tail_latency_paths_quarantined
                        .saturating_add(1);
                }
                DandelionTailLatencyAction::EmergencyPause => {
                ...
                DandelionTailLatencyAction::Release => {
    ...
    pub fn publish_rebate(&mut self, request: PublishRebateRequest) -> Result<String> {
        ensure_capacity(
            self.rebates.len(),
            MAX_REBATES,
            "Dandelion dandelion_tail_latency-phase privacy guard rebates",
        )?;
        let decision = self.decisions.get(&request.decision_id).ok_or_else(|| {
            format!(
                "unknown Dandelion dandelion_tail_latency-phase privacy guard decision {}",
                request.decision_id
            )
        })?;
        ...
        let rebate_id = stable_id(
            "dandelion-tail-latency-liquidity-firewall-low-fee-rebate",
            &[
        ...
        if self.rebates.contains_key(&rebate_id) {
            return Err(format!(
                "Dandelion dandelion_tail_latency-phase privacy guard rebate {rebate_id} already exists"
            ));
        }
        self.rebates.insert(
            rebate_id.clone(),
            LowFeeDandelionTailLatencyFirewallRebate {
    ...
    pub fn publish_redaction_budget(
        ...
        ensure_capacity(
            ...
            "Dandelion dandelion_tail_latency-phase privacy guard redaction budgets",
        )?;
        ...
        let budget_id = stable_id(
            "dandelion-tail-latency-liquidity-firewall-redaction-budget",
            &[
                HashPart::Str(request.scope.as_str()),
                HashPart::Str(&request.cohort_id),
        ...
        if self.redaction_budgets.contains_key(&budget_id) {
            return Err(format!(
                "Dandelion dandelion_tail_latency-phase privacy guard redaction budget {budget_id} already exists"
            ));
        }
    ...
    pub fn publish_operator_summary(
        ...
        ensure_capacity(
            ...
            "Dandelion dandelion_tail_latency-phase privacy guard operator summaries",
        )?;
        self.refresh_live_counts();
        ...
        let summary_id = stable_id(
            "dandelion-tail-latency-liquidity-firewall-operator-summary",
            &[
                HashPart::U64(epoch),
                HashPart::U64(slot),
    ...
    fn refresh_roots(&mut self) {
        ...
        self.roots.state_root = merkle_root(
            "bridge-dandelion-tail-latency-liquidity-firewall:state",
            &[
    ...
    pub fn public_record(&self) -> Value {
        json!({
            ...
            "dandelion_tail_latency_liquidity_firewall_suite": self.config.dandelion_tail_latency_liquidity_firewall_suite,
            ...
            "dandelion_tail_latency_window_slots": self.config.dandelion_tail_latency_window_slots,
            ...
            "policies": self.policies.values().map(|policy| json!({
                ...
                "min_pre_dandelion_tail_latency_reserve_micro_units": policy.min_pre_dandelion_tail_latency_reserve_micro_units,
                "min_dandelion_tail_latency_lane_claim_reserve_micro_units": policy.min_dandelion_tail_latency_lane_claim_reserve_micro_units,
            ...
            "cohorts": self.cohorts.values().map(|cohort| json!({
                ...
                "pre_dandelion_tail_latency_reserve_micro_units": cohort.pre_dandelion_tail_latency_reserve_micro_units,
                "dandelion_tail_latency_lane_claim_reserve_micro_units": cohort.dandelion_tail_latency_lane_claim_reserve_micro_units,
            ...
            "signals": self.signals.values().map(|signal| json!({
                ...
                "dandelion_tail_latency_risk_score_bps": signal.dandelion_tail_latency_risk_score_bps,
...
pub fn demo() -> State {
    ...
    let signal = state
        .submit_signal(SubmitSignalRequest {
            cohort_id,
            kind: SignalKind::DandelionTailLatencyGraphLinkagePressure,
            signal_commitment_root: devnet_root("signal", "demo-viewtag-pressure"),
            observed_claim_tail_latency_path_root: devnet_root(
                "observed-tail_latency_path",
                "demo-viewtag-pressure",
            ),
            dandelion_tail_latency_pressure_root: devnet_root("pressure", "demo-viewtag-pressure"),
            reporter_commitment: "reporter:demo-subaddress-watchtower".to_string(),
            pre_dandelion_tail_latency_reserve_delta_micro_units: -10_000_000,
            dandelion_tail_latency_lane_claim_reserve_delta_micro_units: 8_000_000,
            privacy_set_size: 524_288,
            dandelion_tail_latency_risk_score_bps: 6_200,
    ...
    state
        .submit_attestation(SubmitAttestationRequest {
            signal_id: signal,
            verdict: AttestationVerdict::RequireDandelionTailLatencyCoverage,
            attestor_commitment: "attestor:demo-dandelion-tail-latency-liquidity-firewall"
                .to_string(),
            ...
            pq_security_bits: 256,
            observed_dandelion_tail_latency_risk_score_bps: 6_400,
            observed_privacy_set_size: 524_288,
...
fn object_root<T: Serialize>(domain: &str, value: &T) -> String {
    ...
    domain_hash(
        &format!("bridge-dandelion-tail-latency-liquidity-firewall:{domain}"),
        &[HashPart::Json(&value)],
        32,
    )
}
...
fn devnet_root(domain: &str, label: &str) -> String {
    domain_hash(
        &format!("bridge-dandelion-tail-latency-liquidity-firewall:devnet:{domain}"),
