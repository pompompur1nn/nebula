...
binds the package schema/version identity, package-level launch status,
blocker/remediation counts, readiness report/artifact roots, rooted command map, rooted `next_steps`, and the rooted
...
bootstrap profile in the report root, seal the bootstrap operator-registry,
public-endpoint, and node-probe counts, seal the local finality latency profile
