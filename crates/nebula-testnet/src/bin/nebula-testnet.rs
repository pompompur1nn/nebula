...
fn read_public_testnet_peer_manifest_input(args: &[String], wants_json: bool) -> String {
    read_required_launch_package_input(args, "--public-testnet-peer-manifest", wants_json)
}
...
fn prove_live_rpc_devnet(args: &[String], wants_json: bool) {
        ...
        .any(|arg| ARTIFACT_BOUND_REHEARSAL_FLAGS.contains(&arg.as_str()))
    {
        let inputs = read_launch_package_inputs(args, wants_json);
        let bundle_input = read_launch_package_bundle_input(args, wants_json);
        let public_testnet_peer_manifest_input =
            read_public_testnet_peer_manifest_input(args, wants_json);
        nebula_testnet::prove_live_rpc_devnet_rehearsal_with_peer_manifest_jsons_and_evidence(
            &bundle_input,
            &public_testnet_peer_manifest_input,
            &inputs.deployment_input,
            &inputs.public_status_input,
            &inputs.public_probe_input,
            &inputs.validator_set_input,
...
fn build_live_rpc_devnet_runtime_surface(args: &[String], wants_json: bool) {
    ...
    match nebula_testnet::build_live_rpc_devnet_runtime_surface_evidence_with_peer_manifest_json_pretty(
        &bundle_input,
        &public_testnet_peer_manifest_input,
        &inputs.deployment_input,
        &inputs.public_status_input,
...
fn parse_operators(args: &[String], wants_json: bool) -> Vec<nebula_testnet::OperatorBuildInput> {
    let mut secret_key_files =
        parse_witness_secret_key_files(args, "--operator-secret-key-file", wants_json);
    let operators = parse_rows_range(args, "--operator", 3, 4, wants_json)
        .into_iter()
        .map(|fields| nebula_testnet::OperatorBuildInput {
            operator_id: fields[0].clone(),
            region: fields[1].clone(),
            public_key: fields[2].clone(),
            secret_key_hex: resolve_witness_secret_key(
                "--operator",
                "--operator-secret-key-file",
                &fields[0],
                fields.get(3).cloned(),
                &mut secret_key_files,
                wants_json,
            ),
        })
        .collect();
    reject_unused_witness_secret_key_files(
        "--operator-secret-key-file",
        secret_key_files,
        wants_json,
    );
...
fn parse_observers(args: &[String], wants_json: bool) -> Vec<nebula_testnet::ObserverBuildInput> {
    let mut secret_key_files =
        parse_witness_secret_key_files(args, "--observer-secret-key-file", wants_json);
    let observers = parse_rows_range(args, "--observer", 3, 4, wants_json)
        .into_iter()
...
fn parse_witness_secret_key_files(
    args: &[String],
    name: &str,
    wants_json: bool,
) -> BTreeMap<String, String> {
    let mut secret_key_files = BTreeMap::new();
    for value in arg_values(args, name) {
        let fields = value
            .split(',')
            .map(str::trim)
            .map(str::to_string)
        ...
        if fields.len() != 2 || fields.iter().any(|field| field.is_empty()) {
            print_deployment_attestation_builder_error(
                wants_json,
                &[format!("{name} expected id,path with non-empty fields")],
            );
            process::exit(1);
        }
        match secret_key_files.entry(fields[0].clone()) {
            Entry::Vacant(entry) => {
                let secret_key_hex =
                    read_witness_secret_key_file(name, &fields[0], &fields[1], wants_json);
                entry.insert(secret_key_hex);
            }
            Entry::Occupied(_) => {
                print_deployment_attestation_builder_error(
                    wants_json,
                    &[format!("{name} supplied duplicate id {}", fields[0])],
                );
                process::exit(1);
            }
        }
...
fn read_witness_secret_key_file(name: &str, id: &str, path: &str, wants_json: bool) -> String {
    let secret_key_hex = match read_text_file(path) {
        Ok(secret_key_hex) => secret_key_hex.trim().to_string(),
        Err(error) => {
            print_deployment_attestation_builder_error(wants_json, &[error]);
            process::exit(1);
        }
    };
    if secret_key_hex.is_empty() || secret_key_hex.chars().any(char::is_whitespace) {
        print_deployment_attestation_builder_error(
            wants_json,
            &[format!(
                "{name} for {id} must contain one hex secret key with only surrounding whitespace"
            )],
        );
        process::exit(1);
    }
...
fn resolve_witness_secret_key(
    row_name: &str,
    file_name: &str,
    id: &str,
    inline_secret_key_hex: Option<String>,
    secret_key_files: &mut BTreeMap<String, String>,
    wants_json: bool,
) -> Option<String> {
    let file_secret_key_hex = secret_key_files.remove(id);
    if inline_secret_key_hex.is_some() && file_secret_key_hex.is_some() {
        print_deployment_attestation_builder_error(
            wants_json,
            &[format!(
                "{row_name} {id} supplied both inline secret_key_hex and {file_name}"
            )],
        );
        process::exit(1);
    }
    inline_secret_key_hex.or(file_secret_key_hex)
}

fn reject_unused_witness_secret_key_files(
    name: &str,
    secret_key_files: BTreeMap<String, String>,
    wants_json: bool,
) {
    if secret_key_files.is_empty() {
        return;
    }
    let ids = secret_key_files
        .keys()
        .cloned()
        .collect::<Vec<_>>()
        .join(", ");
    print_deployment_attestation_builder_error(
        wants_json,
        &[format!("{name} supplied for unknown id(s): {ids}")],
    );
    process::exit(1);
}
...
fn help_text() -> &'static str {
    ...
    nebula-testnet --build-public-testnet-launch-certificate --public-observer-confirmation <path> --runtime-surface-evidence <path> --public-testnet-peer-manifest <path> --operator-join-confirmation <path> --validator-join <path> --validator-activation <path> --launch-package-bundle <path> --deployment-attestation <path> --public-status <path> --public-probe <path> --validator-set <path> --operator-handoff <path> --operator-acceptance <path> --genesis-manifest <path>
    nebula-testnet --verify-public-testnet-launch-certificate <path> --public-observer-confirmation <path> --runtime-surface-evidence <path> --public-testnet-peer-manifest <path> --operator-join-confirmation <path> --validator-join <path> --validator-activation <path> --launch-package-bundle <path> --deployment-attestation <path> --public-status <path> --public-probe <path> --validator-set <path> --operator-handoff <path> --operator-acceptance <path> --genesis-manifest <path> [--json]
    nebula-testnet --verify-public-testnet-launch-readiness <path> --public-observer-confirmation <path> --runtime-surface-evidence <path> --public-testnet-peer-manifest <path> --live-rpc-devnet-rehearsal <path> --live-rpc-devnet-runtime-surface-evidence <path> --operator-join-confirmation <path> --validator-join <path> --validator-activation <path> --launch-package-bundle <path> --deployment-attestation <path> --public-status <path> --public-probe <path> --validator-set <path> --operator-handoff <path> --operator-acceptance <path> --genesis-manifest <path> [--json]
...
FINAL PUBLIC LAUNCH READINESS:
    ...
    advertised endpoint, the verified public-testnet peer manifest, a live RPC
    devnet rehearsal report, and the verified loopback-devnet runtime-surface
    evidence whose root the rehearsal reports.
...
LOCAL PUBLIC TESTNET REHEARSAL:
    ...
    live deployment attestation is supplied. When launch artifacts and the
    public-testnet peer manifest are supplied, the report is root-bound to that
...
mod tests {
    ...
    #[test]
    fn help_mentions_public_launch_operator_flags() {
        let help = help_text();
        for needle in [
            "--admin-rpc-bind <addr:port>",
